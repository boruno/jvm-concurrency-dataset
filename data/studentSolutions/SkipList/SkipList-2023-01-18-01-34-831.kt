...
class SkipList<E : Comparable<E>> {
    ...
    init {
        ...
        fun remove(element: E): Boolean {
            ...
            while(true) {
                val found = find(element, preds, succs)
                if (found && element as Int == 10)
                {
                    println(String.format("Found 10. Value is %d. Key is %d. Hashcode is %d", succs[bottomLevel]!!.value, succs[bottomLevel]!!.key, succs[bottomLevel]!!.value.hashCode()))
                }
                if (!found) {
                    return false
                }
        ...
        fun find(x: E, preds: Array<Node<E>?>, succs: Array<Node<E>?>): Boolean {
            ...
            var curr: Node<E>? = null
            var succ: Node<E>?
            if (head.next!![bottomLevel].reference == tail)
            {
                preds[bottomLevel] = head
                succs[bottomLevel] = tail
                return false
            }
            loop@ while(true){
                    pred = head
