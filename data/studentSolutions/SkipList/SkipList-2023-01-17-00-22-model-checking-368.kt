...
class SkipList<E : Comparable<E>> {
    ...
    private fun find(targetElement: E): Boolean {
        val subsets = findSubsetsForElement(targetElement)
        return isElementPresentedInSubsets(targetElement, subsets)
    }

    private fun findNodeOnLevel(prevNode: Node<E>, targetElement: E, level: Int): NodeSubset<E>? {
        var previousNode = prevNode
        var currentNode = previousNode.next[level].value ?: return null
