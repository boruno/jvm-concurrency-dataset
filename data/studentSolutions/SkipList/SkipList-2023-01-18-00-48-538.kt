...
class SkipList<E : Comparable<E>> {
    val head = Node<E>(Integer.MIN_VALUE);
    val tail = Node<E>(Integer.MAX_VALUE);
    init {
        for (i in 0 until head.next!!.size) {
            head.next!![i].set(null, false)
        }
    }
        /**
    ...
    public class Node<T>() {
        ...
        constructor(nodeKey: Int) : this() {
            ...
            value = null
            topLevel = MAX_LEVEL
            next = Array(MAX_LEVEL + 1) {i -> AtomicMarkableReference(null, false) }
        }

        constructor(x: T, height: Int) : this() {
            value = x
            key = x.hashCode()
            next = Array(height + 1) {i -> AtomicMarkableReference(null, false)}
            topLevel = height
        }
    }
