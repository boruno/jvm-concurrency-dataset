...
class MichaelScottQueueWithConstantTimeRemove<E> : QueueWithRemove<E> {
    ...
    private class Node<E>(
        ...
        fun remove(): Boolean {
            ...
            val prevNode1 = prev.value
            val nextNode1 = next.value
            if (prevNode1 != null && nextNode1 != null) {
                prevNode1.next.compareAndSet(this, nextNode1)
                if (prevNode1.extractedOrRemoved) prevNode1.remove()
                if (nextNode1.extractedOrRemoved) nextNode1.remove()
            }

            val prevNode2 = prev.value
            val nextNode2 = next.value
            if (prevNode2 != null && nextNode2 != null) {
                nextNode2.prev.compareAndSet(this, prevNode2)
                if (prevNode2.extractedOrRemoved) prevNode2.remove()
                if (nextNode2.extractedOrRemoved) nextNode2.remove()
            }
