...
class IntIntHashMap {
    ...
    private fun moveCore() {
        while (true) {
            val curCore = core.value
            val next = curCore.next.value ?: return
    ...
    private class Core(capacity: Int) {
        ...
        fun putInternal(key: Int, value: Int, move: Boolean = false): Int {
            ...
            while (true) {
                when (getKey(keyIndex)) {
                    key -> break
                    NULL_KEY -> if (cas(keyIndex, NULL_KEY, key)) break
            ...
            assert(getKey(keyIndex) == key) { "Key mismatch: $keyIndex, $key" }

            if (move) {
                cas(keyIndex + 1, NULL_VALUE, value)
                return NULL_VALUE
            }

            while (true) {
                val oldValue = getValue(valueIndex(keyIndex))
                when {
                    oldValue < 0 -> {
                        val result = next.value?.putInternal(key, value) ?: error("No next core")

                        return if (oldValue != MOVED && result == NULL_VALUE) {
                            oldValue
                        } else {
                            result
                        }
                    ...
                    cas(keyIndex + 1, oldValue, value) -> return oldValue
                }
            }
        }
        ...
        fun rehash() {
            ...
            while (keyIndex < map.size) {
                ...
                if (key != NULL_KEY) {
                    if (isValue(value)) {
                        if (!cas(keyIndex + 1, value, -value)) continue
                        newCore.putInternal(key, value, move = true)
                        cas(keyIndex + 1, -value, MOVED)
                    } else if (value == NULL_VALUE || value == DEL_VALUE) {
                        if (!cas(keyIndex + 1, value, MOVED)) continue
                    } else if (value != MOVED) {
                        newCore.putInternal(key, -value, move = true)
                        cas(keyIndex + 1, value, MOVED)
                    }
