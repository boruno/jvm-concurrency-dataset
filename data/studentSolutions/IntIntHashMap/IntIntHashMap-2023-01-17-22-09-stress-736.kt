    ...
    private class Core internal constructor(capacity: Int) {
        ...
        fun putRehash(key: Int, value: Long): Pair<Int, Boolean> {
            ...
            while (true) { // optimize for successful lookup
                ...
                if (++probes >= MAX_PROBES && value != DEL_VALUE) return Pair(0, true)
                if (index == 0) index = map.size
                index -= 2
            }
        }
        ...
        fun rehash(): Core {
            ...
            while (index < map.size) {
                ...
                if (curValue == NULL_VALUE || curValue == DEL_VALUE) {
                    map[index + 1].compareAndSet(curValue, TRUE_DEL_VALUE)
                }
                if (isValue(curValue.toInt())) {
                    val result = newCore.putRehash(curKey.toInt(), curValue)
