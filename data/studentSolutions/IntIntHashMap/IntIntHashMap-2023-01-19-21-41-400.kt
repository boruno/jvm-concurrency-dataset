...
class IntIntHashMap {
    ...
    private class Core internal constructor(capacity: Int) {
        ...
        val map = atomicArrayOfNulls<Int>(capacity * 2)
...
//        val maxSize: Int = capacity
        ...
        fun getInternal(key: Int): Int {
            var index = index(key)
            var probes = 0
            var cell = toValue(map[index].value)
            while (cell != key) { // optimize for successful lookup
                if (cell == NULL_VALUE) return NULL_VALUE // not found -- no value
                if (++probes >= MAX_PROBES) return NULL_VALUE
                if (index == 0) index = map.size
                index -= 2
                cell = toValue(map[index].value)
            }
        ...
        fun putInternal(key: Int, value: Int): Int {
            ...
            while (cell != key) { // optimize for successful lookup
                if (cell == NULL_VALUE) {
                    ...
                    map[index].compareAndSet(null, key)
                    break
                }
            ...
            val oldValue = toValue(map[index + 1].value)
        ...
        fun rehash(): Core {
            ...
            while (index < map.size) {
                if (isValue(toValue(map[index + 1].value))) {
                    val result = newCore.putInternal(toValue(map[index].value), toValue(map[index + 1].value))
