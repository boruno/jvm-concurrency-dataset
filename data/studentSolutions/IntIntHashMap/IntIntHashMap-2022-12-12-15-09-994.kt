            ...
            while (true) { // optimize for successful lookup
                ...
                if (!isKeyPresented(currentKey)) {
                    ...
                    if (isRemoveOperation(value) && key == 6)
                        println("Remove key: $key, Current value = $currentValue, Index = $index, map: $map")
                    ...
                    if (map.compareAndSet(index + 1, currentValue, value)) {
                        if (isRemoveOperation(value) && key == 6)
                            println("CASed successful. Remove key: $key, Current value = $currentValue, Index = $index, map: $map")
