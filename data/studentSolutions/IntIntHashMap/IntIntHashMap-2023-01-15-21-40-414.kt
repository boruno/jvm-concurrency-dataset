...
class IntIntHashMap {
    ...
    private class Core internal constructor(capacity: Int) {
        ...
        private fun move(key: Int, value: Int) {
            ...
            var probes = 0
            while (true) {
                if (map[index].value == NULL_KEY && map[index].compareAndSet(NULL_KEY, key) && map[index + 1].compareAndSet(NULL_VALUE, key)) return
                if (key == map[index].value && map[index + 1].compareAndSet(NULL_VALUE, value)) return
