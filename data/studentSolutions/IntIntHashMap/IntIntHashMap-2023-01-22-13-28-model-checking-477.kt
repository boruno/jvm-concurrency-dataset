    ...
    private class Core internal constructor(val capacity: Int) {
        ...
        fun putInternal(key: Int, value: Int): Int {
            ...
            loop1@ while (true) {
                ...
                while (expectedKey != key) { // optimize for successful lookup
                    if (expectedKey == NULL_KEY) {
                        ...
                        if (map[index].compareAndSet(NULL_KEY, key)) {
                            ...
                            expectedKey = map[index].value
                            expectedValue = map[index + 1].value
                            if (expectedKey == key) {
                                break
                            }
                        }
                ...
                if (isValue(expectedValue) || expectedValue == DEL_VALUE) {
                    if (currentCore.map[index + 1].compareAndSet(expectedValue, value)) {
                        return expectedValue
                    }
                } else if (expectedValue == NULL_VALUE) {
                    if (isValue(value)) {
                        if (currentCore.map[index + 1].compareAndSet(expectedValue, value)) {
                            return expectedValue
                        }
                    }
                } else { // if S_MARK or negative -- help to rehash it and try again
                    return NEEDS_REHASH
                }
            }
        }
        ...
        fun putInternalOnRehash(key: Int, value: Int) {
            ...
            var index = index(key)
            var expectedKey = map[index].value
            var expectedValue = map[index + 1].value
            while (expectedKey != key) { // optimize for successful lookup
                if (expectedKey == NULL_KEY) {
                    if (map[index].compareAndSet(NULL_KEY, key)) {
                        break
            ...
            // если в новой мапе было значение - ничего не делаем
            if (isValue(expectedValue)) {
                //pass
            } else if (expectedValue == NULL_VALUE) { // если в новой мапе был 0 - обновим его
                if (map[index + 1].compareAndSet(expectedValue, value)) {
                    return
                }
            } else if (expectedValue == DEL_VALUE) {
                // если в мапе удален элемент значит мы уже перехэшировали
                // ничего не делаем
            } else if (expectedValue == S_MARK) {
                // если марка - мы уже перенесли это значение в следующую хэшмапу
                // ничего не делаем
//                throw IllegalStateException("sMark + $expectedValue")
            } else if (isVMod(expectedValue)) {
                if (value != expectedValue * -1) {
                    throw IllegalStateException("mod + $expectedValue")
                }

                if (map[index + 1].compareAndSet(expectedValue, expectedValue * -1)) {
                    return
                }
                // если модифицированное значение

            } else {
                throw IllegalStateException("$expectedValue")
            }
        }
        ...
        fun rehash(): Core {
            ...
            while (index < map.size) {
                ...
                if (expectedValue == S_MARK) {
                    //pass
                } else if (expectedKey == NULL_KEY || expectedValue == NULL_VALUE) {
                    if (!map[index + 1].compareAndSet(NULL_VALUE, S_MARK)) {
                        continue
                    }
                } else {
                    if (isValue(expectedValue)) {
