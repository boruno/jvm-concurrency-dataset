...
//                    }
        ...
        fun rehash(): Core {
            ...
            while (index < map.size) {
                ...
                if (markToRehash(index)) {
                    val value = map[index + 1].value
                    if (value <= 0 && value != REHASHED) {
                        // transfer value
                        next.value!!.receive(map[index].value, -value - 1)
                        markAsTransferred(index)
                    }
                } else {
                    val value = map[index + 1].value
                    if (value != REHASHED && value < 0) {
                        next.value!!.receive(map[index].value, -value - 1)
                        markAsTransferred(index)
                    }
                }
        ...
        fun markToRehash(index: Int) : Boolean {
            // mark value as necessary to transfer
            var value = map[index + 1].value
            while (needsToRehash(value) && !map[index + 1].compareAndSet(value, -value - 1)) {
                value = map[index + 1].value
            }
            if (!needsToRehash(value)) {
                return false
            }

            value = map[index + 1].value
            assert(value <= 0 && value != REHASHED && value != DEL_VALUE)
            return true
        }

        fun needsToRehash(value: Int) : Boolean {
            if (value == DEL_VALUE || value == REHASHED) { return false }
            return value >= 0
        }

        fun markAsTransferred(index: Int) {
            // mark as transferred
            var value = map[index + 1].value
            assert(value <= 0 || value == REHASHED) { value }
            while (!map[index + 1].compareAndSet(value, REHASHED)) {
                value = map[index + 1].value
                assert(value <= 0 || value == REHASHED)
            }
        }
