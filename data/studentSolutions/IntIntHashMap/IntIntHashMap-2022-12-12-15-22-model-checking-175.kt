        ...
        fun putInternal(key: Int, value: Int): Int {
            ...
            while (true) { // optimize for successful lookup
                ...
                if (!isKeyPresented(currentKey)) {
                    ...
                    // Temporary logging
                    if (isRemoveOperation(value) && key == 6 && currentValue == 0)
                        println("[${Thread.currentThread().id}] Remove key: $key, Current value = $currentValue, Index = $index, map: $map")
                    ...
                    if (map.compareAndSet(index + 1, currentValue, value)) {
                        if (isRemoveOperation(value) && key == 6 && currentValue == 0)
                            println("[${Thread.currentThread().id}] CASed successful. Remove key: $key, Current value = $currentValue, Index = $index, map: $map")
