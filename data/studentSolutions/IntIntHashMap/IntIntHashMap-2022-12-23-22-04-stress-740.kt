    ...
    private class Core internal constructor(capacity: Int) {
        ...
        fun getInternal(key: Int): Int {
            ...
            while(true) {
                ...
                if (Int.MIN_VALUE < value && value < 0) // Fixed value is negative
                    return -value;
        ...
        fun putInternal(key: Int, value: Int): Int {
            while(true) {
                ...
                var tryAgain = false
                while (keyValue != key) { // optimize for successful lookup
                ...
                if (oldValue == MOVED_VALUE) // If we have our key moved to new table, we have no power here
                    return NEEDS_REHASH
                if (Int.MIN_VALUE < oldValue && oldValue < 0) // If readonly, that means rehashing in progress and we need to help
                {
                    return NEEDS_REHASH
                }
        ...
        fun helpMoveData(): Boolean {
            ...
            while(true) {
                ...
                while (index < map.size) {
                    if (currentValue < 0 && currentValue != MOVED_VALUE) {
                    ...
                    {
                        if (!map[index + 1].compareAndSet(currentValue, -currentValue)) {
                            needToRestart = true
                            break
                        }
                        if(nextCore.value!!.putInternal(currentKey, currentValue) == NEEDS_REHASH)
