...
class IntIntHashMap {
    ...
    private class Core(capacity: Int) {
        ...
        val moved = atomicArrayOfNulls<Int>(2 * capacity)
        ...
        fun putInternal(key: Int, value: Int): Int {
            ...
            while (true) {
                ...
                if (nextCore.value != null) {
                    val inN = nextCore.value!!.putInternal(key, value)
                    while (true) {
                        val whoWas = moved[index + 1].value
                        if (whoWas == null) { //no move yet, return yourself
                            if (!moved[index + 1].compareAndSet(null, value)) {
                                continue
                            }
                            return prev ?: NULL_VALUE
                        }
                        if (whoWas == prev) { //prev was moved, then someone did stuff in inN, now its us
                            return inN
                        }
                        return prev ?: NULL_VALUE
                    }
        ...
        fun rehash(): Core {
            ...
            while (index < map.size) {
                ...
                if (isValue(v ?: 0) || v == DEL_VALUE) {
                    val k = map[index].value!!
                    val z = newCore.getInternalIndex(k)
                    if (newCore.map[z + 1].compareAndSet(null, v)) {
                        moved[index + 1].compareAndSet(null, v)
                    }
                }
                index += 2
            }
