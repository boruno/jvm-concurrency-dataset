...
//            if (oldValue == NEEDS_HELP) {
//                if(oldCore.help()) {
//                    core.compareAndSet(oldCore, oldCore.next.value!!)
//                }
//            }
            if (oldValue != NEEDS_REHASH && oldValue != NEEDS_HELP) return oldValue
            if (oldCore.help()) {
                core.compareAndSet(oldCore, oldCore.next.value!!)
            } else {
    ...
    private class Core internal constructor(capacity: Int) {
        ...
        fun putInternal(key: Int, value: Int, isRehash: Boolean): Int {
            ...
            if (isRehash) {
                ...
                //if (next.value != null) return NEEDS_REHASH
                if (!map[index + 1].compareAndSet(oldValue, value)) return NEEDS_REHASH
                if (next.value == null) return oldValue
                else return NEEDS_REHASH
            }
        }
...
private const val NEEDS_HELP = -3 // value moved
