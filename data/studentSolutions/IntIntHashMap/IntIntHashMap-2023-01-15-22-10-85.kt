...
class IntIntHashMap {
    ...
    private class Core internal constructor(capacity: Int) {
            ...
            while (true) {
                ...
                return when (map[index + 1].value) {
                    ...
                    in MOVED_VALUE + 1 until 0 -> {
                        val fixedValue = -map[index + 1].value
                        if (nextCore !== null) {
                            nextCore.move(map[index].value, fixedValue)
                            val result = nextCore.getInternal(key)
                            map[index + 1].compareAndSet(map[index + 1].value, MOVED_VALUE)
                            result
                        } else throw IllegalArgumentException("Something wrong")
                    }

                    else -> map[index + 1].value
                }
            }
        }
        ...
        fun putInternal(key: Int, value: Int): Int {
            ...
            var probes = 0
            while (true) {
                if (map[index].value != key) { // optimize for successful lookup
        ...
        fun rehash(): Core {
            ...
            while (index < map.size) {
                while (true) {
                    when (map[index + 1].value) {
                        MOVED_VALUE -> break
                        in MOVED_VALUE + 1 until 0 -> {
                            val fixedValue = -map[index + 1].value
                            newCore.move(map[index].value, fixedValue)
                            if (map[index + 1].compareAndSet(fixedValue, MOVED_VALUE)) break
                        }

                        in 1 until DEL_VALUE -> {
                            val fixedValue = -map[index + 1].value
                            if (map[index + 1].compareAndSet(map[index + 1].value, fixedValue)) {
                                newCore.move(map[index].value, map[index + 1].value)
                                if (map[index + 1].compareAndSet(fixedValue, MOVED_VALUE)) break
                            }
                        }

                        DEL_VALUE -> if (map[index + 1].compareAndSet(map[index + 1].value, MOVED_VALUE)) break
                    }
                }
                index += 2
            }
...
private const val MOVED_VALUE = Int.MIN_VALUE + 1
