...
private class HashMapImpl<K : Any, V : Any> : HashMap<K, V> {
    ...
    override fun remove(key: K): V? =
        if (get(key) != null)
            putInternal(core.value, key, Tombstone, weak = false)
        else
            null
    ...
    private fun putInternal(currentCore: Core<K>, key: K, value: Any, weak: Boolean): V? {
        @Suppress("UNCHECKED_CAST")
        when (val index = currentCore.put(key, weak)) {
            NEEDS_REHASH -> return putInternal(rehash(currentCore), key, value, weak)
            CELL_IS_USED -> return null
            else -> {
                val cell = currentCore.values[index]
                while (true) {
                    when (val cellValue = cell.value) {
                        ...
                        null -> if (cell.compareAndSet(null, value))
                            return null

                        else -> if (weak || cell.compareAndSet(cellValue, value))
                            return if (cellValue is Tombstone)
                                null
                            else
                                cellValue as V?
                    }
        ...
        for (index in 0 until currentCore.capacity) {
            ...
            while (true) {
                when (val cellValue = cell.value) {
                    ...
                    is Tombstone, null -> if (cell.compareAndSet(cellValue, Moved))
                        break

                    else -> {
                        @Suppress("UNCHECKED_CAST")
                        val element = cellValue as V
                        if (cell.compareAndSet(cellValue, Fixed(element))) {
    ...
    private class Core<K>(
        ...
        fun put(key: K, weak: Boolean = false): Int {
            var index = index(key)
            ...
            while (true) {
                val currentKey = keys[index].value
                    ?: if (keys[index].compareAndSet(null, key))
                        ...
                        continue
                if (currentKey == key)
                    return if (weak) CELL_IS_USED else index
                if (++probes == MAX_PROBES) return NEEDS_REHASH
                if (++index == capacity) index = 0
            }
        }
...
private const val NEEDS_REHASH = -2
private const val CELL_IS_USED = -3
