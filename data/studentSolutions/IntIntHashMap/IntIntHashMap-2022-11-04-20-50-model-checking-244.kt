...
class IntIntHashMap {
    ...
    private fun putAndRehashWhileNeeded(key: Int, value: Int): Int {
        while (true) {
            ...
            val oldValue = curCore.putInternal(key, value, false)
    ...
    private fun getAndRehashWhileNeeded(key: Int): Int {
        while (true) {
            val curCore = core.value
            val oldValue = curCore.getInternal(key)
            if (oldValue != NEEDS_REHASH) return oldValue
            rehash(curCore)
        }
    }

    private fun rehash(thisCore: Core) {
        val curCore = core.value
        if (curCore !== thisCore) {
            return
        }
    ...
    private class Core internal constructor(val capacity: Int) {
        ...
        fun putInternal(key: Int, value: Int, onlyIfNullValue: Boolean): Int {
            var index = index(key)
            var probes = 0
            while (true) {
                ...
                if (curKey == NULL_KEY) {
                    ...
                    if (map[index].compareAndSet(NULL_KEY, key) || map[index].value == key) {
                        break
                    }
                }
            ...
            while (true) {
                ...
                if (oldValue != NULL_VALUE && onlyIfNullValue) {
                    return -2
                }
                if (oldValue == MOVED_VALUE || oldValue < 0) {
                    if (oldValue != MOVED_VALUE) { // help to move
                        next.value!!.putInternal(key, -oldValue, true)
                        map[index + 1].value = MOVED_VALUE
                    }
                    return next.value!!.putInternal(key, value, false)
                }
                    ...
                    val success = when (curValue) {
                        ...
                        MOVED_VALUE -> {
                            ...
                            kotlin.run {
                                ...
                                nextCore.putInternal(curKey, abs(curValue), false)
                                true
                            }
                        }
                    }
