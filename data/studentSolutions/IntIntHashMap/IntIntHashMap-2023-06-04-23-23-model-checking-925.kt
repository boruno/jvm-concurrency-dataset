...
class IntIntHashMap {
    ...
    private class Core(capacity: Int) {
        ...
        fun putInternal(key: Int, value: Int): Int {
            var state = ""
            (0 until map.size).forEach {i -> state = state + map[i].value + ", " }
            Logger.getLogger("core").log(Level.INFO, state)
            Logger.getLogger("core").log(Level.INFO, "value: " + value.toString())
            var index = index(key)
            var probes = 0
            ...
            Logger.getLogger("core").log(Level.INFO, "index: " + index.toString())
            Logger.getLogger("core").log(Level.INFO, "old_value: " + map[index+1].value.toString())
