...
class AtomicCounterArray(size: Int) {
    ...
    private inner class IncrementDescriptor(
        ...
        fun applyOperation() {
            val (firstPair, secondPair) = listOf(
                index1 to valueBeforeIncrement1,
                index2 to valueBeforeIncrement2
            ).sortedBy { it.first }

            val (firstIndex, firstValueBeforeIncrement) = firstPair
            val (secondIndex, secondValueBeforeIncrement) = secondPair

            if (!array[firstIndex].compareAndSet(firstValueBeforeIncrement, this)) {
                status.compareAndSet(UNDECIDED, FAILED)
                return
            }

            if (!array[secondIndex].compareAndSet(secondValueBeforeIncrement, this)) {
                status.compareAndSet(UNDECIDED, FAILED)
                return
            }
            ...
            array[firstIndex].compareAndSet(this, firstValueBeforeIncrement + 1)
            array[secondIndex].compareAndSet(this, secondValueBeforeIncrement + 1)
