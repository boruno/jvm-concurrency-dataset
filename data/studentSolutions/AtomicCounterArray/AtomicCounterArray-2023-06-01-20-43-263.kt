...
class AtomicCounterArray(size: Int) {
    ...
    fun inc2(index1: Int, index2: Int) {
        ...
        while (true) {
            val cur1 = get(index1)
            val cur2 = get(index2)
            if (cur1 is IncrementDescriptor) {
                cur1.applyOperation()
                if (cur1.status.value == FAILED) {
                    continue
                }
                if (get(index1) !is Int) throw Exception("no int1")
                if (get(index2) !is Int) throw Exception("no int1")
                break
            }
            if (cur2 is IncrementDescriptor) {
                cur2.applyOperation()
                if (cur2.status.value == FAILED) {
                    continue
                }
                if (get(index1) !is Int) throw Exception("no int2")
                if (get(index2) !is Int) throw Exception("no int2")
                break
            }
            ...
            if (descriptor.status.value == SUCCESS) {
                if (get(index1) !is Int) throw Exception("no int3")
                if (get(index2) !is Int) throw Exception("no int3")
                return
            }
            counter += 1
            if (counter > 10000) {
