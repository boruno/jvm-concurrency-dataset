...
class AtomicCounterArray(size: Int) {
    ...
    private inner class IncrementDescriptor(
        ...
        fun applyOperation() {
            ...
            val resetValue = when (status.value) {
                SUCCESS -> valueBeforeIncrement1 + 1
                FAILED -> valueBeforeIncrement1
                else -> error("Should have not come here")
            }

            array[index1].compareAndSet(this, resetValue)
            array[index2].compareAndSet(this, resetValue)
        }
    }
