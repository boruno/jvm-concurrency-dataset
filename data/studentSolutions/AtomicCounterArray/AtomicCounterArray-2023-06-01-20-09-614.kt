...
class AtomicCounterArray(size: Int) {
    ...
    private inner class IncrementDescriptor(
        ...
        fun applyOperation() {
            while (true) {
                val res1 = array[index1].compareAndSet(valueBeforeIncrement1, array[index1].value)
                val res2 = array[index2].compareAndSet(valueBeforeIncrement2, array[index2].value)
                if (res1 && res2) {
