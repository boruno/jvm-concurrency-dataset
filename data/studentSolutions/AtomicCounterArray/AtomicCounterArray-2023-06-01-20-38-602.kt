...
class AtomicCounterArray(size: Int) {
    ...
    fun get(index: Int): Int {
        fun getFromDescriptor(descriptor: IncrementDescriptor, status: Status): Int {
            return if(status == SUCCESS) {
                val res = if (descriptor.index1 == index) descriptor.valueBeforeIncrement1 + 1
                else descriptor.valueBeforeIncrement2 + 1
                array[index].compareAndSet(descriptor, res)
                res
            } else {
                val res =  if (descriptor.index1 == index) descriptor.valueBeforeIncrement1
