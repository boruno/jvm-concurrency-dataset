    ...
    private inner class IncrementDescriptor(
        ...
        fun applyOperation() {
            if (status.value == UNDECIDED) {
                array[index1].compareAndSet(valueBeforeIncrement1, atomic(this))
                array[index2].compareAndSet(valueBeforeIncrement2, atomic(this))
                status.compareAndSet(UNDECIDED, SUCCESS)
                return
            }
            if (status.value == SUCCESS) {
                array[index1].compareAndSet(atomic(this), valueBeforeIncrement1 + 1)
                array[index2].compareAndSet(atomic(this), valueBeforeIncrement2 + 1)
                return
            }
            array[index1].compareAndSet(atomic(this), valueBeforeIncrement1)
            array[index2].compareAndSet(atomic(this), valueBeforeIncrement2)
        }
    }
