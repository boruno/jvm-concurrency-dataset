            ...
            is IncrementDescriptor -> {
                if (currentValue.status.value == SUCCESS) {
                    if (index == currentValue.index1) {
                        currentValue.valueBeforeIncrement1 + 1
                    } else {
                        currentValue.valueBeforeIncrement2 + 1
                    }
                } else {
                    if (index == currentValue.index1) {
                        currentValue.valueBeforeIncrement1
                    }
                    else {
                        currentValue.valueBeforeIncrement2
                    }
                }
            } else -> {currentValue as Int}
        }
    }
    ...
    fun inc2(index1: Int, index2: Int) {
        ...
        while (true){
            ...
            if (firstValue is IncrementDescriptor){
                ...
                if (array[min(index1, index2)].compareAndSet(minIndexValue, descriptor)) {
    ...
    private inner class IncrementDescriptor(
        ...
        fun applyOperation() {
            ...
            while (true) {
                ...
                val resultValue = array[index2].value ?: continue
                status.compareAndSet(UNDECIDED, SUCCESS)
                if (resultValue is IncrementDescriptor) {
                    resultValue.applyOperation()
                    continue
                }
                if (resultValue == valueBeforeIncrement2 + 1) {
