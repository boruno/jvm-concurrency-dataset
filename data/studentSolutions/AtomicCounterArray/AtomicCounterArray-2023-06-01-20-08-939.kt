...
class AtomicCounterArray(size: Int) {
    ...
    private inner class IncrementDescriptor(
        ...
        fun applyOperation() {
            ...
            if (descriptor !is IncrementDescriptor) {
                val secondDescriptor = array[index2].value
                if (secondDescriptor !is IncrementDescriptor) return
                array[index2].compareAndSet(secondDescriptor, secondDescriptor.valueBeforeIncrement2+1)
                return
            }
