    ...
    private inner class IncrementDescriptor(
        ...
        fun applyOperation() {
            if (status.value == SUCCESS) {
                array[leftIndex].compareAndSet(this, leftValueBeforeIncrement + 1)
                array[rightIndex].compareAndSet(this, rightValueBeforeIncrement + 1)
                return
            }
            ...
            if (leftValue is IncrementDescriptor && leftValue != this) {
                leftValue.applyOperation()
            }
            if (leftValue == this || array[leftIndex].compareAndSet(leftValueBeforeIncrement, this)) {
                val rightValue = array[rightIndex].value
                if (rightValue is IncrementDescriptor && rightValue != this) {
                    rightValue.applyOperation()
                }
                if (rightValue == this || array[rightIndex].compareAndSet(rightValueBeforeIncrement, this)) {
