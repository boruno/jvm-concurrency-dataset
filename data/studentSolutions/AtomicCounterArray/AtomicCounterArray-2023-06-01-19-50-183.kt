...
class AtomicCounterArray(size: Int) {
    ...
    fun inc2(index1: Int, index2: Int) {
        ...
        if (firstValue is IncrementDescriptor) {
            ...
            val descriptior = IncrementDescriptor(index1, firstValue as Int, index2, secondValue as Int)
            array[index1].compareAndSet(firstValue, descriptior)
            descriptior.applyOperation()
        }
    }
    ...
    private inner class IncrementDescriptor(
        ...
        fun applyOperation() {
            ...
            array[index1].compareAndSet(descriptor.valueBeforeIncrement1, descriptor.valueBeforeIncrement1+1)

            // physically change first value
            array[index2].compareAndSet(descriptor.valueBeforeIncrement2, descriptor.valueBeforeIncrement2+1)
        }
    }
