@file:Suppress("DuplicatedCode")

//package day4

import AtomicCounterArray.Status.*
import kotlinx.atomicfu.*

// This implementation never stores `null` values.
class AtomicCounterArray(size: Int) {
    private val array = atomicArrayOfNulls<Any>(size)

    init {
        // Fill array with zeros.
        for (i in 0 until size) {
            array[i].value = 0
        }
    }

    fun get(index: Int): Int {
        // TODO: the cell can store a descriptor.
        return when (array[index].value) {
            is Int -> array[index].value as Int
            is IncrementDescriptor -> {
                val desc = (array[index].value as IncrementDescriptor)
                if (index == desc.index1 && (desc.status.value == UNDECIDED || desc.status.value == FAILED)) {
                    desc.valueBeforeIncrement1
                } else if (index == desc.index1) {
                    desc.valueBeforeIncrement1 + 1
                } else if (index == desc.index2 && (desc.status.value == UNDECIDED || desc.status.value == FAILED)) {
                    desc.valueBeforeIncrement2
                } else if (index == desc.index2) {
                    desc.valueBeforeIncrement2 + 1
                } else {
                    throw RuntimeException()
                }
            }
            else -> { throw RuntimeException() }
        }
    }

    fun inc2(index1: Int, index2: Int) {
        require(index1 != index2) { "The indices should be different" }
        // TODO: This implementation is not linearizable!
        // TODO: Use `IncrementDescriptor` to perform the operation atomically.

        while (true) {
            val value1 = get(index1)
            val value2 = get(index2)

            val desc = IncrementDescriptor(
                index1, value1,
                index2, value2
            )

            desc.applyOperation()

            if (desc.status.value === SUCCESS) return

//            if (value1 != null && value2 != null) {
//                val desc = IncrementDescriptor(
//                    index1, value1,
//                    index2, value2
//                )
//                if (!array[index1].compareAndSet(value1, desc)) continue
//                if (!array[index2].compareAndSet(value2, desc)) continue
//                desc.applyOperation()
//            } else {
//                if (value1 == null) {
//                    val desc1 = array[index1].value as IncrementDescriptor
//                        return
//                    }
//                }
//            }

        }
    }

    // TODO: Implement the `inc2` operation using this descriptor.
    // TODO: 1) Read the current cell states
    // TODO: 2) Create a new descriptor
    // TODO: 3) Call `applyOperation()` -- it should try to increment the counters atomically.
    // TODO: 4) Check whether the `status` is `SUCCESS` or `FAILED`, restarting in the latter case.
    private inner class IncrementDescriptor(
        val index1: Int, val valueBeforeIncrement1: Int,
        val index2: Int, val valueBeforeIncrement2: Int
    ) {
        val status = atomic(UNDECIDED)

        // TODO: Other threads can call this function
        // TODO: to help completing the operation.
        fun applyOperation() {
            // TODO: Use the CAS2 algorithm, installing this descriptor
            // TODO: in `array[index1]` and `array[index2]` cells.
            if (!array[index1].compareAndSet(valueBeforeIncrement1, valueBeforeIncrement1+1)) {
                status.compareAndSet(UNDECIDED, FAILED)
            }
            if (!array[index2].compareAndSet(valueBeforeIncrement2, valueBeforeIncrement2+1)) {
                status.compareAndSet(UNDECIDED, FAILED)
            }
            status.compareAndSet(UNDECIDED, SUCCESS)
        }
    }

    private enum class Status {
        UNDECIDED, FAILED, SUCCESS
    }
}