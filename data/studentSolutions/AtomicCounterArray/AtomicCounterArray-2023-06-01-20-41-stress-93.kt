...
class AtomicCounterArray(size: Int) {
    ...
    fun inc2(index1: Int, index2: Int) {
        ...
        val updatedIndex1 = min(index1, index2)
        val updatedIndex2 = max(index1, index2)
        while (true) {
            val before1 = get(updatedIndex1)
            val before2 = get(updatedIndex2)
            val descriptor = IncrementDescriptor(updatedIndex1, before1, updatedIndex2, before2)
