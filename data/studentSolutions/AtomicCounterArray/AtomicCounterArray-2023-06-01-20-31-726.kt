...
class AtomicCounterArray(size: Int) {
    ...
    private inner class IncrementDescriptor(
        ...
        fun applyOperation() {
            while (true) {
                val cellIncremented1 = valueBeforeIncrement1.inc()
                val cellIncremented2 = valueBeforeIncrement2.inc()
                val res1 = array[index1].compareAndSet(valueBeforeIncrement1,cellIncremented1)
                val res2 = array[index2].compareAndSet(valueBeforeIncrement2, cellIncremented2)
                if (res1 && res2) {
                    status.compareAndSet(UNDECIDED,SUCCESS)
                    status.compareAndSet(FAILED,SUCCESS)
                    return
                } else {
                    array[index1].compareAndSet(cellIncremented1, valueBeforeIncrement1)
                    array[index2].compareAndSet(cellIncremented2,valueBeforeIncrement2)
