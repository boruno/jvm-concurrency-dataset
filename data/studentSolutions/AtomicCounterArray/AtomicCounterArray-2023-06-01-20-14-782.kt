...
class AtomicCounterArray(size: Int) {
    ...
    private inner class IncrementDescriptor(
        ...
        fun applyOperation() {
            while (true) {
                val res1 = array[index1].compareAndSet(valueBeforeIncrement1, valueBeforeIncrement1 + 1)
                val res2 = array[index2].compareAndSet(valueBeforeIncrement2, valueBeforeIncrement2  + 1)
                if (res1 && res2) {
                    status.lazySet(SUCCESS)
                    return
                } else {
                    status.lazySet(FAILED)
                    continue
                }
            }
