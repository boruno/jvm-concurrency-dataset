...
class ConcurrentHashTableWithoutResize<K : Any, V : Any>(initialCapacity: Int) : HashTable<K, V> {
    ...
    class Table<K : Any, V : Any>(val capacity: Int) {
        ...
        tailrec fun put(key: K, value: V): Any? {
            var index = index(key)
            ...
            repeat(MAX_PROBES) {
                ...
                when (curKey) {
                    ...
                    key -> {
                        ...
                        return if (values.compareAndSet(index, oldValue, value)) oldValue else put(key, value)
                    }
            ...
            repeat(MAX_PROBES) {
                ...
                when (curKey) {
                    ...
                    key -> {
                        ...
                        val oldValue = values[index]
                        if (!values.compareAndSet(index, oldValue, null)) return oldValue
                        return oldValue
                    }
                    // Empty cell.
                    null -> {
