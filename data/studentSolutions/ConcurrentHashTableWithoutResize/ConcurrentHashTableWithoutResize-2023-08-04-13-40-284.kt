            ...
            repeat(MAX_PROBES) {
                ...
                when (curKey) {
                    ...
                    key -> {
                        ...
                        while (!values[index].compareAndSet(oldValue, value))
                        return oldValue
                    }
                    // The cell does not store a key.
                    null -> {
                        // Insert the key/value pair into this cell.
                        while(!keys[index].compareAndSet(null, key))
                        while(!values[index].compareAndSet(null, value))
            ...
            repeat(MAX_PROBES) {
                ...
                when (curKey) {
                    ...
                    key -> {
                        ...
                        val oldValue = values[index].value
                        while(!values[index].compareAndSet(oldValue, null))
                        return oldValue
                    }
                    // Empty cell.
                    null -> {
