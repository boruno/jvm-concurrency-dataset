...
class ConcurrentHashTableWithoutResize<K : Any, V : Any>(initialCapacity: Int) : HashTable<K, V> {
    ...
    class Table<K : Any, V : Any>(val capacity: Int) {
        ...
        fun put(key: K, value: V): Any? {
            ...
            repeat(MAX_PROBES) {
                // Read the key.
                val keyRef = keys[index]
                val curKey = keyRef.value
                when (curKey) {
                    // The cell contains the specified key.
                    key -> {
                        // Update the value and return the previous one.
                        val ref = values[index]
                        val oldValue = ref.value
                        if (ref.compareAndSet(oldValue, value)) {
                            return oldValue
                        }
                    }
                    ...
                    null -> {
                        ...
                        if (keyRef.compareAndSet(null, key)) {
                            val valueRef = values[index]
                            val oldValue = valueRef.value
                            if (valueRef.compareAndSet(oldValue, value)) {
                                return oldValue
                            }
                        }
                    }
