...
class ConcurrentHashTableWithoutResize<K : Any, V : Any>(initialCapacity: Int) : HashTable<K, V> {
    ...
    class Table<K : Any, V : Any>(val capacity: Int) {
        ...
        fun put(key: K, value: V): Any? {
            ...
            findKey@ for (probe in 0 until MAX_PROBES + 1) {
                if (probe >= MAX_PROBES) return NEEDS_REHASH
                ...
                when (curKey) {
                    ...
                    else -> index = (index + 1) % capacity // Process the next cell, use linear probing.
                }
            }
            ...
            while (true) {
                ...
                if (!values.compareAndSet(index, oldValue, value)) continue
                return oldValue
            }
        }
