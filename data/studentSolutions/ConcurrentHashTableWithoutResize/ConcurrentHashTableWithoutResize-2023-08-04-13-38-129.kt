            ...
            repeat(MAX_PROBES) {
                ...
                when (curKey) {
                    ...
                    key -> {
                        ...
                        values[index].compareAndSet(oldValue, value)
                        return oldValue
                    }
                    // The cell does not store a key.
                    REMOVED_KEY -> {
                        // Insert the key/value pair into this cell.
                        keys[index].compareAndSet(REMOVED_KEY as K, key)
...
//                        keys[index].value = REMOVED_KEY                                   ------------------update is here
                        // Read the value associated with the key and replace it with `null`.
                        val oldValue = values[index].value
                        values[index].compareAndSet(oldValue, null)
                        return oldValue
                    }
                    // Empty cell.
                    null -> {
