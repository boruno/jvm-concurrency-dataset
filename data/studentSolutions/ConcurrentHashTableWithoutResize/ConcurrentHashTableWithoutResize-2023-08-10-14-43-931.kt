...
class ConcurrentHashTableWithoutResize<K : Any, V : Any>(initialCapacity: Int) : HashTable<K, V> {
    ...
    class Table<K : Any, V : Any>(val capacity: Int) {
            ...
            repeat(MAX_PROBES) {
                ...
                when (curKey) {
                    ...
                    key -> {
                        ...
                        // the key will never be deleted from the map
        ...
        private fun addKey(index: Int, key: K): Boolean {
            return keys.compareAndExchange(index, null, key) == key
//            return keys.compareAndSet(index, null, key) || keys.get(index) == key
        }

        private fun updateValue(index: Int, value: V?): V? {
