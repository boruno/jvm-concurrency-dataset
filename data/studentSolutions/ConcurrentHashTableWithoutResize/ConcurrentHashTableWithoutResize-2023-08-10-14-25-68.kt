...
class ConcurrentHashTableWithoutResize<K : Any, V : Any>(initialCapacity: Int) : HashTable<K, V> {
    ...
    class Table<K : Any, V : Any>(val capacity: Int) {
        ...
        fun put(key: K, value: V): Any? {
                    ...
                    null, REMOVED_KEY -> {
                        ...
                        if (keys.compareAndSet(index, null, key)) {
                            if (values.compareAndSet(index, null, value)) {
                                ...
                                return null
                            } else keys.compareAndSet(index, key, null)
                        }
                    }
                }
            ...
            // Inform the caller that the table should be rehashed.
            return NEEDS_REHASH
        }
