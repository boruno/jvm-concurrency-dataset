...
class ConcurrentHashTable<K : Any, V : Any>(initialCapacity: Int) : HashTable<K, V> {
    ...
    override fun put(key: K, value: V): V? {
        while (true) {
            ...
            val table = table.value.next.value ?: table.value

            val putResult = table.put(key, value)
    ...
    private fun resize() {
        ...
        if (!currentTable.next.compareAndSet(null, newTable)) {
            // Another thread already resized the table and we don't need to do anything
            return
        }
                ...
                when (val value = currentTable.values[index].value) {
                    ...
                    is Moving<*> -> {
                        ...
                        if (currentTable.values[index].compareAndSet(value, movingValue)) {
                            ...
                            if (currentTable.values[index].compareAndSet(movingValue, Moved)) {
                                break
                            }
                        }
                    }
