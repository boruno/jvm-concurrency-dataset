...
class ConcurrentHashTable<K : Any, V : Any>(initialCapacity: Int) : HashTable<K, V> {
    ...
    private fun moveFixedValue(
        ...
        val value = curTable.values[i]
        if (value is Fixed<*>) {
            val x = put(curTable.keys[i] as K, value.value as V, onlyNotInitialized = false, newTable)
                    ...
                    key -> {
                        ...
                        return when {
                            ...
                            oldValue === Moved || oldValue is Fixed<*> -> {
                                ...
                                this@ConcurrentHashTable.put(key, value, onlyNotInitialized, table.get())
                            }
                ...
                when (curKey) {
                    ...
                    key -> {
                        ...
                        if (oldValue === Moved || oldValue is Fixed<*>) {
                            applyStartedResize(this)
                            return this@ConcurrentHashTable.remove(key)
                        }
