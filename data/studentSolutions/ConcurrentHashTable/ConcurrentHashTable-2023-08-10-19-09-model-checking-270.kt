...
class ConcurrentHashTable<K : Any, V : Any>(initialCapacity: Int) : HashTable<K, V> {
    ...
    private fun resize() {
        ...
        curTable.nextTable.compareAndSet(null, newTable)
        finishResize()
    }

    private fun finishResize() {
        val curTable = table.get()
        val nextTable = curTable.nextTable.get() ?: return
    ...
    class Table<K : Any, V : Any>(val capacity: Int) {
        ...
        fun put(key: K, value: V): Any? {
            ...
            while(true) {
                // If the next table changed, start from scratch with probes = 0
                if (nextTable.get() != curNextTable) {
                ...
                when (curKey) {
                    ...
                    key -> {
                        ...
                        if (oldValue is Moved) {
                            ...
                            return nextTable.get().put(key, value)
                        }
        ...
        fun get(key: K): V? {
            ...
            while (true) {
                // If the next table is already changed, start again with probes = 0
                if (nextTable.get() != curNextTable) {
                    return get(key)
                }
                // Read the key.
