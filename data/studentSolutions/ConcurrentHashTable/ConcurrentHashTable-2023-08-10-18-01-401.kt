                    ...
                    when (curKey) {
                        ...
                        key -> {
                            ...
                            when (val oldValue = values[index]) {
                                S -> {
                                    (next.get() ?: continue).put(key, value)
                                }

                                is ConcurrentHashTable<*, *>.Fixed -> {
                                    // helping
                                    (next.get() ?: continue).put(key, oldValue.value as V)
                                    values[index] = S
                                    continue
                                }
                        ...
                        null -> {
                            ...
                            if (keys.compareAndSet(index, null, key)) {
                                while (true) {
                                    when (val oldValue = values[index]) {
                                        S -> return (next.get() ?: continue).put(key, value)
                                        is ConcurrentHashTable<*, *>.Fixed -> {
                                            // helping
                                            (next.get() ?: continue).put(key, oldValue.value as V)
                                            values[index] = S
            ...
            repeat(MAX_PROBES) {
                ...
                when (curKey) {
                    ...
                    key -> {
                        ...
                        while (true) {
                            when (val oldValue = values[index]) {
                                S -> return (next.get() ?: continue).remove(key)
                                is ConcurrentHashTable<*, *>.Fixed -> {
                                    // helping
                                    (next.get() ?: continue).put(key, oldValue.value as V)
                                    values[index] = S
