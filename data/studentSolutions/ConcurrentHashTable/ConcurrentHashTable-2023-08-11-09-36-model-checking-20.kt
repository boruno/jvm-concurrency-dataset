...
class ConcurrentHashTable<K : Any, V : Any>(initialCapacity: Int) : HashTable<K, V> {
    ...
    private fun resize() {
        ...
        while (true) {
            ...
            val newCap = 2 * curTail.capacity
            val newTail = Table<K, V>(newCap)
            for (i in 0 until newCap) {
                newTail.values[i] = T
            }
        ...
        if (tab.state.get() == TableState.REHASHING) {
                ...
                while (true) {
                    ...
                    when (key) {
                        null -> {
                            ...
                            val v = tab.values.get(idx)
                            if (v == S) {
                                break
                            }
                            if (v is ConcurrentHashTable<*, *>.Copying) {
                                v.finish()
                            }
                        }
                        else -> {
                                ...
                                S -> {
                                    ...
                                    val copying = if (curVal is ConcurrentHashTable<*, *>.Copying) {
                                        ...
                                        if (tab.values.compareAndSet(idx, curVal, copying)) {
                                            ...
                                            // failed to lock value, repeat in the loop
                                            continue
                                        }
                                    }
                                    copying.finish()
            ...
            for (i in 0 until nextTab.capacity) {
                nextTab.values.compareAndSet(i, T, null)
            }
                ...
                when (curKey) {
                    ...
                    key -> {
                        while (true) {
                            ...
                            when (v) {
                                S -> {
                                    state.compareAndSet(TableState.LIVE, TableState.REHASHING)
                                    return NEEDS_REHASH
                                }
                                ...
                                is ConcurrentHashTable<*, *>.Copying -> {
                                    ...
                                    if (witness != v) {
                                        // we failed, some other thread succeeded,
                                        // treat as if we succeeded, but were overwritten
                                        //return v as V?
                                    } else {
                                        return witness as V?
                                    }
        ...
        fun move(key: K, value: V) {
            ...
            repeat(capacity) {
                ...
                when (curKey) {
                    // The cell contains the specified key.
                    key -> {
                        values.compareAndSet(index, T, value)
                        return
                    }
                    // The cell does not store a key.
                    null -> {
