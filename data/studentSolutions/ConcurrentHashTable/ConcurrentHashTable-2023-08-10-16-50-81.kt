...
class ConcurrentHashTable<K : Any, V : Any>(private val initialCapacity: Int) : HashTable<K, V> {
    ...
    private fun resize() {
        // TODO:
        val newTable = Table<K, V>(initialCapacity)
        var index = 0
        while (index < initialCapacity) {
            val curTable = table.get()
            if (curTable.values.get(index) != null) {
                val key = curTable.keys.get(index) as? K? ?: continue
                val value = curTable.values.get(index) ?: continue
