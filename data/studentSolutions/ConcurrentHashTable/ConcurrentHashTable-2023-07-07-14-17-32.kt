...
class ConcurrentHashTable<K : Any, V : Any>(initialCapacity: Int) : HashTable<K, V> {
    ...
    private fun resize() {
        val oldTableValue = table.value
        val newTable = oldTableValue.resize()
        table.compareAndSet(oldTableValue, newTable)
    }
    ...
    class Table<K : Any, V : Any>(val capacity: Int) {
        ...
        fun resize(): Table<K, V> {
            val newTableAttempt = Table<K, V>(capacity * 2)
            ...
            for (i in 0 until keys.size) {
                while (true) {
                    ...
                    if (oldValue is MovedValue<*>) {
                        newTable.put(keys[i].value!!, oldValue.value as V)
                        values[i].value = Moved
                    }
                    if (oldValue != null) {
                        if (values[i].compareAndSet(oldValue, MovedValue(oldValue))) {
                            newTable.put(keys[i].value!!, oldValue as V)
                            values[i].value = Moved
                            break
                        }
                    } else {
                        values[i].compareAndSet(oldValue, Moved)
                    }
                }
            }
            return newTable
