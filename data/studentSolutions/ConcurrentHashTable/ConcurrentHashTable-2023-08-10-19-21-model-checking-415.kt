...
//    private val nextTable: AtomicReference<Table<K, V>?> = AtomicReference(null)
    ...
    override fun put(key: K, value: V): V? {
        while (true) {
            ...
            val putResult = tableNode.get().table.put(key, value)
    ...
    override fun get(key: K): V? {
        while (true) {
            val curTable = tableNode.get().table
    ...
    private fun resize() {
        val cur = tableNode.get()
        val curNext = cur.next.get()

        if (curNext != null) {
            moveAndSet()
        } else {
            val newTable = Table<K, V>(cur.table.capacity * 2)

            if (cur.next.compareAndSet(null, TableNode(newTable))) {
                moveAndSet()
            }
        }
    }
    ...
    private fun moveAndSet() {
        ...
        if (curNext != null) {
            cur.table.moveElements(curNext.table)
            tableNode.compareAndSet(cur, curNext)
        }
    }
