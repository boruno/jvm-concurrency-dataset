...
class MSQueueWithConstantTimeRemove<E> : QueueWithRemove<E> {
    ...
    override fun enqueue(element: E) {
        ...
        while (true){
            ...
            if (curTail.next.compareAndSet(null, newNode)) {
                ...
                val nextValue = curTail.next.value
                if (nextValue is Node) {
                    tail.compareAndSet(curTail, nextValue)
                }
            }
        }
    }
    ...
    override fun dequeue(): E? {
        ...
        while (true){
            ...
            if (head.compareAndSet(curHead, nextValue) && nextValue.markExtractedOrRemoved()) {
                val element = nextValue.element
                nextValue.element = null
                nextValue.prev.value = null
                curHead.next.value = null
                return element
            } else {
    ...
    private inner class Node(
        ...
        fun remove(): Boolean {
            ...
            val isMarkedNow = markExtractedOrRemoved()


            // if the node is tail, postpone removal
            if (next.value == null){
                return isMarkedNow
            }
            ...
            // if prev node is head
            if (prevNode.prev.value == null && prevNode != this@MSQueueWithConstantTimeRemove.head.value) {
                // dequeue has already started
                return isMarkedNow
            }
...
//                    // we found the previous node and removed the node from the queue
//                    // we are sure that the next.value is not null
//                    // we can not safely set next.value to null, since it may confuse enqueue()
//                    break
//                }
//
//                if (nextNode == null){
...
//                } else {
//                    prevNode = nextNode
//                    nextNode = prevNode.next.value
//                }
//
//            }
            if (prevNode.extractedOrRemoved) prevNode.remove()
            if (nextNode != null && nextNode.extractedOrRemoved) nextNode.remove()
            return isMarkedNow
        }
    }
}