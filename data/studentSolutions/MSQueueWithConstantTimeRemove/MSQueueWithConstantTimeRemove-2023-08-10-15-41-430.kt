...
//        val newNode = Node(element)
        while (true) {
            ...
            val newNode = Node(element, curTailNode)
    ...
    override fun dequeue(): E? {
        ...
        while (true) {
            ...
            // todo че-то убрать указатель назад
    ...
    private class Node<E>(
        ...
        private fun phyRem() {
            ...
            curNext.prev.setIfNotNul(curPrev)
            if (curPrev.extractedOrRemoved) curPrev.phyRem()
            if (curNext.extractedOrRemoved) curNext.phyRem()
        }

        fun <T> AtomicReference<T>.setIfNotNul(value: T) {
            while (true) {
                val cur = get()
                if (cur == null) break
...
//        outside@ while (true) {
//            var somePrev :Node? = nodeNext
//            inside@ while (true) {
//                somePrev = somePrev!!.prev.get()
//                if (somePrev == null) {
//                    node.prev.set(null)
//                    break@outside
//                }
//                if (!somePrev.extractedOrRemoved) {
//                    node.prev.set(somePrev)
//                    break@inside
//                }
//            }
//            if (!node.prev.get()!!.extractedOrRemoved) {
//                break@outside
//            }
//        }
//
...
//        nodeNext.prev.set(nodePrev)
//
//        if (nodeNext.extractedOrRemoved) removeNodePhysicallyThreadUnsafe(nodeNext)
//        if (nodePrev.extractedOrRemoved) removeNodePhysicallyThreadUnsafe(nodePrev)
//
////        var iter = nodeNext
////        while (true) {
////            nodePrev.next.set(iter)
////            if (iter != null && iter != tail && iter.extractedOrRemoved) {
////                iter = iter.next.get()
////            }
////            else {
////                break
