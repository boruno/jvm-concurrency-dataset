...
class MSQueueWithConstantTimeRemove<E> : QueueWithRemove<E> {
    ...
    override fun enqueue(element: E) {
        ...
        while (true) {
            ...
            var  newNode : Node<E>
            if (curTailPrev == null) {
                newNode = Node(element, head.get())
            } else {
                newNode = Node(element, curTail)
            }
//            if (curTailPrev != null) {
...
//                if (curTailPrev?.extractedOrRemoved == true) curTailPrev.remove()
    ...
    override fun dequeue(): E? {
        ...
        while (true) {
            val curHead = head.get()  // O
            val curHeadNext = curHead.next.get() ?: return null  // O
            val curHeadPrev = curHead.prev.get()
            if (curHeadPrev != null) {
                if (curHeadPrev.extractedOrRemoved) curHeadPrev.remove()
            }
            if (head.compareAndSet(curHead, curHeadNext)) {  // O
                ...
                if (curHeadNext.remove()) {  // O
                    return curHeadNext.element  // O
                }
            }
        }
    }
