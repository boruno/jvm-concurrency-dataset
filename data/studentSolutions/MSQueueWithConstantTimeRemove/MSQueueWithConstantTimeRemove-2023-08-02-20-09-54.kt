...
class MSQueueWithConstantTimeRemove<E> : QueueWithRemove<E> {
    ...
    override fun enqueue(element: E) {
        while (true) {
            val currentNode = tail.value
            val newNode = Node(element, currentNode)

            if (currentNode.next.compareAndSet(null, newNode)) {
                tail.compareAndSet(currentNode, newNode)

                if (currentNode.extractedOrRemoved) {
                    currentNode.remove()
                }

                return
            } else {
                tail.compareAndSet(currentNode, currentNode.next.value!!)
            }
        }
    }
