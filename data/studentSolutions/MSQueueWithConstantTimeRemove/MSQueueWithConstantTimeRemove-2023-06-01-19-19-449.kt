...
class MSQueueWithConstantTimeRemove<E> : QueueWithRemove<E> {
    ...
    override fun dequeue(): E? {
        ...
        while (true) {
            ...
            val nodeToDeque: Node<E> = oldHead.next.value ?: return null
            val nextNode = nodeToDeque.next.value

            // prepare new head
            val newHead = Node<E>(null, null)
            newHead.next.value = nextNode
            newHead.markExtractedOrRemoved()

            if (!nodeToDeque.extractedOrRemoved && head.compareAndSet(oldHead, newHead)) {
                if (nextNode != null) nextNode.prev.compareAndSet(nodeToDeque, newHead)
                return nodeToDeque.element
            }
        }
    }
