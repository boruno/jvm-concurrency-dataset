...
class MSQueueWithConstantTimeRemove<E> : QueueWithRemove<E> {
    ...
    override fun enqueue(element: E) {
        ...
        while (true) {
            ...
            if (curTail == tail.value) {
                if (next == null) {
                    if (curTail.next.compareAndSet(null, null)) {
                        tail.compareAndSet(curTail, Node(element, curTail))
    ...
    private fun removePhysically(node: Node) {
        if (node.next.value == null) return

//        node.prev.value?.next?.compareAndSet(node, node.next.value)
//        node.next.value?.prev?.compareAndSet(node, node.prev.value)
    }
