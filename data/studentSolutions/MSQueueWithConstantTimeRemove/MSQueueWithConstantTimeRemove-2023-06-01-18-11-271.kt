...
//            }
            check(nodeNextPrev == node) {
                "node.next.prev != node; `node` contains ${node.element}, `node.next` contains ${nodeNext.element}. ==> Head: ${head.value.dump()} ==> Tail: ${tail.value.dump()} ==> Node: ${node.dump()} Operations: ${operations.joinToString(", ")}"
            }
