...
class MSQueueWithConstantTimeRemove<E> : QueueWithRemove<E> {
    ...
    override fun enqueue(element: E) {
        while (true) {
            ...
            if (curTail.next.compareAndSet(null, node)) {
                if (tail.compareAndSet(curTail, node) && curTail != head.value) {
                    updateTailAndRemoveIfNeeded(curTail)
                }
                return
            } else {
                val next = curTail.next.value ?: continue
                tail.compareAndSet(curTail, next)
                if (curTail != head.value) {
                    updateTailAndRemoveIfNeeded(curTail)
                }
            }
        }
    ...
    private fun updateTailAndRemoveIfNeeded(curTail: Node<E>) {
    ...
    private class Node<E>(
        ...
        fun removePhysically() {
            ...
            val prevNode = this.prev.value ?: return
            val nextNode = this.next.value

            // Remove the current node by linking the previous node directly to the next node
            prevNode.next.compareAndSet(this, nextNode)

            // Update the 'prev' reference of the next node to point to the previous node, if nextNode is not null
            nextNode?.let {
                while(true) {
                    val currPrev = it.prev.value
                    if (currPrev == this) {
                        if (it.prev.compareAndSet(currPrev, prevNode)) break
                    } else {
                        // If `currPrev` isn't the current node, it means another node has been inserted between,
                        // so we break the loop and don't update the 'prev' reference
                        break
                    }
                }
            }

            // If the previous or next nodes have been logically removed, remove them physically as well
            if (prevNode.extractedOrRemoved) prevNode.removePhysically()
            nextNode?.let { if (it.extractedOrRemoved) it.removePhysically() }
        }
    }
}