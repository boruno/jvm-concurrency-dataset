...
class MSQueueWithConstantTimeRemove<E> : QueueWithRemove<E> {
    ...
    private inner class Node<E>(
        ...
        fun removePhysically() {
            val curPrev = prev.value ?: return // if prev is null => it is head => not touch head (it is fictive)
            val curNext = next.value
            if (curNext != null) {
                curNext.prev.value = curPrev
            }
            curPrev.next.value = curNext // doesn't matter if curNext is null it is ok
