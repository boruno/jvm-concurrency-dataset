...
class MSQueueWithConstantTimeRemove<E> : QueueWithRemove<E> {
    ...
    private fun getRealTail(): Node<E>? {
        while (true) {
            val curTailNode = tail.get()
            if (curTailNode.next.get() == null) return curTailNode
    ...
    override fun enqueue(element: E) {
        ...
        while (true) {
            val node = Node(element, getRealTail())
            val curTailNode = tail.get()
            if (curTailNode.next.compareAndSet(null, node)) {

                tail.compareAndSet(curTailNode, node)

                node.prev.set(node.findPrevNode(node.prev.get()))
    ...
    private fun Node<E>.findPrevNode(startFrom: Node<E>?): Node<E>? {
        var prev: Node<E>? = startFrom
        while (true) {
            if (prev?.next?.get() == this) return prev
            val node = prev?.next?.get()
                ?: return null
            prev = node
        }
    }
    ...
    override fun dequeue(): E? {
        ...
        while (true) {
            ...
            val curHeadNextNode = curHeadNode.next.get() ?: return null
            if (head.compareAndSet(curHeadNode, curHeadNextNode)) {
                head.get().prev.set(null)
                if (curHeadNextNode.markExtractedOrRemoved()) {
                    return curHeadNextNode.element
                }
            }
        }
    }
