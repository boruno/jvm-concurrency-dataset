...
class MSQueueWithConstantTimeRemove<E> : QueueWithRemove<E> {
    ...
    override fun remove(element: E): Boolean {
        ...
        while (true) {
            ...
            if (next.element == element && next.remove()) {
                while (true) {
                    val currHead = head.get()
                    val currHeadNext = currHead.get(currHead.next)
                    if (currHeadNext != null && currHeadNext.extractedOrRemoved) {
                        currHeadNext.prev.set(null)
                        head.compareAndSet(currHead, currHeadNext)
                    } else {
                        return true
                    }
                }
