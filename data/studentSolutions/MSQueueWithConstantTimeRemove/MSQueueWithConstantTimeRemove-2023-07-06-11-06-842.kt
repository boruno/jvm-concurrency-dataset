...
class MSQueueWithConstantTimeRemove<E> : QueueWithRemove<E> {
    ...
    override fun dequeue(): E? {
        while (true) {
            ...
            if (head.compareAndSet(curHead, value) && value.markExtractedOrRemoved()) {
                value.prev.compareAndSet(curHead, value)
    ...
    override fun checkNoRemovedElements() {
        check(head.value.prev.value == head.value) {
            "`head.prev` must be itself"
        }
        ...
        while (true) {
            ...
            check(nodeNextPrev != nodeNext) {
                "The `prev` pointer of node with element ${nodeNext.element} is `itself`, while the node is in the middle of the queue"
            }
    ...
    private class Node<E>(
        ...
        fun remove(): Boolean {
            ...
            if (next.value == null || prev.value == this) {
