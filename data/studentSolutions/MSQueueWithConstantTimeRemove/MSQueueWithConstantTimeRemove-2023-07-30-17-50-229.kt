...
class MSQueueWithConstantTimeRemove<E> : QueueWithRemove<E> {
    ...
    override fun enqueue(element: E) {
        ...
        var toEnqueue = Node(element, tail.value)
        while (true) {
            if (tail.value.next.compareAndSet(null, toEnqueue)) {
                if (tail.compareAndSet(tail.value, toEnqueue) && tail.value.extractedOrRemoved && tail.value != head.value) {
                    tail.value.deleteNode()
                }
                return
            } else {
                val afterTail = tail.value.next.value ?: continue
                tail.compareAndSet(tail.value, afterTail)
                if (tail.value.extractedOrRemoved && tail.value != head.value) {
                    tail.value.deleteNode()
                }
            }
            toEnqueue = Node(element, tail.value)
        }
    }
    ...
    override fun dequeue(): E? {
        ...
        while (true) {
            val afterHead = head.value.next.value ?: return null
            if (head.compareAndSet(head.value, afterHead)) {
                afterHead.prev.getAndSet(null)
                check (afterHead.markExtractedOrRemoved()) { return afterHead.element }
            }
        }
    }
    ...
    private class Node<E>(
        ...
        fun deleteNode() {
            if (this.next.value == null || this.prev.value == null) { return }

            val nextNode = next.value
            val previousNode = prev.value ?: return
            previousNode.next.getAndSet(nextNode)
            if (nextNode != null) {
                while (true) {
                    if(nextNode.prev.value == null) { break }
                    if (nextNode.prev.compareAndSet(nextNode.prev.value, previousNode)) { break }
                }
            }
            if (previousNode.extractedOrRemoved) {
                previousNode.deleteNode()
            }
            if (nextNode != null && nextNode.extractedOrRemoved) {
                nextNode.deleteNode()
            }
        }
