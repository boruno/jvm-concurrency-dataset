...
class MSQueueWithConstantTimeRemove<E> : QueueWithRemove<E> {
    ...
    private val fixEdges = atomic(false)
    ...
    override fun dequeue(): E? {
        ...
        println("dequeue start")
        while (true) {
            val curHead = head.value
            val curHeadNext = curHead.next.value ?: return null
            println("Head Next: ${curHead.element} ${curHeadNext.element}")
    ...
    override fun checkNoRemovedElements() {
        ...
        while (true) {
            ...
            check(nodeNextPrev == node) {
                "node.next.prev != node; `node` contains ${node.element}, `node.next` contains ${nodeNext.element} node.next.prev=${nodeNextPrev.element}"
            }
    ...
    private inner class Node<E>(
        ...
        fun printTop5Methods() {
            val exception = Exception()
            val stackTraceElements = exception.stackTrace
            val methods = mutableListOf<String>()

            for (i in 1..5) {
                if (i >= stackTraceElements.size) break
                val methodName = stackTraceElements[i].methodName
                methods.add(methodName)
            }

            println(methods.joinToString(separator = ", "))
        }

        fun removePhysically() {
            println("removePhys $element")
            printTop5Methods()
            val curPrev = prev.value ?: return
            val curNext = next.value ?: return
            if (curNext.prev.compareAndSet(this, curPrev)) {
                println("Next Prev: ${curNext.element} ${curPrev.element}")
                if (curPrev.extractedOrRemoved) {
                    curPrev.removePhysically()
                }
            }
            if (curPrev.next.compareAndSet(this, curNext)) {
                println("Prev Next: ${curPrev.element} ${curNext.element}")
