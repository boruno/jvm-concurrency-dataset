...
class MSQueueWithConstantTimeRemove<E> : QueueWithRemove<E> {
    ...
    override fun dequeue(): E? {
        ...
        while (true) {
            ...
            val shift2 = curHeadNext.next.value
            if (shift2!=null){
                if(head.compareAndSet(curHead, shift2))
                    if(curHeadNext.markExtractedOrRemoved()){
                        curHead.next.value = shift2
                        shift2.prev.value = curHead
    ...
    private class Node<E>(
        ...
        fun remove(): Boolean {
            ...
            if (!(this.prev.value == null || this.next.value == null)) {
                val nextRef = next.value
                val prevRef = prev.value
                if (prevRef!=null) {
                    prevRef.next.compareAndSet(this, nextRef)
                    if (prevRef.extractedOrRemoved) prevRef.remove()
                }
                if (nextRef!=null) {
                    nextRef.prev.compareAndSet(this,prevRef)
                    if (nextRef.extractedOrRemoved) nextRef.remove()
                }
