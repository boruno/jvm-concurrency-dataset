...
class MSQueueWithConstantTimeRemove<E> : QueueWithRemove<E> {
        ...
        while (true) {
            if (node !== head.value && node !== tail.value) {
                check(!node.extractedOrRemoved) {
                    "$node -- Removed node with element ${node.element} found in the middle of the queue"
                }
            }
            ...
            check(nodeNextPrev != null) {
                "$node -- The `prev` pointer of node with element ${nodeNext.element} is `null`, while the node is in the middle of the queue"
            }
            check(nodeNextPrev == node) {
                "$node -- node.next.prev != node; `node` contains ${node.element}, `node.next` contains ${nodeNext.element}"
            }
    ...
    private class Node<E>(
        ...
        override fun toString(): String = (if (prev.value == null) "<null" else "<prev") + "($element)" + (if (next.value == null) "null>" else "next>")
    }
}