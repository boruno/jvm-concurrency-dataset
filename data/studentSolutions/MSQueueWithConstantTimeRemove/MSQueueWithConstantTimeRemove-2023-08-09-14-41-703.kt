    ...
    private class Node<E>(
        ...
        fun remove(): Boolean {
            ...
            while (true) {
                ...
                if (this.prev.get() == null) { // if previous node is null then current is head so don't remove it
                    break
                }

                // previous node could be dummy

                if (this.next.get() == null) { // if next node is null then current is tail so don't remove it
                    break
                }

                val prevSet = this.prev.get()?.next?.compareAndSet(this, this.next.get()) ?: break
                val nextSet = this.next.get()?.prev?.compareAndSet(this, this.prev.get()) ?: break
