...
class MSQueueWithConstantTimeRemove<E> : QueueWithRemove<E> {
    ...
    override fun enqueue(element: E) {
        while (true) {
            ...
            if (currentTail.next.compareAndSet(null, newNode)) {
                setTail(currentTail, newNode)
                return
            }
            else setTail(currentTail, currentTail.next.value)
        }
    }
    ...
    private fun setTail(expectedTail: Node<E>, newTail: Node<E>?) {
        if (newTail == null)
            return
        tail.compareAndSet(expectedTail, newTail)
        if (expectedTail.extractedOrRemoved)
            expectedTail.remove()
    }
    ...
    override fun validate() {
        ...
        while (true) {
            ...
            check(nodeNextPrev == node) {
                "node.next.prev != node; `node` contains ${node.element}, `node.next` contains ${nodeNext.element} but its previous is ${nodeNextPrev.element}"
            }
    ...
    class Node<E>(var element: E?, prev: Node<E>? = null) {
        ...
        fun remove(): Boolean {
            ...
            if (curNext != null) {
                ...
                if (curPrev != null && curPrev.extractedOrRemoved)
                    curPrev.remove()
            }
            return removed
        }
