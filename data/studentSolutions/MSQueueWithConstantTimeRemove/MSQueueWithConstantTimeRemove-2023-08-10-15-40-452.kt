...
class MSQueueWithConstantTimeRemove<E> : QueueWithRemove<E> {
    ...
    override fun validate() {
        ...
        while (true) {
            ...
            check(nodeNextPrev == node) {
                "node.next.prev != node; `node` contains ${node.element}, `node.next` contains ${nodeNext.element} ${node.element} ${nodeNext.element} ${nodeNextPrev.element}"
            }
    ...
    private class Node<E>(
         ...
         */
        fun remove(): Boolean {
            val markExtractedOrRemoved = markExtractedOrRemoved()
        ...
        fun physicalRemove() {
            ...
            if (curPrev != null && curNext != null) {
                ...
                if (curPrev.extractedOrRemoved) {
                    curPrev.physicalRemove()
                }
                if (curNext.extractedOrRemoved) {
                    curNext.physicalRemove()
                }
            }
        }

        private fun AtomicReference<Node<E>?>.setIfNotNull(curPrev: Node<E>?) {
            while (true) {
                val curNextPrev = get() ?: break
                if (compareAndSet(curNextPrev, curPrev)) break
            }
        }
    }
}