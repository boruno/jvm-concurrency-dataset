...
class MSQueueWithConstantTimeRemove<E> : QueueWithRemove<E> {
    ...
    override fun dequeue(): E? {
        while (true) {
            ...
            if (head.compareAndSet(curHead, chNext)) {
                if (chNext.remove()) {
                    val prev = chNext.prev.value
                    if (prev != null) prev.next.value = null
                    chNext.prev.value = null
                    val next = chNext.next.value
                    if (next != null) next.prev.value = chNext
    ...
    private class Node<E>(
        ...
        fun removePhysically() {
            ...
            val prev = curNext.prev.value

            if (prev != null) {
                curNext.prev.value = prev.prev.value
                prev.next.value = curNext
            } else {
                curNext.prev.value = null
            }

            if (curNext.extractedOrRemoved) curNext.removePhysically()
            if (prev != null && prev.extractedOrRemoved) prev.removePhysically()
        }
    }
}