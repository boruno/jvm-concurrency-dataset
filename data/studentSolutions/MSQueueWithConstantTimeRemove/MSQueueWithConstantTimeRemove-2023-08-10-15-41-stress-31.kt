...
class MSQueueWithConstantTimeRemove<E> : QueueWithRemove<E> {
    ...
    override fun dequeue(): E? {
        while (true) {
            ...
            nextHead.prev.set(null) // current head is now removed node and prev could be dummy
    ...
    private class Node<E>(
        ...
        fun physicalRemove() {
            ...
            val previousNode = this.prev.get() ?: return // if previous node is null then current is head so don't remove it
            val nextNode = this.next.get() ?: return  // if next node is null then current is tail so don't remove it

            previousNode.next.set(nextNode)
            previousNode.prev.setIfNotNull(previousNode)

            if (previousNode.extractedOrRemoved) previousNode.physicalRemove()
            if (previousNode.extractedOrRemoved) previousNode.physicalRemove()
        }
    }
}
