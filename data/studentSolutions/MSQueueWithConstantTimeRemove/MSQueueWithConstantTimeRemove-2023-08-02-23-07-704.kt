...
class MSQueueWithConstantTimeRemove<E> : QueueWithRemove<E> {
    ...
    override fun enqueue(element: E) {
        ...
        while (true) {
            ...
            val newNode = Node(element, currentTail.value)
            ...
            if (next.compareAndSet(null, newNode)) {
                ...
                //
                //тк в remove мы хваст физически не удалили
                if (value.extractedOrRemoved) {
                    removePhisically(value)
                }
                //
                return
            } else {
    ...
    private fun removePhisically(value: Node<E>) {
        val curNext = value.next.value
        if (curNext == null) return

        val prev: Node<E>? = findPrev(value)

        if (prev == null) return
        //update the pointer to the current next
        prev.next.value = curNext
        //
        if (curNext.extractedOrRemoved) {
            removePhisically(curNext)
        }
    }

    private fun findPrev(value: Node<E>): Node<E>? {
        var curNode: Node<E> = head.value
        while (true) {
            val curNodeNext = curNode.next.value
            if (curNodeNext == null) {
                return null
            }
            if (curNodeNext == value) {
                return curNode
            }
            curNode = curNodeNext
        }
        //we can't find prev pointer if it's already extracted
    }
    ...
    private class Node<E>(
        ...
        fun removePhisically() {
            val curNext = next.value
            if (curNext == null) return

            val prev: Node<E>? = findPrev()
