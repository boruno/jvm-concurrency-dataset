...
class MSQueueWithConstantTimeRemove<E> : QueueWithRemove<E> {
    ...
    override fun dequeue(): E? {
        ...
        val prevHead = curHead.prev.get() ?: return null
        prevHead.prev.set(null)
        return if (head.compareAndSet(curHead, nextHead) && nextHead.markExtractedOrRemoved()) nextHead.element else dequeue()
    }
    ...
    private class Node<E>(
         ...
         */
        fun remove(): Boolean {
            return markExtractedOrRemoved().also { if (it) removePhysically() }
        }
        fun removePhysically() {
            val nextValue = next.get() ?: return
            val prevValue = prev.get() ?: return
            prevValue.next.set(nextValue)
            nextValue.prev.setIfNotNullYet(prevValue)
            if (prevValue.extractedOrRemoved) {
                prevValue.removePhysically()
            }
            if (nextValue.extractedOrRemoved) {
                nextValue.removePhysically()
            }
        }
        
        fun <T> AtomicReference<T>.setIfNotNullYet(value: T) {
            do {
                val oldValue = get() ?: break
