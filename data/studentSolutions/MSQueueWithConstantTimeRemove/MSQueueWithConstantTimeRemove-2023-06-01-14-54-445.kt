...
class MSQueueWithConstantTimeRemove<E> : QueueWithRemove<E> {
    ...
    override fun checkNoRemovedElements() {
        ...
        while (true) {
            ...
            check(nodeNextPrev == node) {
                "node.next.prev != node; `node` contains ${node.element}, `node.next` contains ${nodeNext.element}. == ${node.dump()}"
            }
    ...
    private class Node<E>(
        ...
        fun remove(): Boolean {
            ...
            val prevNode = prev.value ?: return result
            val nextNode = next.value ?: return result
        ...
        override fun toString(): String =
            ...
            ("($element" + if (extractedOrRemoved) ",extractedOrRemoved)" else ")") +
            (if (next.value == null) "" else "->")

        public fun dump() : String = (if (prev.value != null) prev.value.toString() else "") + toString() + (if (next.value != null) next.value.toString() else "")

        public fun dumpNext() : String = if (next.value == null) toString() else toString() + next.value.toString()

        public fun dumpPrev() : String = if (prev.value == null) toString() else prev.value.toString() + toString()
    }
}