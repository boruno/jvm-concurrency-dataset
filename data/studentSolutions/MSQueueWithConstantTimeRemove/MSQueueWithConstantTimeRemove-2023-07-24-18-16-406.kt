...
//                }
    ...
    override fun checkNoRemovedElements() {
        check(head.value.prev.value == null) {
            "`head.prev` must be null (${head.value.prev.value!!.element})"
        }
        check(tail.value.next.value == null) {
            "tail.next must be null (${tail.value.next.value!!.element})"
        }
        ...
        while (true) {
            ...
            check(nodeNextPrev == node) {
                "node.next.prev (${nodeNextPrev.element}) != node; `node` contains ${node.element}, `node.next` contains ${nodeNext.element}"
            }
    ...
    private class Node<E>(
        ...
        fun remove(): Boolean {
            ...
            if (curPrev == null) return mark // Don't delete first element
