...
class MSQueueWithConstantTimeRemove<E> : QueueWithRemove<E> {
    ...
    override fun enqueue(element: E) {
        ...
        while (true) {
            ...
            val toEnqueue = Node(element, currentTail)

            if (currentTail.next.compareAndSet(null, toEnqueue)) {
                if (tail.compareAndSet(currentTail, toEnqueue) && currentTail != head.value) {
                    checkAndRemove(currentTail)
                }
                return
            } else {
                val afterTail = currentTail.next.value ?: continue
                tail.compareAndSet(currentTail, afterTail)
    ...
    override fun dequeue(): E? {
        ...
        while (true) {
            ...
            if (head.compareAndSet(head.value, afterHead)) {
                ...
                if (afterHead.markExtractedOrRemoved()) {
                    return afterHead.element
                }
            }
        }
    }
    ...
    private fun checkAndRemove(toCheck: Node<E>) {
        if (toCheck.extractedOrRemoved) {
            toCheck.deleteNode()
        }
    }
        ...
        fun deleteNode() {
            ...
            if (nextNode != null) {
                while (true) {
                    ...
                    if (nextNode.prev.compareAndSet(nextNode.prev.value, previousNode)) {
                        break
                    }
                }
            }
