...
class MSQueueWithConstantTimeRemove<E> : QueueWithRemove<E> {
    ...
    private class Node<E>(
        ...
        fun firstNotDeletedNext(): Node<E>? {
            var next = next.value
            while (next != null && !next.extractedOrRemoved) {
                next = next.next.value
            }
            return next
        }

        fun firstNotDeletedPrev(): Node<E>? {
            var prev = prev.value
            while (prev != null && !prev.extractedOrRemoved) {
        ...
        fun removePhysically() {
            val next = firstNotDeletedNext() ?: return
            val prevNode = firstNotDeletedPrev() ?: return
            next.prev.compareAndSet(this, prevNode)
            prevNode.next.compareAndSet(this, next)
