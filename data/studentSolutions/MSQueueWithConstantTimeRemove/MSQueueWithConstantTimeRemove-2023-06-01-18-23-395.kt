...
class MSQueueWithConstantTimeRemove<E> : QueueWithRemove<E> {
    ...
    override fun enqueue(element: E) {
        ...
        var currTail = tail.value
        var newNode = Node(element, currTail)
        while (!currTail.next.compareAndSet(null, newNode)) {
            tail.compareAndSet(currTail, currTail.next.value!!)
            currTail = tail.value
            newNode = Node(element, currTail)
        }
    ...
    override fun dequeue(): E? {
        ...
        var currHead = head.value
        var newHead = currHead.next.value ?: return null
        while (!head.compareAndSet(currHead, newHead) || !newHead.markExtractedOrRemoved()) {
            currHead = head.value
            currHead.prev.value = null
            newHead = currHead.next.value ?: return null
        }
    ...
    private class Node<E>(
        ...
        fun remove(): Boolean {
            ...
            // TODO: the algorithm.
            val murderer = markExtractedOrRemoved()
            val wasPrev = prev.value ?: return murderer
            val wasNext = next.value ?: return murderer
            if (wasPrev.next.value != this) return murderer
            if (wasNext.prev.value != this) return murderer
            wasPrev.next.compareAndSet(this, wasNext)
            wasNext.prev.compareAndSet(this, wasPrev)
            if (wasPrev.extractedOrRemoved) wasPrev.remove()
            if (wasNext.extractedOrRemoved) wasNext.remove()
            return murderer
        }
    }
}