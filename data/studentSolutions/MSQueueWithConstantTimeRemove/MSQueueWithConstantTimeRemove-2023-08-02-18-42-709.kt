...
class MSQueueWithConstantTimeRemove<E> : QueueWithRemove<E> {
    ...
    override fun dequeue(): E? {
        ...
        while (true) {
            ...
            val curHeadPrev = curHead.prev.value
            if (head.compareAndSet(curHead, curHeadNext) && head.value.prev.compareAndSet(curHead, null))
    ...
    private class Node<E>(
        ...
        fun remove(): Boolean {
            ...
            // TODO: the algorithm.
            val isMarkedExtractedOrRemoved = markExtractedOrRemoved()

            val nextNode = next.value
            if (nextNode == null) return isMarkedExtractedOrRemoved

            val curPrev = prev.value
            if (curPrev == null) return isMarkedExtractedOrRemoved

            nextNode.prev.value = curPrev
            if (nextNode.extractedOrRemoved) return nextNode.remove()

            curPrev.next.value = nextNode
            if (curPrev.extractedOrRemoved) return curPrev.remove()

            return isMarkedExtractedOrRemoved
        }
    }
}