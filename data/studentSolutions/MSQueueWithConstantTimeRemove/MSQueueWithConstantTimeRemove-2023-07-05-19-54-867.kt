...
class MSQueueWithConstantTimeRemove<E> : QueueWithRemove<E> {
    ...
    override fun enqueue(element: E) {
        while (true) {
            ...
            val nextTail = Node(element, prev = currentTail)
            ...
            if (currentTail.next.compareAndSet(null, nextTail)) {
                ...
                if (currentTail.extractedOrRemoved) {
                    val prev = currentTail.prev.value
                    val next = currentTail.next.value

                    if (currentTail != head.value && prev != null && next != null) {
                        next.prev.compareAndSet(currentTail, prev)
                        prev.next.compareAndSet(currentTail, next)
