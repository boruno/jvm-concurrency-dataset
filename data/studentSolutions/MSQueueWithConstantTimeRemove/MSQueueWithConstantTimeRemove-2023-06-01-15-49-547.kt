...
class MSQueueWithConstantTimeRemove<E> : QueueWithRemove<E> {
    ...
    override fun enqueue(element: E) {
        while (true) {
            ...
            if (curTail.next.compareAndSet(null, node)) {
                node.prev.compareAndSet(null, curTail)
                tail.compareAndSet(curTail, node)
                if (curTail.extractedOrRemoved) curTail.remove()
                return
            } else {
                val curTailNext = curTail.next.value ?: continue
                tail.compareAndSet(curTail, curTailNext) // not sure that it happens each time
                curTailNext.prev.compareAndSet(null, curTail)
                if (curTail.extractedOrRemoved) curTail.remove()
            }
        }
    }
    ...
    override fun dequeue(): E? {
        while (true) {
            ...
            // TODO delete previous pointer or not?
            if (head.compareAndSet(curHead, next) && next.markExtractedOrRemoved()) {
                next.prev.compareAndSet(curHead, null)
                return next.element
            }
        }
    }
    ...
    override fun checkNoRemovedElements() {
        ...
        while (true) {
            ...
            check(nodeNextPrev == node) {
                "node.next.prev != node; `node ${printMeta(node)}` contains ${node.element}, `node.next ${printMeta(nodeNext)}` contains ${nodeNext.element}"
            }
    ...
    private fun printMeta(node: Node<E>) = "(previous - ${node.prev.value}) (current - $node) (next - ${node.next.value})"
