    ...
    override fun enqueue(element: E) {
        ...
        while (true) {
            ...
            if (curTail == tail.value) {
                if (next == null) {
                    val newNode = Node(element, if (curTail == head.value) head.value else curTail)
...
//            }
    ...
    private fun removePhysically(node: Node) {
        ...
        val next = node.next.value
        if (next != null) next.prev.compareAndSet(node, prev)
        if (prev != null) prev.next.compareAndSet(node, next)
        else {
