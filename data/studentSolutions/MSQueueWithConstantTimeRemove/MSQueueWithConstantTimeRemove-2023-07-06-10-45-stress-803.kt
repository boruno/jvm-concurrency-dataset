...
class MSQueueWithConstantTimeRemove<E> : QueueWithRemove<E> {
    ...
    override fun dequeue(): E? {
        while (true) {
            ...
            if (head.compareAndSet(curHead, curNextHead) && curNextHead.markExtractedOrRemoved()) {
                curNextHead.prev.value = null
    ...
    private inner class Node<E>(
        ...
        fun remove(): Boolean {
            ...
            next.value?.let { it.prev.compareAndSet(curNext, prev.value) }
            prev.value?.let {it.next.compareAndSet(this, next.value) }

//            if (prev.value?.extractedOrRemoved!!) prev.value?.remove()
//            if (next.value?.extractedOrRemoved!!) next.value?.remove()
