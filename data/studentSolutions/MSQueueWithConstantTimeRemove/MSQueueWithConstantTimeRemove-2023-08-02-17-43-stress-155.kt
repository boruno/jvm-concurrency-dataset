...
class MSQueueWithConstantTimeRemove<E> : QueueWithRemove<E> {
    ...
    override fun enqueue(element: E) {
        while (true) {
            ...
            if (oldTail.next.compareAndSet(null, newTail)) {
                ...
                if (oldTail.extractedOrRemoved && oldTail.prev.value != null)
                    oldTail.replace()
                return
            } else {
                oldTail.next.value?.let { next ->
    ...
    class Node<E>(
        ...
        fun replace() {
            prev.value?.let { it.next.value = next.value }
            next.value?.let { it.prev.value = prev.value }
        }
