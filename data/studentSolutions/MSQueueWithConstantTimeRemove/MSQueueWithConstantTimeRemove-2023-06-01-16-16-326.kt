...
class MSQueueWithConstantTimeRemove<E> : QueueWithRemove<E> {
    ...
    override fun dequeue(): E? {
        ...
        while (true) {
            ...
            if (head.compareAndSet(oldHead, newHead) && newHead.prev.compareAndSet(oldHead, null)) {
                if (newHead.markExtractedOrRemoved()) {
                    return newHead.element
                } else {
                    newHead.prev.compareAndSet(null, oldHead)
                    head.compareAndSet(newHead, oldHead)
                }
            }
