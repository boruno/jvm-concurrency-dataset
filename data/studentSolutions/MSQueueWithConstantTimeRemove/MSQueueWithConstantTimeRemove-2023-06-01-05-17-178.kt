...
class MSQueueWithConstantTimeRemove<E> : QueueWithRemove<E> {
    ...
    private class Node<E>(
        ...
        fun remove(): Boolean {
            ...
            var p = prev.value ?: return result
            var n = next.value ?: return result
            var p2 = prev.value ?: return result

            while (p != p2) {
                p = prev.value ?: return result
                n = next.value ?: return result
                p2 = prev.value ?: return result
            }

            p.next.value = n
            n.prev.value = p

            if (p.extractedOrRemoved) p.remove()
            if (n.extractedOrRemoved) n.remove()
