...
class MSQueueWithConstantTimeRemove<E> : QueueWithRemove<E> {
    ...
    override fun dequeue(): E? {
        ...
        while (true) {
            ...
            // either this or another dequeue will update the node, so it's safe to do that
            curHeadNextNode.prev.set(null)

            if (head.compareAndSet(curHeadNode, curHeadNextNode)) {
                if (curHeadNextNode.markExtractedOrRemoved()) {
                    val element = curHeadNextNode.element
                    curHeadNextNode.element = null
                    return element
                }
            }
        }
