...
//                    check(newNode.prev.value == currentTail)
                }
                ...
                if (tail.compareAndSet(currentTail, currentTail.next.value!!)) {
//                    next.prev.compareAndSet(currentTail)
                }
            }
        }
    }
    ...
    override fun dequeue(): E? {
        while (true) {
            ...
            check(currentHead.prev.value == null)

            val currentHeadNext = currentHead.next.value ?: return null

            if (currentHeadNext.prev.compareAndSet(currentHead, null) && head.compareAndSet(
                    currentHead,
                    currentHeadNext
                )
            ) {
...
//            }
            ...
            while (true) {
                ...
                val previousNode = this.prev.value

                if (nextNode == null || previousNode == null) {
                    return removed
                }

                if (previousNode.next.compareAndSet(this, nextNode)) {
                    while (true) {
                        if (nextNode.prev.compareAndSet(this, previousNode)) {
