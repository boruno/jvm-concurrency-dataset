...
class AtomicArrayWithCAS2Simplified<E : Any>(size: Int, initialValue: E) {
    ...
    fun cas2(
        ...
        if (index1 < index2) {
            desc = CAS2Descriptor(index1, expected1, update1, index2, expected2, update2)
        } else {
            desc = CAS2Descriptor(index2, expected2, update2, index1, expected1, update1)
        }
        desc.apply()
...
//                if (curState !is AtomicArrayWithCAS2Simplified<*>.CAS2Descriptor && curState != expected1) {
//                    // там прям число, причем не наше. это точно фейл
//                    return
//                } else if (curState !is AtomicArrayWithCAS2Simplified<*>.CAS2Descriptor && curState == expected1) {
//                    // число но наше. стоит попытаться поставить дескриптор
//                    // тут в результате может стать
//                } else if (curState is AtomicArrayWithCAS2Simplified<*>.CAS2Descriptor && curState == this) {
//                    // наш дескриптор уже стоит. можно двигаться дальше
//                } else {
//                    // не наш дескриптор. придется ему помогать
//                }
