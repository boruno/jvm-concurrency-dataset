...
class AtomicArrayWithCAS2Simplified<E : Any>(size: Int, initialValue: E) {
    ...
    inner class CAS2Descriptor(
        ...
        private fun installDescriptor() {
            ...
            val fistIndex = if(index1 < index2) index1 else index2
            ...
            while (true) {
                if (array[fistIndex].compareAndSet(firstExpected, this)) {
                    if(array[secondIndex].compareAndSet(secondExpected, this)){
                        break;
                    }
                }

                val firstCell = if(index1 < index2) array[index1].value else array[index2].value
                val secondCell = if (index1 < index2) array[index2].value else array[index1].value

                if(firstCell is AtomicArrayWithCAS2SingleWriter<*>.CAS2Descriptor && firstCell != this){
                    firstCell.apply()
                }
                
                if(secondCell is AtomicArrayWithCAS2SingleWriter<*>.CAS2Descriptor && secondCell != this){
                    secondCell.apply()
                }
            }
        }
