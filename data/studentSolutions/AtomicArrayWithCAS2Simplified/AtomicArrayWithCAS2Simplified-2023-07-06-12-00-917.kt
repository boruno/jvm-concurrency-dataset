...
class AtomicArrayWithCAS2Simplified<E : Any>(size: Int, initialValue: E) {
    ...
    inner class CAS2Descriptor(
        ...
        fun apply() {
            while (!array[indexF].compareAndSet(expectedF, this)) {
                val value = array[indexF].value
                if (value == this) {
                    break
                }
            ...
            while (!array[indexS].compareAndSet(expectedS, this)) {
                val value = array[indexS].value
                if (value == this) {
                    break
                }
                if (value is AtomicArrayWithCAS2Simplified<*>.CAS2Descriptor) {
