...
class AtomicArrayWithCAS2Simplified<E : Any>(size: Int, initialValue: E) {
    ...
    fun get(index: Int): E {
        ...
        return if (cell is AtomicArrayWithCAS2Simplified<*>.CAS2Descriptor) {
            val (expected, updated) = when (index) {
                cell.casDataA.index -> with(cell.casDataA) { expected as E to updated as E }
                cell.casDataB.index -> with(cell.casDataB) { expected as E to updated as E }
    ...
    fun cas2(
        ...
        val triple1 = CasData(index1, expected1, update1)
        val triple2 = CasData(index2, expected2, update2)

        val (a, b) = if (index1 < index2) {
            triple1 to triple2
        } else {
            triple2 to triple1
        }
