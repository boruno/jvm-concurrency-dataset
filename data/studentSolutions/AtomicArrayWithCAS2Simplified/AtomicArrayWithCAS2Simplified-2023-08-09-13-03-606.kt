...
class AtomicArrayWithCAS2Simplified<E : Any>(size: Int, initialValue: E) {
    ...
    inner class CAS2Descriptor(
        ...
        fun apply() {
            ...
            val firstDescIfExists = toDescriptor(array[index1])
            if(firstDescIfExists != null &&  (firstDescIfExists.status.get() == SUCCESS || firstDescIfExists.status.get() == FAILED)) {
                if (firstDescIfExists.status.get() == SUCCESS) firstDescIfExists.changeToUpdate() else firstDescIfExists.changeToExpected()
                return
            } else if (firstDescAlreadyInstalled || array.compareAndSet(index1, expected1, this)) {
                val secDescAlreadyInstalled = array.compareAndSet(index2, this, this)
                val secondDescIfExists = toDescriptor(array[index2])
                if(secondDescIfExists != null &&  (secondDescIfExists.status.get() == SUCCESS || secondDescIfExists.status.get() == FAILED)) {
                    if (secondDescIfExists.status.get() == SUCCESS) secondDescIfExists.changeToUpdate() else secondDescIfExists.changeToExpected()
                    return
                }
