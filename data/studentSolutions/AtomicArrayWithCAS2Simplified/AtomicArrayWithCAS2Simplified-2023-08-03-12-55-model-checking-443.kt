...
class AtomicArrayWithCAS2Simplified<E : Any>(size: Int, initialValue: E) {
    ...
    inner class CAS2Descriptor(
        ...
        fun apply() {
            ...
            if (cell1Value != this && cell1Value is AtomicArrayWithCAS2Simplified<*>.CAS2Descriptor) {
                cell1Value.apply()
                return
            }

            val cell2Value = cell2.value
            if (cell2Value != this && cell2Value is AtomicArrayWithCAS2Simplified<*>.CAS2Descriptor) {
                cell2Value.apply()
                return
            }
            ...
            when (currentStatus) {
                UNDECIDED -> {
                    var wasFailed = false
                    if (cell1Value != this && !cell1.compareAndSet(expected1, this)) {
                        if (!status.compareAndSet(UNDECIDED, FAILED)) return
                        wasFailed = true
                    }

                    if (cell2Value != this && !cell2.compareAndSet(expected2, this)) {
                        if (!status.compareAndSet(UNDECIDED, FAILED)) return
                        wasFailed = true
                    }
