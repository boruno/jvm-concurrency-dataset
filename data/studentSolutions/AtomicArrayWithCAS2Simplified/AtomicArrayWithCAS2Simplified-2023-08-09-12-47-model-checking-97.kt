                    ...
                    if (val1 === this) {
                        ...
                        if (val1 is AtomicArrayWithCAS2Simplified<*>.CAS2Descriptor) {
                            ...
                            when (val1.status.get()) {
                                UNDECIDED -> {
                                    var liveLock = false
                                    if (val1.index2 == index1 && idx == index2 && idx > index1) {
                                        // live lock
                                        updateCell() // roll back cells
                                        liveLock = true
                                    }
                                    //if (val1.index2 == index1) return false
                                    ...
                                    if (liveLock) {
                                        val retryFirst = installDescriptor(index1, expected1)
                                        if (!retryFirst) return false
                                    }
                                    continue
                                }
                                FAILED -> {
