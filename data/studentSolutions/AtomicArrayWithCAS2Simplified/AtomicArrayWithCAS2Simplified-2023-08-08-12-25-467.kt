        ...
        private fun installTo(idx: Int, expected: E): Boolean {
            while (true) {
                when (val value = array[idx].value) {
                    ...
                    is AtomicArrayWithCAS2Simplified<*>.CAS2Descriptor -> when (value.status.value) {
                        UNDECIDED -> value.apply()
                        SUCCESS -> updateValues(value, true)
                        FAILED -> updateValues(value, false)
                    }
                    expected -> {
