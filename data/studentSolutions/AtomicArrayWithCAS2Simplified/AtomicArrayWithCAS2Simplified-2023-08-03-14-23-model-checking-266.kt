...
class AtomicArrayWithCAS2Simplified<E : Any>(size: Int, initialValue: E) {
    ...
    inner class CAS2Descriptor(
        ...
        fun apply() {
            while(true) {
                if (this.isUndecided()) {
                    val actual1 = tryInstall(index1, expected1)
                    if (actual1.needsHelp()) {
                        (actual1 as AtomicArrayWithCAS2Simplified<*>.CAS2Descriptor).apply()
                        continue
                    }
                    val actual2 = tryInstall(index2, expected2)
                    if (actual2.needsHelp()) {
                        (actual2 as AtomicArrayWithCAS2Simplified<*>.CAS2Descriptor).apply()
                        continue
                    }
                    updateStatus()
                }
        ...
        // returns true if no help provided
        private fun tryInstall(index: Int, expected: E): Any? {
            if (array[index].compareAndSet(expected, this))
                return null
            return array[index].value
        }

        private fun Any?.needsHelp(): Boolean = when(this) {
            null -> false
            this@CAS2Descriptor -> false
            is AtomicArrayWithCAS2Simplified<*>.CAS2Descriptor -> true
            else -> false
        }

        // returns true if help provided
//        private fun provideHelp(index: Int): Boolean =
...
//                true
//            else if (actual1 != this && actual1 != null) {
//                actual1.apply()
//                null
//            }
//            else if (actual2 != this && actual2 != null) {
//                actual2.apply()
//                null
//            }
//            else false
