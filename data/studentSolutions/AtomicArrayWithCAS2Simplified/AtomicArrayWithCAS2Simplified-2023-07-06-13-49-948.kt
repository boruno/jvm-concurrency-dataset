...
class AtomicArrayWithCAS2Simplified<E : Any>(size: Int, initialValue: E) {
    ...
    fun CAS2Descriptor<E>.apply(): Boolean{
        ...
        if (array[firstIndex].value == this && array[secondIndex].value == this) {
            status.compareAndSet(UNDECIDED, SUCCESS)
        } else if (array[firstIndex].value==this){
            if(array[secondIndex].compareAndSet(secondExpected, this)){
                status.compareAndSet(UNDECIDED, SUCCESS)
            }else{
                (array[secondIndex].value as? CAS2Descriptor<E> ?: return false).apply()
            }
        }
        while (true) {
            val firstDescriptorInstalled = array[firstIndex].compareAndSet(firstExpected, this)
            if (firstDescriptorInstalled) break
            val desc = array[firstIndex].value as? CAS2Descriptor<E> ?: return false
            if(!desc.apply()) continue
            break
        }
        if (array[secondIndex].compareAndSet(secondExpected, this)) {
            status.compareAndSet(UNDECIDED, SUCCESS)
        ...
        nextDescriptor.apply()
        return nextDescriptor.status.value == SUCCESS
    }
