    ...
    inner class CAS2Descriptor(
        ...
        fun apply() {
            ...
            if (cell1 is AtomicArrayWithCAS2Simplified<*>.CAS2Descriptor && cell1.status.value == UNDECIDED){
                if (array[index2].compareAndSet(cell1.expected2, this)) {
                    array[index1].compareAndSet(cell1, cell1.update1)
                    array[index2].compareAndSet(this, cell1.update2)
                    cell1.status.value = SUCCESS
                }
            }
            if (cell2 is AtomicArrayWithCAS2Simplified<*>.CAS2Descriptor && cell2.status.value == UNDECIDED){
                if(array[index1].compareAndSet(cell2.expected1, this)) {
                    array[index1].compareAndSet(this, cell2.update1)
                    array[index2].compareAndSet(cell2, cell2.update2)
