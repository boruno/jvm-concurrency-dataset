    ...
    inner class CAS2Descriptor(
        ...
        fun apply() {
            ...
            if (status.value == UNDECIDED) {
                updateStatus(installDescriptor(index1, expected1) && installDescriptor(index2, expected2))
            }
            updateCells()
        }
        ...
        private fun installDescriptor(idx: Int, exp: E): Boolean {
            while (true) {
                val cell = array[idx].value
                if (cell is AtomicArrayWithCAS2Simplified<*>.CAS2Descriptor) {
                    if (cell != this) { // foreign descriptor
                        cell.apply()
                        continue
                    }
                    return true // our descriptor
                }
                val installed = array[idx].compareAndSet(exp, this)
                if (!installed && array[idx].value != this) { continue }
                return installed 
            }
        }
...
//        private fun installDescriptor(idx: Int, exp: E): Boolean {
//            while (true) {
//                if (!array[idx].compareAndSet(exp, this)) {
//                    val cell = array[idx].value
//                    if (cell is AtomicArrayWithCAS2Simplified<*>.CAS2Descriptor) {
//                        if (cell != this) {
//                            cell.apply()
...
//                }
//                val installed = array[idx].compareAndSet(exp, this)
//                if (installed || array[idx].value == this) { return true }
//            }
//        }
