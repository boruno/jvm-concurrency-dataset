...
class AtomicArrayWithCAS2Simplified<E : Any>(size: Int, initialValue: E) {
    ...
    inner class CAS2Descriptor(
        ...
        private fun helpOthers() {
            while (true) {
                val otherDescriptor1 = array[index1].value
                if (otherDescriptor1 is AtomicArrayWithCAS2Simplified<*>.CAS2Descriptor) {
                    when (otherDescriptor1.status.value) {
                        Status.UNDECIDED -> Unit
                        Status.SUCCESS, Status.FAILED -> {
                            otherDescriptor1.tryApplyValues()
                            continue
                        }
                    }
                }

                val otherDescriptor2 = array[index2].value
                if (otherDescriptor2 is AtomicArrayWithCAS2Simplified<*>.CAS2Descriptor) {
                    when (otherDescriptor2.status.value) {
                        Status.UNDECIDED -> Unit
                        Status.SUCCESS, Status.FAILED -> {
                            otherDescriptor2.tryApplyValues()
                            continue
                        }
                    }
                }
        ...
        private fun tryApplyValues(): Boolean =
            when (status.value) {
                Status.SUCCESS -> {
