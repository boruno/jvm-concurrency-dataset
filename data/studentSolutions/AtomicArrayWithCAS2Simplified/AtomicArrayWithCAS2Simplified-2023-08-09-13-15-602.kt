...
class AtomicArrayWithCAS2Simplified<E : Any>(size: Int, initialValue: E) {
    ...
    inner class CAS2Descriptor(
        ...
        private fun helpToProcessDescriptor(anotherDescriptor: CAS2Descriptor): Boolean {
            ...
            if (!array.compareAndSet(indexA, expectedA, anotherDescriptor))
                if (array.get(indexA) != anotherDescriptor) {
                    anotherDescriptor.status.compareAndSet(UNDECIDED, FAILED)
                    return false
                }
            if (!array.compareAndSet(indexB, expectedB, anotherDescriptor))
                if (array.get(indexB) != anotherDescriptor) {
                    anotherDescriptor.status.compareAndSet(UNDECIDED, FAILED)
                    return false
                }

            if (anotherDescriptor.status.compareAndSet(UNDECIDED, SUCCESS) || anotherDescriptor.status.get() == SUCCESS) {
