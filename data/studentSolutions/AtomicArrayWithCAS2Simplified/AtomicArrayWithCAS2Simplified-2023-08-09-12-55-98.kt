...
class AtomicArrayWithCAS2Simplified<E : Any>(size: Int, initialValue: E) {
    ...
    inner class CAS2Descriptor(
        ...
        private fun installDescriptor(idx: Int) : Boolean {
            while (true) {
                if (!array.compareAndSet(idx, getExpectedValue(idx), this)) {
                    val curValue = array[idx]
                    if (curValue is AtomicArrayWithCAS2Simplified<*>.CAS2Descriptor && curValue.status.get() == UNDECIDED) {
                        val anotherIdx = if (idx == curValue.index1) curValue.index2 else curValue.index1
                        val anotherCell = array[anotherIdx]
                        if (anotherCell == curValue || curValue.installDescriptor(anotherIdx)) {
                            curValue.status.compareAndSet(UNDECIDED, SUCCESS)
                            curValue.finish()
                        } else {
                            return false
                        }
