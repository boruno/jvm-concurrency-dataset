...
class AtomicArrayWithCAS2Simplified<E : Any>(size: Int, initialValue: E) {
    ...
    fun get(index: Int): E {
        ...
        while (true) {
            ...
            when {
                value !is AtomicArrayWithCAS2Simplified<*>.CAS2Descriptor -> return value as E
                else -> value.apply()
            }
        }
    }
    ...
    inner class CAS2Descriptor(
        ...
        fun read(isSuccess: Boolean, index: Int): E = when (isSuccess) {
            true -> if (index == index1) update1 else update2
        ...
        fun apply(visited: MutableSet<AtomicArrayWithCAS2Simplified<*>.CAS2Descriptor> = hashSetOf()) {
            val isSuccess = installDescriptor(
                index1,
                expected1,
                visited,
            ) && installDescriptor(
            ...
            updateCell(index1, read(isSuccess, index1))
            updateCell(index2, read(isSuccess, index2))
        ...
        fun installDescriptor(
            index: Int,
            expected: E,
            visited: MutableSet<AtomicArrayWithCAS2Simplified<*>.CAS2Descriptor>
        ): Boolean {
            val sell = array[index]
