    ...
    inner class CAS2Descriptor(
        ...
        private fun installCell(index: Int, expected: E) {
            while (true) {
                ...
                val curValue = array[index].value
                when {
                    curValue == this -> return
                    curValue is AtomicArrayWithCAS2Simplified<*>.CAS2Descriptor -> curValue.apply()
                    curValue == expected && status.value == UNDECIDED-> if (array[index].compareAndSet(expected, this)) return else continue
                    curValue != expected -> return
                }
            }
        }
