...
class AtomicArrayWithCAS2Simplified<E : Any>(size: Int, initialValue: E) {
    ...
    inner class CAS2Descriptor(
        ...
        private fun updateStatus() {
            val firstOk = checkIndex(minOf(index1, index2))
            val secondOk = checkIndex(maxOf(index1, index2))
            val newStatus = if (firstOk && secondOk) SUCCESS else FAILED
        ...
        private fun checkIndex(i: Int): Boolean {
            when (val actual = array[i].value) {
                this -> return true
                is AtomicArrayWithCAS2SingleWriter<*>.CAS2Descriptor -> actual.apply()
            }
            return false
        }
