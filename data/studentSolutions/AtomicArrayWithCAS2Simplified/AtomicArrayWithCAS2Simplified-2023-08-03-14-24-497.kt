    ...
    inner class CAS2Descriptor(
        ...
        private fun installDescriptor() {
            while (true) {
                ...
                when {
                    current1 == expected1 -> array[index1].compareAndSet(expected1, this)
                    current1 is AtomicArrayWithCAS2Simplified<*>.CAS2Descriptor -> current1.apply()
                    else -> shouldBreak = true
                }
                when {
                    current2 == expected2 -> array[index2].compareAndSet(expected2, this)
                    current2 is AtomicArrayWithCAS2Simplified<*>.CAS2Descriptor -> current2.apply()
                    else -> shouldBreak = true
                }
