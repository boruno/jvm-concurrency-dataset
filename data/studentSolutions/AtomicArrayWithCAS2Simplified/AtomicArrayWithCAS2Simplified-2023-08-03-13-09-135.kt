    ...
    inner class CAS2Descriptor(
        ...
        private fun install(): Boolean {
            while (true) {
                ...
                if (array[index1].compareAndSet(expected1, this) || curValue1 == this) {
                    if (array[index2].compareAndSet(expected2, this)  || curValue2 == this) {
                        return true
                    } else if (curValue2 is AtomicArrayWithCAS2Simplified<*>.CAS2Descriptor) {
                        if (curValue2.status.value == SUCCESS) {
                            curValue2.updatePhysically()
                        } else {
                            curValue2.apply()
                        }
                        continue
                    }
                } else if (curValue1 is AtomicArrayWithCAS2Simplified<*>.CAS2Descriptor)  {
                    if (curValue1.status.value == SUCCESS) {
                        curValue1.updatePhysically()
                    } else {
                        curValue1.apply()
                    }
                    continue
