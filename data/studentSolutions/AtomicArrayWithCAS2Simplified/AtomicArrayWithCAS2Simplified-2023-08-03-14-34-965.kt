...
//        }
        ...
        fun apply() {
            ...
            if (cell1 is AtomicArrayWithCAS2Simplified<*>.CAS2Descriptor && cell1.status.value == UNDECIDED){ //todo install descriptor ?
...
//                }
            ...
            if (!array[index1].compareAndSet(expected1, this)) { // here it could be decriptor - todo
                status.value = FAILED
                return
            }
            if (!array[index2].compareAndSet(expected2, this)) { // here it could be decriptor - todo
                array[index1].compareAndSet(this, expected1)
                status.value = FAILED
                return
            }
            if (status.value != SUCCESS){ // check if someone already helped
                status.value = SUCCESS
                updateCells()
            }
