        ...
        fun apply() {
            ...
            while (true) {
                ...
                when (currentStatus) {
                    UNDECIDED -> {
                        if (cell1Value == this) {
                            status.compareAndSet(UNDECIDED, A_SUCCESS)
                        } else if (cell1.compareAndSet(expected1, this)) {
                            status.compareAndSet(UNDECIDED, A_SUCCESS)
                        } else {
                    ...
                    A_SUCCESS -> {
                        if (cell2Value == this) {
                            status.compareAndSet(A_SUCCESS, SUCCESS)
                        } else if (cell2.compareAndSet(expected2, this)) {
                            status.compareAndSet(A_SUCCESS, SUCCESS)
                        } else {
                            status.compareAndSet(UNDECIDED, FAILED)
                        }
