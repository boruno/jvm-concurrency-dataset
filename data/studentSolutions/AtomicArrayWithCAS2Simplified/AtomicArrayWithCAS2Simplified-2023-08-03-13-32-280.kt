        ...
        private fun installDescriptor(): CAS2Descriptor {
            while (true) {
                if (array[index1].compareAndSet(expected1, this)) {
                    if (array[index2].compareAndSet(expected2, this)) return this
                    val currentValue2 = array[index2].value
                    if (currentValue2 != this && currentValue2 is AtomicArrayWithCAS2Simplified<*>.CAS2Descriptor && currentValue2.status.value != SUCCESS) {
                        currentValue2.apply()
                    }
                } else {
                    val currentValue1 = array[index1].value
                    if (currentValue1 != this && currentValue1 is AtomicArrayWithCAS2Simplified<*>.CAS2Descriptor && currentValue1.status.value != SUCCESS) {
                        currentValue1.apply()
                    }
                }
            }
