...
class AtomicArrayWithCAS2Simplified<E : Any>(size: Int, initialValue: E) {
    ...
    inner class CAS2Descriptor(
        ...
        fun apply() {
            while(true) {
                if (this.isUndecided()) {
                    val helpProvided1 = !tryInstall(index1, expected1)
                    val helpProvided2 = !tryInstall(index1, expected2)
                    if (helpProvided1 || helpProvided2)
                        continue
                    updateStatus()
                }
                updateCells()
        ...
        private fun tryInstall(index: Int, expected: E): Boolean {
            if (array[index].compareAndSet(expected, this))
                return true
            return !provideHelp(index)
        }
