        ...
        private fun applyLogically() {
            ...
            while (true) {
                if (!array.compareAndSet(index1, expected1, this)) {
                    var desc = array.get(index1) as? AtomicArrayWithCAS2Simplified<*>.CAS2Descriptor
                    if (desc != null && desc != this) {
                        desc.apply()
                        continue
                    }
                    desc = array.get(index2) as? AtomicArrayWithCAS2Simplified<*>.CAS2Descriptor
                    if (desc != null && desc != this) {
                        desc.apply()
                        continue
                    }
                }
                array.compareAndSet(index2, expected2, update2)
                break
            }
