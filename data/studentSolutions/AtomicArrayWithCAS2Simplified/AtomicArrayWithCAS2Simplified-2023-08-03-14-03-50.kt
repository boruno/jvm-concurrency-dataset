...
class AtomicArrayWithCAS2Simplified<E : Any>(size: Int, initialValue: E) {
    ...
    inner class CAS2Descriptor(
        ...
        private val Any?.isDescriptor
            get() = this is AtomicArrayWithCAS2Simplified<*>.CAS2Descriptor
        ...
        private fun installDescriptor() {
            while (true) {
                ...
                if (!array[index1].compareAndSet(expected1, this)) {
                    ...
                    if (current1 != this) success = false
                    if (current1.isDescriptor) {
                        (current1 as AtomicArrayWithCAS2Simplified<*>.CAS2Descriptor).apply()
                        continue
                    }
                    break
                }
                if (!array[index2].compareAndSet(expected2, this)) {
                    ...
                    if (current2 != this) success = false
                    if (current2.isDescriptor) {
                        (current2 as AtomicArrayWithCAS2Simplified<*>.CAS2Descriptor).apply()
                        continue
                    }
                    break
                }
