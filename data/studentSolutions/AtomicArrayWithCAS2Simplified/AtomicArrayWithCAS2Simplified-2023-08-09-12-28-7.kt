...
class AtomicArrayWithCAS2Simplified<E : Any>(size: Int, initialValue: E) {
    ...
    inner class CAS2Descriptor(
            ...
            when(val value = array[cellIndex]) {
                is AtomicArrayWithCAS2Simplified<*>.CAS2Descriptor -> {
                    ...
                    when (value.status.get()) {
                        SUCCESS -> value.settleUpdate()
                        UNDECIDED -> {
                            val installed = if (cellIndex == index1) {
                                installForeignCell(index1, value) // index 1 is equal to some index in foreign descriptor
                                // install index2 cell
                            } else {
                                installForeignCell(index2, value) // index 2 is equal to some index in foreign descriptor
                                // install index1 cell
                            }
                            value.updateStatus(installed)
                            value.updateCells()
                            // continue from update status
                        }
                        FAILED -> value.restoreState()
                        else -> {}
                    }
        ...
        private fun helpFromInstallPhase(
        ...
        private fun installForeignCell(
            myIndex: Int,
            foreignDescriptor: AtomicArrayWithCAS2Simplified<*>.CAS2Descriptor
        ): Boolean = if (foreignDescriptor.index1 == myIndex) { // find cell equal to my index and update another
