    ...
    inner class CAS2Descriptor(
        ...
        fun apply() {
            ...
            while (status.value != SUCCESS) {
                when (val curValueF = array[indexF].value) {
                    this -> break
                    ...
                    expectedF -> {
                        if (array[indexF].compareAndSet(curValueF, this)) {
                            break
                        }
                    }
            ...
            while (status.value != SUCCESS && !array[indexS].compareAndSet(expectedS, this)) {
                when (val curValueS = array[indexS].value) {
                    this -> break
                    is AtomicArrayWithCAS2Simplified<*>.CAS2Descriptor -> {
                        curValueS.apply()
                        continue
                    ...
                    expectedS -> {
                        if (array[indexS].compareAndSet(curValueS, this)) {
                            break
                        }
                    }
