...
class AtomicArrayWithCAS2Simplified<E : Any>(size: Int, initialValue: E) {
    ...
    inner class CAS2Descriptor(
        ...
        private fun applyLogically() {
            ...
            install(index1)
            install(index2)
            status.compareAndSet(UNDECIDED, if (bothEq(index1, index2, this)) SUCCESS else FAILED)
        }
        ...
        private fun install(index: Int) {
            while (true) {
                ...
                array.compareAndSet(index, getExpected(index), getUpdate(index))
                return
            }
        }

        private fun getExpected(index: Int) = if (index == index1) expected1 else expected2

        private fun getUpdate(index: Int) = if (index == index1) update1 else update2
