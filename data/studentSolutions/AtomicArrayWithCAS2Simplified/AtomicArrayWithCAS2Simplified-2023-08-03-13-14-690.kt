...
class AtomicArrayWithCAS2Simplified<E : Any>(size: Int, initialValue: E) {
    ...
    inner class CAS2Descriptor(
        ...
        private fun installDescriptor(): Boolean =
            (array[index1].compareAndSet(expected1, this) || helpIndex(index1)) && (array[index2].compareAndSet(expected1, this) || helpIndex(index2))

        private fun helpIndex(i: Int): Boolean =
            when (val actual = array[i].value) {
                this -> true
                is AtomicArrayWithCAS2Simplified<*>.CAS2Descriptor -> {
                    actual.isUndecided() && run {
                        actual.apply()
                        installDescriptor()
                    }
                }
        ...
        private fun updateStatus() {
            val success = checkIndex(index1) && checkIndex(index2)
