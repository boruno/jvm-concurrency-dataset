@file:Suppress("DuplicatedCode", "UNCHECKED_CAST")

//package day3

import day3.AtomicArrayWithCAS2Simplified.Status.*
import java.util.concurrent.atomic.AtomicReference
import java.util.concurrent.atomic.AtomicReferenceArray


// This implementation never stores `null` values.
class AtomicArrayWithCAS2Simplified<E : Any>(size: Int, initialValue: E) {
    private val array = AtomicReferenceArray<Any?>(size)

    init {
        // Fill array with the initial value.
        for (i in 0 until size) {
            array[i] = initialValue
        }
    }

    fun get(index: Int): E {
        // TODO: the cell can store CAS2Descriptor

        return when (val value = array[index]) {
            is AtomicArrayWithCAS2Simplified<*>.CAS2Descriptor -> when (value.status.get()) {
                SUCCESS -> value.updateForIndex(index)
                else -> value.expectedForIndex(index)
            }

            else -> value
        } as E
    }

    fun cas2(
        index1: Int, expected1: E, update1: E,
        index2: Int, expected2: E, update2: E
    ): Boolean {
        require(index1 != index2) { "The indices should be different" }
        val descriptor = CAS2Descriptor(
            index1 = index1, expected1 = expected1, update1 = update1,
            index2 = index2, expected2 = expected2, update2 = update2
        )
        descriptor.apply()
        return descriptor.status.get() === SUCCESS
    }

    inner class CAS2Descriptor(
        private val index1: Int,
        private val expected1: E,
        private val update1: E,
        private val index2: Int,
        private val expected2: E,
        private val update2: E
    ) {
        val status = AtomicReference(UNDECIDED)

        fun updateForIndex(index: Int): E = if (index == index1) update1 else update2
        fun expectedForIndex(index: Int): E = if (index == index1) expected1 else expected2

        fun apply() {
            // TODO: Install the descriptor, update the status, and update the cells;
            // TODO: create functions for each of these three phases.

            if (install()) {
                statusSuccess()
                updateCells()
            } else {
                statusFailed()
            }
        }

        private fun install(): Boolean {
            while (true) {
                if (array.compareAndSet(index1, expected1, this)) {
                    return if (array.compareAndSet(index2, expected2, this)) {
                        true
                    } else {
                        array.set(index1, expected1)
                        false
                    }
                } else {
                    if (!tryHelpForeignDescriptor(index1)) {
                        return false
                    }
                }
            }
        }

        private fun statusSuccess() {
            status.set(SUCCESS)
        }

        private fun statusFailed() {
            status.set(FAILED)
        }

        private fun updateCells() {
            array.set(index1, update1)
            array.set(index2, update2)
        }

        private fun tryHelpForeignDescriptor(index: Int): Boolean {
            val value = array.get(index)
            if (value is AtomicArrayWithCAS2Simplified<*>.CAS2Descriptor) {
                val success = if (index == value.index1) {
                    array.compareAndSet(value.index2, value.expected2, value.update2)
                } else {
                    array.compareAndSet(value.index1, value.expected1, value.update1)
                }
                value.status.compareAndSet(UNDECIDED, if (success) SUCCESS else FAILED)
                return true
            }
            return false
        }

    }

    enum class Status {
        UNDECIDED, SUCCESS, FAILED
    }
}