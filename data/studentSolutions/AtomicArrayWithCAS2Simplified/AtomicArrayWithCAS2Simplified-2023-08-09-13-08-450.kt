...
class AtomicArrayWithCAS2Simplified<E : Any>(size: Int, initialValue: E) {
    ...
    inner class CAS2Descriptor(
        ...
        private fun installDescriptor(idx: Int, exp: Any): Boolean {
            if (array.compareAndSet(idx, exp, this)) {
                ...
                if (val1 === this) {
                    ...
                    if (val1 is AtomicArrayWithCAS2Simplified<*>.CAS2Descriptor) {
                        ...
                        if (val1.status.get() == UNDECIDED) {
                            if (array.get(val1.index1) == array.get(val1.index2)) {
                                // success other descriptor
                                val1.updateStatus(SUCCESS)
                                val1.updateCell()
                            } else if (index1 == val1.index2 && idx < index1) {
                                // help other desc
                                array.compareAndSet(index1, this, expected1)
                                val1.apply()
                            } else {
                                // fail other descriptor
                        ...
                        /*
                            when (val1.status.get()) {
                                UNDECIDED -> {
                                    if (index1 == val1.index2 && idx < index1) {
                                        // help other desc
                                        array.compareAndSet(index1, this, expected1)
                                    }
                                    val1.apply()
