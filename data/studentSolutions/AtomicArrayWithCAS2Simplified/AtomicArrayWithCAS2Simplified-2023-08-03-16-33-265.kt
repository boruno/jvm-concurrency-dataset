...
class AtomicArrayWithCAS2Simplified<E : Any>(size: Int, initialValue: E) {
    ...
    inner class CAS2Descriptor(
        ...
        fun apply() {
            val idx1 = install(index1, expected1)
            val idx2 = install(index2, expected2)
            setStatus(idx1, idx2)
            cleanup()
        }
        ...
        fun install(index: Int, expected: E): Boolean {
            while (true) {
                when (val value = array[index1].value) {
                    this -> return true
                    is AtomicArrayWithCAS2Simplified<*>.CAS2Descriptor -> value.apply()
                    expected -> if (array[index1].compareAndSet(expected1, this)) return true
                    else -> return false
                }
            }
        }
