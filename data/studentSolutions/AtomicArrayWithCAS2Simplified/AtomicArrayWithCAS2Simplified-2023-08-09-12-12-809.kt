        ...
        private fun installDescriptor(): Boolean {
            if (array.compareAndSet(index1, expected1, this)) {
                if (array.compareAndSet(index2, expected2, this)) {
                    ...
                    val index2Value = array.get(index2)
                    if (index2Value is AtomicArrayWithCAS2SingleWriter<*>.CAS2Descriptor) {
                        index2Value.apply()
                        return true
                    } else {
                        return false
                ...
                val index1Value = array.get(index1)
                if (index1Value is AtomicArrayWithCAS2SingleWriter<*>.CAS2Descriptor) {
                    index1Value.apply()
                    return true
                } else {
                    return false
