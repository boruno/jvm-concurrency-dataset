...
class AtomicArrayWithCAS2Simplified<E : Any>(size: Int, initialValue: E) {
    ...
    inner class CAS2Descriptor(
        ...
        private fun tryHelp(index: Int): Boolean {
            val valueDescriptor = array[index].value

            if (valueDescriptor == this)
                return false

            if (valueDescriptor is AtomicArrayWithCAS2Simplified<*>.CAS2Descriptor) {
                return if (valueDescriptor.status.value == Status.UNDECIDED) {
                    valueDescriptor.apply()
                    true
                } else
                    false
