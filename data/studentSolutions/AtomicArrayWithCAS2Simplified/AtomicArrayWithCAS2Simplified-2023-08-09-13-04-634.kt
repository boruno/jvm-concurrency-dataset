...
class AtomicArrayWithCAS2Simplified<E : Any>(size: Int, initialValue: E) {
    ...
    inner class CAS2Descriptor(
        ...
        private fun installCellDescriptor(i: Int): Boolean {
            val cellExpected = if (i == index1) expected1 else expected2
            val cellUpdated = if (i == index1) update1 else update2

            while (true) {
                val result = array.compareAndSet(i, cellExpected, cellUpdated)
                if (result) {
                    return true
                }
