    ...
    inner class CAS2Descriptor(
        ...
        private fun install(): Status {
            while (true) {
                ...
                if (element1 is AtomicArrayWithCAS2Simplified<*>.CAS2Descriptor) {
                    if (array[index2].compareAndSet(element1.expected2, element1)) {
                        break
                    }
                    if (array[index2].value == element1 && element1 != this) continue
                }
                else break
            }
            if ((array[index1].value is AtomicArrayWithCAS2Simplified<*>.CAS2Descriptor || array[index1].compareAndSet(
                    expected1,
                    this
                ))
                && array[index2].value is AtomicArrayWithCAS2Simplified<*>.CAS2Descriptor || array[index2].compareAndSet(
                    expected2,
                    this
                )
            ) {
