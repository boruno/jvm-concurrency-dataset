            ...
            while (true) {
                ...
                if (cell1 is AtomicArrayWithCAS2Simplified<*>.CAS2Descriptor) {
                    if (cell1 != this) { // foreign descriptor -> help
                        cell1.apply()
                        continue
                    } else { // our descriptor -> installed!

                    }
                } else { // try to install the descriptor
                    if (!array[index1].compareAndSet(expected1, this)) return false // Failure to install
                } // if we are here, the first descriptor has been installed
