...
class AtomicArrayWithCAS2Simplified<E : Any>(size: Int, initialValue: E) {
    ...
    inner class CAS2Descriptor(
        ...
        private fun install1(): Boolean {
            return matchesExpected(index1, expected1) && array[index1].compareAndSet(expected1, this)
        }

        private fun install2(): Boolean {
            return matchesExpected(index2, expected2) && array[index2].compareAndSet(expected2, this)
        }
        ...
        private fun matchesExpected(index: Int, expected: E): Boolean {
            val valueOrDescriptor = array[index].value
            if (valueOrDescriptor is AtomicArrayWithCAS2Simplified<*>.CAS2Descriptor) {
                valueOrDescriptor.apply()
                return valueOrDescriptor.getValue(index) === expected
            }
            return valueOrDescriptor === expected
        }
    }
