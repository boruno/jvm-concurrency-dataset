...
class AtomicArrayWithCAS2Simplified<E : Any>(size: Int, initialValue: E) {
    ...
    inner class CAS2Descriptor(
        ...
        fun apply() {
            ...
            var curValueF = array[indexF].value
            while (status.value != SUCCESS && !array[indexF].compareAndSet(expectedF, this)) {
                if (curValueF == this) {
                    break
                }
                if (curValueF is AtomicArrayWithCAS2Simplified<*>.CAS2Descriptor) {
                    curValueF.apply()
                    curValueF = array[indexF].value
                    continue
                }
            ...
            var curValueS = array[indexS].value
            while (status.value != SUCCESS && !array[indexS].compareAndSet(expectedS, this)) {
                if (curValueS == this) {
                    break
                }
                if (curValueS is AtomicArrayWithCAS2Simplified<*>.CAS2Descriptor) {
                    curValueS.apply()
                    curValueS = array[indexS].value
                    continue
                }
                if (!status.compareAndSet(UNDECIDED, FAILED) && status.value != FAILED) {
                    break
