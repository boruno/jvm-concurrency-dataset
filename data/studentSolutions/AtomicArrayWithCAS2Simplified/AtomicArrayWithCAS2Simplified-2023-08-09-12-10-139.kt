...
class AtomicArrayWithCAS2Simplified<E : Any>(size: Int, initialValue: E) {
    ...
    inner class CAS2Descriptor(
        ...
        private fun installDescriptor(): Boolean {
            if (array.compareAndSet(index1, expected1, this)) {
                if (array.compareAndSet(index2, expected2, this)) {
                    ...
                    val unexpected2 = array.get(index2)
                    if (unexpected2 is AtomicArrayWithCAS2Simplified<*>.CAS2Descriptor) {
                        val unexpectedDescriptor = unexpected2 as AtomicArrayWithCAS2Simplified<E>.CAS2Descriptor
                        if (unexpectedDescriptor == this) return true
                ...
                val unexpected1 = array.get(index2)
                if (unexpected1 is AtomicArrayWithCAS2Simplified<*>.CAS2Descriptor) {
                    val unexpectedDescriptor = unexpected1 as AtomicArrayWithCAS2Simplified<E>.CAS2Descriptor
                    if (unexpectedDescriptor == this) return true
                    unexpectedDescriptor.apply()
                    return installDescriptor()
                } else {
        ...
        fun applyLogically(installed: Boolean): Boolean {
            return when {
