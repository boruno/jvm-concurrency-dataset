        ...
        return when (val value = array[index].value) {
            is AtomicArrayWithCAS2SingleWriter<*>.CAS2Descriptor -> {
                if (value.status.value == AtomicArrayWithCAS2SingleWriter.Status.SUCCESS) {
                    if (index == value.index1) return value.update1 as? E ?: error("kek")
                    else return value.update2 as? E ?: error("kek")
                } else {
                    if (index == value.index1) return value.expected1 as? E ?: error("kek")
                    else return value.expected2 as? E ?: error("kek")
                }
            }
            else -> {
                value as? E ?: error("kek")
            }
        }
    }
