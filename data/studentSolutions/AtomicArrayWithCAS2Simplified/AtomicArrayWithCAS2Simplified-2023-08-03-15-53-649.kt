...
//            if (installCell(index1, expected1)) installCell(index2, expected2)
        ...
        private fun installCell(index: Int, expected: E): Boolean {
            ...
            while (true) {
                when (val curValue = array[index].value) {
                    this -> return true
                    is AtomicArrayWithCAS2Simplified<*>.CAS2Descriptor -> curValue.apply()
                    expected -> if (array[index].compareAndSet(expected, this)) return true else continue
                    else -> return false
                }
            }
        }
