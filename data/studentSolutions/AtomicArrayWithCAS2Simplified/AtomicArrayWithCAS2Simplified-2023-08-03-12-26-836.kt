...
//        }
        ...
        fun apply() {
            ...
            while (true) {
                when (old1) {
                    is AtomicArrayWithCAS2SingleWriter<*>.CAS2Descriptor -> old1.apply()
                }
                when (old2) {
                    is AtomicArrayWithCAS2SingleWriter<*>.CAS2Descriptor -> old2.apply()
                }
                old1 = array[index1].value
                old2 = array[index2].value

                if (old1 is AtomicArrayWithCAS2SingleWriter<*>.CAS2Descriptor ||
                    old2 is AtomicArrayWithCAS2SingleWriter<*>.CAS2Descriptor) { continue }

                if (!array[index1].compareAndSet(expected1, this)) {
                    if (array[index1].value is AtomicArrayWithCAS2SingleWriter<*>.CAS2Descriptor) { continue }
                ...
                if (!array[index2].compareAndSet(expected2, this)) {
                    if (array[index2].value is AtomicArrayWithCAS2SingleWriter<*>.CAS2Descriptor) {
                        continue
                    }
