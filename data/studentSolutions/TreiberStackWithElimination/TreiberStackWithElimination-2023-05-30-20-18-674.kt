...
class TreiberStackWithElimination<E> : Stack<E> {
    ...
    private fun tryPushElimination(element: E): Boolean {
        ...
        if (current.compareAndSet(CELL_STATE_EMPTY, element)){
            repeat (ELIMINATION_WAIT_CYCLES){}
        }

        if (current.compareAndSet(CELL_STATE_RETRIEVED, CELL_STATE_EMPTY))  {
            return true
        }
    ...
    private fun tryPopElimination(): E? {
        ...
        val index = randomCellIndex()
        val current = eliminationArray[index]
        val curValue = eliminationArray[index].value

        if (current.compareAndSet(CELL_STATE_EMPTY, CELL_STATE_RETRIEVED)){
            return stack.pop()
        }
