    ...
    fun push(x: E) {
        ...
        if (top.compareAndSet(node.next, node)) {
            ...
            for (i in 0 until ELIMINATION_ARRAY_SIZE) {
                println((eliminationArray[i].value as Node<E>).x)
            }

            var him = eliminationArray[random % ELIMINATION_ARRAY_SIZE].value
            while (!eliminationArray[random % ELIMINATION_ARRAY_SIZE].compareAndSet(him, Node(x, top.value))) {
            ...
            if (him != null) {
                while (true) {
                    val node2 = Node((him as Node<E>).x, top.value)
