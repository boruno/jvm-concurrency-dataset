...
class TreiberStackWithElimination<E> {
    ...
    fun pop(): E? {
        val elimIndex = ThreadLocalRandom.current().nextInt(ELIMINATION_ARRAY_SIZE)
        while (true) {
            val value = eliminationArray[elimIndex]
            val vall = value.value
            if (vall != null && eliminationArray[elimIndex].compareAndSet(vall, null)) {
                return vall as E
            }

//            if (!eliminationArray[elimIndex].compareAndSet(null, null)) {
//                val value = eliminationArray[elimIndex].value
//                if (value != null && eliminationArray[elimIndex].compareAndSet(value, null)) {
//                    return value as E
//                }
//            }
            val curTop = top.value ?: return null
