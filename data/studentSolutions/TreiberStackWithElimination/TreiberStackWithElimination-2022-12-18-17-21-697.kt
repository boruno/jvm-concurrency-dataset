...
class TreiberStackWithElimination<E> {
    ...
    fun push(x: E) {
        if (!putAndWait(x)) {
            while (true) {
                if (top.compareAndSet(top.value, Node(x, top.value))) {
                    return
                }
            }
        }
    ...
    fun pop(): E? {
        for (i in 0 until ELIMINATION_ARRAY_SIZE) {
            if (eliminationArray[i].value == null || eliminationArray[i].value == done) {
                continue
            }

            if (eliminationArray[i].compareAndSet(eliminationArray[i].value, done)) {
                return (eliminationArray[i].value as Node<E>).x
            }
        }
