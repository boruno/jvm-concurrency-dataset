...
class TreiberStackWithElimination<E> {
    ...
    fun pop(): E? {
        for (i in ELIMINATION_TRY_FIND_CYCLES downTo 0) {
            ...
            {
                val result = eliminationArray[eliminationIndex].value ?: break;
                if (eliminationArray[eliminationIndex].compareAndSet(result, Operation(result.x, true)))
                {
                    return result.x;
                }
                else
...
private const val ELIMINATION_TRY_FIND_CYCLES = 4