...
class TreiberStackWithElimination<E> {
    ...
    private fun tryToEliminatePush(x: E): Boolean {
        ...
        if (eliminationArray[index].value != null) {
            val index2 = index + 1 % ELIMINATION_ARRAY_SIZE
            if (eliminationArray[index2].value != null) {
                return false
            }
        }

        val push_value = eliminationArray[index].value
        if (eliminationArray[index].compareAndSet(push_value, x)) {
    ...
    private fun tryToEliminatePop(): E? {
        ...
        if (eliminationArray[index].value != null) {
            ...
            if (eliminationArray[index].compareAndSet(pop_result, null)) {
                return pop_result
            } else {
                return null
            }
