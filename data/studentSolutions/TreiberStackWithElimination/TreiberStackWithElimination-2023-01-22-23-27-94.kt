...
class TreiberStackWithElimination<E> {
     ...
     */
    fun push(x: E) {
        val rand: Int = ThreadLocalRandom.current().nextInt(ELIMINATION_SIZE)
        for (i in max(0, rand - SEARCHING_RANGE) until min(ELIMINATION_SIZE, rand + SEARCHING_RANGE)) {
     ...
     */
    fun pop(): E? {
        val rand = ThreadLocalRandom.current().nextInt(ELIMINATION_SIZE)
        for (i in max(0, rand - SEARCHING_RANGE) until min(ELIMINATION_SIZE, rand + SEARCHING_RANGE)) {
            val el = eliminationArray[i]
            val value = el.value
            if (value != null) {
