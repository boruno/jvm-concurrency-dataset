...
class TreiberStackWithElimination<E> {
    ...
    private fun eliminatePush(x: E): Boolean {
        ...
        for (i in 0..COUNT_RETRY) {
            ...
            if (oldElement == null) {
                if (eliminationArray[pos].compareAndSet(oldElement, x)) {
    ...
    private fun eliminatePop(): E? {
        ...
        for (i in 0..COUNT_RETRY) {
            oldElement = eliminationArray[pos].value as E?
            if (oldElement != null) {
                if (eliminationArray[pos].compareAndSet(oldElement, null)) {
                    return oldElement
                }
            }
            pos = random.nextInt(ELIMINATION_ARRAY_SIZE)
...
private const val COUNT_RETRY = ELIMINATION_ARRAY_SIZE * 3
