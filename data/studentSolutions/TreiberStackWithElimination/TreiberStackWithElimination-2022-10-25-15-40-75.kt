...
class TreiberStackWithElimination<E> {
    ...
    companion object {
        const val FIND_ATTEMPTS = 2
        const val WAIT_LOOP_ITERS = 100
    }
     ...
     */
    fun push(x: E) {
        if (!try_elimination_push(x)) base_push(x)
    }
    ...
    private fun try_elimination_pop() : E? {
        val index = getRandomIndex()
        for (attempt in 0 until FIND_ATTEMPTS) {
            val curIndex = (index + attempt) % ELIMINATION_ARRAY_SIZE
            var el = eliminationArray.get(curIndex)
            var value = el.getAndSet(null)
            if (value != null) {
                return value
            }
        }
