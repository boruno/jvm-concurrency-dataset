...
class TreiberStackWithElimination<E> {
    ...
    fun pop(): E? {
        ...
        val elim_top = eliminationArray[randInd].value

        if (elim_top?.state == NodeState.PUSHED && eliminationArray[randInd].compareAndSet(elim_top, poppedNode)) {
//        if (positionReference.value?.state == NodeState.PUSHED) {
//            val el = positionReference.getAndSet(poppedNode)
//            if (el?.element != null) {
                return elim_top.element
//            }
        }
