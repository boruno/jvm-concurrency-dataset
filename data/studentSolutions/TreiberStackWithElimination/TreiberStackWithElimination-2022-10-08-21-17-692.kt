        ...
        while(true)
        {
            ...
            else
            {
                for (i in ELIMINATION_TRY_FIND_CYCLES downTo 0) {
                    ...
                    {
                        val result_elimination = eliminationArray[eliminationIndex].value ?: break;
                        if (result_elimination.done)
                            break;
                        if (eliminationArray[eliminationIndex].compareAndSet(result_elimination, Operation(result_elimination.x, true)))
                        {
                            return result_elimination.x;
                        }
                        else
                            break;
                    }
