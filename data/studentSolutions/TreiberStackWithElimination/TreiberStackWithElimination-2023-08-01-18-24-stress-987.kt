...
//        for (i in 0 until waitCycles) {
            val cellValue = eliminationArray[randomIdx].value
            if (cellValue == CELL_STATE_EMPTY) {
                if (eliminationArray[randomIdx].compareAndSet(cellValue, element)) {
                    for (j in 0 until waitCycles) {
                        Thread.sleep(10)
                        val cellValueAfterWaiting = eliminationArray[randomIdx].value
                        if (cellValueAfterWaiting == CELL_STATE_RETRIEVED) {
                            eliminationArray[randomIdx].compareAndSet(CELL_STATE_RETRIEVED, CELL_STATE_EMPTY)
                            return true
                        }
                    }
                    eliminationArray[randomIdx].compareAndSet(element, CELL_STATE_EMPTY)
                    return false
                }
            }
//        }
