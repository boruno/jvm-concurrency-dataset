...
class TreiberStackWithElimination<E> : Stack<E> {
    ...
    private fun tryPushElimination(element: E): Boolean {
        ...
        if (atomicRef.compareAndSet(CELL_STATE_EMPTY, element)) return false
    ...
    private fun tryPopElimination(): E? {
        ...
        val result = currentCell.getAndUpdate { value ->
            if (value == CELL_STATE_EMPTY) {
                return@getAndUpdate CELL_STATE_EMPTY
            } else {
                CELL_STATE_RETRIEVED
            }
        }
        if (result == CELL_STATE_EMPTY || result == CELL_STATE_RETRIEVED) return null
        return result as E
    }
