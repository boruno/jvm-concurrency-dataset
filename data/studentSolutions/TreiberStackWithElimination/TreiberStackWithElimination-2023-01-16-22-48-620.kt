...
class TreiberStackWithElimination<E> {
    ...
    fun push(x: E) {
        ...
        if (top.compareAndSet(node.next, node)) {
            ...
            eliminationArray[random % ELIMINATION_ARRAY_SIZE].getAndSet(x) as E?

            for (i in 0..10) {
                if (eliminationArray[random % ELIMINATION_ARRAY_SIZE].compareAndSet(null, null)) {
                    return
                }
            }

            eliminationArray[random % ELIMINATION_ARRAY_SIZE].compareAndSet(x, null)
            while (true) {
