...
class TreiberStackWithElimination<E> {
     ...
     */
    fun push(x: E) {
        val el = 0 //random.nextInt(ELIMINATION_ARRAY_SIZE)
        ...
        if (eliminationState == 1) {
            for (j in 0 until ELIMINATION_CYCLE_COUNT) {
                if (eliminationArray[j].compareAndSet(null, null)) {
                    eliminationState = 2
                    break
                }
            }
    ...
    private object EliminationSuccess
//    private inner class EliminationValue(val e: E) : EliminationArrayElement()
}

private class Node<E>(val x: E, val next: Node<E>?)

private const val ELIMINATION_ARRAY_SIZE = 1 // DO NOT CHANGE IT
