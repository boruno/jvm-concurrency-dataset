...
class TreiberStackWithElimination<E> {
    ...
    fun push(x: E) {
        ...
        if (eliminationArray[ind].compareAndSet(null, x)) {
            for (i in 1..100) {
                if (eliminationArray[ind].value != x) return
            }
    ...
    fun pop(): E? {
        val ind = Random.nextInt(ELIMINATION_ARRAY_SIZE)
        val res = eliminationArray[ind].value as E?

        if (res != null && eliminationArray[ind].compareAndSet(res, Node(null, null))) return res
