...
class TreiberStackWithElimination<E> : Stack<E> {
    ...
    private fun tryPushElimination(element: E): Boolean {
        ...
        val item = eliminationArray[number]

        if (item.compareAndSet(null, element)) {
            var i = 0
            ...
            item.compareAndSet(element, null)
            return false
        }
...
//        item.compareAndSet(element, DONE)
//        if (element != null) return element as? E

        val element = item.getAndSet(DONE)

        if (element != DONE) return element as E?
//        if (element != DONE)
