...
open class TreiberStackWithElimination<E> : Stack<E> {
    ...
    private fun tryPopElimination(): E? {
        ...
        val randomIndex = randomCellIndex()
        val randomVal = eliminationArray.getAndUpdate(randomIndex) {
            if (it != CELL_STATE_RETRIEVED && it != CELL_STATE_EMPTY) {
                CELL_STATE_RETRIEVED
            } else it
        }
        val randomValue = eliminationArray.get(randomIndex)
        if (randomValue != CELL_STATE_RETRIEVED) {
            return randomValue as E?
        }
        return null
    }
