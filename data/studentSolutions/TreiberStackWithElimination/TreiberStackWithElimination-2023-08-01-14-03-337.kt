...
class TreiberStackWithElimination<E> : Stack<E> {
    ...
    private fun tryPushElimination(element: E): Boolean {
        ...
        val slot = randomCellIndex()
        if (!eliminationArray[slot].compareAndSet(CELL_STATE_EMPTY, element))
            return false
        ...
        while (numAttempt < ELIMINATION_WAIT_CYCLES) {
            if (eliminationArray[slot].compareAndSet(CELL_STATE_RETRIEVED, CELL_STATE_EMPTY))
                return true
            numAttempt++
        }
        eliminationArray[slot].compareAndSet(element, CELL_STATE_EMPTY)
        return false
    }
    ...
    private fun tryPopElimination(): E? {
        ...
        val slot = randomCellIndex()
        val value = eliminationArray[slot].value
        if (value == CELL_STATE_EMPTY || value == CELL_STATE_RETRIEVED)
            return null

        return if (eliminationArray[slot].compareAndSet(value, CELL_STATE_RETRIEVED))
            value as E
        else
            null
    }
