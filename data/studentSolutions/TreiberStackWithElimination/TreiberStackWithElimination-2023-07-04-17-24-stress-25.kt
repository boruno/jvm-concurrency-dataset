...
class TreiberStackWithElimination<E> : Stack<E> {
    ...
    private fun tryPushElimination(element: E): Boolean {
        ...
        val eliminationSucceeded = eliminationArray[cellIndex].value == CELL_STATE_RETRIEVED
        if (eliminationSucceeded) {
            eliminationArray[cellIndex].compareAndSet(CELL_STATE_RETRIEVED, CELL_STATE_EMPTY)
        }
        else {
            eliminationArray[cellIndex].compareAndSet(element, CELL_STATE_EMPTY)
        }
        return eliminationSucceeded
...
//        TODO("Implement me!")
    ...
    private fun tryPopElimination(): E? {
        ...
        val atomicRef = eliminationArray[randomCellIndex]
        val element = atomicRef.value
        if (element != CELL_STATE_EMPTY && element != CELL_STATE_RETRIEVED) {
            atomicRef.compareAndSet(element, CELL_STATE_RETRIEVED)
            return element as E?
        }
