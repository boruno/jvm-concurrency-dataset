...
class TreiberStackWithElimination<E> {
    ...
    private fun tryEliminate(type: String, value: E?): Pair<Boolean, E?> {
        ...
        var limitTime = System.nanoTime() + timeout
        while (System.nanoTime() < limitTime) {
            ...
            if (currentSlotValue == null) {
                if (eliminationArray[index].compareAndSet(null, Operation("wait", type, value))) {
                    while (System.nanoTime() < limitTime) {
                        val waitingValue = eliminationArray[index].value ?: break

                        val xyu = eliminationArray[((limitTime - System.nanoTime()).toInt())].value

                        if (waitingValue.status == "busy") {
                            val newValue = eliminationArray[index].getAndSet(null)
...
private class Operation<E>(val status: String, val type: String?, val x: E?) {
    override fun toString(): String {
        return "Status: $status, type: $type, value: $x, currentTime: ${System.nanoTime()}"
    }
}
