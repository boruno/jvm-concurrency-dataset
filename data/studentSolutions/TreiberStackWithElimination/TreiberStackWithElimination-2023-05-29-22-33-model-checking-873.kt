...
class TreiberStackWithElimination<E> : Stack<E> {
    ...
    private fun tryPushElimination(element: E): Boolean {
        ...
        if (cell.compareAndSet(null, element)) {
            for (i in 0 until ELIMINATION_WAIT_CYCLES) {
                ...
                if (value != null) {
                    if (cell.compareAndSet(value, null)) {
                        // Element successfully eliminated by a concurrent pop operation
                        return true
                    } else {
                        // Another thread attempted to push an element concurrently
                        return false
                    }
                }
            }
            // Reset the cell to the empty state
            cell.compareAndSet(element, null)
        }
        return false
    }
    ...
    private fun tryPopElimination(): E? {
        ...
        if (value != null && cell.compareAndSet(value, null)) {
            // Element successfully retrieved by a concurrent push operation
            return value as E
        }
        return null
    }
