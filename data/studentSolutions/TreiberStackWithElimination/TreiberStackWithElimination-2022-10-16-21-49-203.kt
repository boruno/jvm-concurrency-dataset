        ...
        while (true) {
            ...
            if (element == null) {
                if (eliminationArray[randomPosition].compareAndSet(null, x)) {
                    var currentTime = System.nanoTime().toDuration(unit = DurationUnit.NANOSECONDS)
                    while (currentTime < (currentTime + WAITING_TIMEOUT)) {
                        element = eliminationArray[randomPosition].value
                        if (element != null) {
                            eliminationArray[randomPosition].value = null
                            return element
                        }
                        currentTime = System.nanoTime().toDuration(unit = DurationUnit.NANOSECONDS)
                    }
                }
