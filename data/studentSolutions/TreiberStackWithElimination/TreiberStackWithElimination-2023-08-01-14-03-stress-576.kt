...
//        return false
        ...
        if (!eliminationArray[rand].compareAndSet(CELL_STATE_EMPTY, element)) {
            return false
        }
        ...
        while (iteration++ < ELIMINATION_WAIT_CYCLES) {
            if (eliminationArray[rand].value == CELL_STATE_RETRIEVED) {
                eliminationArray[rand].compareAndSet(CELL_STATE_RETRIEVED, CELL_STATE_EMPTY)
                return true
            }
        }
...
//        return null
        ...
        if (eliminationArray[rand].compareAndSet(assumeValue, CELL_STATE_RETRIEVED)) {
