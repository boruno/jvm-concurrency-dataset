...
class TreiberStackWithElimination<E> {
    ...
    private val POPPED_NODE = QueueNode<E?>(null, NodeState.POPPED)
...
//            randInd = (randInd + i) % ELIMINATION_ARRAY_SIZE
        ...
        val newNode = QueueNode<E?>(x, NodeState.PUSHED)
        if (positionReference.compareAndSet(null, newNode)) {
            for (j in 0 until WAIT_ITERATIONS) {
                if (positionReference.compareAndSet(POPPED_NODE, null)) {
                    return
                }
            }
            if (!positionReference.compareAndSet(newNode, null)) {
                return
            }
        }
//        }
...
//        for (i in 0 until ELIMINATION_ARRAY_SIZE) {
//            randInd = (randInd + i) % ELIMINATION_ARRAY_SIZE
        val el = eliminationArray[randInd].getAndSet(POPPED_NODE)
        if (el?.element != null) {
            return el.element
        }
//        }
