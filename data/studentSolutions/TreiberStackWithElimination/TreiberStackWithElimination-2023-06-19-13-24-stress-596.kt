...
class TreiberStackWithElimination<E> : Stack<E> {
    ...
    private fun tryPushElimination(element: E): Boolean {
        ...
        if (cell.compareAndSet(CELL_STATE_EMPTY, element)) {
            for (i in 0 until ELIMINATION_WAIT_CYCLES) {
                if (cell.compareAndSet(element, CELL_STATE_RETRIEVED)) return true
            }
    ...
    private fun tryPopElimination(): E? {
        ...
        var element = cell.value as? E

        if (element != null && cell.compareAndSet(element, CELL_STATE_RETRIEVED)) {
            return element
        }
