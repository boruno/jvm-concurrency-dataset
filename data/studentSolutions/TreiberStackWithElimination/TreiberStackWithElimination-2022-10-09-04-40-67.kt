...
class TreiberStackWithElimination<E> {
        ...
        while (curIndexTry < ELIMINATION_TRIES_COUNT) {
            ...
            if (eliminationArray[randomIndex].compareAndSet(null, x)) {
                ...
                while (curValueTry < ELIMINATION_TRIES_COUNT) {
                    if (eliminationArray[randomIndex].compareAndSet("DONE", null)) {
                        return
                    }
                    curValueTry++
                }
    ...
    fun pop(): E? {
        ...
        while (curIndexTry < ELIMINATION_TRIES_COUNT) {
            val randomIndex = Random.nextInt(0, eliminationArray.size)
            val element = eliminationArray[randomIndex].value
            if (element != null && eliminationArray[randomIndex].compareAndSet(element, "DONE")) {
                return element as E
            }
            curIndexTry++
        }
