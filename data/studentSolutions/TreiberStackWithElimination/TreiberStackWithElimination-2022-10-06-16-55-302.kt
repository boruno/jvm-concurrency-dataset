...
class TreiberStackWithElimination<E> {
    ...
    fun push(x: E) {
        while (true) {
            ...
            if (eliminationArray[ind].compareAndSet(null, x)) {
                ...
                val until = LocalDateTime.now().plusNanos(1000)
                while (until < LocalDateTime.now()) {
                    if (eliminationArray[ind].value == null) return
                }
    ...
    fun pop(): E? {
        while (true) {
            val curTop = top.value;
            if (curTop == null && counter.value == 0) return null
