...
open class TreiberStackWithElimination<E> : Stack<E> {
    ...
    protected open fun tryPushElimination(element: E): Boolean {
        ...
        val index = 0 //randomCellIndex()
        val elementToInject = ElementWrapper(element, false)

        if (!eliminationArray.compareAndSet(index, null, elementToInject))
            return false;

        TimeUnit.MILLISECONDS.sleep(10)

        if (eliminationArray.compareAndSet(index, elementToInject, null))
            return false

        val newElement = ElementWrapper(element, true)
        if (eliminationArray.compareAndSet(index, newElement, null))
            return true
