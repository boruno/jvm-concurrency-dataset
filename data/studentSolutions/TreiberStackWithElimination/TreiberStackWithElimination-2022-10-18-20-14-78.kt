...
class TreiberStackWithElimination<E> {
    ...
    fun push(x: E) {
        ...
        while(true) {
            if (top.value == null) {
                ...
                {
                    for (i in ELIMINATION_TRY_FIND_CYCLES_PUSH downTo 0) {
    ...
    fun pop(): E? {
        ...
        while(true)
        {
            ...
            {
                for (i in ELIMINATION_TRY_FIND_CYCLES_POP downTo 0) {
                    val eliminationIndex = Random.nextInt(0, ELIMINATION_ARRAY_SIZE);
                    if (eliminationArray[eliminationIndex].value == null)
                        continue;
                    else
                    {
                        val resultElimination = eliminationArray[eliminationIndex].value ?: break
                        if (eliminationArray[eliminationIndex].compareAndSet(resultElimination, null))
                        {
...
private const val ELIMINATION_WAIT_CYCLES = 25

private const val ELIMINATION_TRY_FIND_CYCLES_POP = 2
private const val ELIMINATION_TRY_FIND_CYCLES_PUSH = 3