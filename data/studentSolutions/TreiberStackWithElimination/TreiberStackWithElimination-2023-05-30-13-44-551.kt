...
class TreiberStackWithElimination<E> : Stack<E> {
    ...
    private fun tryPushElimination(element: E): Boolean {
        ...
        for (i in 0 until ELIMINATION_WAIT_CYCLES) {
            ...
            if (d.compareAndSet(CELL_STATE_RETRIEVED, CELL_STATE_EMPTY)) {
                return true
            }
        }
        return false
//        while (true) { // infinite
//            val d = eliminationArray[ind]
//            if (d.compareAndSet(d, CELL_STATE_EMPTY))
//                return false
//        }
    }
    ...
    private fun tryPopElimination(): E? {
        ...
        val ind = randomCellIndex()
        val data = eliminationArray[ind]
        if (data.compareAndSet(CELL_STATE_EMPTY, CELL_STATE_EMPTY))
            return null
        if (data.compareAndSet(data, CELL_STATE_RETRIEVED))
