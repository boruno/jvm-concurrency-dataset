...
class TreiberStackWithElimination<E> {
    ...
    fun push(x: E) {
        ...
        if (old1.compareAndSet(null, x)) {
            for (i in 0..10) {
                if (old1.compareAndSet(DIRTY, null)) return
            }
            ...
            if (old2.compareAndSet(null, x)) {
                for (i in 0..10) {
                    if (old2.compareAndSet(DIRTY, null)) return
                }
                if (!old2.compareAndSet(x, null)) {
                    return
                }
            }
        }
    ...
    fun pop(): E? {
        ...
        if (value1 != null && old1.compareAndSet(value1, DIRTY)) {
            return value1 as E?
        }
        val old2 = eliminationArray[1]
        val value2 = old2.value
        if (value2 != null && old2.compareAndSet(value2, DIRTY)) {
            return value2 as E?
        }
        while (true) {
