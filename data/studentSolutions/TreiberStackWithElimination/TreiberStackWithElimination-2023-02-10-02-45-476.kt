        ...
        for (i in 0..10){
            ...
            if (eliminationArray[index].compareAndSet(null, EliminationValue(null, "pop") )){
                for (j in 0.. 10){
                    ...
                    if(x != null ){
                        eliminationArray[index].compareAndSet(EliminationValue(x, "pop"), null)
                        return x
                    }
                }
            }
            val element = eliminationArray[index].value
            if (element?.command =="push" &&
                element.x != null && eliminationArray[index].compareAndSet(element, null)) {
                return element.x
            }
        }
        return popLockFree()
