...
class TreiberStackWithElimination<E> {
     ...
     */
    fun push(x: E) {
        val idx: Int = Random.nextInt(ELIMINATION_ARRAY_SIZE)
        val eliminatedX: Value<E>? = eliminationArray[idx].value
        val newEliminatedX = Value(x)
        if (eliminatedX == null) {
            if (eliminationArray[idx].compareAndSet(null, newEliminatedX)) {
                repeat(TIME_WAITING) {}
                if (eliminationArray[idx].compareAndSet(newEliminatedX, null)) {
                    defaultPush(x)
                }
            } else {
                defaultPush(x)
...
private const val TIME_WAITING = 1000
