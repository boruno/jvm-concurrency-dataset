...
class TreiberStackWithElimination<E> : Stack<E> {
    ...
    fun generateRandomNumber(MAX: Int): Int {
        require(MAX >= 0) { "MAX must be a non-negative number." }
        return Random.nextInt(MAX + 1)
    }

    private fun tryPushElimination(element: E): Boolean {
        var i = 0
        val max = eliminationArray.size - 1
        var idx = randomCellIndex()
        if (eliminationArray[idx].value == null){
            eliminationArray[idx].value = element
        }
    ...
    private fun tryPopElimination(): E? {
        val max = eliminationArray.size - 1
        var idx = generateRandomNumber(max)
        return if (eliminationArray[idx].value != null) {
            eliminationArray[idx].value
        } else {
            null
        }
