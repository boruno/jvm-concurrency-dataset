...
class TreiberStackWithElimination<E> {
    ...
    private val eliminationArray = atomicArrayOfNulls<Pair<E, String>?>(ELIMINATION_ARRAY_SIZE)
 ...
 */
    ...
    private fun tryPush(x: E): Boolean {
        ...
        if (eliminationArray[index].compareAndSet(null, Pair(x, WAIT))) {
            for (i in (0..ELIMINATION_AWAIT)) {
                if (eliminationArray[index].compareAndSet(Pair(x, DONE), null)) {
                    return true
                }
            }
            return !eliminationArray[index].compareAndSet(Pair(x, WAIT), null) && eliminationArray[index].compareAndSet(Pair(x, DONE), null)
        }
    ...
    private fun tryPop(): E? {
        ...
        for (i in (0..ELIMINATION_AWAIT)) {
            val x = eliminationArray[index].value
            if (x != null && eliminationArray[index].compareAndSet(x, Pair(x.first, DONE))) {
                return x.first
