...
class TreiberStackWithElimination<E> : Stack<E> {
    ...
    private fun tryPushElimination(element: E): Boolean {
        // TODO: Choose a random cell in `eliminationArray` and try to install the element there.
        val randomCellIndex = randomCellIndex()
        val cell = eliminationArray[randomCellIndex]
        // TODO: Wait `ELIMINATION_WAIT_CYCLES` loop cycles in hope that a concurrent `pop()` grabs the element. If so, clean the cell and finish, returning `true`. Otherwise, move the cell to the empty state and return `false`.
