...
class TreiberStackWithElimination<E> : Stack<E> {
    ...
    private fun tryPushElimination(element: E): Boolean {
        ...
        for (i in 0 until ELIMINATION_WAIT_CYCLES) {
            ...
            if (data.compareAndSet(CELL_STATE_RETRIEVED, CELL_STATE_EMPTY)) {
                return true
            }
        }
        while (true) { // infinite
    ...
    private fun tryPopElimination(): E? {
        ...
        val ind = randomCellIndex()
        while (true) {
            val data = eliminationArray[ind].value
            if (data == CELL_STATE_EMPTY)
                return null
            if (eliminationArray[ind].compareAndSet(data, CELL_STATE_RETRIEVED))
                return data as? E
        }
    }
