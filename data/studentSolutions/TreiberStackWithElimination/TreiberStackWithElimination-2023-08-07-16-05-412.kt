...
open class TreiberStackWithElimination<E> : Stack<E> {
    ...
    protected open fun tryPushElimination(element: E): Boolean {
        ...
        if (eliminationArray.compareAndSet(index, CELL_STATE_EMPTY, element)) {
            var count = ELIMINATION_WAIT_CYCLES
            while (count --> 0);

            if (eliminationArray.compareAndSet(index, CELL_STATE_RETRIEVED, CELL_STATE_EMPTY)) {
                return true
            }
            return !eliminationArray.compareAndSet(index, element, CELL_STATE_EMPTY) || !eliminationArray.compareAndSet(index, CELL_STATE_RETRIEVED, CELL_STATE_EMPTY)
        }
        return false
