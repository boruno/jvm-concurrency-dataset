...
class TreiberStackWithElimination<E> {
     ...
     */
    fun push(x: E) {
        val elIndex = ThreadLocalRandom.current().nextInt(ELIMINATION_ARRAY_SIZE)
        val el = eliminationArray[elIndex].value
        if(el == null && eliminationArray[elIndex].compareAndSet(null, x)) {
            for(i in 1..10000) {
                if(eliminationArray[elIndex].compareAndSet(x, x))
                    return
            }
