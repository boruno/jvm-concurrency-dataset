...
class FAAQueue<E> {
    ...
    fun enqueue(element: E) {
        while (true) {
            ...
            if (segment.elements[(index % SEGMENT_SIZE).toInt()].compareAndSet(null, Maybe(element)))
                return
        }
    }
    ...
    fun dequeue(): E? {
        while (true) {
            ...
            if (segment.elements[(index % SEGMENT_SIZE).toInt()].compareAndSet(null, Maybe.none()))
                continue
...
private class Segment<T>(val id: Long) {
    ...
    private fun cas(i: Int, expect: Maybe<T>?, update: Maybe<T>?) = elements[i].compareAndSet(expect, update)
