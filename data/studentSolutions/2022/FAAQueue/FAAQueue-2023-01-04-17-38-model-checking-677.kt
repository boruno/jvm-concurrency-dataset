...
class FAAQueue<E> {
    ...
    private fun findSegment(start: Segment, idx: Long) : Segment {
        ...
        if (start.startIndex.toLong() > idx) {
            println("!!!>" + start.startIndex + " " + idx + "||" + head.value.startIndex + " " + tail.value.startIndex)
            TODO("what")
        }
        while (true) {
    ...
    fun enqueue(element: E) {
        while (true) {
            ...
            val s = findSegment(cur_tail.value, i / SEGMENT_SIZE)
            moveTailForward(s);
    ...
    fun dequeue(): E? {
        while (true) {
            ...
            val s = findSegment(cur_head.value, i / SEGMENT_SIZE)
            moveHeadForward(s)
