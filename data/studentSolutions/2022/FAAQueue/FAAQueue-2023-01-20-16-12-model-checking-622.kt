...
class FAAQueue<E> {
    ...
    fun enqueue(element: E) {
        while (true) {
            ...
            if (idx >= SEGMENT_SIZE) {
                val newTail = Segment()
                newTail.put(idx.toInt(), element)
                enqIdx.getAndIncrement()

                if (curTail.next?.cas(idx.toInt(), null, newTail)!!) {
                ...
                if (curTail.elements[idx.toInt()].compareAndSet(null, element)) {
                    return
                }
            }
        }
    ...
    fun dequeue(): E? {
        while (true) {
            ...
            val res = curHead.elements[idx.toInt()].getAndSet(Any()) ?: continue
            return res as E?
        }
    }
