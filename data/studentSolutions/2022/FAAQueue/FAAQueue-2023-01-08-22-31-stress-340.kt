...
//            println("i = $i")
            ...
            assert(s.id == i / SEGMENT_SIZE)
//            println("s.id = " + (s.id).toString())
//            if (s.elements[(i % SEGMENT_SIZE).toInt()].value != null) {
//                println("not null " + s.elements[(i % SEGMENT_SIZE).toInt()].value.toString())
//            }
            if (s.elements[(i % SEGMENT_SIZE).toInt()].compareAndSet(null, element)) {
//                println("DONE " + element.toString())
                return
            }
        }
...
//            }
            ...
            println("DEQUE RESULT: " + s.elements[(i % SEGMENT_SIZE).toInt()].value.toString())
...
//        while (true) {
        for (aaa in 1..100) {
            ...
            if (elem.next.value != null || elem.next.compareAndSet(null, Segment(elem.id + 1))) {
                elem = elem.next.value!!
            }
        }
        throw Exception("find segment, id = " + id.toString() + ", elem.id = " + elem.id.toString())
    }
}
