...
class FAAQueue<E> {
    ...
    private fun findSegment(start: Segment, id: Long): Segment {
        ...
        if (currentSegment.id.value >= id) {
            return start
        }
        while (true) {
            if (currentSegment.next.value == null) {
                val newSegment = Segment()
                newSegment.id.value = currentSegment.id.value + 1L
                if (!currentSegment.next.compareAndSet(null, newSegment)) {
                    continue
                }
                if (newSegment.id.value == id) {
                    return newSegment
                } else {
    ...
    private fun moveTailForward(newTail: Segment) {
            val currentTail = tail
            if (newTail.id.value < currentTail.value.id.value) {
                return
            }
    ...
    private fun moveHeadForward(newHead: Segment) {
            val currentHead = head
            if (newHead.id.value < currentHead.value.id.value) {
                return
            }
