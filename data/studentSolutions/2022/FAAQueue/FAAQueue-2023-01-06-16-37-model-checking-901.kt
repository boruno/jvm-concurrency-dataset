...
class FAAQueue<E> {
    ...
    fun enqueue(element: E) {
        while (true) {
            ...
            val elementIdx = (idx % SEGMENT_SIZE).toInt()
    ...
    fun dequeue(): E? {
        while (true) {
            ...
            if (curHead.idx < segmentIdx) {
                if (curHead.next.compareAndSet(null, Segment(segmentIdx))) {
                    head.compareAndSet(curHead, curHead.next.value!!)
                }
                curHead = curHead.next.value!!
            }

            if (curHead.elements[elementIdx].compareAndSet(null, BROKEN_MARKER)) {
                continue
            } else {
                return curHead.elements[elementIdx].value as E?
            }
        }
    }
    ...
    private fun findSegment(segment: Segment, idx: Long): Segment {
        ...
        while (tmpSegment.next.value != null && tmpSegment.idx < idx) {
