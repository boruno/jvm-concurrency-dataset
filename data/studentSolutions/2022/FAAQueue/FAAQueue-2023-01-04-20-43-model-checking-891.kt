...
class FAAQueue<E> {
    ...
    private fun moveTailForward(segment: Segment) {
        while (true) {
            val currentTail = tail.value
            if (currentTail.id < segment.id) if (tail.compareAndSet(currentTail, segment)) return
            return
        }
    }
    ...
    private fun moveHeadForward(segment: Segment) {
        while (true) {
            val currentHead = head.value
            if (currentHead.id < segment.id) if (head.compareAndSet(currentHead, segment)) return
            return
        }
    }
