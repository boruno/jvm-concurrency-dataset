...
class FAAQueue<E> {
    ...
    fun enqueue(element: E) {
        while (true) {
            ...
            if (moveTailForward(segment)) { }

            /*if (curTail.getId() > segment.getId()) continue
            else if (curTail.getId() < segment.getId()) tail.value = segment*/
            if (tail.value.elements[index % SEGMENT_SIZE].compareAndSet(null, element)) return
        }
    }
    ...
    fun dequeue(): E? {
        while (true) {
            ...
            if (isEmpty)
            {
                ...
                {
                    this.head.compareAndSet(curHead, curHeadNext)
                }
            }
            else
            {
                ...
                if (moveHeadForward(segment)) { }

                /* if (curHead.getId() >= segment.getId()) continue
                 if (curHead.getId() < segment.getId()) head.compareAndSet(curHead, segment)*/

                if (curHead.elements[index % SEGMENT_SIZE].compareAndSet(null, "⟂")) continue
                return segment.elements[index % SEGMENT_SIZE].value as E?
            }
        }
    }
