        ...
        for (i in 0 until FC_ARRAY_SIZE) {
            when (val op = fcArray[i].value) {
                is FcArrayElement.FcOperation.Add<*> -> {
                    ...
                    fcArray[i].compareAndSet(op, FcArrayElement.FcAnswer.Add)
                }
                is FcArrayElement.FcOperation.Poll -> {
                    fcArray[i].compareAndSet(op, FcArrayElement.FcAnswer.Poll(q.poll()))
                }
                is FcArrayElement.FcOperation.Peek -> {
                    fcArray[i].compareAndSet(op, FcArrayElement.FcAnswer.Peek(q.peek()))
                }
                else -> Unit
            }
        }
