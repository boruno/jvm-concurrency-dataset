...
class FCPriorityQueue<E: Comparable<E>> {
    ...
    private val queue = TreeSet<E> { a, b ->
        when {
            a > b -> 1
            a < b -> -1
            else -> -1
    ...
    private fun solve_tasks() {
        for (index in 0 until OPERATION_ARRAY_SIZE) {
            ...
            when (task.type) {
                OperationTypes.POLL -> {
                    operations[index].compareAndSet(task, Operation(OperationTypes.DONE, queue.pollFirst()))
                }
                OperationTypes.PEEK -> {
                    if (queue.isEmpty()) {
                        operations[index].compareAndSet(task, Operation(OperationTypes.DONE, null))
                    } else {
                        operations[index].compareAndSet(task, Operation(OperationTypes.DONE, queue.first()))
                    }
                }
                OperationTypes.ADD -> {
