    ...
    fun poll(): E? {
        ...
        while (true) {
            if (lock.tryLock()) {
                ...
                if (msgToCheck == null)
                {
                    msgToCheck = getHelpedRequest(req.id)
                }
                if (msgToCheck?.op == ReqType.Response) {
                    return msgToCheck.getMessage()
                } else {
                    if (index == 0) {
    ...
    fun add(element: E) {
        ...
        while (true) {
            if (lock.tryLock()) {
                ...
                if (msgToCheck?.op == ReqType.Response) { // if request was processed
