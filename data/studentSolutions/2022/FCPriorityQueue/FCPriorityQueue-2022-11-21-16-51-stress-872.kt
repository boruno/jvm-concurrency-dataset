...
class FCPriorityQueue<E : Comparable<E>> {
    ...
    private val fcArray = atomicArrayOfNulls<Operation>(size)
    ...
    fun poll(): E? {
        if (!lock.compareAndSet(expect = false, update = true)) {
            val place = ThreadLocalRandom.current().nextInt(size)
            ...
            if (fcArray[place].compareAndSet(null, operation)) {
                while (true) {
                    ...
                    if (lock.compareAndSet(expect = false, update = true)) {
                        val elem: E?
                        if (fcArray[place].value == operation) {
                             elem = q.poll()
                        } else {
                            assert(fcArray[place].value == result)
                            elem = result.result as E?
                        }
