...
class FCPriorityQueue<E : Comparable<E>> {
    ...
    private val res_array = MutableList<E?>(ARRAY_SIZE) { null }

    // private fun get(idx: Int): E = fun_array.getAndSet(null)

    // private fun put(element: E) = 
        ...
        while (true) {
            if (tryLock()) {
                ...
                if (!fun_array[idx].compareAndSet(null, {
                    if (res_array[idx] == null)
                        res_array[idx] = queue.poll()
                    }
                )) continue

                while (true) {
                    val res = res_array[idx]
                    if (res != null) {
                        res_array[idx] = null
                        fun_array[idx].value = null
                        return res
                    } else if (tryLock()) {
                        var value = res_array[idx]
                        if (value != null) {
                            res_array[idx] = null
                            fun_array[idx].value = null
                        } else {
