        ...
        while (true) {
            if (tryLock()) {
                ...
                while (true) {
                    ...
                    while (flatCombiningArray[index].value === null) {
                        flatCombiningArray[index].compareAndSet(null, operation)
                    }
                    if (flatCombiningArray[index].value === null) throw NullPointerException("1")
                    val value = flatCombiningArray[index].value
                    if (flatCombiningArray[index].value === null) throw NullPointerException("2")
                    if (value !== null) {
                        if (flatCombiningArray[index].value === null) throw NullPointerException("3")
                        if (value.status === OperationType.DONE) {
                            if (flatCombiningArray[index].value === null) throw NullPointerException("4")
                            flatCombiningArray[index].compareAndSet(operation, null)
                            if (flatCombiningArray[index].value === null) throw NullPointerException("5")
        ...
        for (i in 0 until flatCombiningArray.size) {
            ...
            when (operation.status) {
                OperationType.POLL -> {
                    ...
                    if (flatCombiningArray[i].value === null) throw NullPointerException("6")
                }

                OperationType.PEEK -> {
                    val result = q.peek()
                    flatCombiningArray[i].compareAndSet(operation, Operation(OperationType.DONE, result))
                    if (flatCombiningArray[i].value === null) throw NullPointerException("7")
                }

                OperationType.ADD -> {
                    q.add(operation.element)
                    flatCombiningArray[i].compareAndSet(operation, Operation(OperationType.DONE))
                    if (flatCombiningArray[i].value === null) throw NullPointerException("8")
                }
