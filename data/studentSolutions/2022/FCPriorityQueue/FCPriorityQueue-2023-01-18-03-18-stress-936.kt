...
class FCPriorityQueue<E : Comparable<E>>() {
    ...
    fun poll(): E? {
        ...
        while (!lock.tryLock()) {
            if (taskIdx == -1) {
                ...
                if (ar[taskIdx2].compareAndSet(null, Operation(OP.POOL, STATUS.WAITING, null, null))) {
                    taskIdx = taskIdx2
                }
            } else {
    ...
    fun peek(): E? {
        ...
        while (!lock.tryLock()) {
            if (taskIdx == -1) {
                val taskIdx2 = ThreadLocalRandom.current().nextInt(0, 50)
                if (ar[taskIdx2].compareAndSet(null, Operation(OP.PEEK, STATUS.WAITING, null, null))) {
                    taskIdx = taskIdx2
                }
            } else {
                if (ar[taskIdx].value?.status == STATUS.DONE) {
    ...
    fun add(element: E) {
        ...
        while (!lock.tryLock()) {
            if (taskIdx == -1) {
                val taskIdx2 = ThreadLocalRandom.current().nextInt(0, 50)
                if (ar[taskIdx2].compareAndSet(null, Operation(OP.ADD, STATUS.WAITING, element, null))) {
                    taskIdx = taskIdx2
                }
            } else {
                if (ar[taskIdx].value?.status == STATUS.DONE) {
