...
class DynamicArrayImpl<E> : DynamicArray<E> {
    ...
    override fun put(index: Int, element: E) {
        while (true) {
            ...
            var cur_value = core.value.get(index)!!
            if (cur_value.first == Status.READY
                && core.value.cas(index, -1, cur_value, Pair(Status.READY, element))) {
                break
            } else if (cur_value.first == Status.MODIFIED && new_core.value.size > index) {
                cur_value = new_core.value.get(index)!!
                if (new_core.value.cas(index, -1, cur_value, Pair(Status.READY, element))
                    && s == core.value.size) {
                    break
                }
            } else if (cur_value.first == Status.MODIFIED
                && new_core.value.add(index, -1, Pair(Status.READY, element))) {
                break
            }
        }
    }
    ...
    override fun pushBack(element: E) {
        while (true) {
            ...
            if (core.value.add(size, cur_capacity, Pair(Status.READY, element))) {
                break
            }
        }
    }
    ...
    fun update(cur_capacity: Int) {
        ...
        while (true) {
            ...
            var now = core.value.get(i)!!
            while (core.value.capacity == cur_capacity) {
                now = core.value.get(i)!!
                if (now.first == Status.MODIFIED
                    || core.value.cas(i, cur_capacity, now, Pair(Status.MODIFIED, now.second))) {
                    break
                }
            }

            new_core.value.add(i, cur_capacity, Pair(Status.READY, now.second))
...
private class Core<E>(
    ...
    fun cas(index: Int, cap: Int, expect: E?, element: E?): Boolean {
        require(index < size)
        if ((cap == capacity || cap == -1) && array[index].compareAndSet(expect, element)) {
    ...
    fun add(index: Int, cap: Int, element: E?): Boolean {
        if (index >= array.size || (cap != capacity && cap != -1)) {
            return false
        }
