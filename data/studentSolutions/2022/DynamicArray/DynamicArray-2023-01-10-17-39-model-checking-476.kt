...
class DynamicArrayImpl<E> : DynamicArray<E> {
    ...
    override fun put(index: Int, element: E) {
        ...
        head.put(index, element)

        while (true) {
            val nextNode = head.next.value ?: break
            val value = head.get(index)
    ...
    override fun pushBack(element: E) {
        while (true) {
            ...
            val curSize = oldCore.size.value
            if (curSize < oldCore.capacity()) {
                oldCore.size.compareAndSet(curSize, curSize + 1)
                if (oldCore.array[curSize].compareAndSet(null, element)) {
                    return
                }
            }
            else {
