...
class DynamicArrayImpl<E> : DynamicArray<E> {
    ...
    override fun get(index: Int): E {
        ...
        var tmpElem = curCore.get(index)?.value
        while(curCore.next.value != null) {
            curCore = curCore.next.value!!
            tmpElem = curCore.get(index)?.value
        }
        val ls = curCore.get(index)
        return if (ls == null) {
            tmpElem as E
        } else {
            ls.value
            ...
            if (curLen + 1 > curSize) {
                ...
                for (i in 0 until curSize) {
                    while (true) {
                        ...
                        if (!cCore.cas(i, transferElem, Elem(transferElem.value, true))) continue
