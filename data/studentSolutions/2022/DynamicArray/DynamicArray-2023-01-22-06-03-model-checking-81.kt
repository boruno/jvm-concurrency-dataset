        ...
        while (true) {
            ...
            if (index < localCore.capacity) {
                if (localCore.array[index].compareAndSet(null, element)) {
                    ...
                    localCore.size.incrementAndGet()
                    var nextLocalCore = localCore.next.value
                    while (nextLocalCore != null) {
                        nextLocalCore.size.incrementAndGet()
                        nextLocalCore = nextLocalCore.next.value
                    }
                    return
                }
                continue
