...
class DynamicArrayImpl<E> : DynamicArray<E> {
    ...
    override fun put(index: Int, element: E) {
        //var head =
        if (index >= core.value.size.value) throw IllegalArgumentException()
        core.value.array[index].getAndSet(element)
        while (true) {
            when {
                core.value.next.value != null -> {
                    if (core.value.array[index].value != null) core.value.next.value!!.array[index].getAndSet(core.value.array[index].value)
