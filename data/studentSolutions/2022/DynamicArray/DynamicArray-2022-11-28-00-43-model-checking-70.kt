...
private class Core<E>(
    ...
    fun tryPushBackWithoutExtending(element: E): PushBackAttemptResult {
        ...
        if (pushIndex >= getCapacity())
            return PushBackAttemptResult(isSuccessful = false, pushIndex = pushIndex)

        return if (array[pushIndex].compareAndSet(null, CommonElement(element))) {
            PushBackAttemptResult(isSuccessful = true, pushIndex = pushIndex)
        } else PushBackAttemptResult(isSuccessful = false, pushIndex = pushIndex)
    }
