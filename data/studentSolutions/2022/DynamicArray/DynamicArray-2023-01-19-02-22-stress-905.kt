...
class DynamicArrayImpl<E> : DynamicArray<E> {
    ...
    private inner class Operation(val idx: Int, val e: E)
    private val operationStack = LinkedBlockingQueue<Operation>()
...
//        println("${Thread.currentThread().id} : pb> $element | ${core.value.capacity}")
        val op = Operation(index.getAndIncrement(), element)
        val idx = op.idx
...
//        println("${Thread.currentThread().id} : $now>>${now.next.value} ? ${now.completed.value} || ${core.value}")
    ...
    private fun completeDelayed() {
        while (operationStack.isNotEmpty()) {
            var op = operationStack.peek() ?: return

            expand(op.idx)

            op = operationStack.poll() ?: return
