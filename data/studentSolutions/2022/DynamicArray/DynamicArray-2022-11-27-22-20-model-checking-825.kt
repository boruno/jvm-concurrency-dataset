    ...
    override fun pushBack(element: E) {
        while (true) {
            ...
            if (currentCore.tryPushBackWithoutExtending(element))
                break
            else markCoreAsExtending(currentCore)
            ...
            // Fix element -> Copy it to the extended array -> Mark element as Moved (so it couldn't be used by reader anymore)
            for (index in 0 until currentSize) {
                ...
                newCore.casElement(index, BaseElement(null), CommonElement(movableElement.element))

                val movedElement = currentCore.get(index)
                currentCore.casElement(index, movedElement, MovedElement(movedElement.element))
            }

            if (newCore.casElement(currentSize, BaseElement(null), CommonElement(element)))
                break
        }
    }
