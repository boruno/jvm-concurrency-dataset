...
class DynamicArrayImpl<E> : DynamicArray<E> {
    ...
    override fun pushBack(element: E) {
        loop@ while (true) {
            ...
            var isDone = false
            while (true) {
                if (current.size.value == current.capacity) {
                    current.next.compareAndSet(null, Core(current.size.value * 2))
                    core.compareAndSet(current, current.makeNewTable()!!)
                    continue@loop
                }
                if (current.cas(current.size.value, null, Wrapper(element))) isDone = true
                incrementSize(current, current.size.value)
                if (isDone) return
            }
        }
    }
...
private class Core<E>(
    ...
    fun makeNewTable(): Core<E>? {
        loop@ for (index in 0 until size.value) {
            while (true) {
                val value = get(index)
                if (value is Moved) continue@loop
                if (value is Raw || cas(index, value, Raw(value.element))) help(index, value)
            }
        }
        return next.value
    }
