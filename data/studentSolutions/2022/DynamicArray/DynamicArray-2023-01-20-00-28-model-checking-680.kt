...
class DynamicArrayImpl<E> : DynamicArray<E> {
    ...
    override fun pushBack(element: E) {
        val index = core.value._size.getAndIncrement()
        while (true) {
            val curCore = core.value
            try {
                curCore.pushBack(element, index)
                return
            } catch (e : PrincessIsInAnotherCastleException) {
                core.compareAndSet(curCore, curCore.nextCore.value!!)
            }
...
private class Core<E>(
    ...
    fun pushBack(element : E, index : Int) {
        while (true) {
            if (index < capacity) {
                val newValue = Storage(StorageType.VALUE, element)
                if (array[index].compareAndSet(null, newValue)) {
                    return
                }
            } else {
                if (nextCore.compareAndSet(null, Core(capacity * 2))) {
                    nextCore.value!!._size.compareAndSet(0, index + 1)
                }
                helpMove()
