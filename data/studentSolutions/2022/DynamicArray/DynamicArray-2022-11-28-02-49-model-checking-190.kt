...
class DynamicArrayImpl<E> : DynamicArray<E> {
    ...
    override fun get(index: Int): E {
        ...
        while (true) {
            ...
            val element = currentElement?.element
            return if (element != null) element else throw NullPointerException("Expression 'currentElement?.element' must not be null. Size is $size")
        }
    }
    ...
    override val size: Int get() {
        ...
        val min = min(currentCore.getSize(), currentCore.getCapacity())
        println(min)
        if (currentCore.next.value == null)
            return min
