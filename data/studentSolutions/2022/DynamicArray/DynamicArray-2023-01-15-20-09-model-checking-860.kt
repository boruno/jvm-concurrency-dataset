...
private class Core<E : Any>(
    ...
    fun get(index: Int): E? {
        ...
        return when(val cell = array[index].value) {
            ...
            is Moved -> {
                val x = this.nextCore.value!!.array[index].value
                return if (x == null) { cell.x as E? } else { x.x as E? }
            }
            null -> return null
        }
    }
        ...
        while (true) {
            when(val cell = array[index].value) {
                is Moved -> this.nextCore.value!!.put(index, element)
                    ...
                    if(array[index].compareAndSet(cell, element)) {
                        return
                    } else {
                        continue
                    }
    ...
    fun resize() {
        ...
        for(i in 0 until array.size) {
            while (true) {
                ...
                if (x is Moved) {
                    nextCore.value!!.array[i].compareAndSet(null, Current(x.x))
                    break
                } else if(array[i].compareAndSet(x, Moved(x.x))) {
                    nextCore.value!!.array[i].compareAndSet(null, Current(x.x))
                    break
                }
            }
...
sealed class Cell(val x: Any)
class Moved(x: Any): Cell(x)
class Current(x: Any): Cell(x)
