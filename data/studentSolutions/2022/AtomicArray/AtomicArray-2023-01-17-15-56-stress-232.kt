...
class AtomicArray<E>(size: Int, initialValue: E) {
    ...
    fun cas2(
        ...
        val descriptor = CASNDescriptor<E>(indexA, expectedA, updateA, indexB, expectedB, updateB)
        while (true) {
        ...
        DCSS(descriptorDCSS)
        if(descriptorDCSS.outcome.value == result.FAIL) {
        ...
        fun complete() {
            if (outcome.get() == result.UNDECIDED) {
                while(true) {
                    ...
                    if (bValue is AtomicArray<*>.CASNDescriptor<*> && bValue == this) {
                    ...
                    val descriptorDCSS = RDCSSDescriptor(outcome, result.UNDECIDED, indexB, expectB, this)
                    DCSS(descriptorDCSS)
                    if (descriptorDCSS.outcome.value == result.FAIL) {
    ...
    fun DCSS(d: AtomicArray<*>.RDCSSDescriptor<*>) {
        while (true) {
