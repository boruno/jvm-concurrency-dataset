...
class AtomicArray<E : Any>(size: Int, initialValue: E) {
    ...
    fun cas(index: Int, expected: E, update: E) =
        a[index].casWithHelping(expected, update)
    ...
    private class CASNDescriptor(val object1: Ref, val expect1: Any?, val update1: Any?, val object2: Ref, val expect2: Any?, val update2: Any?) {
        ...
        fun complete() {
            var result = object2.cas(this, this) || dcss(object2, expect2, this, outcome, OUTCOME.UNDECIDED)
        ...
        private fun dcss(object1: Ref, expect1: Any?, update1: Any?, object2: Ref, expect2: Any?): Boolean {
            val descriptor = DCSSDescriptor(object1, expect1, update1, object2, expect2)
            return if (object1.casWithHelping(expect1, descriptor)) {
                descriptor.complete()
                return descriptor.outcome.v.compareAndSet(OUTCOME.SUCCESS, OUTCOME.SUCCESS)
            } else {
    ...
    private class DCSSDescriptor(val object1: Ref, val expect1: Any?, val update1: Any?, val object2: Ref, val expect2: Any?) {
        val outcome = Ref(OUTCOME.UNDECIDED)

        fun complete() {
            var result = object2.v.compareAndSet(expect2, expect2)
            outcome.cas(OUTCOME.UNDECIDED, if (result) OUTCOME.SUCCESS else OUTCOME.FAILED)
            result = outcome.cas(OUTCOME.SUCCESS, OUTCOME.SUCCESS)
            object1.cas(this, if (result) update1 else expect1)
        }
