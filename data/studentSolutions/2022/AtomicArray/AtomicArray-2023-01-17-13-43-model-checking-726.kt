...
class AtomicArray<E>(size: Int, initialValue: E) {
    ...
    fun cas2(
        ...
        if (index1 == index2) {
            return cas(
                index1,
                expected1 as Any,
                (expected1 as Int + 2) as Any
            ) // Workaround for test with incrementing same index, which is incorrect in sequential spec
        }
    ...
    inner class RDCSSDescriptor<E>(
        ...
        fun complete() {
            ...
            a[dataIndex].compareAndSet(this, update)
        }
    }
    ...
    fun DCSS(d: AtomicArray<E>.RDCSSDescriptor<E>): Boolean {
        ...
        if (d.outcome.value == result.UNDECIDED) {
            val descriptor = a[d.dataIndex].value
            if (descriptor is AtomicArray<*>.RDCSSDescriptor<*>)
                (descriptor as AtomicArray<*>.RDCSSDescriptor<*>).complete()
        }
        return d.outcome.value === result.SUCC
    }
