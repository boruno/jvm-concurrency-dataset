...
class SynchronousQueue<E> {
    ...
    suspend fun send(element: E) {
        while (true) {
            ...
            if (curHead == curTail || curTail.operation.value == "SEND") {
                if (!coroutine(curTail, Node("SEND", element))) return
            } else {
                if (curHead.next.value!!.cor.value != null && head.compareAndSet(curHead, curHead.next.value!!)) {
                    curHead.next.value!!.element.compareAndSet(curHead.next.value!!.element.value, element)
                    curHead.next.value!!.cor.value!!.resume(false)
                    return
                }
            }
        }
    ...
    suspend fun receive(): E {
        while (true) {
            val curHead = head.value
            val curTail = tail.value
            if (curHead == curTail || curTail.operation.value == "RECEIVE") {
               // val node =
                if (!coroutine(curTail, Node<E>("RECEIVE", null))) return Node<E>("RECEIVE", null).element.value!!
            } else {
    ...
    private suspend fun coroutine(curTail: Node<E>, node: Node<E>): Boolean {
        return suspendCoroutine sc@{ cor ->
            node.cor.value = cor
            if (curTail.next.compareAndSet(null, node)) tail.compareAndSet(curTail, node)
            else {
