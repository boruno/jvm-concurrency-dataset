    ...
    suspend fun send(element: E) {
        ...
        while (true) {
            if (s < receiveIdx.value) {
                val cur = head.value
                val cur_value = cur.value.value as Pair<Continuation<Any>, Optional<E>?>?
                if (cur_value != null && cur_value.second == null) {
                    if (cur.value.compareAndSet(cur_value, Pair(cur_value.first, Optional.ofNullable(element)))) {
                ...
                val res = suspendCoroutine<Any> sc@ { cont ->
                    next.value.compareAndSet(null, Pair(cont, Optional.ofNullable(element)))
    ...
    suspend fun receive(): E {
        ...
        while (true) {
            if (r < sendIdx.value) {
                val cur = tail.value
                val cur_value = cur.value.value as Pair<Continuation<Any>, Optional<E>?>?
                if (cur_value?.second != null && !cur_value.second!!.isEmpty) {
                    if (cur.value.compareAndSet(cur_value, Pair(cur_value.first, Optional.empty<E>()))) {
                        cur_value.first.resume(true)
                ...
                if (res == false) {
                    ...
                    return (next.value.value as Pair<Continuation<Any>, Optional<E>>).second.get()
                }
            }
        }
    }
