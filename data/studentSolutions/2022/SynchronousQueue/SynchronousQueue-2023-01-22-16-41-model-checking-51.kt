...
class SynchronousQueue<E> {
    ...
    suspend fun send(element: E) {
        while (true) {
            val t = tail.value
            val h = head.value
            if (h.next.value == null || t is Sender<*>) {
                if (enqueueAndSuspend(element, t)) return
    ...
    private suspend fun enqueueAndSuspend(element: E, t: Coroutine<E>): Boolean {
        val res = suspendCoroutine<Any?> sc@{ cont ->
            ...
            if ((head.value.next.value == null || cur_tail is Sender<*>)) {
    ...
    suspend fun receive(): E {
        while (true) {
            ...
            if (h.next.value == null || t is Receiver<*>) {
                val res = enqueueAndSuspend(t)
    ...
    private suspend fun enqueueAndSuspend(t: Coroutine<E>): E? {
        val res = suspendCoroutine sc@{ cont ->
            val receiver: Receiver<E> = Receiver(cont)
            val cur_tail = tail.value
            if ((head.value.next.value == null || cur_tail is Receiver<*>)) {
