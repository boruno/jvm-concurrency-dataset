...
//        while (true) {
        val id = ThreadLocalRandom.current().nextInt(100)
        println("--------------------------- $id")
        for (aaa in 1..1000) {
            val t = tail.value
            val h = head.value
            ...
            if (h.next.value == null || t.isSender) {
                println("pushback $element")
                val res = suspendCoroutine<Any> sc@{ cont ->
                    if (!enqueue(Node(cont, element, true), t)) {
                        cont.resume(RETRY)
                        return@sc
                    }
                }
...
//        while (true) {
        for (aaa in 1..1000) {
            ...
            if (h.next.value == null || !t.isSender) {
                val res = suspendCoroutine<Any> sc@{ cont ->
                    if (!enqueue(Node(cont, null, false), t)) {
                        cont.resume(RETRY)
                        return@sc
                    }
                }
