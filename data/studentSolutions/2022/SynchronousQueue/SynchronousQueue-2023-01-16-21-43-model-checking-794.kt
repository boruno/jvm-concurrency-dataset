...
//        while (true) {
        for (aaa in 1..100) {
            ...
            if (h.next.value == null || t.type.value == 1) {
                val res = suspendCoroutine<Any> sc@{ cont ->
                    if (!enqueue(Node(cont, element, 1), t)) {
                        cont.resume(RETRY)
                        return@sc
                    }
                }
                ...
                return
            } else if (t.type.compareAndSet(2, 0)) {
                val deq = dequeue(h)
                if (deq == null) {
                    t.type.compareAndSet(0, 2)
                    continue
                }
...
//        while (true) {
        for (aaa in 1..100) {
            ...
            if (h.next.value == null || t.type.value == 2) {
                val res = suspendCoroutine<Any> sc@{ cont ->
                    if (!enqueue(Node(cont, null, 2), t)) {
                        cont.resume(RETRY)
                        return@sc
                    }
                }
                ...
                return res as E
            } else if (t.type.compareAndSet(1, 0)) {
                val deq = dequeue(h)
                if (deq == null) {
                    t.type.compareAndSet(0, 1)
                    continue
                }
...
type:
    ...
    0 - dummy
 */
private class Node<E>(val cont: Any?, val element: E?, val c: Int) {
