...
class SynchronousQueue<E> {
    ...
    private fun dequeueAndResume(requestNode: Node<E>): Boolean {
        ...
        if (requestNode.isSender() && node.isReceiver()) {
            node.updateElement(requestNode.element)
            node.coroutine?.resume(requestNode.element!!)
            return true
        } else if (requestNode.isReceiver() && node.isSender()) {
            node.updateElement(node.element)
            node.coroutine?.resume(Unit)
        }
