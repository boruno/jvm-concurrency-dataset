...
class LinkedListSet<E : Comparable<E>> {
    ...
    fun add(element: E): Boolean {
        var node = head.value
        var prevUndel : Node<E>? = null
        while ( node.next != null ) {
            val nodenext = node.next!!.element.value!!
            if ( node.element.value!!.value!! < element && node.next!!.element.value!!.value!! >= element ) {
                if ( ( node.next!!.element.value!!.value!! == element ) && ( node.next!!.element.value is Undeleted<*>) ) {
                    return false
                } else {
                    val newNode = Node<E>(prev = node, element = Undeleted<E>(element), next = node.next)
                    if ( node.element.value is Undeleted<*> )
                        prevUndel = node
                    if ( nodenext != node.next!!.element.value!! ) {
                        node = head.value
                        prevUndel = null
                        continue
                    }
