...
class LinkedListSet<E : Comparable<E>> {
    ...
    fun add(element: E): Boolean {
        while (true) {
            ...
            val node = Node(window.prev, element, window)
            if (window.casPrev(window.prev, node)) {
                head.compareAndSet(head.value, window.prev!!)
                return true
            }
        }
    }
...
private class Node<E : Comparable<E>>(prev: Node<E>?, element: E?, next: Node<E>?) {
    ...
    private val _isRemoved = atomic(false)
    val isRemoved get() = _isRemoved.value

    fun setIsRemoved(value: Boolean) {
        _isRemoved.value = value
    }
