...
class LinkedListSet<E : Comparable<E>> {
    ...
    fun add(element: E): Boolean {
        outer@ while (true) {
            if (first.node.next == last) {
                val answer = SomeNode(Node(null, element, last))
                if (first.node.casNext(last, answer)) {
                    return true
                }
                continue
            }
            ...
            while (current != null && current != last) {
                ...
                if (current.node != last && prev.node.element <= element && (current == last || element < current.node.element)) {
                ...
                if (current.node == last || prev.node.element > element) {
                    return false
                }
                prev = current
