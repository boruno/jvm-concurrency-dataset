...
class IntIntHashMap {
    ...
    private class Core internal constructor(capacity: Int) {
        ...
        fun getInternal(key: Int): Int {
            ...
            if (map[index + 1].value == MOVED) {
                index = index(key)
                probes = 0
                while (next.value!!.map[index].value != key) { // optimize for successful lookup
                    if (++probes >= MAX_PROBES) return NULL_VALUE
                    if (index == 0) index = map.size
                    index -= 2
                }
                println(next.value!!.map[index + 1].value)
                return next.value!!.map[index + 1].value
            }
            // found key -- return value
            return toUnLockedValue(map[index + 1].value)
        }
        ...
        fun putInternal(key: Int, value: Int): Int {
            ...
            if (isLockedValue(oldValue) || oldValue == MOVED) {
                return NEEDS_REHASH
            }
        ...
        fun placeInternal(key: Int, value: Int) {
            if (value == MOVED) return
            if (value == DEL_VALUE) return
            ...
            // found key -- update value
            map[index + 1].compareAndSet(NULL_VALUE, transformedValue)
        }
        ...
        private fun helpInternal() {
            ...
            while (index < map.size) {
                ...
                if (value == NULL_VALUE) {
                    ...
                    map[index + 1].compareAndSet(value, toLockedValue(value))
                }
