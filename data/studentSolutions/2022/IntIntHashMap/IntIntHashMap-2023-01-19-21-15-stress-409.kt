...
//        val maxSize: Int = capacity
        ...
        fun getInternal(key: Int): Int {
            var index = index(key)
            var probes = 0
            var cell = map[index].value
            while (cell != key) { // optimize for successful lookup
                if (cell == null) return NULL_VALUE // not found -- no value
                if (++probes >= MAX_PROBES) return NULL_VALUE
                if (index == 0) index = map.size
                index -= 2
                cell = map[index].value
            }
            // found key -- return value
            return map[index + 1].value as Int
        }
        ...
        fun rehash(): Core {
            ...
            while (index < map.size) {
                if (isValue(map[index + 1].value as Int)) {
                    val result = newCore.putInternal(map[index].value as Int, map[index + 1].value as Int)
