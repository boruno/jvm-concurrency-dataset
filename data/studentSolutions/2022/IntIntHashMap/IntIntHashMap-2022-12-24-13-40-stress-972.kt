...
class IntIntHashMap {
    ...
    fun remove(key: Int): Int {
        require(key > 0) { "Key must be positive: $key" }
        return toValue(putAndRehashWhileNeeded(key, NULL_VALUE))
    }
            ...
            while(true) {
                ...
                while (keyValue != key && keyValue != -key) { // optimize for successful lookup
                    if (keyValue == NULL_KEY) {
                        ...
                        if (value == NULL_VALUE) return NULL_VALUE // remove of missing item, no need to claim slot
                ...
                if (map[index].value < 0) // Key is readonly, so we need to help
                    return NEEDS_REHASH
                if (tryAgain)
                    continue
            ...
            while(true) {
                ...
                while (index < map.size) {
                    if (currentKey < 0 && currentValue != MOVED_VALUE && currentValue != DEL_VALUE) {
                        nextCore.value!!.putInternalWithCAS(-currentKey, currentValue, NULL_VALUE)
