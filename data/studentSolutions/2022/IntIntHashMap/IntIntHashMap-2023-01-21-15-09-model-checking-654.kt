...
class IntIntHashMap {
    ...
    private class Core constructor(capacity: Int) {
        ...
        init {
            ...
            for (i in 0 until map.size) map[i].value = 0
        }
        ...
        fun putInternal(key: Int, value: Int): Int {
            ...
            while (true) {
                ...
                while (curKey != key) { // optimize for successful lookup
                    if (curKey == NULL_KEY) {
                        if (value == DEL_VALUE) return NULL_VALUE // be careful
                ...
                if (map[index + 1].value == DEL_VALUE) println(map[index + 1].value)
                if (map[index + 1].compareAndSet(DEL_VALUE, value)) return oldValue // maybe if with Moved check to optimize
                if (map[index + 1].compareAndSet(oldValue, value)) return oldValue // maybe if with Moved check to optimize
            }
        }
            ...
            while (index < map.size) {
                while(true) {
                    ...
                    if (isValue(value)) {
                        ...
                        newCore.putInternal(map[index].value!!, value)
...
private const val MOVED_VALUE = Int.MIN_VALUE // mark for removed value
...
private fun toValue(value: Int): Int {
    return if (isValue(value)) {
        ...
        if (isFixed(value)) value and -1 else 0
    }
}