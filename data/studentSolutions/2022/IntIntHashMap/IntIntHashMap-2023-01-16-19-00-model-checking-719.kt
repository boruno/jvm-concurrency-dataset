...
class IntIntHashMap {
    ...
    private class Core(capacity: Int) {
        ...
        fun getInternal(key: Int): Int {
            ...
            if (res == MOVED) {
                return nextCore.value!!.getInternal(key)
            }
        ...
        fun putInternalLow(key: Int, value: Int, toFind: Int?): Int {
            var index = index(key)
            var probes = 0
            while (true) { // optimize for successful lookup
                val mkv = map[index].value
                ...
                if (mkv == null) {
                    if (toFind != null && toFind != DEL_VALUE) {
            ...
            while (true) {
                ...
                val oldValue = map[index + 1].value
                if (oldValue != toFind) {
                    if (toFind == DEL_VALUE && oldValue != null) {
                        return INVALID_VALUE
                    } else if (toFind != DEL_VALUE) {
                        return INVALID_VALUE
                    }
                }
                if (map[index + 1].compareAndSet(oldValue, value)) {
        ...
        fun rehash(): Core {
            ...
            while (index < map.size) {
                var past_value : Int? = null
                while (true) {
                    val k = map[index].value ?: 0
                    val v = map[index + 1].value
                    if (isValue(v ?: 0) || v == DEL_VALUE) {
                        val result = newCore.putInternalLow(k, v ?: 0, past_value)
                        if (result == INVALID_VALUE) { //we have already moved
                            break
                        }
                        past_value = v ?: 0
                        ...
                        break;//some other thread said "moved"
                    }
                }
                index += 2
            }
...
private const val INVALID_VALUE = -17
