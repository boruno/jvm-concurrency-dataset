...
class TreiberStackWithElimination<E> {
    ...
    fun pop(): E? {
        ...
        if (task != null) {
            if (task.status.compareAndSet(Status.TODO, Status.IN_WORK)) {
                val answer = task.value.value
                if (task == eliminationArray[0].value) {
        ...
        if (task1 != null) {
            if (task1.status.compareAndSet(Status.TODO, Status.IN_WORK)) {
                val answer = task1.value.value
                if (task1 == eliminationArray[1].value) {
                    task1.status.value = Status.DONE
                    return answer
                }
            }
        }
