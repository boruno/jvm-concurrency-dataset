...
class TreiberStackWithElimination<E> {
     ...
     */
    fun push(x: E) {
        val index = Rand.nextInt(0, ELIMINATION_ARRAY_SIZE)
        if (eliminationArray[index].compareAndSet(null, x)) {
            for (i in 0 until RETRY_COUNT) {
                if (eliminationArray[index].compareAndSet(State.DONE, null)) {
                    return
                }
            }
        }
        ...
        eliminationArray[index].compareAndSet(State.DONE, null)
    }
    ...
    fun pop(): E? {
        ...
        if (value != null && value != State.DONE && eliminationArray[index].compareAndSet(value, State.DONE)) {
            return value as E?
        }
