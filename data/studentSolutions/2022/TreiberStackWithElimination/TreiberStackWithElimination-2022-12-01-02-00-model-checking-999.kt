...
class TreiberStackWithElimination<E> {
    ...
    fun push(x: E) {
        ...
        val new = Node(x) // use Node object as wrapper for x
        val el = eliminationArray[elIndex].value
        if(el == null) {
            if(eliminationArray[elIndex].compareAndSet(null, new)) {
                for(i in 1..10000);
                if(eliminationArray[elIndex].compareAndSet(new, null))
                    push0(x)
            } else push0(x)
        } else push0(x)
    }
...
private class Node<E>(val x: E, val next: Node<E>? = null)
