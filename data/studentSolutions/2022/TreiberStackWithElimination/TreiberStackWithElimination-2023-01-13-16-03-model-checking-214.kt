...
class TreiberStackWithElimination<E> {
    ...
    fun push(x: E) {
        ...
        while (true) {
            ...
            if (top.compareAndSet(head, newHead)) {
                if (idx != null) {
                    while (eliminationArray[idx].value!!.status.value == Status.IN_WORK) {
                    }
                    eliminationArray[idx].value = null
                }
                return
    ...
    fun pop(): E? {
        if (eliminationArray[0].value != null) {
            if (eliminationArray[0].value!!.status.compareAndSet(Status.TODO, Status.IN_WORK)) {
                val answer = eliminationArray[0].value!!.value.value
                eliminationArray[0].value!!.status.value = Status.DONE
                return answer
            }
        }
        if (eliminationArray[1].value != null) {
            if (eliminationArray[1].value!!.status.compareAndSet(Status.TODO, Status.IN_WORK)) {
                val answer = eliminationArray[1].value!!.value.value
                eliminationArray[1].value!!.status.value = Status.DONE
                return answer
            }
        }
        while(true) {
