...
class AtomicArrayNoAba<E>(size: Int, initialValue: E) {
    ...
    private class casnDescriptor<E>(
        ...
        fun complete() {
            ...
            val res = dcss.dcssTwoIndexes(actual2, expected2, this, outcome, Res.undecided)
    ...
    private class dcssDescriptor<E1, E2>(
        ...
        ): Any? {// если вызвал на ячейке в которой стоит casn дескриптор
...
//            val update = if (actual2.getValue() == expected2) update1 else expected1
            val actVal = actual2.getValueDcss()
        ...
        fun getValue(): T {
            v.loop { cur ->
                when (cur) {
                    ...
                    is casnDescriptor<*> -> cur.complete() // getValue вызвал это, тут casn -> dcss -> casn(тк он лежит в самом низу)
