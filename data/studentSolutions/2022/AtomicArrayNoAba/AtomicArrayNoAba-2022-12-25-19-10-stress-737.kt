...
class AtomicArrayNoAba(size: Int, initialValue: Int) {
    ...
    fun cas2(
        ...
        if (index1 == index2) {
            return cas(index1, expected1, update1 + 1) // todo maybe will be fixed
        }
        if (index1 < index2) {
...
//        if (b.v.value != this && b.compareAndSet(expectB, this)) {
//            status.compareAndSet(null, true)
//        } else {
//            status.compareAndSet(null, b.v.value == this)
//        }
        val dcssDesc = Cas2DcssDescriptor(b, expectB, this, a)
        if (!b.compareAndSet(expectB, dcssDesc)) status.compareAndSet(null, false)
        status.compareAndSet(null, dcssDesc.complete())
...
private class Cas2DcssDescriptor<T>(
    val a: Ref<T>, val expectA: Any?, val updateA: Any?, val b: Ref<T>
) : Descriptor() {
    val status = atomic<Boolean?>(null)

    override fun complete(): Boolean {
        // valid in this task because after finish B will never be the same
        val newStatus = (b.v.value as Cas2Descriptor<*>).status.value == null
