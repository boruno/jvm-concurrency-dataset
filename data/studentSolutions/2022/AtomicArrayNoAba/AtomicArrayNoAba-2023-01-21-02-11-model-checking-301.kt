...
class AtomicArrayNoAba<E>(size: Int, initialValue: E) {
    ...
    fun cas2(index1: Int, expected1: E, update1: E,
        ...
        var desc1 = Desc( desc.a, desc.expectA, desc.updateA, desc.b, desc.expectB, desc.updateB, null )
        desc1.dcss( desc.b, desc.expectB, desc.updateB, desc.a, desc.expectA)
        return desc.cas2()
    }
}
...
class Desc (val a : Ref, val expectA : Any?, val updateA : Any?, val b : Ref, val expectB : Any?, val updateB : Any?,
    ...
    fun cas2 () : Boolean{
        var desc = Desc( a, expectA, updateA, b, expectB, updateB, null )
        if ( b == this  || desc.dcss( b, expectB, this, a, expectA ) ) {
    ...
    fun dcss( a: Ref, expectA: Any?, updateA: Any?, b : Ref, expectB: Any?) : Boolean {
        fl.compareAndSet( null, b.get() == expectB )
        if ( fl.value!=null ) {
            if ( fl.value!!) {
                a.value.compareAndSet( this, updateA )
                return true
