    ...
    fun get(index: Int): E {
        return when(val currentValue = a[index].value) {
            is Descriptor<*> -> {
                ...
                if (index == currentValue.index1 && status == Status.SUCCESS) {
                    currentValue.update1 as E
                }
                else if (index == currentValue.index1)
                    currentValue.expected1 as E
                else if (index == currentValue.index2 && status != Status.SUCCESS)
                    currentValue.expected2 as E
                else
                    currentValue.update2 as E
            }
...
//                return true
            if (descriptor.casStatus(Status.UNDECIDED, Status.FAILED)) {
    ...
    private fun tryPut(index: Int, descriptor: Descriptor<E>, expected: E): Boolean {
        return when (val currentValue = a[index].value) {
            ...
            else -> a[index].compareAndSet(expected, descriptor)
        }
    }
