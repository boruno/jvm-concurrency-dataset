...
class AtomicArrayNoAba<E>(size: Int, initialValue: E) {
    ...
    fun cas2(
        ...
        return if (index1 > index2) {
            Cas2Helper(index2, expected2, update2, index1, expected1, update1)
        } else {
            Cas2Helper(index1, expected1, update1, index2, expected2, update2)
        }
    ...
    private fun Cas2Helper(index1: Int, expected1: E, update1: E, index2: Int, expected2: E, update2: E): Boolean {
        val descriptor = DescriptorCAS2(index1, expected1, update1, index2, expected2, update2)

        if (casHelper(index1, expected1, descriptor)) {
            descriptor.complete()
    ...
    private fun casHelper(index: Int, expected: E, update: Any): Boolean {
        while (true) {
            val value = get(index)
    ...
    private inner class DescriptorCAS2(
        ...
        val consensus = atomic(State.UNDECIDED)
        fun complete() {
//            val one = a[index1]
//            val two = a[index2]
            if (a[index2].value == this) {
                consensus.compareAndSet(State.UNDECIDED, State.COMPLETED)
                a[index1].compareAndSet(this, update1)
                a[index2].compareAndSet(this, update2)
            } else {
                consensus.compareAndSet(State.UNDECIDED, State.FAILED)
