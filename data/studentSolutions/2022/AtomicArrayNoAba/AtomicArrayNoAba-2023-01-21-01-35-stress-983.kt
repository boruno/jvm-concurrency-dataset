...
class AtomicArrayNoAba<E>(size: Int, initialValue: E) {
    ...
    private inner class Desc(
        ...
        fun complete() {
            if (a[indexB].compareAndSet(expB, this) || a[indexB].value == this) {
                completed.value = true
            }
            if (completed.value) {
                a[indexA].compareAndSet(this, updA)
                a[indexB].compareAndSet(this, updB)
            } else {
                a[indexA].compareAndSet(this, expA)
                a[indexB].compareAndSet(this, expB)
            }
        }
    }
