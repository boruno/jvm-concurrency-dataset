...
//    fun get(index: Int): E = throw AssertionError("wtf aasdasd")
    ...
    private class casnDescriptor<E>(
        ...
        fun complete1(): Boolean {
            // а если операция?
            if (outcome.myGetValue() == Res.undecided) {
                ...
                outcome.setValue(status)
            }

            if (outcome.myGetValue() == Res.success) {
...
//            val update = if (actual2.getValue() == expected2) update1 else expected1
            val actVal = actual2.v.value//остановился тут
            if (actVal == expected2) {
