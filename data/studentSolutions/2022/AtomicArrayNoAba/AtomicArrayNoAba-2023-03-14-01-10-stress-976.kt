...
//        return true
        var desc = DWCASDescriptor(index1, expected1, update1, index2, expected2, update2)
        if (index1 > index2){
            desc = DWCASDescriptor(index2, expected2, update2, index1, expected1, update1)
        }
        val old1 = get(desc.index1)
        val old2 = get(index2)
        if (cas(desc.index1, desc.expected1, desc.update1)) {
            if (cas(desc.index2, desc.expected2, desc.update2)) {
                return true
            } else {
                cas(desc.index1, desc.update1, old1)
                return false
            }
        } else {
            return false
...
//        val updateB: E
//        if (index1 < index2) {
//            x = a[index1]; expectedA = expected1; updateA = update1
//            y = a[index2]; expectedB = expected2; updateB = update2
//        } else {
//            x = a[index2]; expectedA = expected2; updateA = update2
//            y = a[index1]; expectedB = expected1; updateB = update1
//        }
