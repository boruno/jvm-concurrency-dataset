...
class ADescriptor(
    ...
    override fun complete() {
        if (outcome.value == "UNDECIDED") {
            val descr = BDescriptor(b, expectB, updateB, a, expectA, updateA)
            b.get()

            if (dcssMod(b, expectB, descr, a, expectA)) {
                outcome.compareAndSet("UNDECIDED", "SUCCESS")
                descr.outcome.compareAndSet("UNDECIDED", "SUCCESS")
                a.v.compareAndSet(this, updateA)
                b.v.compareAndSet(descr, updateB)
                return
            }
