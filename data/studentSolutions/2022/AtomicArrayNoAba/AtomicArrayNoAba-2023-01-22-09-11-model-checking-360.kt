...
//    }
    ...
    inner class DescDCSS ( val a: Ref, val expectA: Any?, val updateA: Any?, val b: Ref, val expectB: Any?) {
        ...
        fun complete() : Boolean {
            ...
            if ( cop !is AtomicArrayNoAba<*>.DescDCSS) {
                if (!a.value.compareAndSet(expectA, this)) {
                    fl.value.compareAndSet( null, false )
                    return false
                }
            }
            fl.value.compareAndSet( null, b.value.value == expectB )
...
//            descDCSS.complete()
            if ( b.value.value === this || ( descDCSS.complete() && descDCSS.fl.value.value as Boolean ) ) {
