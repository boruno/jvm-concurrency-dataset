...
class FineGrainedBank(n: Int) {
    ...
    private val condLock = CountDownLatch(1)
    ...
    fun getAmount(index: Int): Long {
        ...
        val result: Long
        synchronized(accounts[index].mutex) {
    ...
    fun totalAmount(): Long {
        ...
        synchronized(accounts) {
            accounts.forEach { it.mutex.wait() }
        }
        result = accounts.sumOf { account ->
            account.amount
        }
        synchronized(accounts) {
            accounts.forEach { it.mutex.notify() }
        }
        return result
    }
    ...
    fun transfer(fromIndex: Int, toIndex: Int, amount: Long) {
        ...
        val firstLock: Object
        val secondLock: Object
