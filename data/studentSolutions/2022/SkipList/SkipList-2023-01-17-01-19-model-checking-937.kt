...
class SkipList<E : Comparable<E>> {
    private val maxLevelCount: Int = 10
    ...
    fun add(element: E): Boolean {
        val maxNodeLevel: Int = ThreadLocalRandom.current().nextInt(0, maxLevelCount)

        while (true) {
            if (contains(element))
                return false

            val newNode = Node(element, maxNodeLevel, maxLevelCount)
            ...
            val bottomNewNextNode = bottomSubset.next ?: bottomSubset.curr!!
    ...
    fun remove(element: E): Boolean {
        while (true) {
            ...
            for (level in removingNode.levelsCount - 1 downTo  bottomLevel + 1) {
                var prevNode = removingNodeSubset.prev!!

                while (true) {
                    if (prevNode.next[level].compareAndSet(removingNode, deletedNode))
                        break

                    prevNode = removingNodeSubset.prev
                }
            }
    ...
    private fun findSubsetOnLevel(element: E, level: Int): NodeSubset<E> {
        var prevNode = head
        var curNode: Node<E>? = head.next[level].value ?: return NodeSubset(prevNode, null, null)
        ...
        return NodeSubset(prevNode, curNode, curNode.next[level].value)
    }
...
open class Node<E : Comparable<E>>(element: E?, levelsCount: Int, maxLevelsCount: Int = levelsCount) {
    private val _element: E? = element
    val element get() = _element

    private val _next = atomicArrayOfNulls<Node<E>?>(maxLevelsCount)
    val next get() = _next

    private val _levelsCount: Int = levelsCount
    val levelsCount get() = _levelsCount

    private val _maxLevelsCount: Int = levelsCount
    val maxLevelsCount get() = _maxLevelsCount
}
