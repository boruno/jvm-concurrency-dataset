...
class TreiberStack<E> {
    ...
    fun push(x: E) {
        ...
        if (elimination[i].value == null) {
           return vanillaPush(x)
        }
        timeout = 0

        while (timeout < TIMEOUT && elimination[i].value!!.type.value != EliminationRecordType.Done) {
            timeout++
        }

        while (!elimination[i].value!!.type.compareAndSet(EliminationRecordType.Push, EliminationRecordType.Terminated)) {}
        while (!elimination[i].compareAndSet(record, null)) {}

        if (record.type.value == EliminationRecordType.Push) {
            return vanillaPush(x)
        }
