    ...
    fun enqueue(element: E) {
        while (true) {
            ...
            if (index < SEGMENT_SIZE) {
                if(curTail.value.cas(index,null, element))
                    return
            }
            else {
    ...
    fun dequeue(): E? {
        while (true) {
            ...
            if (deqIdx.value <= enqIdx.value)
                return null
            val index = deqIdx.getAndDecrement().toInt()
            if (index < SEGMENT_SIZE) {
                val element = curHead.value.get(index) ?: continue
                curHead.value.cas(index, element, null)
                return element as E?
            }
