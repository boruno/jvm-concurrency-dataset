...
class FAAQueue<E> {
    ...
    fun enqueue(element: E) {
        while (true) {
            ...
            var currentSegmentBound = currentTail.id * SEGMENT_SIZE + (SEGMENT_SIZE - 1)

            while (index > currentSegmentBound) {
                ...
                if (currentTail.next.compareAndSet(null, newSegment)) {
                    ...
                    currentSegmentBound = currentTail.id * SEGMENT_SIZE + (SEGMENT_SIZE - 1)
                }
            }
...
//                            "Seg: ${currentHead.elements[0].value}, ${currentHead.elements[1].value}")
            ...
            var currentSegmentBound = currentHead.id * SEGMENT_SIZE + (SEGMENT_SIZE - 1)
            ...
            while (index > currentSegmentBound) {
                ...
                if (currentHead.next.compareAndSet(null, newSegment)) {
                    while (!head.compareAndSet(currentHead, newSegment)) {
                    ...
                    continue
//                    else {
//                        throw Exception("Invalid state for ${head.value.id}, ${currentHead.id}, ${newSegment.id}")
//                    }
                }
                currentHead = currentHead.next.value!!
                currentSegmentBound = currentHead.id * SEGMENT_SIZE + (SEGMENT_SIZE - 1)
            }
