...
class FAAQueue<E> {
    ...
    fun dequeue(): E? {
        while (true) {
            ...
            moveHeadForward(findSegment(head.value, index / SEGMENT_SIZE))
            val indexInSegment: Int = (index % SEGMENT_SIZE).toInt()
            if (findSegment(head.value, index / SEGMENT_SIZE).cas(indexInSegment, null, BROKEN)) {
                continue
            }
            return findSegment(head.value, index / SEGMENT_SIZE).get(indexInSegment) as E?
        }
    }
