...
class FAAQueue<E> {
    ...
    fun enqueue(element: E) {
        while (true) {
            ...
            val enqIdx1 = enqIdx.getAndIncrement()
            if (enqIdx1 < SEGMENT_SIZE) {
                if (tail.cas(enqIdx1.toInt(),null, element)) {
                    return
                }
            } else {
                val newT = Segment()
                newT.put(0, element)
                if (tail.next?.cas(0, null, newT) == true) {
    ...
    fun dequeue(): E? {
        while (true) {
            ...
            return head.get(deqIdx.toInt()) as E?
        }
    }
