...
class FAAQueue<E> {
    ...
    fun findSegment(start: Segment, id: Long): Segment {
        ...
        while (newStart.getIndex() < id) {
            if (newStart.next.value == null) {
                newStart.next.value = Segment(newStart.getIndex() + 1)
            }
    ...
    fun enqueue(element: E) {
        while (true) {
            ...
            val s: Segment = findSegment(start = curTail.value, id = i / SEGMENT_SIZE)
            moveTailForward(s)
    ...
    fun dequeue(): E? {
        while (true) {
            ...
            val curHead = head
            val i = enqIdx.getAndIncrement()
            val s: Segment = findSegment(start = curHead.value, id = i / SEGMENT_SIZE)
            moveHeadForward(s)
