...
class FAAQueue<E> {
    ...
    private fun moveTailForward(segment: Segment) {
        while (segment.id > tail.value.id) tail.compareAndSet(tail.value, tail.value.next!!)
    }

    private fun moveHeadForward(segment: Segment) {
        while (segment.id > head.value.id) head.compareAndSet(head.value, head.value.next!!)
    }
