...
class FAAQueue<E> {
    ...
    fun dequeue(): E? {
        while (true) {
            ...
            return if (curHead.elements[(idx % SEGMENT_SIZE).toInt()]
                    .compareAndSet(null, BROKEN_MARKER)
            ) {
                null
            } else {
                curHead.elements[(idx % SEGMENT_SIZE).toInt()].value as E?
            }
        }
    }
     ...
     */
    val isEmpty: Boolean
        get() = (head.value.elements[(deqIdx.value % SEGMENT_SIZE).toInt()].value == null)
}
