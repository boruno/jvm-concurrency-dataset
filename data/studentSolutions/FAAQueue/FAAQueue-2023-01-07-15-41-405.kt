...
//            }
            ...
            if ((i.toInt() / SEGMENT_SIZE) > segment.getId()    /*(i.toInt() % SEGMENT_SIZE) == 0*/   /*i >= SEGMENT_SIZE*/) {
...
//                        enqIdx.value = 1
                ...
                if (i.toInt() == 0) {
                    if (tail.value.elements[i.toInt()].compareAndSet(null, element)) { return }
                } else {
                    if (tail.value.elements[i.toInt() % SEGMENT_SIZE].compareAndSet(null, element)) { return }
                }
            }
        }
    }
