...
class FAAQueue<E> {
    private val head: AtomicRef<Segment> = atomic(Segment(0))
    private val tail: AtomicRef<Segment> = atomic(Segment(0))
    ...
    fun dequeue(): E? {
        while (true) {
            ...
            while (true) {
                if (head.value.index < getSegment(curHead, index / 2).index && head.compareAndSet(head.value, getSegment(curHead, index / 2))) {
                       break
                } else {
                    break
                }
            ...
            if (getSegment(curHead, index / 2).cas((index % 2).toInt(), null, false)) {
                continue
            }

            return getSegment(curHead, index / 2).get((index % 2).toInt()) as E?
        }
    }
