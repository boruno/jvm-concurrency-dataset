...
class FAAQueue<E> {
    ...
    fun enqueue(element: E) {
        while (true) {
           // System.out.println("E " + deqIdx.value + " " + enqIdx.value)
            val curTail = tail.value
            val index = enqIdx.getAndIncrement()
            val segment : Segment// = findSegment(curTail, index)
            var cur = curTail
            ...
            if (cur.index > curTail.index)
            {
                tail.compareAndSet(curTail, cur)
            }
            if (cur.cas((index % SEGMENT_SIZE).toInt(),null, element)) {
                return
            }
        }
    }
    ...
    fun dequeue(): E? {
        while (true) {
            //System.out.println("D " + deqIdx.value + " " + enqIdx.value)
