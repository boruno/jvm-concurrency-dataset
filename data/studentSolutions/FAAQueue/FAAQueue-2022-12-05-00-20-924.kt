    ...
    fun dequeue(): E? {
        while (true) {
            ...
            if (i > SEGMENT_SIZE - 1) {
                ...
                val el = head.value.elements[i].value
                if (el == null) {
                    continue
                }
                if (head.value.elements[i].compareAndSet(el, BROKEN)) {
                    return el as E
                }
            }
        }
        ...
        get() {
            while (true) {
                ...
                if (cur_head.next.value == null) {
                    if (head.value == tail.value && (enqIdx.value <= deqIdx.value || deqIdx.value > SEGMENT_SIZE) ) {
                        return true
                    }
                    return false
                }
