...
class FAAQueue<E> {
        ...
        while (true) {
            ...
            val curIndex = deqIdx.getAndUpdate {
                if (deqIdx.value >= enqIdx.value) {
                    println("< empty ${enqIdx.value}|${deqIdx.value}")
                    return null
                } else {
                    it + 1
                }
            ...
            val my = moveHead(curHead, curIndex) ?: return null
    ...
    private fun moveHead(curHead: Segment, idx: Long): Segment? {
        var my = curHead
        while (!my.inRange(idx)) {
