...
//            println("\n DEqUEUE----------")
            ...
            if (head.compareAndSet(currentHead, segment)) {
                if (i.toInt() == 0) {
                    val value = head.value.elements[i.toInt()].getAndSet(null)
                    if (value != null) {
                        return value as E?
                    }
                } else {
                    val value = head.value.elements[(i % SEGMENT_SIZE).toInt()].getAndSet(null)
                    if (value != null) {
                        return value as E?
                    }
                }
...
//                println("-------\n")
                ...
                if ((head.value == tail.value && deqIdx.value >= enqIdx.value) || deqIdx.value >= enqIdx.value) {
                    val currentHeadNext = currentHead.next.value
                    if (currentHeadNext != null) {
                        head.compareAndSet(currentHead, currentHeadNext)
                    } else {
    ...
    fun findSegment(start: Segment, index: Int) : Segment {
        ...
        while (startSegment.getId() != id) {
            ...
            if (nextSegment != null) {
                ...
                startSegment.next.compareAndSet(next, newSegment)
