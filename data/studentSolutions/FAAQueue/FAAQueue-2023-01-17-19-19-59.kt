...
class FAAQueue<E> {
    ...
    fun enqueue(element: E) {
        while (true) {
            if (deqIdx.value < enqIdx.value || deqIdx.compareAndSet(deqIdx.value, enqIdx.value)) {
    ...
    fun dequeue(): E? {
        while (true) {
            ...
            val s = findSegment2(curHead, i / SEGMENT_SIZE);
            ...
            head.value.cas((i % SEGMENT_SIZE).toInt(), res, null);
            if (res == BAD_ELEM) {
                return null
            }
            return res as E;
    ...
    private fun findSegment2(curHead: Segment, id: Long): Segment {
        var cur : Segment? = curHead;
        while (true) {
            if (cur != null) {
                if (cur.index == id || cur.next == null) {
                    return cur;
                }
                cur = cur.next;
            } else {
