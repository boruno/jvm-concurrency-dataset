...
class FAAQueue<E> {
    ...
    fun enqueue(element: E) {
        while (true) {
            ...
            val i: Int = enqIdx.getAndIncrement().toInt()
            if (enqIdx.value >= SEGMENT_SIZE) {
                ...
                this.tail.compareAndSet(curTail, curTail.next!!)
            } else if (tail.value.elements[i].compareAndSet(null, element)) {
                return
            }
        }
    ...
    fun dequeue(): E? {
        while (true) {
            ...
            val i: Long = deqIdx.getAndIncrement()
            if (i >= SEGMENT_SIZE) {
                val nextHead = curHead.next ?: return null
                head.compareAndSet(curHead, nextHead)
                continue
            }
            val res = curHead.elements[i.toInt()].getAndSet(Any()) ?: continue
            return res as E?
        }
    }
