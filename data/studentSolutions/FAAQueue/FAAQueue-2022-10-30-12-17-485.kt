...
class FAAQueue<E> {
    ...
    fun dequeue(): E? {
        while (true) {
            ...
            if (currentHead.id != segment.id) {
                ...
                } else if (!head.compareAndSet(currentHead, segment))
                    continue
            }
            ...
            val element = segment.get((index % SEGMENT_SIZE).toInt()) as E?
    ...
    private fun findSegment(startSegment: Segment, index: Long): Segment {
        var bound = (startSegment.id * SEGMENT_SIZE) + (SEGMENT_SIZE - 1)
        ...
        while (index > bound) {
            if (segment.next.value != null) {
                ...
                segment = Segment(index / SEGMENT_SIZE)
            }

            bound = (segment.id * SEGMENT_SIZE) + (SEGMENT_SIZE - 1)
        }
