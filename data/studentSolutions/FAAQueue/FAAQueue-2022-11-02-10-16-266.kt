...
class FAAQueue<E> {
    ...
    fun enqueue(element: E) {
        while (true) {
            ...
            while (index > getSegmentUpperBound(tail.value.id)) {
                val nextTail = tail.value.next.value ?: break

                tail.compareAndSet(tail.value, nextTail)
            }
...
//                if (head.value.next.value == null)
            ...
            while (index > getSegmentUpperBound(head.value.id)) {
                val nextHead = head.value.next.value ?: break

                head.compareAndSet(head.value, nextHead)
            }
