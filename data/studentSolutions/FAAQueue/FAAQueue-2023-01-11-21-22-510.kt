...
//                    head.compareAndSet( cur_head.value, cur_head.value.next.value!! )
            ...
            if ( head.compareAndSet( head_copy, cur_head ) ) {
                if ( head.value.elements[(i % SEGMENT_SIZE).toInt()].compareAndSet(null, -1333)) {
                    continue
                } else {
                    if ( cur_head.elements[(i % SEGMENT_SIZE).toInt()].value!!.equals(-1333))
