...
class FAAQueue<E> {
    ...
    fun enqueue(element: E) {
        while(true) {
            ...
            if (enqIdx.mod(SEGMENT_SIZE) == 0) {
                ...
                val newTail = Segment()
                newTail.elements[enqIdx.mod(SEGMENT_SIZE)].value = element
                println("enqueu " + element)
                if (curTail.next == null) {
                ...
                if (curTail.elements[enqIdx.mod(SEGMENT_SIZE)].compareAndSet(null, element)) {
                    println("enqueu " + element)
    ...
    fun dequeue(): E? {
        while (true) {
            ...
            if ((deqIdx.mod(SEGMENT_SIZE)) == 0) {
                if (curHead.next != null) {
                    head.compareAndSet(curHead, curHead.next!!)
                    continue
                } else {
                    println("dequeue " + null)

                    return null
                }
            }
            val res = curHead.elements[deqIdx.mod(SEGMENT_SIZE)].getAndSet(DONE)
