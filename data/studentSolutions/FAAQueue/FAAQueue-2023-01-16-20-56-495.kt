...
class FAAQueue<E> {
    ...
    fun dequeue(): E? {
        while (true) {
            ...
            if (i / SEGMENT_SIZE < curHead.next.value!!.id) {
                continue
            }
            val value = curHead.next.value!!.elements[(i % SEGMENT_SIZE).toInt()].value
            if (value != null && value != "0") {
                if (curHead.next.value!!.elements[(i % SEGMENT_SIZE).toInt()].compareAndSet(value, "0")) {
                    // println("dequeue $value id $i")
                    return value as E
                }
            } else {
                if (!curHead.next.value!!.elements[(i % SEGMENT_SIZE).toInt()].compareAndSet(null, "0")) {
                    val valueNew = curHead.next.value!!.elements[(i % SEGMENT_SIZE).toInt()].value
                    if (valueNew != "0") {
                        // println("this code shouldn't work, but IDK, maybe work")
                        return valueNew as E
                    }
