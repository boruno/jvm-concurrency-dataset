...
class FAAQueue<E> {
    ...
    fun enqueue(element: E) {
        while (true) {
            ...
            println("[${Thread.currentThread().id}] enc to seg ${curSegment.id}")
    ...
    fun dequeue(): E? {
        while (true) {
            ...
            println("[${Thread.currentThread().id}] deq from seg ${curSegment.id}")
            if (curSegment.id > curHead.id) { //??
                tail.compareAndSet(curHead, curSegment)
            }
