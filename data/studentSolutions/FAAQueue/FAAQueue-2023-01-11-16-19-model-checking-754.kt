...
class FAAQueue<E> {
  ...
  fun dequeue(): E? {
    while (true) {
      ...
      if (currentHead.deqIdx.value >= SEGMENT_SIZE || currentHead.deqIdx.value >= currentHead.enqIdx.value) {
        val next = currentHead.next.value ?: if (isEmpty) return null else continue
  ...
  val isEmpty: Boolean
    get() {
      return head.value.deqIdx.value >= head.value.enqIdx.value && head.value.get(head.value.enqIdx.value.toInt()) == DONE
    }
}
