...
class FAAQueue<E> {
    ...
    fun dequeue(): E? {
        while (true) {
            ...
            moveHeadForward(findSegment(head.value, deqIdx.getAndAdd(1) / SEGMENT_SIZE))
            val indexInSegment: Int = (deqIdx.getAndAdd(1) % SEGMENT_SIZE).toInt()
            if (findSegment(head.value, deqIdx.getAndAdd(1) / SEGMENT_SIZE).cas(indexInSegment, null, BROKEN)) {
                continue
            }
            return findSegment(head.value, deqIdx.getAndAdd(1) / SEGMENT_SIZE).get(indexInSegment) as E?
        }
    }
