...
//                throw ExceptionInInitializerError()
            while ( i > ( cur_tail.value.id + 1 ) * SEGMENT_SIZE ) {
                val new_seg = Segment()
            ...
            if ( tail.value.elements[ ( ( i + 1 ) % SEGMENT_SIZE ).toInt() ].compareAndSet( tail.value.elements[ ( ( i + 1) % SEGMENT_SIZE ).toInt() ], element ) ) {
                return
            } else {
