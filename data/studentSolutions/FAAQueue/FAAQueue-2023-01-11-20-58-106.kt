...
class FAAQueue<E> {
    ...
    fun enqueue(element: E) {
        while ( true ) {
            val copy_tail = tail
            val cur_tail = copy_tail
            val i = enqIdx.incrementAndGet()
            var seg = Segment()
            if ( cur_tail.value.next.compareAndSet( null, seg ) ) {
                cur_tail.value.next.value!!.id = cur_tail.value.id + 1
            }
            while ( i > cur_tail.value.id * SEGMENT_SIZE ) {
                var seg1 = Segment()
                if ( cur_tail.value.next.compareAndSet( null, seg1 ) ) {
                    cur_tail.value.next.value!!.id = cur_tail.value.id + 1
                }
                cur_tail.getAndSet( cur_tail.value.next.value!! )
            }
            tail.compareAndSet( copy_tail.value, cur_tail.value )
