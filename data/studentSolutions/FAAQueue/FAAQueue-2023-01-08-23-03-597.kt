...
class FAAQueue<E> {
    ...
    fun dequeue(): E? {
        while (true) {
            ...
            val expectedValue = segment.get((i % SEGMENT_SIZE).toInt()) ?: continue

            if (segment.cas((i % SEGMENT_SIZE).toInt(), expectedValue, null)) {
                if (head.compareAndSet(currentHead, segment)) {
                    return expectedValue as E
                }
            }
//            if (head.compareAndSet(currentHead, segment)) {
//                if (segment.cas((i % SEGMENT_SIZE).toInt(), null, Any())) {
//                    continue
//                } else {
//                    val value = segment.get((i % SEGMENT_SIZE).toInt())
//                    return value as E?
//                }
//            }
        }
