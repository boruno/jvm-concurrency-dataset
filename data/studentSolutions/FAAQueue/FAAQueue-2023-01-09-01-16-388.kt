...
class FAAQueue<E> {
    ...
    fun dequeue(): E? {
        while (true) {
            ...
            val (dequed, vt) = oldHead.deque()
            val isTop = oldHead.isTop(vt)
            val isBot = oldHead.isBottom(vt)
            if (isTop || isBot) {
                println("top or bot")
            }
            if (dequed != null) {
...
private class Segment<E>() {
    ...
    fun deque(): Pair<E?, ValueType?> {
        ...
        var ret: ValueType? = null
        if (oldDeqIdx < SEGMENT_SIZE) {
            ret = elements[oldDeqIdx].value!!.type
        }
        return null to ret
    }

    fun isTop(vt: ValueType?): Boolean {
        return vt != null && vt == ValueType.TOP
    }

    fun isBottom(vt: ValueType?): Boolean {
        return vt != null && vt == ValueType.BOTTOM
    }
}
