...
class FAAQueue<E> {
    ...
    fun dequeue(): E? {
        while (true) {
            ...
            moveHeadForward(cur_head, s)
            if (s.cas((i % SEGMENT_SIZE).toInt(), null, "hui")) continue
            // s.get(i)
            // try {
            val res = s.get(i.toInt() % SEGMENT_SIZE) ?: return null
            println("ok, $i")
            return res as E?
            // } catch (e: Exception) {
            //     println("IDX $i")
            //     // println(head.value.elements.)
            //     println("first ${head.value.firstId}")
            //     // println("array ${head.value.get(0)}")
            //     //
            //
            //     // println(head.value.next)
            // }
        }
    }
