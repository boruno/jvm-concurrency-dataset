...
class FAAQueue<E> {
    ...
    fun enqueue(element: E) {
        while (true) {
            ...
            if (!curTail.inRange(curIndex)) {
               if (tail.value.next.compareAndSet(null, Segment(curIndex, Segment.Element(element)))) {
                   return
               }
                continue
            }
    ...
    val isEmpty: Boolean
        get() {
            return enqIdx.value == deqIdx.value || head.value.next.value == null && !head.value.inRange(deqIdx.value)
        }

    companion object {
        @JvmStatic
        fun main(args: Array<String>) {
            val test = FAAQueue<Int>()
