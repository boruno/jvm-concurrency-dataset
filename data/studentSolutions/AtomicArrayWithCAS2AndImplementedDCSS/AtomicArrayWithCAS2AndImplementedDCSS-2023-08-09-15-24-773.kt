...
//                        if (array.compareAndSet(index, expected, this)) return true
                        if (dcss(index, expected, this, status, UNDECIDED)) return true else return false
                    }
                    else -> {
                        // Unexpected value
                        return false
        ...
        private fun updatePhysically() {
            ...
            if (success) {
                if (dcss(index1, this, update1, status, SUCCESS))
                    dcss(index2, this, update2, status, SUCCESS)
            } else {
                if (dcss(index1, this, expected1, status, FAILED))
