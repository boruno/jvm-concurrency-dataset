...
class AtomicArrayWithCAS2AndImplementedDCSS<E : Any>(size: Int, initialValue: E) {
    ...
    fun dcss(
        ...
        if (array.compareAndSet(index, expectedCellState, AnotherFuckingDescriptor())) {
            if (statusReference.compareAndSet(expectedStatus, DCSS)) {
                array.set(index, updateCellState)
                statusReference.set(DCSS)
            } else {
    ...
    class AnotherFuckingDescriptor {
