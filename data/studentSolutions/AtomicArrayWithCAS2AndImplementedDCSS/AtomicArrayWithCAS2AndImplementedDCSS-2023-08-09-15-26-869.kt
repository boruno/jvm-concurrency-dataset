...
class AtomicArrayWithCAS2AndImplementedDCSS<E : Any>(size: Int, initialValue: E) {
        ...
        private fun install(idx: Int, exp: Any): Boolean {
            while (true) {
                ...
                when (val cellState = array.get(idx)) {
                    ...
                    is AtomicArrayWithCAS2AndImplementedDCSS<*>.DcssDescriptior -> cellState.apply()
                    exp -> {
    ...
    inner class DcssDescriptior(
        ...
        fun install(idx: Int, exp: Any?): Boolean {
            while (true) {
                if (status.get() != UNDECIDED || statusReference.get() != expectedStatus) return false
                when (val cellState = array.get(idx)) {
                    this -> return true
                    is AtomicArrayWithCAS2AndImplementedDCSS<*>.CAS2Descriptor -> cellState.apply()
                    is AtomicArrayWithCAS2AndImplementedDCSS<*>.DcssDescriptior -> cellState.apply()
    ...
    fun dcss(
        ...
        dcss.apply()
        return dcss.status.get() == SUCCESS
    }
}