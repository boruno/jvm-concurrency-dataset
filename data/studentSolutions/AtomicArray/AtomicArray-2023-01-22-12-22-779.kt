...
class AtomicArray<E>(size: Int, initialValue: E) {
    ...
    fun get(index: Int) =
        a[index].value!!.valueOut as E

    fun set(index: Int, value: E) {
        a[index].value!!.valueOut = value
    }
    ...
    private class Ref<T>(initial: T) {
        val v: AtomicRef<Any?> = atomic(initial as Any?)

        var valueOut: T
            get() {
                v.loop { cur ->
                    when(cur) {
                        is RDCSSDescriptor<*, *> -> cur.complete()
    ...
    private class RDCSSDescriptor<A, B> (
        val a: Ref<A>, val expectedA: A, val updatedA: Descriptor<*>,
        val b: Ref<B>, val expectedB: B
    ) {
        fun complete() {
            if (b.valueOut === expectedB) {
                a.v.compareAndSet(this, updatedA)
            } else {
                a.v.compareAndSet(this, expectedA)
            }
