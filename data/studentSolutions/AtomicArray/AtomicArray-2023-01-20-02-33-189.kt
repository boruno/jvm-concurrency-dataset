...
class AtomicArray<E : Any>(size: Int, initialValue: E) {
    ...
    class CASNDescriptor<E : Any>(
        ...
        var desc : RDCSSDescriptor<*>? = null
        override fun complete(): Boolean {
            if(b.v.value!=this){
                if(desc==null){
                    desc = RDCSSDescriptor(b, expectB, this)
                }
                if(b.CAS(expectB, desc!!)||b.v.value==desc){
                    desc!!.complete()
                }
            }
            if (b.v.value==this) {
    ...
    class RDCSSDescriptor<E : Any>(
        ...
        fun dcss():Boolean{
            if(!a.CAS(expectA,this)){
                if(a.v.value is RDCSSDescriptor<*>){
