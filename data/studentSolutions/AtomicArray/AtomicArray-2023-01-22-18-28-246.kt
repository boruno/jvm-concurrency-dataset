...
class AtomicArray<E>(size: Int, initialValue: E) {
    ...
    fun cas2(index1: Int, expected1: E, update1: E,
        ...
        return if (index1 == index2) {
            ...
            return when {
                index1 > index2 ->
                    CAS2Descriptor(a[index2], expected2, update2, a[index1], expected1, update1).complete()//.run(a[index2], expected2)
                else ->
                    CAS2Descriptor(a[index1], expected1, update1, a[index2], expected2, update2).complete()//run(a[index1], expected1)
            }
        }
    }
}
...
class CAS2Descriptor<E>(
    ...
    override fun complete() : Boolean{
        if(!value1.cas(expected1, this)) return false
