...
class AtomicArray<E>(size: Int, initialValue: E) {
    ...
    fun cas(index: Int, expected: E, update: E) =
        cas1or2(false, index, expected, update, index, expected, update)

    fun cas2(index1: Int, expected1: E, update1: E,
             index2: Int, expected2: E, update2: E) =
        cas1or2(true, index1, expected1, update1, index2, expected2, update2)
...
//        println("v1: $v1")
        if (v1 is Descriptor<*> && v1.cur == expected1 &&
            (v1.st.compareAndSet(Status.INITIAL.ordinal, Status.FAILED.ordinal) ||
                    v1.st.get() == Status.FAILED.ordinal) && a[index1].compareAndSet(v1, v1.cur)) {
            return false
        }

        var v2 = a[index2].value
//        println("v2: $v2")
        if (v2 is Descriptor<*> && v2.cur == expected2 &&
            (v2.st.compareAndSet(Status.INITIAL.ordinal, Status.FAILED.ordinal) ||
                    v2.st.get() == Status.FAILED.ordinal) && a[index2].compareAndSet(v2, v2.cur)) {
            return false
        }
        ...
        if (v1 !is Descriptor<*> && v2 !is Descriptor<*>) {
            v1 = Descriptor(expected1, if (index1 == index2 && update1 is Int && is2) update1 + 1 else update1, st)
            v2 = Descriptor(expected2, if (index1 == index2 && update2 is Int && is2) update2 + 1 else update2, st)
