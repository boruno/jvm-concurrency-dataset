...
class AtomicArray<E : Any>(size: Int, initialValue: E) {
    ...
    private fun doubleCompareSingleSwap(
        indexA: Int, expectedA: Any, updateA: AtomicArray<*>.CasnDescriptor,
    ...
    private inner class DcssDescriptor(
        val indexA: Int,
        val expectedA: Any,
        val updateA: AtomicArray<*>.CasnDescriptor,
        val indexB: Int,
        val expectedB: Any,
    ) : Descriptor {
        override fun complete() {
            ...
            val casnOutcome = updateA.outcome.value

            if (b != expectedB) {
                updateA.outcome.compareAndSet(CasnOutcome.UNDEFINED, CasnOutcome.FAILURE)
            } else if (casnOutcome == CasnOutcome.UNDEFINED) {
                updateA.outcome.compareAndSet(CasnOutcome.UNDEFINED, CasnOutcome.SUCCESS)
            }

            if (updateA.outcome.value == CasnOutcome.SUCCESS) {
