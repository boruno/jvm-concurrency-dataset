...
class AtomicArray<E : Any>(size: Int, initialValue: E) {
    ...
    fun cas2(index1: Int, expected1: E, update1: E,
        ...
        if (!doubleCompareSingleSwap(descriptor.indexA, descriptor.expectedA, descriptor, descriptor.indexB, descriptor.expectedB, descriptor))
            return false

        // Check for '!= this' to prevent failing when other operation helps.
        if (!doubleCompareSingleSwap(descriptor.indexB, descriptor.expectedB, descriptor, descriptor.indexA, descriptor, descriptor) && a[descriptor.indexB].value != descriptor) {
    ...
    private fun doubleCompareSingleSwap(
        ...
        indexB: Int, expectedB: Any, descriptor: CasnDescriptor
    ): Boolean {
        if (a[indexA].value != expectedA || a[indexB].value != expectedB)
            return false

        if (descriptor.outcome.compareAndSet(CasnOutcome.UNDEFINED, CasnOutcome.UNDEFINED))
        ...
        val updateB: E)
    {
        ...
        fun complete() {
            ...
            if (!doubleCompareSingleSwap(indexA = indexB, expectedA = expectedB, updateA = this, indexB = indexA, expectedB = this, descriptor =  this) && a[indexB].value != this) {
