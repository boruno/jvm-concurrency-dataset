...
class AtomicArray<E : Any>(size: Int, initialValue: E) {
    ...
    class CASNDescriptor<E : Any>(
        ...
        override fun complete(): Boolean {
            val res = b.v.value == this || RDCSSDescriptor(b, expectB, this).dcss()
//            if(b.v.value!=this){
//                val desc = RDCSSDescriptor(b, expectB, this)
...
//            }
    ...
    class RDCSSDescriptor<E : Any>(
        ...
        fun dcss(): Boolean {
            ...
            complete()
            return a.v.value == updateA
        }

        override fun complete(): Boolean {
            when (updateA.outcome.value) {
                OUTCOME.UNDECIDED -> outcome.compareAndSet(OUTCOME.UNDECIDED, OUTCOME.SUCCESS)
                else -> outcome.compareAndSet(OUTCOME.UNDECIDED, OUTCOME.FAILED)
            }
            when (outcome.value) {
                OUTCOME.FAILED -> a.v.compareAndSet(this, expectA)
                OUTCOME.SUCCESS -> a.v.compareAndSet(this, updateA)
            }
