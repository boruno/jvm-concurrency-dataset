...
class FineGrainedBank(n: Int) {
    ...
    fun getAmount(index: Int): Long {
        ...
        accounts[index].cnt.incrementAndGet()
        val amount = accounts[index].amount
        if (accounts[index].cnt.decrementAndGet() == 0)
    ...
    fun deposit(index: Int, amount: Long): Long {
        ...
        try {
            ...
            if (account.cnt.decrementAndGet() == 0)
                account.lock.unlock()
        }
    }
    ...
    fun transfer(fromIndex: Int, toIndex: Int, amount: Long) {
        ...
        try {
            if (fromIndex <= toIndex) {
                from.lock.lock()
                from.cnt.incrementAndGet()
                to.lock.lock()
                to.cnt.incrementAndGet()
            } else {
                to.lock.lock()
                to.cnt.incrementAndGet()
                from.lock.lock()
            ...
            if (from.cnt.decrementAndGet() == 0)
                from.lock.unlock()
            if (to.cnt.decrementAndGet() == 0)
                to.lock.unlock()
        }
    }
