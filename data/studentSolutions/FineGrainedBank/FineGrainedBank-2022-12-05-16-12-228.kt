...
class FineGrainedBank(n: Int) {
    ...
    fun totalAmount(): Long {
        ...
        try {
            ...
            for (index in accounts.indices) {
                accounts[accounts.size - index - 1].lock.unlock()
            }
        }
    }
    ...
    fun transfer(fromIndex: Int, toIndex: Int, amount: Long) {
        ...
        accounts[min(toIndex, fromIndex)].lock.lock()
        accounts[max(toIndex, fromIndex)].lock.lock()
        try {
            ...
            accounts[max(toIndex, fromIndex)].lock.unlock()
            accounts[min(toIndex, fromIndex)].lock.unlock()
        }
    }
