...
class FineGrainedBank(accountsNumber: Int) : Bank {
    ...
    override fun transfer(fromId: Int, toId: Int, amount: Long) {
        ...
        val fromAmount = from.lock.withLock {
            from.amount
        }
        check(amount <= fromAmount) { "Underflow" }
        val to = accounts[toId]
        val toAmount = to.lock.withLock {
            to.amount
        }
        check(!(amount > MAX_AMOUNT || toAmount + amount > MAX_AMOUNT)) { "Overflow" }
        from.lock.withLock {
            from.amount -= amount
        }
        to.lock.withLock {
