...
class FineGrainedBank(n: Int) {
    ...
    fun getAmount(index: Int): Long {
        ...
        while (!accounts[index].locked.compareAndSet(false, true)) {}
        val amount = accounts[index].amount
        accounts[index].locked.set(false)
        return amount
    }
    ...
    fun deposit(index: Int, amount: Long): Long {
        ...
        try {
            while (!account.locked.compareAndSet(false, true)) {}
    ...
    fun transfer(fromIndex: Int, toIndex: Int, amount: Long) {
        ...
        try {
            if (fromIndex <= toIndex) {
                while (!from.locked.compareAndSet(false, true)) {}
                while (!to.locked.compareAndSet(false, true)) {}
            } else {
                while (!to.locked.compareAndSet(false, true)) {}
                while (!from.locked.compareAndSet(false, true)) {}
            }
