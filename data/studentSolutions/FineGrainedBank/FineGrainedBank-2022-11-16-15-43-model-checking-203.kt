...
class FineGrainedBank(n: Int) {
    ...
    fun getAmount(index: Int): Long {
        val amount: Long
        kotlin.synchronized(accounts[index]) {
    ...
    fun transfer(fromIndex: Int, toIndex: Int, amount: Long) {
        ...
        val lessIndex = minOf(fromIndex, toIndex)
        val greaterIndex = maxOf(fromIndex, toIndex)
        if (lessIndex == fromIndex) {
            kotlin.synchronized(accounts[fromIndex]) {
                kotlin.synchronized(accounts[toIndex]) {
