...
class FineGrainedBank(accountsNumber: Int) : Bank {
    ...
    // Correct version - Order locks
//    override fun transfer(fromId: Int, toId: Int, amount: Long) {
//        // TODO: Make this operation thread-safe via fine-grained locking.
//        require(amount > 0) { "Invalid amount: $amount" }
//        require(fromId != toId) { "fromId == toId" }
//        val fromLock = accounts[fromId] to fromId
//        val toLock = accounts[toId] to toId
//        val from = accounts[fromId]
//        val to = accounts[toId]
//        listOf(fromLock, toLock).sortedBy { it.second }.map { it.first }.let {
//            it[0].locked {
//                it[1].locked {
//                    check(amount <= from.amount) { "Underflow" }
//                    check(!(amount > MAX_AMOUNT || to.amount + amount > MAX_AMOUNT)) { "Overflow" }
