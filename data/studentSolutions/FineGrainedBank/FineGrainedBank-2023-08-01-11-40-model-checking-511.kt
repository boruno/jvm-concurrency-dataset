...
class FineGrainedBank(accountsNumber: Int) : Bank {
    ...
    override fun transfer(fromId: Int, toId: Int, amount: Long) {
        ...
        listOf(fromId, toId).sortedBy { it }.map { accounts.get(it) }.forEach { account -> account.lock.lock() }
                check(amount <= from.amount) { "Underflow" }
                check(!(amount > MAX_AMOUNT || to.amount + amount > MAX_AMOUNT)) { "Overflow" }
                from.amount -= amount
                to.amount += amount
        listOf(fromId, toId).sortedBy { it }.map { accounts.get(it) }.forEach { account -> account.lock.unlock() }
