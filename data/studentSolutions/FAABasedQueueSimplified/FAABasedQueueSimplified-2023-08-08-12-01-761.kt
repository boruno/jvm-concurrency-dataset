...
//        enqIdx.set(i + 1)
        while (true) {
            ...
            if (infiniteArray.compareAndSet(i.toInt(), null, element)) break
        }
...
//        if (deqIdx.get() >= enqIdx.get()) return null
        while (true) {
            if (shouldWeSkipThisIteration()) continue
            // TODO: Increment the counter atomically via Fetch-and-Add.
            // TODO: Use `getAndIncrement()` function for that.
//        val i = deqIdx.get()
//        deqIdx.set(i + 1)
...
//        return infiniteArray.get(i.toInt()) as E
    ...
    private fun shouldWeSkipThisIteration(): Boolean {
        while (true) {
            val curEndIdx = enqIdx.get()
            val curDeqIdx = deqIdx.get()
            if (curEndIdx == enqIdx.get()) {
                return curDeqIdx >= curEndIdx
            }
        }
    }
