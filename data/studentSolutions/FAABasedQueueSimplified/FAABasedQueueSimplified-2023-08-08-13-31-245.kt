    ...
    override fun enqueue(element: E) {
        ...
        while (true) {
            ...
            if (infiniteArray.compareAndSet(i.toInt(), null, element)) {
                println("Enqueued $element ${Thread.currentThread().id} encIdx: $i")
                return
            }
        }
...
//                continue
//            }
//            return infiniteArray.get(curDeq.toInt()) as E
            if (isQueueEmpty()) {
                return null
            }
            val curDeq = deqIdx.getAndIncrement()
            val item = infiniteArray.get(curDeq.toInt())

            // Check if the item is null, which means this cell is already poisoned
            if (item == null) {
                continue
            }

            // Try to set the cell to null (poison it)
            if (infiniteArray.compareAndSet(curDeq.toInt(), item, null)) {
                // If successful, return the item
                return item as E
            }
        }
