...
//            val i = enqIdx.get()
//            enqIdx.set(i + 1)
            val nextIndex = enqIdx.getAndIncrement()

            // TODO: Atomically install the element into the cell
            // TODO: if the cell is not poisoned.
//            infiniteArray.set(i.toInt(), element)
            if (infiniteArray.compareAndSet(nextIndex.toInt(), null, element)) {
                return
            }
        }
    }
...
//            val i = deqIdx.get()
//            deqIdx.set(i + 1)
            val nextIndex = deqIdx.getAndIncrement()

            // TODO: Try to retrieve an element if the cell contains an
            // TODO: element, poisoning the cell if it is empty.
//            return infiniteArray.get(i.toInt()) as E
            if (infiniteArray.compareAndSet(nextIndex.toInt(), null, POISONED)) {
                continue
            }

            return infiniteArray.get(nextIndex.toInt()) as? E
        }
    }

    private fun shouldDequue(): Boolean {
        while (true) {
