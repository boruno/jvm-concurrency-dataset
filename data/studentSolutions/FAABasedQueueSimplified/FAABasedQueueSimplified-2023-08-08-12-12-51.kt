...
//        infiniteArray.set(i.toInt(), element)
    ...
    override fun dequeue(): E? {
        while (true) {
            ...
            val element = infiniteArray[i.toInt()]
            if (infiniteArray.compareAndSet(i.toInt(), element, null)) {
                return element as E
            }
            infiniteArray[i.toInt()] = POISONED
        }
//
//        // Is this queue empty?
...
//        return infiniteArray.get(i.toInt()) as E
    ...
    private fun isQueueEmpty(): Boolean {
        while (true) {
            val enqIdx1 = enqIdx.get()
            val deqInx = deqIdx.get()
            val enqIdx2 = enqIdx.get()
            if (enqIdx1 == enqIdx2) {
                return enqIdx1 <= deqInx
            }
        }
    }
}
