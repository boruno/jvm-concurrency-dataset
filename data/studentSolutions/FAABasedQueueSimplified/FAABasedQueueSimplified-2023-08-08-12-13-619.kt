...
class FAABasedQueueSimplified<E> : Queue<E> {
    ...
    override fun enqueue(element: E) {
        while (true) {
            val currentEnqIdx = enqIdx.getAndIncrement().toInt()
            if (infiniteArray.compareAndSet(currentEnqIdx, null, element)) {
                return
            }
        }
    }
    ...
    override fun dequeue(): E? {
        while (true) {
            ...
            val currentDeqIdx = deqIdx.getAndIncrement().toInt()
            val currentValue = infiniteArray.get(currentDeqIdx)
            if (infiniteArray.compareAndSet(currentDeqIdx, currentValue, null)) {
                return currentValue as E
            }
            infiniteArray.compareAndSet(currentDeqIdx, null, POISONED)
        }
    }
