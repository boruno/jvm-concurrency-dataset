...
class FAABasedQueueSimplified<E> : Queue<E> {
    ...
    override fun enqueue(element: E) {
        while (true) {
            ...
            if (infiniteArray[enqIdx.get().toInt()] != POISONED) {
                infiniteArray.compareAndSet(enqIdx.get().toInt(), null, element)
                return
            }
        }
    }
    ...
    override fun dequeue(): E? {
        while (true) {
            ...
            if (infiniteArray.compareAndSet(deqIdx.get().toInt(), null, POISONED))
                continue
            else
                return infiniteArray.get(deqIdx.get().toInt()) as E
        }
    }
