...
class FAABasedQueueSimplified<E> : Queue<E> {
    ...
    override fun enqueue(element: E) {
        while (true) {
            ...
            if (infiniteArray.compareAndSet(enqueIdx.toInt(), null, element)) {
                return
            }
        }
    }
    ...
    override fun dequeue(): E? {
        while (true) {
            if (deqIdx.get() > enqIdx.get()) return null // Changed
            val dequeIdx = deqIdx.getAndIncrement()
            if (infiniteArray.compareAndSet(dequeIdx.toInt(), null, POISONED)) {
                continue
            }
            return infiniteArray.get(dequeIdx.toInt()) as E?
        }
    }
