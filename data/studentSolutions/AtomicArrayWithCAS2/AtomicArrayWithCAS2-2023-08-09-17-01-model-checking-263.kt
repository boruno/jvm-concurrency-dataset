...
class AtomicArrayWithCAS2<E : Any>(size: Int, initialValue: E) {
    ...
    fun get(index: Int): E {
        return when (val state = array[index]) {

            is AtomicArrayWithCAS2<*>.DcssDescriptor -> {
                if (state.status.get() == SUCCESS) {
                    extract(index, state.update)
                } else {
                    extract(index, state.expected)
                }
            }
    ...
    private fun extract(index: Int, element: Any?): E {
        return when (element) {
            is AtomicArrayWithCAS2<*>.CAS2Descriptor -> {
                return if (element.status.get() == SUCCESS) {
                    element.getUpdateValue(index) as E
                } else {
                    element.getExpectedValue(index) as E
                }
            }
