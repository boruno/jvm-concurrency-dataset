...
class AtomicArrayWithCAS2<E : Any>(size: Int, initialValue: E) {
    ...
    fun cas(index: Int, expected: E, update: E): Boolean {
        val descriptor = Descriptor(-1, UNIVERSAL, UNIVERSAL, index, expected, update)
        descriptor.apply()
    ...
    private inner class Descriptor(
        ...
        fun apply() {
            if (status.value == UNDECIDED) {
                val install1 = if (index1 == -1) true else installOrHelp(true)
        ...
        private fun installOrHelp(first: Boolean): Boolean {
            ...
            while (true) {
                ...
                if (current === this) {
                    return true
                } else if (current is AtomicArrayWithCAS2<*>.Descriptor) {
                    current.apply()
