        ...
        private fun installCell(cellIndex: Int, expected: E?): Boolean {
            while (true) {
                ...
                when(val value = array[cellIndex]) {
                    ...
                    is AtomicArrayWithCAS2<*>.DcssDescriptor -> if (value.getNextDescriptorValue(cellIndex) == expected) { // todo: could change to next value
...
//            } && statusReference.get() == expectedStatus
            array.compareAndSet(index, expectedCellState, this) && statusReference.get() == expectedStatus

        private fun updateCells(installed: Boolean): Boolean =
            if (installed) {
                settleUpdate()
            } else {
                restoreState()
