...
class AtomicArrayWithCAS2<E : Any>(size: Int, initialValue: E) {
    ...
    fun cas2(
        ...
        while (true) {
            ...
            if (firstValue != expected1 || secondValue != expected2) return false
    ...
    private inner class DCSSDescriptor(
        ...
        val caS2Descriptor: CAS2Descriptor, val expected2: Any?
    ) {
        val status = atomic(Status.UNDECIDED)
        fun applyOperation() {
            while (true) {
                val resultValue = array[index1].value
                if (resultValue is AtomicArrayWithCAS2<*>.DCSSDescriptor) {
                    resultValue.applyOperation()
                    continue
                } else if (resultValue is AtomicArrayWithCAS2<*>.CAS2Descriptor) {
                    resultValue.applyOperation()
                    continue
                } else if (resultValue == this) {
                    if (caS2Descriptor.status.value == expected2) {
                        status.compareAndSet(Status.UNDECIDED, Status.SUCCESS)
                    } else {
                        status.compareAndSet(Status.UNDECIDED, Status.FAILED)
                    }
