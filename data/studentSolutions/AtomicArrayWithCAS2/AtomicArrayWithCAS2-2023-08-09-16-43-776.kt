        ...
        private fun installCell(cellIndex: Int, expected: E?): Boolean {
            while (true) {
                ...
                when(val value = array[cellIndex]) {
                    ...
                    is AtomicArrayWithCAS2<*>.DcssDescriptor -> if (value.getDescriptorValue(cellIndex) == expected) {
...
//        }
    ...
    private fun dcss(
        ...
        return if (descriptor.status.get() == Status.SUCCESS) {
            array.compareAndSet(index, descriptor, descriptor.getDescriptorValue(index))
        } else {
            false
        }
    }
    ...
    inner class DcssDescriptor(
        ...
        private fun install(): Boolean =
            when (val arrayItem = array[index]) {
                is AtomicArrayWithCAS2<*>.DcssDescriptor -> if (getDescriptorValue(index) == expectedCellState) {
