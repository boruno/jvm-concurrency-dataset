...
class AtomicArrayWithCAS2<E : Any>(size: Int, initialValue: E) {
        ...
        return when (val e = array[index].value) {
            is AtomicArrayWithCAS2<*>.DCSS -> {
                if (e.status.value == Status.SUCCESS) {
                    getValue(e.casDescriptor, index) as E
                }
                else e.expected as E
            }
            is AtomicArrayWithCAS2<*>.CAS2Descriptor -> getValue(e, index) as E
            else -> e as E
        }
    }

    fun getValue(casDescriptor: AtomicArrayWithCAS2<*>.CAS2Descriptor, index: Int): Any {
    ...
    inner class CAS2Descriptor(
        ...
        fun install(i: Int, expected: E): Boolean {
            while (true) {
                ...
                when {
                    e == this -> return true
                    e == expected -> if (dcss(i, expected, this, Status.UNDECIDED)) return true
        ...
        fun dcss(index: Int, expected: E,
                 cas: CAS2Descriptor, expectedStatus: Status): Boolean {
            val dcss = DCSS(index, expected, cas, expectedStatus)
            dcss.apply()
    ...
    inner class DCSS(
        ...
        fun apply() {
            if (status.value == Status.UNDECIDED) {
                if (installDescriptor() && casDescriptor.status.value == expectedStatus) {
                    status.compareAndSet(Status.UNDECIDED, Status.SUCCESS)
                } else {
                    status.compareAndSet(Status.UNDECIDED, Status.FAILED)
                }
