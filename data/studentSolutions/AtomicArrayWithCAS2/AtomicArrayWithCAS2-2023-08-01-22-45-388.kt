...
class AtomicArrayWithCAS2<E : Any>(size: Int, initialValue: E) {
            ...
            return when (val currentValue = array[index].value) {
                is AtomicArrayWithCAS2<*>.CAS2Descriptor -> {
                    if (currentValue.status.value == Status.SUCCESS) {
                        if (index == currentValue.index1) currentValue.update1 as E? else currentValue.update2 as E?
                    } else {
                        if (index == currentValue.index1) currentValue.expected1 as E? else currentValue.expected2 as E?
                    }
                }
                is AtomicArrayWithCAS2<*>.DCSSDescriptor<*> -> {
                    currentValue.applyOperation()
                    currentValue.takeIf { it.status.value == Status.SUCCESS }?.update1 as E?
                }
    ...
    fun cas(index: Int, expected: E?, update: E?): Boolean {
        ...
        while (true) {
            when (val currentValue = array[index].value) {
                is AtomicArrayWithCAS2<*>.DCSSDescriptor<*> -> currentValue.applyOperation()
                is AtomicArrayWithCAS2<*>.CAS2Descriptor -> currentValue.applyOperation()
    ...
    fun cas2(
        ...
        while (true) {
            ...
            val minInd = minOf(index1, index2)
