...
class AtomicArrayWithCAS2<E : Any>(size: Int, initialValue: E) {
    ...
    fun CAS2Descriptor.get(index: Int): E = when (status.get()!!) {
            Status.UNDECIDED, Status.FAILED ->
                when (index) {
                    index1 -> expected1 as E
                    index2 -> expected2 as E
    ...
    fun get(index: Int): E? {
        ...
        if (v is AtomicArrayWithCAS2<*>.CAS2Descriptor)
            return (v as AtomicArrayWithCAS2<E>.CAS2Descriptor).get(index)
        else if (v is AtomicArrayWithCAS2<*>.DCSSDescriptor){
            return when (v.status.get()!!) {
                Status.UNDECIDED, Status.FAILED -> when (v.expected) {
                    is AtomicArrayWithCAS2<*>.CAS2Descriptor -> (v.expected as AtomicArrayWithCAS2<E>.CAS2Descriptor).get(index)
                    else -> v.expected as E?
                }
                Status.SUCCESS -> when (v.update) {
                    is AtomicArrayWithCAS2<*>.CAS2Descriptor -> (v.update as AtomicArrayWithCAS2<E>.CAS2Descriptor).get(index)
                    else -> v.update as E?
                }
            }
        }
