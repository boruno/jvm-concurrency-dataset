    ...
    fun get(index: Int): E? {
        ...
        return when (val currentValue = array[index].value) {
            ...
            is AtomicArrayWithCAS2<*>.DCSSDescriptor<*> -> {
                ...
                if (currentValue.status.value == Status.SUCCESS) currentValue.update1 as E?
                else currentValue.expected1 as E?
            }
            else -> {
                currentValue as E?
            }
    ...
    private inner class CAS2Descriptor(
        ...
        fun applyOperation() {
            ...
            while (true) {
                ...
                val isSuccess = dcss(index2, expected2, this, index2, Status.UNDECIDED)
    ...
    private inner class DCSSDescriptor<E: Enum<E>>(
        ...
        fun applyOperation() {
            while (true) {
                ...
                val secondValue = when (val currentValue = array[index2].value) {
                    is AtomicArrayWithCAS2<*>.DCSSDescriptor<*> -> {
                        if (currentValue.status.value == Status.SUCCESS) currentValue.update1
                        else currentValue.expected1
                    }
                    is AtomicArrayWithCAS2<*>.CAS2Descriptor -> {
                        if (currentValue.status.value == Status.SUCCESS) {
                            if (index2 == currentValue.index1) currentValue.update1
                            else currentValue.update2
                        } else {
                            if (index2 == currentValue.index1) currentValue.expected1
