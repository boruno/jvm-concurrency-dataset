...
//        return true
    ...
    inner class DCSSDescriptor(
        ...
        private fun install(): Boolean {
            return when (val cell = array[index]) {
                this -> true
                else -> array.compareAndSet(index, expected, this)
            }
        }

        private fun updateStatus(ok: Boolean) {
            dcssStatus.compareAndSet(UNDECIDED, if (ok && statusReference.get() == expectedStatus) SUCCESS else FAILED)
        }
