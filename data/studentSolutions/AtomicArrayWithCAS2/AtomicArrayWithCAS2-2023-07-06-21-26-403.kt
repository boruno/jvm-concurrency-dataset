...
class AtomicArrayWithCAS2<E : Any>(size: Int, initialValue: E) {
    ...
    inner class CAS2Descriptor(
        ...
        fun install(i: Int, expected: E): Boolean {
            while (true) {
                ...
                when (e) {
                    this -> return true
                    expected -> if (array[i].compareAndSet(expected, this)) return true
                    is AtomicArrayWithCAS2<*>.CAS2Descriptor -> e.apply()
                    is AtomicArrayWithCAS2<*>.DCSS -> e.apply()
                    else -> return false
                }
            }
        }
    ...
    inner class DCSS(
        val index: Int,
        val expected: E?,
        val casStatus: AtomicRef<Status>,
        ...
        fun apply() {
            val updated = if (installDesctiptor() && casStatus.value == expectedStatus) {
                status.compareAndSet(Status.UNDECIDED, Status.SUCCESS)
            } else {
                status.compareAndSet(Status.UNDECIDED, Status.FAILED)
            }
        ...
        private fun installDesctiptor(): Boolean {
            while (true) {
