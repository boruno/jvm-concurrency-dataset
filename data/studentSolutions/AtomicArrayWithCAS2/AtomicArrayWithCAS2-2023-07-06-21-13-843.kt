...
class AtomicArrayWithCAS2<E : Any>(size: Int, initialValue: E) {
    ...
    inner class CAS2Descriptor(
        ...
        fun apply(): Boolean {
            val descriptor = this
            if (!setDescriptor(index1, expected1)) {
                if (!descriptor.status.compareAndSet(UNDECIDED, FAILED) || descriptor.status.value === FAILED) {
                    return false
                }
            }
            return applyNext()
        ...
        fun setDescriptor(index: Int, expected: E): Boolean {
            ...
            while (descriptor.status.value === UNDECIDED) {
                if (!DCSSDescriptor(index, expected, descriptor, descriptor, UNDECIDED).apply_()) {
                    val value = ref.value
                    if (value === descriptor) {
                        return true
                    }
        ...
        fun applyNext(): Boolean {
            val descriptor = this
            if (!setDescriptor(index2, expected2)) {
    ...
    inner class DCSSDescriptor(
        ...
        fun apply_(): Boolean {
            setDescriptor_(index1, expected1)
            return applyNext_()
        }

        fun setDescriptor_(index: Int, expected: Any) {
            val descriptor = this
            val ref = array[index]
