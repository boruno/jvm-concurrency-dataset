...
class AtomicArrayWithCAS2<E : Any>(size: Int, initialValue: E) {
    ...
    fun get(index: Int): E? {
        ...
        if (cellState is AtomicArrayWithCAS2AndImplementedDCSS<*>.CAS2Descriptor) {
            val descriptor = cellState as AtomicArrayWithCAS2AndImplementedDCSS<E>.CAS2Descriptor
            ...
            while(true) {
                ...
                when(val cellState = array[index]) {
                    ...
                    is AtomicArrayWithCAS2AndImplementedDCSS<*>.CAS2Descriptor -> {
                        cellState as AtomicArrayWithCAS2AndImplementedDCSS<E>.CAS2Descriptor
                        cellState.apply()
                    }
                    else -> {
                        return false
    ...
    inner class DcssDescriptor(
        ...
        private fun installDescriptor(): Boolean {
            while(true) {
                if (statusReference.get() == expectedCas2Status) {
                    return false
                } else if (array.compareAndSet(index, expectedB, this)) {
                    return true
                }
            }
        }
