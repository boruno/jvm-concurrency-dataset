...
class AtomicArrayWithCAS2<E : Any>(size: Int, initialValue: E) {
    ...
    inner class DcssDescriptor(
        ...
        fun apply(index: Int): Boolean {
            while (true) {
                if (status.get() != expectedStatus) return false
                when (val cell = array[index]) {
                    this -> if (array.compareAndSet(index, this, update)) return true
                    is AtomicArrayWithCAS2<*>.DcssDescriptor -> cell.apply(index)
