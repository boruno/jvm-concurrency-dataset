...
class AtomicArrayWithCAS2<E : Any>(size: Int, initialValue: E) {
    ...
    fun get(index: Int): E {
        ...
        else if (item is AtomicArrayWithCAS2<*>.DcssDescriptor) {
            return if (item.status.value !== SUCCESS) item.expected as E else item.update as E
        } else {
            return item as E
        }
    }
    ...
    inner class CAS2Descriptor(
        ...
        private fun tryInstallDescriptors(): Boolean {
            if (!tryInstallDescriptor(index1, expected1, viaDcss = false)) return false
            if (!tryInstallDescriptor(index2, expected2, viaDcss = true)) return false
            return true
        }

        private fun tryInstallDescriptor(index: Int, expected: E, viaDcss: Boolean): Boolean {
            while (true) {
        ...
        private fun dcss(index: Int, expected: E): Boolean {
            val descriptor = DcssDescriptor(index, expected, this, this, UNDECIDED)
    ...
    inner class DcssDescriptor(
        ...
        fun installAndApply() {
            val success = tryInstallDescriptor(index, expected)
            if (success) {
                apply()
            } else {
                status.compareAndSet(UNDECIDED, FAILED)
        ...
        fun apply() {
            if (descriptor.status.value == expectedStatus) {
                status.compareAndSet(UNDECIDED, SUCCESS)
            } else {
                status.compareAndSet(UNDECIDED, FAILED)
            }
