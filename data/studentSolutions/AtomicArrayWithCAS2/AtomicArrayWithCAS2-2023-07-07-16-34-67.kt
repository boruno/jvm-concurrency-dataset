...
class AtomicArrayWithCAS2<E : Any>(size: Int, initialValue: E) {
    ...
    inner class CAS2Descriptor(
        ...
        val dcss = DCSSDescriptor(index2, expected2, update2, this)
        ...
        private fun installDescriptor(index: Int, expected: E?, update: Any? = null): Boolean {
            while (true) {
                val state = array[index].value
                when {
                    (state === this) -> {
                    ...
                    (state === expected) -> {
                        if (update is AtomicArrayWithCAS2<*>.DCSSDescriptor && update.status.value !== Status.UNDECIDED){
                            return true
                        }
                        if (cas(index, expected, update ?: this)) {
                            return true
                        } else {
                            continue
                        }
        ...
        private fun tryInstallDescriptors() {
            if (installDescriptor(index1, expected1) && installDescriptor(index2, expected2, dcss)) {
        ...
        private fun updateValues() {
            ...
            if (status.value === Status.SUCCESS) {
                ...
                array[index2].compareAndSet(dcss, expected2)
            }
        }
