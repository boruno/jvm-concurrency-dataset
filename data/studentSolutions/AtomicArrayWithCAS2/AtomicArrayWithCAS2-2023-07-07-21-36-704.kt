...
class AtomicArrayWithCAS2<E : Any>(size: Int, initialValue: E) {
    ...
    fun cas2(
        ...
        firstCellDescriptor.secondDescriptor = secondCellDescriptor
        secondCellDescriptor.secondDescriptor = firstCellDescriptor
    ...
    private inner class MyDCSSDescriptor(
        ...
        var secondDescriptor: DCSSDescriptor<E>? = null
        ...
        override fun help() {
            val isBValid = isBAsExpected(indexB, expectedB)
            setStatus(isDescriptorSet = true, isBAsExpected = isBValid)
            if (isFailed()) {
                setValue(indexA, expectedA)
            }
            if (isSuccess() && secondDescriptor?.isSuccess() == true) {
                setValue(indexA, updateA)
            }
        }
