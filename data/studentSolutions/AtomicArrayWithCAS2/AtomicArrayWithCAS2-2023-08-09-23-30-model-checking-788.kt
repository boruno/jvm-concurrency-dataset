...
class AtomicArrayWithCAS2<E : Any>(size: Int, initialValue: E) {
    ...
    inner class DcssDescriptor(
        ...
        fun apply(index: Int): Boolean {
            while (true) {
                ...
                when {
                    status.get() != expectedStatus -> return false
                    cell is AtomicArrayWithCAS2<*>.DcssDescriptor ->
                        if (array.compareAndSet(index, this, update)) return true else cell.apply(index)
                    else ->
                        if (!array.compareAndSet(index, expected, this)) return false
                }
            }
        }
    }
    ...
    ): Boolean = DcssDescriptor(expectedCellState, updateCellState, statusReference, expectedStatus).apply(index)
}