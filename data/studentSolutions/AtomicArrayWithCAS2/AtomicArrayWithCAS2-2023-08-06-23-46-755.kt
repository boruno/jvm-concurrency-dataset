...
class AtomicArrayWithCAS2<E : Any>(size: Int, initialValue: E) {
            ...
            while (true) {
                when (val value = array[idx].value) {
                    ...
                    expected -> {
                        ...
                        return dcssWithStatus(idx, expected, this)
                    }
                    else -> return false
                }
            }
    ...
    private fun updateValues(desc: AtomicArrayWithCAS2<*>.CAS2Descriptor, success: Boolean) {
        if (success) {
            ...
            //array[desc.index1].compareAndSet(desc.dcssDesc.value, desc.expected1)
            //array[desc.index2].compareAndSet(desc, desc.expected2)
            //array[desc.index2].compareAndSet(desc.dcssDesc.value, desc.expected2)
        }
    }
