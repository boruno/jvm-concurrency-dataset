...
class AtomicArrayWithCAS2<E : Any>(size: Int, initialValue: E) {
    ...
    inner class CAS2Descriptor(
        ...
        private fun install(): Boolean {
            ...
            return installInto(index1, expected1) && installInto(index2, expected2)
        }
    ...
    inner class DCSSDescriptor(
        ...
        val status = atomic(AtomicArrayWithDCSS.Status.UNDECIDED)

        fun getValue(): E? {
            return if (status.value == AtomicArrayWithDCSS.Status.SUCCESS) {
                update1
            } else {
                expected1
            }
        ...
        private fun install(): Boolean {
            while (true) {
                if (status.value != AtomicArrayWithDCSS.Status.UNDECIDED) return false
                ...
                when {
                    ...
                    value1 is AtomicArrayWithDCSS<*>.DCSSDescriptor -> value1.help()
                    value2 is AtomicArrayWithDCSS<*>.DCSSDescriptor -> value2.help()
        ...
        private fun updateStatus(installed: Boolean) {
            if (installed) {
                status.compareAndSet(AtomicArrayWithDCSS.Status.UNDECIDED, AtomicArrayWithDCSS.Status.SUCCESS)
            } else {
                status.compareAndSet(AtomicArrayWithDCSS.Status.UNDECIDED, AtomicArrayWithDCSS.Status.FAILED)
            }
        }

        private fun updateCell() {
            if (status.value == AtomicArrayWithDCSS.Status.SUCCESS) {
                array[index1].compareAndSet(this, update1)
            } else {
                array[index1].compareAndSet(this, expected1)
            }
