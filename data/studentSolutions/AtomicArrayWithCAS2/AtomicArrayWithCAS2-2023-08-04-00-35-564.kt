...
class AtomicArrayWithCAS2<E : Any>(size: Int, initialValue: E) {
    ...
    fun get(index: Int): E {
        val value = when (val valueOrDescriptor = array.get(index)) {
            is AtomicArrayWithCAS2<*>.CAS2Descriptor -> valueOrDescriptor.getValue(index)
            is AtomicArrayWithCAS2<*>.DCSSDescriptor -> {
                val valueOrDescriptor2 = valueOrDescriptor.getValue()
                if (valueOrDescriptor2 is AtomicArrayWithCAS2<*>.CAS2Descriptor) {
                    valueOrDescriptor2.getValue(index)
                } else {
                    valueOrDescriptor
                }
            }
            else -> valueOrDescriptor
        }
