        ...
        private fun handleCell (flag : Boolean) {
            ...
            while (true) {
                ...
                if (old is AtomicArrayWithCAS2<*>.CAS2Descriptor) {
                    ...
                    if (old == expected) {
                        // TODO: if (dcss(index, expected, this, status, Status.UNDECIDED)) { return }
                        // TODO: DIDN'T GET HOW IT SHOULD WORK since status is not an index and this is not E?
                        // TODO: (stress) tests fails but not always
                        if (status.value == Status.SUCCESS || status.value == Status.FAILED) { return } // new
