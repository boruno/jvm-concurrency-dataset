    ...
    fun cas(index: Int, expected: E?, update: E?): Boolean {
        while (true) {
            when (val value = array[index].value) {
                is AtomicArrayWithCAS2<*>.CAS2Descriptor -> value.proceed()
    ...
    private inner class CAS2Descriptor(
        ...
        fun apply() {
            while (true) {
                when (val value = array[firstIndex].value) {
                    ...
                    else -> statusRef.compareAndSet(Status.UNDECIDED, Status.FAILED)
                }
            }
        ...
        fun proceed() {
            while (true) {
                ...
                if (array[firstIndex].value !== this)
                    return
                ...
                when (statusRef.value) {
                    ...
                    Status.UNDECIDED -> {
                        when (val value = array[secondIndex].value) {
                            this -> break
                            is AtomicArrayWithCAS2<*>.CAS2Descriptor -> value.proceed()
                            secondExpected -> if (array[secondIndex].compareAndSet(secondExpected, this)) {
                                if (array[firstIndex].value !== this)
                                    return rollback()
                                statusRef.compareAndSet(Status.UNDECIDED, Status.SUCCESS)
                                break
                            }
                            else -> {
                                statusRef.compareAndSet(Status.UNDECIDED, Status.FAILED)
                                break
                            }
                        }
                    }
            ...
            if (statusRef.value == Status.SUCCESS)
                commit()
            else
                rollback()
        }
...
//                when (val value = array[firstIndex].value) {
//                    this -> {
//                        if (array[firstIndex].compareAndSet(this, firstUpdate)) {
