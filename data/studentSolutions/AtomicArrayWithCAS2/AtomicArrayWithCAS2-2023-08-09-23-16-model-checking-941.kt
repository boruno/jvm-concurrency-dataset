...
class AtomicArrayWithCAS2<E : Any>(size: Int, initialValue: E) {
    ...
    inner class DcssDescriptor(
        ...
        fun apply(index: Int): Boolean {
            return if (status.get() == expectedStatus) {
                applyPhysically(index)
            } else {
                false
            }
        }

        fun applyPhysically(index: Int): Boolean {
            return array.compareAndSet(index, this, update)
        }
    }
    ...
    fun dcss(
        ...
        while (true) {
            ...
            when {
                statusReference.get() != expectedStatus -> return false
                cell is AtomicArrayWithCAS2<*>.DcssDescriptor -> if (cell.applyPhysically(index)) return true
                else ->  if (!array.compareAndSet(index, expectedCellState, dcssDescriptor)) return false
            }
        }
    }
}