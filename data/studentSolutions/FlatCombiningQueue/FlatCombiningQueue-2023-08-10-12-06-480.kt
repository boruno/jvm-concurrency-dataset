...
class FlatCombiningQueue<E> : Queue<E> {
    ...
    override fun enqueue(element: E) {
        while (true) {
            ...
            val isSuccess = tasksForCombiner.compareAndSet(cellIndex, null, element)
            if (isSuccess) {
                while (tasksForCombiner[cellIndex] != null) {
                    withLock {
                        tasksForCombiner[cellIndex] = null
                        runTasks()
                        return
                    }
                }
    ...
    override fun dequeue(): E? {
        while (true) {
            ...
            val cellIndex = randomCellIndex()
            val isSuccess = tasksForCombiner.compareAndSet(cellIndex, null, Dequeue)
            if (isSuccess) {
                while (tasksForCombiner[cellIndex] != null) {
                    withLock {
                        val removedElement = tasksForCombiner[cellIndex] as Result<*>
                        tasksForCombiner[cellIndex] = null
                        runTasks()
                        return removedElement.value as E
                    }
                }
            }
        }
    ...
    private fun doTask(index: Int) {
        when (val task = tasksForCombiner[index]) {
            null -> {}
            is Result<*> -> {}
            is Dequeue -> {
                val removedValue = queue.removeFirstOrNull()
                tasksForCombiner[index] = Result(removedValue)
            }

            else -> {
                val elem = task as E
                queue.addLast(elem)
                tasksForCombiner[index] = Result(elem)
            }
        }
    }

    private inline fun withLock(body: () -> Unit) {
