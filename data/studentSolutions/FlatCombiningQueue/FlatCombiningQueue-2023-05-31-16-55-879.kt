...
class FlatCombiningQueue<E> : Queue<E> {
    ...
    override fun enqueue(element: E) {
        ...
        while (true) {
            if (combinerLock.compareAndSet(expect = false, update = true)) {
                ...
                if (!placed || tasksForCombiner[cell].value != PROCESSED) queue.addLast(element)
                if (placed) tasksForCombiner[cell].value = null
    ...
    override fun dequeue(): E? {
        ...
        while (true) {
            if (combinerLock.compareAndSet(expect = false, update = true)) {
                val ans = if (!placed || tasksForCombiner[cell].value != PROCESSED) {
