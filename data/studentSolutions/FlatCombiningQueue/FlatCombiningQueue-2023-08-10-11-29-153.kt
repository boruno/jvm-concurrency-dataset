...
class FlatCombiningQueue<E> : Queue<E> {
    ...
    override fun enqueue(element: E) {
        ...
        var i = Random.nextInt(TASKS_FOR_COMBINER_SIZE)
        var isPlaced = false
        while (true) {
            if (combinerLock.compareAndSet(false, true)) {
                queue.addLast(element)
                for (i in 0..TASKS_FOR_COMBINER_SIZE) {
                    if (tasksForCombiner.get(i) is Dequeue) {
                ...
                if (!isPlaced) {
                    while (!tasksForCombiner.compareAndSet(i, null, element)) {
                        i = Random.nextInt(TASKS_FOR_COMBINER_SIZE)
                    }
                    isPlaced = true
                } else {
                    if (tasksForCombiner.compareAndSet(i, Result(element), null)) {
                        return
                    }
                }
            }
            ...
            if (combinerLock.compareAndSet(false, true)) {
                ...
                if (!isPlaced) {
                    ...
                    if (tasksForCombiner.get(i) is Result<*>) {
                        return tasksForCombiner.getAndSet(i, null) as E
                    }
                }
            }
        }
