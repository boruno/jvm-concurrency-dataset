...
class FlatCombiningQueue<E> : Queue<E> {
        ...
        while (true) {
            if (combinerLock.compareAndSet(expect = false, update = true)) {
                ...
                if (tasksForCombiner[index].compareAndSet(null, request)) {
                    if (tasksForCombiner[index].compareAndSet(request, null)) continue
    ...
    private fun combine() {
        val dequeState = State(DEQUE_TASK)
        for (i in 0 until TASKS_FOR_COMBINER_SIZE) {
            val task = tasksForCombiner[i].value
            val state = task?.operationState ?: continue
            if (state == PROCESSED) continue
            val element = queue.firstOrNull()
            if (tasksForCombiner[i].compareAndSet(dequeState, State(PROCESSED, element))) {
                queue.removeFirstOrNull()
            } else {
                queue.add(state as E)
        ...
        while (true) {
            if (combinerLock.compareAndSet(expect = false, update = true)) {
                ...
                if (tasksForCombiner[index].compareAndSet(null, request)) {
                    ...
                    val value = tasksForCombiner[index].value
                    tasksForCombiner[index].compareAndSet(value, null)
                    return value?.operationParam as? E?
                }
            }
        }
    }
    ...
    private data class State(val operationState: Any?, val operationParam: Any? = null)
}
