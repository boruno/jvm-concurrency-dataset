            ...
            if (combinerLock.compareAndSet(false, true)) {
                ...
                for (i in 0 until TASKS_FOR_COMBINER_SIZE) {
                    when (val value = tasksForCombiner[i].value) {
                        Dequeue -> {
                            val result = queue.removeLastOrNull()
                ...
                when (val value = tasksForCombiner[i].value) {
                    ...
                    is Result<*> -> {
                        // clean up
                        tasksForCombiner[i].compareAndSet(value, null)
                        // finish
                        return
                    }

                    // empty, set the element
                    null -> tasksForCombiner[i].compareAndSet(null, element)
        ...
        while (true) {
            if (combinerLock.compareAndSet(false, true)) {
                ...
                for (i in 0 until TASKS_FOR_COMBINER_SIZE) {
                    when (val value = tasksForCombiner[i].value) {
                        Dequeue -> {
                            val element = queue.removeLastOrNull()
                            tasksForCombiner[i].compareAndSet(value, Result(element))
                        }
