...
class FlatCombiningQueue<E> : Queue<E> {
    ...
    private fun tryOperation(operation: Any?): Result<E?> {
        while (true) {
            if (tryLock()) {
                return doCombinerStuff(operation)
            }
            val index = randomCellIndex()
            if (tasksForCombiner[index].compareAndSet(null, operation)) {
                while (true) {
                    if (tryLock()) {
                        return doCombinerStuff(operation, index)
                    }
    ...
    private fun doCombinerStuff(operation: Any?, index: Int? = null): Result<E?> {
        val result: Result<E?> = if (index != null) {
            val value = tasksForCombiner[index].value
            if (value is Result<*>) {
                @Suppress("UNCHECKED_CAST")
                value as Result<E?>
    ...
    private fun processWaitList() {
        repeat(tasksForCombiner.size) { i ->
            val maybeOperation = tasksForCombiner[i].value
            if (maybeOperation != null && maybeOperation !is Result<*>) {
                doOperation(maybeOperation)
            }
        }
    }
