        ...
        if (lock()) {
            ...
            for (i in 0 until tasksForCombiner.size) {
                when (val task = tasksForCombiner[i].value) {
                    is Result<*> -> throw IllegalStateException("Found abandoned result")
                    is Dequeue -> {
                        val toAdd = Result(queue.removeFirstOrNull())
                        tasksForCombiner[i].value = toAdd
                    }
                    null -> {}
                    else -> {
                ...
                if (tasksForCombiner[randomCellIndex].compareAndSet(null, element)) {
                    while (true) {
                        if (tasksForCombiner[randomCellIndex].value is Result<*>) {
                            tasksForCombiner[randomCellIndex].value = null
                            return
                        } else if (lock()) {
                            if (tasksForCombiner[randomCellIndex].value !is Result<*>) {
                                queue.addLast(element)
        ...
        if (lock()) {
            ...
            while (true) {
                ...
                if (tasksForCombiner[randomCellIndex].compareAndSet(null, Dequeue)) {
                    while (true) {
                        if (tasksForCombiner[randomCellIndex].value is Result<*>) {
                            val res = (tasksForCombiner[randomCellIndex].value as Result<*>).value as E
                            tasksForCombiner[randomCellIndex].value = null
                            return res
                        } else if (lock()) {
                            val res = if (tasksForCombiner[randomCellIndex].value is Result<*>) {
                                (tasksForCombiner[randomCellIndex].value as Result<*>).value as E
                            } else {
