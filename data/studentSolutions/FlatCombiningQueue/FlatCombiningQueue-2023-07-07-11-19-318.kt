...
class FlatCombiningQueue<E> : Queue<E> {
    ...
    private fun dequeueUnderLock(): E {
        require(combinerLock.value)
        return queue.removeFirstOrNull() as E
    }
            ...
            while (true) {
                ...
                if (tasksForCombiner[index].compareAndSet(null, Dequeue)) {
                    while (true) {
                        ...
                        if (combinerLock.compareAndSet(false, true)) {
                            val result2 = tasksForCombiner[index].value

                            if (result2 is Result<*>) {
                                tasksForCombiner[index].compareAndSet(result, null)
                                combinerLock.value = false
                                return result2.value as E
                            }
    ...
    fun helpOthers() {
        ...
        for (i in 0 until TASKS_FOR_COMBINER_SIZE) {
            ...
            if (value is Dequeue) {
                ...
                tasksForCombiner[i].compareAndSet(value, Result(dequeueValue))
            } else {
