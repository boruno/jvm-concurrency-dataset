...
class FlatCombiningQueue<E> : Queue<E> {
    ...
    private fun processViaTask(task: Any?, a: () -> Result<E?>): Result<E?>? {
        ...
        while (true) {
            val v = tasksForCombiner[idx].value as? Result<*>
                ?: tryCombine(a, idx)
            if (v is Result<*>) {
                tasksForCombiner[idx].value = null
                return v as Result<E?>
            }
            val res = tryCombine(a, idx, task)
            if (res != null) {
                tasksForCombiner[idx].value = null
                return res
            }
    ...
    private fun tryCombine(a: () -> Result<E?>, idx: Int? = null, task: Any? = null): Result<E?>? {
        if (combinerLock.compareAndSet(false, true)) {
            try {
                ...
                if (idx != null) tasksForCombiner[idx].value = null
