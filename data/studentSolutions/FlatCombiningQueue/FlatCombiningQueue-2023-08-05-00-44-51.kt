...
class FlatCombiningQueue<E> : Queue<E> {
    ...
    private fun execute(task: Task<E>): E? {
        ...
        while (true) {
            ...
            if (lock.compareAndSet(expect = false, update = true)) try {
                ...
                val result = (if (taskIndex == -1) task else takeTask(taskIndex)).execute()
                ...
                if (tasks[taskIndex].compareAndSet(null, task)) {
                    val result = waitForTask(taskIndex) ?: continue
    ...
    private fun executeTasks() {
        for (i in 0 until tasks.size) {
            val task = tasks[i].value ?: continue
            val result = Return(
                when (task) {
    ...
    private fun waitForTask(index: Int): Return? {
        ...
        while (true) {
            ...
            when {
                ...
                value is Return -> {
                    check(tasks[index].compareAndSet(value, null)) {
                        "Combiner task #$index expected value is not '$value' but got ${tasks[index].value}"
                    }
                    return value
                }
