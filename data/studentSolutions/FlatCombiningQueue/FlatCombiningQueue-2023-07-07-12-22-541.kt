        ...
        while (true) {
            if (combinerLock.compareAndSet(expect = false, update = true)) {
                if (posted) {
                    ...
                    if (mayBeResult == op) {
                        tasksForCombiner[randomIdx].value = null
                        return underLock()
                    } else {
                        val resultImpl = (mayBeResult as Result<*>)
                        combineForOthers()
                        unlock()
                        try {
                            return resultImpl.value as R?
            ...
            if (posted) {
                val mayBeResult = tasksForCombiner[randomIdx].value
                if (mayBeResult != op) {
                    check(mayBeResult is Result<*>)
                    check(tasksForCombiner[randomIdx].compareAndSet(mayBeResult, null))
                    return (mayBeResult as Result<*>).value as R?
                }
            } else {
        ...
        for (i in 0 until TASKS_FOR_COMBINER_SIZE) {
            ...
            when (next) {
                ...
                is Dequeue -> {
                    ...
                    check(tasksForCombiner[i].compareAndSet(next, Result(Unit)))
                }
            }
        }
    }
