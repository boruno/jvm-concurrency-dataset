...
class FlatCombiningQueue<E> : Queue<E> {
    ...
    override fun enqueue(element: E) {
        ...
        return performFlatCombainingOperation({ queue.addLast(element) }, Dequeue)
        while (true) {
            if (lockCombinerLock()) {
                ...
                if (!tasksForCombiner.compareAndSet(randomIdx, null, element)) continue
    ...
    override fun dequeue(): E? {
        ...
        // TODO:      updates to `Result` (do not forget to clean it in this case),
        // TODO:      or `combinerLock` becomes available to acquire.
        return performFlatCombainingOperation({ queue.removeFirstOrNull() }, Dequeue)
        while (true) {
            if (lockCombinerLock()) {
                ...
                val randomIdx = randomCellIndex()
                // if the index is bad, we will just try again
                if (!tasksForCombiner.compareAndSet(randomIdx, null, Dequeue)) continue
    ...
    private fun waitForResult(randomIdx: Int): Result<*> {
        ...
        return tasksForCombiner.getAndSet(randomIdx, null) as Result<*>
    }

    private fun <T> performFlatCombainingOperation(operation: () -> T, token: Any): T {
        while (true) {
            if (lockCombinerLock()) {
                val element = operation()
                ...
                val randomIdx = randomCellIndex()
                // if the index is bad, we will just try again
                if (!tasksForCombiner.compareAndSet(randomIdx, null, token)) continue
