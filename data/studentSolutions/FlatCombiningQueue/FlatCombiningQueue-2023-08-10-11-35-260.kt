...
class FlatCombiningQueue<E> : Queue<E> {
            ...
            if (tryLock()) {
                ...
                for (index in 0..tasksForCombiner.length()) {
                    when (val cell = tasksForCombiner.get(index)) {
                        // dequeue
                        is Dequeue -> {
                            tasksForCombiner.compareAndSet(index, Dequeue, Result(queue.removeFirstOrNull()))
                        }
                        ...
                        // enqueue request, put
                            ...
                            tasksForCombiner.compareAndSet(index, cell, Result(cell))
                        }
                    }
                }
                unlock()
    ...
    override fun dequeue(): E? {
        ...
        while (true) {
            ...
            if (tryLock()) {
                val rez = queue.removeFirstOrNull()
                unlock()
                return rez
            } else {
                val cellIndex = randomCellIndex()
