    ...
    override fun enqueue(element: E) {
        ...
        try {
            for (i in 0 until tasksForCombiner.size) {
                ...
                if (i == myCellIdx) {
                    if (value != PROCESSED_ENQ)
                        queue.addLast(value as E)
                    c.value = null
                } else if (value == null || value == PROCESSED_ENQ || value is ProcessedDequeue)
                    continue
                else if (value == DEQUE_TASK) {
                    c.value = ProcessedDequeue(queue.removeFirstOrNull())
                } else {
                    queue.addLast(value as E)
                    c.value = PROCESSED_ENQ
                }
            }
        } finally {
    ...
    override fun dequeue(): E? {
        ...
        while (true) {
            if (myCell.value != DEQUE_TASK) {
                return (myCell.getAndSet(null) as ProcessedDequeue).value as E?
            }
        ...
        try {
            ...
            for (i in 0 until tasksForCombiner.size) {
                ...
                if (i == myCellIdx) {
                    myResult = if (value == DEQUE_TASK) queue.removeFirstOrNull() else (value as ProcessedDequeue).value as E?
                    c.value = null
                } else if (value == null || value == PROCESSED_ENQ)
                    continue
                else if (value == DEQUE_TASK) {
                    val dequeued = queue.removeFirstOrNull()
                    c.value = ProcessedDequeue(dequeued)
                } else {
                    queue.addLast(value as E)
                    c.value = PROCESSED_ENQ
                }
...
private val DEQUE_TASK = "DEQUE_TASK"

// TODO: Put this token in `tasksForCombiner` when the task is processed.
private val PROCESSED_ENQ = "PROCESSED_ENQ"

class ProcessedDequeue(val value: Any?)