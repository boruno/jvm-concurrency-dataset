        ...
        while (true) {
            if (combinerLock.compareAndSet(expect = false, update = true)) {
                ...
                if (tasksForCombiner[index].compareAndSet(null, operation)) {
                    while (true) {
                        if (combinerLock.compareAndSet(expect = false, update = true)) {
                            tasksForCombiner[index].compareAndSet(operation, null)
                            help()
                            queue.addLast(element)
                            combinerLock.compareAndSet(true, update = false)
                        } else {
                            if (operation.state.compareAndSet(PROCESSED, null)) return
                        }
                    }
                }
            }
    ...
    private fun help() {
        for (i in 0 until tasksForCombiner.size) {
            ...
            if (operation.task == DEQUE_TASK) {
                ...
                while (true) {
                    if (operation.state.compareAndSet(null, PROCESSED)) return
                }
            }
            else {
                    ...
                    while (true) {
                        if (combinerLock.compareAndSet(expect = false, update = true)) {
                            ...
                            if (operation.state.compareAndSet(PROCESSED, null)) {
                                return tasksForCombiner[index].value as E
                            }
                        }
                    }
                }
