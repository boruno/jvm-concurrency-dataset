...
//                dequeueResults[idx].value = queue.removeFirstOrNull()
            ...
            if (tasksForCombiner[idx].compareAndSet(Dequeue, queue.removeFirstOrNull())) {
                continue
            }
    ...
    override fun dequeue(): E? {
        ...
        while(true) {
            if (tasksForCombiner[rci].compareAndSet(null, Dequeue)) { // set the random cell
                while (!tryLock()) {
                    if (tasksForCombiner[rci].value != Dequeue) { // wait for the result to appear
                ...
                return (tasksForCombiner[rci].value as E).also {
                    tasksForCombiner[rci].compareAndSet(tasksForCombiner[rci].value, null)
                    help()
                }
            }
        }
