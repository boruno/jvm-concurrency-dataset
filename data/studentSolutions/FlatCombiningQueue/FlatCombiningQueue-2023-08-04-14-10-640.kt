...
class FlatCombiningQueue<E : Any> : Queue<E> {
    ...
    override fun dequeue(): E? {
        val result = perform(Dequeue) {
            queue.removeFirstOrNull()
        }
        check(result == null || result as? E != null) {
            "Unexpected type of $result: ${result!!::class.java.canonicalName}"
        }
        return result as E?
    }
                ...
                combine {
                    if (index > -1) {
                        ...
                        when {
                            ...
                            else -> error("No task for index=$index")
                        }.also {
