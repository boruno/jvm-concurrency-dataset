...
class FlatCombiningQueue<E> : Queue<E> {
    ...
    fun helpOthers() {
        for (i in 0..tasksForCombiner.length() - 1) {
        ...
        if (combinerLock.compareAndSet(false, true)) {
            ...
            while (true) {
                ...
                if (tasksForCombiner.compareAndSet(index, null, element)) {
                    while (true) {
                        if (tasksForCombiner.compareAndSet(index, Result(element), null) || combinerLock.compareAndSet(
                                false,
                                true
                            )
                        ) {
        ...
        if (combinerLock.compareAndSet(false, true)) {
            ...
            while (true) {
                ...
                if (tasksForCombiner.compareAndSet(index, null, Dequeue)) {
                    while (true) {
                        if (tasksForCombiner.compareAndSet(index, Result(Dequeue), null) || combinerLock.compareAndSet(
                                false,
                                true
                            )
                        ) {
