...
class FlatCombiningQueue<E> : Queue<E> {
    ...
    fun helpOthers() {
        for (i in 0 until tasksForCombiner.length()) {
            when (val elem = tasksForCombiner.get(i)) {
        ...
        if (combinerLock.compareAndSet(false, true)) {
            ...
            while (true) {
                ...
                if (tasksForCombiner.compareAndSet(index, null, element)) {
                    while (true) {
                        if (tasksForCombiner.get(index) is Result<*>) return
                        if (combinerLock.compareAndSet(false, true)) {
        ...
        if (combinerLock.compareAndSet(false, true)) {
            ...
            while (true) {
                ...
                if (tasksForCombiner.compareAndSet(index, null, Dequeue)) {
                    while (true) {
                        if (tasksForCombiner.get(index) is Result<*> || combinerLock.compareAndSet(false, true)) {
                            combinerLock.set(false)
                            return dequeue()
                        }
                    }
