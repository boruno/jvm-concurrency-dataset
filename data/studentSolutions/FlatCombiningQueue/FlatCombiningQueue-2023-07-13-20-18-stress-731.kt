...
class FlatCombiningQueue<E> : Queue<E> {
    ...
    private fun processTasks(): List<E?> {
        ...
        if (processedTasks.isNotEmpty()) {
            val index = randomCellIndex()
            var releaseLock = true // Flag to track whether the lock should be released

            while (releaseLock && !combinerLock.compareAndSet(expect = false, update = true)) {
                // Keep trying to acquire the lock until releaseLock is false
                releaseLock = false
            }

            for (i in processedTasks) {
                // Check if the task is still marked as PROCESSED
                if (tasksForCombiner[i].value == PROCESSED) {
                    tasksForCombiner[i].value = null // Clean the processed task
                } else {
                    releaseLock = false // A task is not processed, so don't release the lock
                }
            }

            if (releaseLock) {
                combinerLock.value = false // Release the combiner lock
            }
        }
