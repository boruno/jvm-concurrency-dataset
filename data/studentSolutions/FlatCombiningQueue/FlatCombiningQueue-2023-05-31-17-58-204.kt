            ...
            if (combinerLock.compareAndSet(expect = false, update = true)) {
                ...
                for (i in 0 until TASKS_FOR_COMBINER_SIZE) {
                    ...
                    if (tasksForCombiner[cellNumber].compareAndSet(null, element)) {
                        ...
                        //smb took task here
                        if (tasksForCombiner[cellNumber].compareAndSet(element, null)) {
                            break;
                        } else {
                            tasksForCombiner[cellNumber].compareAndSet(PROCESSED, null)
                            return
                        }
                    }
                }
            ...
            if (combinerLock.compareAndSet(expect = false, update = true)) {
                ...
                for (i in 0 until TASKS_FOR_COMBINER_SIZE) {
                    ...
                    if (tasksForCombiner[cellNumber].compareAndSet(null, DEQUE_TASK)) {
                        repeat (TASKS_FOR_COMBINER_SIZE) {
                            if (tasksForCombiner[cellNumber].value != DEQUE_TASK) {
                                val removed = tasksForCombiner[cellNumber].value
                                tasksForCombiner[cellNumber].compareAndSet(removed, null)
                                return removed as E?
                            }
                        }
                        //nobody took the task
                        if (tasksForCombiner[cellNumber].compareAndSet(DEQUE_TASK, null)) {
                            break;
                        } else {
                            val removed = tasksForCombiner[cellNumber].value
                            tasksForCombiner[cellNumber].compareAndSet(removed, null)
                            if (removed == PROCESSED || removed == DEQUE_TASK) {
                                println("123");
                            }
                            return removed as E?
                        }
