...
class FlatCombiningQueue<E> : Queue<E> {
        ...
        if (combinerLock.compareAndSet(false, true)) {
            ...
            while (true) {
                ...
                if (tasksForCombiner[randomCellIndex].compareAndSet(null, element)) {
                    while (tasksForCombiner[randomCellIndex].value == element){}
                    break
                }
    ...
    private fun help() {
        for (i in 0 until tasksForCombiner.size) {
            val op = tasksForCombiner[i].getAndSet(null) ?: continue
            if (op is Dequeue) {
                ...
                if (tasksForCombiner[randomCellIndex].compareAndSet(null, Dequeue)) {
                    while (true) {
                        ...
                        if (value != null && value != Dequeue) {
                           if (tasksForCombiner[randomCellIndex].compareAndSet(value, null)) {
                               result = value as E
                               return result
                           }
                        }
