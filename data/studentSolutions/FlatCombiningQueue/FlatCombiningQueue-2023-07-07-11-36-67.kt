                ...
                if (tasksForCombiner[index].compareAndSet(null, element)) {
                    while (true) {
                        ...
                        if (result is Result<*>) {
                            ...
                            if (combinerLock.tryLock()) {
                                if (tasksForCombiner[index].compareAndSet(result, null)) {
                                    queue.addLast(element)
                                }
                                combinerTasks()
                                combinerLock.unlock()
                ...
                if (tasksForCombiner[index].compareAndSet(null, Dequeue)) {
                    while (true) {
                        ...
                        if (result is Result<*>) {
                            ...
                            if (combinerLock.tryLock()) {
                                val result1 = if (tasksForCombiner[index].compareAndSet(result, null)) {
                                   queue.removeFirstOrNull()
                                } else {
                                    (tasksForCombiner[index].value as Result<*>).value
                                }
                                combinerTasks()
                                combinerLock.unlock()
                                return result1 as E
