...
class FlatCombiningQueue<E> : Queue<E> {
    ...
    override fun enqueue(element: E) {
        ...
        if (combinerLock.compareAndSet(false, true)) {
            ...
            while (!tasksForCombiner[i].compareAndSet(PROCESSED, null)) { }
        }
//        queue.addLast(element)
    }
    ...
    override fun dequeue(): E? {
        ...
        if (combinerLock.compareAndSet(false, true)) {
            ...
            while (true) {
                i = randomCellIndex()
                if (tasksForCombiner[i].compareAndSet(null, DEQUE_TASK)) break
            }
            while (true) {
                if (tasksForCombiner[i].compareAndSet(PROCESSED, null)) {
                    break
                }
                if (combinerLock.compareAndSet(false, true)) {
                    doCombine()
...
//        return queue.removeFirstOrNull()
    ...
    private fun doCombine() {
        for (i in 0 until tasksForCombiner.size) {
            val item = tasksForCombiner[i]
            if (item.compareAndSet(DEQUE_TASK, PROCESSED)) {
                queue.removeFirstOrNull()
            } else if (item.value != PROCESSED && item.value != null) {
                queue.addLast(item.value as E)
