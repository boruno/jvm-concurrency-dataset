...
class FlatCombiningQueue<E : Any> : Queue<E> {
    ...
    private fun execute(descriptor: Any, operation: () -> Any?): Any? {
        var index = -1
        while (true) {
                ...
                combine {
                    if (index > -1) {
                        ...
                        when {
                            ...
                            else -> executeCombinerTask(task)
                        } ?: error("Unexpected task '$task' type '${task::class.java.canonicalName}'")
                    } else operation()
                }
            } finally {
    ...
    private fun combine(operation: () -> Any?): Any? {
        ...
        for (i in 0 until tasksForCombiner.size) {
            ...
            val result = Result(executeCombinerTask(task) ?: continue)
    ...
    private fun executeCombinerTask(task: Any) = when (task) {
