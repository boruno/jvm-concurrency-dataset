...
class FlatCombiningQueue<E> : Queue<E> {
    ...
    override fun dequeue(): E? {
        while (true) {
            ...
            if (combinerLock.compareAndSet(false, true)) {
                ...
                if (!taskCell.compareAndSet(null, DEQUE_TASK)) continue

                repeat(TASK_WAIT_CYCLES) {
                    val task = taskCell.value

                    if (
                        task is ProcessedDequeue
                        && taskCell.compareAndSet(task, null)
                    ) {
    ...
    @JvmInline
    private value class EnqueueTask(val element: Any?)

    @JvmInline
    private value class ProcessedDequeue(val result: Any?)
}
