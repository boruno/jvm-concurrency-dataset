...
class FlatCombiningQueue<E> : Queue<E> {
    ...
    private fun doDirtyWork() {
        for (i in 0 until tasksForCombiner.size) {
            when (val elem = tasksForCombiner[i].value) {
                null, PROCESSED_ENQUEUE -> {}
                DEQUE_TASK -> {
                    tasksForCombiner[i].value = DequeResult(queue.removeFirstOrNull())
                }
                else -> {
            ...
            if (combinerLock.compareAndSet(false, true)) {
                ...
                when (val prev = tasksForCombiner[i].value) {
                    ...
                    DEQUE_TASK -> throw Exception("DEQUE_TASK enqueue lock")
                        ...
                        if (prev != element && prev != PROCESSED_ENQUEUE) {
                            throw Exception("found value enqueue lock")
                        }
                        if (prev as? DequeResult<E> != null) {
                            throw Exception("DequeResult enqueue lock")
                        }
                        if (prev == element)
                            queue.addLast(element)
                ...
                when (val prev = tasksForCombiner[i].value) {
                    ...
                    PROCESSED_ENQUEUE -> return
                        ...
                        if (prev as? DequeResult<E> != null) {
                            throw Exception("DequeResult enqueue")
                        }
                        if (prev == element)
                            continue
            ...
            if (combinerLock.compareAndSet(false, true)) {
                ...
                when (val prev = tasksForCombiner[i].value) {
                    ...
                    PROCESSED_ENQUEUE -> throw Exception("PROCESSED_ENQUEUE dequeue lock")
                        ...
                        if (prev as? DequeResult<E> == null)
                            throw Exception("dequeue value found but not DequeResult lock")

                        doDirtyWork()
                        combinerLock.value = false
                        return (prev as? DequeResult<E>)!!.element
                    }
                }
            } else {
                ...
                when (val prev = tasksForCombiner[i].value) {
                    ...
                    PROCESSED_ENQUEUE -> throw Exception("PROCESSED_ENQUEUE dequeue")
                        ...
                        tasksForCombiner[i].value = null
                        if (prev as? DequeResult<E> == null)
                            throw Exception("dequeue value found but not DequeResult")
                        return prev.element
                    }
                }
            }
    ...
    class DequeResult<E>(val element: E)
}
