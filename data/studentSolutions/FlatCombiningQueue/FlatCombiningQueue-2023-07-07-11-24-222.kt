        ...
        if (combinerLock.compareAndSet(false, true)) {
            ...
            while (true) { // this loop is for retry
                ...
                if (tasksForCombiner[randomIdx].compareAndSet(null, Dequeue)) {
                    while(true) {
                        // asked to enqueue the element -> expect Result(Unit) here
                        if (tasksForCombiner[randomIdx].value !is Dequeue) {
                            val value = tasksForCombiner[randomIdx].value as? E ?: error("Expected value of the element to be E but found: ${tasksForCombiner[randomIdx].value}")
    ...
    private fun helpOthers() {
        for (i in 0 until tasksForCombiner.size) {
            ...
            if (task is Dequeue) {
                ...
                val enqTaskValue = tasksForCombiner[i].value as? E ?: error("Expected value of the element to be E but found: ${tasksForCombiner[i].value}")
                queue.addLast(enqTaskValue)
                tasksForCombiner[i].value = Unit
            }
        }
    }
