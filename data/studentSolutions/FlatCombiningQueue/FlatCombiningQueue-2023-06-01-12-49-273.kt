...
FlatCombiningQueue<E> : Queue<E> {
    ...
    fun combinerPass()
    {
        for (i in 1 .. TASKS_FOR_COMBINER_SIZE)
        {
            val task = tasksForCombiner[i].value ?: continue
            if (task == DEQUE_TASK) {
                queue.removeFirstOrNull() ?: continue
                if (tasksForCombiner[i].compareAndSet(DEQUE_TASK, task)) {
                    continue // all is good
                } else {
                    queue.addFirst(task as E) // return to previous state
                }
            } else { // push request
    ...
    override fun enqueue(element: E) {
        ...
        while (true)
        {
            ...
            {
                // becomes combiner
                if (currentCell == null || tasksForCombiner[currentCell] != PROCESSED)
            ...
            else
            {
                ...
                tasksForCombiner[currentCell].value = element
    ...
    override fun dequeue(): E? {
        ...
        while (true)
        {
            ...
            else
            {
                // publishes and waits
                currentCell = randomCellIndex()
                tasksForCombiner[currentCell].value = DEQUE_TASK
