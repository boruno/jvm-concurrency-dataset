        ...
        for (i in 0 until TASKS_FOR_COMBINER_SIZE) {
            while (true) {
                when (val state = tasksForCombiner[i]) {
                    (state is Result<*> || state == null) -> {
                        break
                    }
                    (state is Dequeue) -> {
                        if (tasksForCombiner.compareAndSet(i, state, Result(queue.removeFirstOrNull()))) {
                            break
                        }
                    }
                    else -> {
                        if (tasksForCombiner.compareAndSet(i, state, Result(state))) {
                            queue.addLast(state as E)
                            break
                        }
                    }
                }
