...
class FlatCombiningQueue<E> : Queue<E> {
    ...
    override fun enqueue(element: E) {
        ...
        val task = Element(element, true)
        while (!combinerLock.compareAndSet(false, true)) {
            if (tasksForCombiner[index].compareAndSet(null, task) || tasksForCombiner[index].compareAndSet(PROCESSED, task)) {
    ...
    override fun dequeue(): E? {
        ...
        var index = randomCellIndex()
        while (!combinerLock.compareAndSet(false, true)) {
            if (tasksForCombiner[index].compareAndSet(null, DEQUE_TASK) || tasksForCombiner[index].compareAndSet(PROCESSED, DEQUE_TASK)) {
                while (tasksForCombiner[index].value !is Element<*>) { }
                val result = (tasksForCombiner[index].value as Element<E>).value
                tasksForCombiner[index].compareAndSet(tasksForCombiner[index].value, null)
                return result
            }
            index = randomCellIndex()
        }
    ...
    private fun processTasks() {
        for (taskIndex in 0 until tasksForCombiner.size) {
            val localTask = tasksForCombiner[taskIndex].value ?: continue
            when (localTask) {
                is Element<*> -> {
                    queue.addLast(localTask.value as E)
                    tasksForCombiner[taskIndex].compareAndSet(localTask, PROCESSED)
                }
                DEQUE_TASK -> {
                    val dequeued = queue.removeFirstOrNull()
                    tasksForCombiner[taskIndex].compareAndSet(localTask, Element(dequeued, false))
                }
            }
        }
    }
