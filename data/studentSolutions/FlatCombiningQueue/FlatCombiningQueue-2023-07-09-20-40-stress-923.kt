...
class FlatCombiningQueue<E> : Queue<E> {
    ...
    override fun dequeue(): E? {
        ...
        while (true) {
            if (combinerLock.compareAndSet(false, true)) {
                ...
                msg(-100, "LOCK remove $res")
                processQueue()
                combinerLock.compareAndSet(true, false)
                return res.also { msg(-100, "ACTUALLY RETURNED1 $res") }
            } else {
                val id = randomCellIndex()
                    ...
                    while (true) {
                        if (combinerLock.compareAndSet(false, true)) {
                            val res = if (tasksForCombiner[id].compareAndSet(Dequeue, null)) {
                                ...
                                msg(id, "LOCK2 manual removed $v")
                                v
                            } else {
                                val v = deCheckAndClear(id)
                                msg(id, "LOCK2 checked and found $v")
                                v
                            }
                            ...
                            combinerLock.compareAndSet(true, false)
                            return res.also { msg(-100, "ACTUALLY RETURNED2 $res") }

                        } else {
                            deCheckAndClear(id)?.let { return it.also { msg(-100, "ACTUALLY RETURNED3 $it") } }
                        }
                    }
                }
            }
    ...
    private fun msg(id: Int, other: String) =
        println("this:${this.hashCode()} tid:${Thread.currentThread().id} id:$id $other")
