...
class AtomicArrayNoAba<E>(size: Int, initialValue: E) {
    ...
    fun cas2(index1: Int, expected1: E, update1: E,
        ...
        val cas2Descriptor = CAS2Descriptor(index1, expected1, update1, index2, expected2, update2, OUTCOME.UNDECIDED)

        if (a[index1].value!!.cas(expected1 as Any, cas2Descriptor)) {
            if (cas2Descriptor.outcome == OUTCOME.UNDECIDED && a[index2].value!!.cas(expected2 as Any, cas2Descriptor)) {
                cas2Descriptor.outcome = OUTCOME.SUCCESS
    ...
    inner class CAS2Descriptor<E>(
        ...
        override fun complete() {
            val value = a[index2].value
            if (value == expected2) {
                outcome = OUTCOME.SUCCESS
                a[index2].value!!.cas(value as Any, update2 as Any)
            } else {
                outcome = OUTCOME.FAILED
                a[index1].value!!.cas(update1 as Any, expected1 as Any)
            }
        }
    }
}
