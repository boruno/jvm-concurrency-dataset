...
class AtomicArrayNoAba<E>(size: Int, initialValue: E) {
    ...
    fun cas2(index1: Int, expected1: E, update1: E,
        ...
        return when (expected1) {
            is Int -> cas(index1, expected1, (expected1 + 1) as E)
            expected2 -> cas(index1, expected1, update1)
            else -> false
        }
    }
}
...
class Ref<T>(initial: T) {
    ...
    fun cas(expected: Any?, update: Any?): Boolean {
        v.loop {
            if (it == expected) { if (v.compareAndSet(it, update)) return true }
            else return false
        }
    }
}
...
private class CAS2Descriptor<A, B>(
    ...
    override fun complete() {
        val tmp = if (bRef.cas(bExpect, this)) Outcome.Success else Outcome.Fail
        outcome.compareAndSet(Outcome.Undecided, tmp)
        val aRes = if (outcome.value == Outcome.Success) aUpdate else aExpect
        val bRes = if (outcome.value == Outcome.Success) bUpdate else bExpect

        outcome.compareAndSet(Outcome.Undecided, Outcome.Success)
        aRef.v.compareAndSet(this, aRes)
        bRef.v.compareAndSet(this, bRes)
    }
}