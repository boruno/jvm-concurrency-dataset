...
class CasnDescriptor<E>(
    ...
    val outcome = Ref(UNDECIDED)

    override fun complete() {
        if (dcss(b, expectedB, this, outcome, UNDECIDED)) {
...
fun <A, B> dcss(
    a: Ref<A>, expected1: A, update1: Any,
    b: Ref<B>, expected2: B
): Boolean {
...
class DCSSDescriptor<A, B>(
    private val a: Ref<A>, private val expectedA: A, private val updateA: Any,
    private val b: Ref<B>, private val expectedB: B
) : Descriptor {
    val outcome = Ref(UNDECIDED)
