...
class CASNDescriptor<E>(val cds: List<CellDescriptor<E>>) {
    ...
    fun eq(other: CASNDescriptor<E>?): Boolean {
        if (other == null) {
            return false
        }
        if (cds.size != other.cds.size) {
            return false
        }
        var same = true
        for (i in 0 until cds.size) {
            same = same
                && cds[i].index    == other.cds[i].index
                && cds[i].expected == other.cds[i].expected
                && cds[i].update   == other.cds[i].update
        }
        return same
    }
...
class AtomicArrayNoAba<E>(size: Int, initialValue: E) {
    ...
    private fun readInternal(index: Int, desc: CASNDescriptor<E>?): Pair<Cell<E>, E> {
        while (true) {
            ...
            if (oldVal is CellDescriptor) {
                ...
                if (!parent.eq(desc) && parent.getStatus() == Status.ACTIVE) {
                    casN(parent)
                    continue
                }
    ...
    fun cas(index: Int, expected: E, update: E): Boolean {
        ...
        val desc = CASNDescriptor(listOf(c1))
        c1.parent = desc
        return casN(desc)
    }
    ...
    fun cas2(index1: Int, expected1: E, update1: E,
        ...
        val desc = CASNDescriptor(listOf(c1, c2))
        c1.parent = desc
        c2.parent = desc
        return casN(desc)
    }
