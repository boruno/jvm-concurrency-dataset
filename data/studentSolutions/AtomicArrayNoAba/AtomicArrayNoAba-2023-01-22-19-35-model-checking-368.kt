    ...
    fun complete(): Boolean {
        ...
        if ( IWONNADIEPLEASEKILLME != this ) {
            if ( IWONNADIEPLEASEKILLME is DescDCSS ) {
                ...
                while (true) {
                    var cop = b.get()
                    if (cop !is DescDCSS && cop !is DescCAS2 && cop != expectB) {
                        dcss.res.value.compareAndSet(null, false)
                    }
...
class DescDCSS ( val a: Ref, val expectA: Any?, val descCAS2: DescCAS2 ) {
    ...
    fun complete() : Boolean {
        if ( descCAS2.res.value.value == null ) {
            res.value.compareAndSet( null, true )
            a.value.compareAndSet( this, descCAS2 )
            return true
        } else {
            return false
        }
...
//todo придумать другую лекцию по касну, эта говно.