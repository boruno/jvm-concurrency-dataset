...
class AtomicArrayNoAba<E>(size: Int, initialValue: E) {
    ...
    fun DCAS(desc: DCASDescriptor<E>) : Boolean {
        ...
        if (success) {
            a[desc0.index].compareAndSet(desc0.expected, desc0.update)
            a[desc1.index].compareAndSet(desc1.expected, desc1.update)
            desc.status.compareAndSet(StatusType.ACTIVE, StatusType.SUCCESSFUL)
            aD[desc0.index].value = null
            aD[desc1.index].value = null
            return true
        } else {
