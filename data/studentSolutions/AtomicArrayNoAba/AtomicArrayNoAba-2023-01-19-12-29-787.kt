...
class AtomicArrayNoAba<E>(size: Int, initialValue: E) {
    ...
    fun DCAS(desc: DCASDescriptor<E>) : Boolean {
        ...
        if (desc.status.value == StatusType.SUCCESSFUL) {
            return true;
        } else if (desc.status.value == StatusType.FAILED) {
            return false
        }
        if (success) {
            desc.status.compareAndSet(StatusType.ACTIVE, StatusType.COMMITED)
            updateValues(desc)
            return true
        } else {
            desc.status.compareAndSet(StatusType.ACTIVE, StatusType.ROLLBACK)
            getBackValues(desc)
            return false
        }
    }

    fun updateValues(desc: DCASDescriptor<E>) {
        ...
        desc.status.compareAndSet(StatusType.COMMITED, StatusType.SUCCESSFUL)
    }

    fun getBackValues(desc: DCASDescriptor<E>) {
        val desc0 = desc.descriptors[0]!!
        val desc1 = desc.descriptors[1]!!
        a[desc0.index].compareAndSet(desc0, Value(desc0.old))
        a[desc1.index].compareAndSet(desc1, Value(desc1.old))
        desc.status.compareAndSet(StatusType.ROLLBACK, StatusType.FAILED)
    }
        ...
        while (true) {
            ...
            if (value is Value) {
                ...
                if (parent != desc && parent.status.value == StatusType.ACTIVE) {
                    DCAS(parent)
                    continue
                } else if (parent != desc && parent.status.value == StatusType.COMMITED) {
                    updateValues(parent)
                    continue
                } else if (parent != desc && parent.status.value == StatusType.ROLLBACK) {
                    getBackValues(parent)
                    continue
                }
                else {
    ...
    enum class StatusType {ACTIVE, SUCCESSFUL, FAILED, COMMITED, ROLLBACK}
}