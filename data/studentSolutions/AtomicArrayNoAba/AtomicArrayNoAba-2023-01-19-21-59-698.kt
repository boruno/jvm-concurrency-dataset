...
class AtomicArrayNoAba<E>(size: Int, initialValue: E) {
    ...
    fun cas2(index1: Int, expected1: E, update1: E,
        ...
        if ( copy!!.a.value == a[ command.indexA ].value!!.a.value && a[command.indexA].compareAndSet( copy, command ) ) {
            if ( copy1!!.a.value == a[command.indexB].value!!.a.value && a[ command.indexB!! ].compareAndSet( copy1, command1 ) ) {
                command.c = SUCC
                a[command.indexA].compareAndSet( command, Value( command.updateA, null, null, null, null, null, null) )
                a[command.indexB].compareAndSet( command1, Value( command.updateB, null, null, null, null, null, null ) )
                return true
            } else {
                command.c = FAIL
