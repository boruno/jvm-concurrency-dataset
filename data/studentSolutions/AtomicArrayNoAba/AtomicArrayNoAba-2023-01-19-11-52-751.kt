...
class CASNDescriptor<E>(
    ...
    val status = atomic(Status.Undefined)

    fun complete() {
        if (b.v.compareAndSet(expectB, this)) {
            status.compareAndSet(Status.Undefined, Status.Success)
        } else {
            status.compareAndSet(Status.Undefined, Status.Failed)
        }
        ...
        while (true) {
            ...
            if (value1.v.compareAndSet(expected1, descriptor)) {
                ...
                when (descriptor.status.value) {
                    ...
                    Status.Failed, Status.Undefined -> return false
                }
            } else {
                return false
            }
