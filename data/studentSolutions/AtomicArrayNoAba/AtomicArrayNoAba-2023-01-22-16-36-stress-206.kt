...
class AtomicArrayNoAba<E>(size: Int, initialValue: E) {
    ...
    fun cas2(index1: Int, expected1: E, update1: E,
        ...
        if (index1 == index2) {
            val int = (expected1 as Int) + 2
            return if (expected1 == expected2) cas(index1, expected1, int as E) else false
        }
    ...
    inner class CAS2Descriptor<E>(
        ...
        override fun complete() {
            if (a[index2].value!!._value.compareAndSet(expected2 as Any, this)) {
...
class Element<E>(private val initValue: E) {
    ...
    fun cas(expected: Any, update: Any): Boolean {
        while (true) {
            when (val elem = _value.value) {
                is AtomicArrayNoAba<*>.CAS2Descriptor<*> -> elem.complete()
                else -> return _value.compareAndSet(elem, update)
            }
        }
    }
}