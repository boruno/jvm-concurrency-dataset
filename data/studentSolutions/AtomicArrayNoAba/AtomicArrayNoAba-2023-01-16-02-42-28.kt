...
class AtomicArrayNoAba<E : Any>(val size: Int, initialValue: E) {
    ...
    class Ref<E>(init: E) {
        ...
        var value: E
            get() {
                inValue.loop {
                    when (it) {
                        !is CASDescriptor<*> -> return it as E/*{it.complete()}*/
                    }
                }
            }
            set(upd) {
                inValue.loop {
                    when (it) {
                        !is CASDescriptor<*> -> if (inValue.compareAndSet(it, upd)) return/*it.complete()*/
                    }
                }
            }
    }
    ...
    class RDCSSDescriptor<A, B> (val value1: Ref<A>, val expected1: A,
        ...
        override fun complete(): Boolean{
            ...
            status = if (value2.inValue.compareAndSet(expected2, update) && status != Result.FAIL)
