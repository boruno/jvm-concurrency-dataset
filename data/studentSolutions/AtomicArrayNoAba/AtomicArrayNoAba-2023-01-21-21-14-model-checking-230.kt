...
class AtomicArrayNoAba<E>(size: Int, initialValue: E) {
    ...
    fun cas2(
        ...
        if (a[i1]!!.v.compareAndSet(exp1, descriptor) && a[i2]!!.v.compareAndSet(exp2, upd2)) {
    ...
    inner class Descriptor<E>(
        ...
        fun complete() {
            if (outcome.value == OutCome.UNDECIDED && a[index2]!!.v.compareAndSet(expected2, this)) {
                outcome.value = OutCome.SUCCESS
                a[index1]!!.v.compareAndSet(this, update1)
                a[index2]!!.v.compareAndSet(this, update2)
            } else {
                a[index1]!!.v.compareAndSet(this, expected1)
