...
class Ref<T>(initial: T) {
    ...
    var value: T
        get() {
            v.loop {
                if (it is Descriptor) it.complete() else return it as T
            }
        }
        set(new) {
            v.loop {
...
private class CAS2Descriptor<A, B>(
    ...
    override fun complete() {
        val t = if (bRef.cas(bExpect, this)) Outcome.Success else Outcome.Fail
        outcome.compareAndSet(Outcome.Undecided, t)
