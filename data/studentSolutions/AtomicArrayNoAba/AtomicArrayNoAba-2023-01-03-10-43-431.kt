...
class RDCSSDescriptor(
    val a: MyRef<Any?>, val expectA: Any?, val updateA: Any?,
    val b: AtomicRef<DescriptorStatus>, val expectB: DescriptorStatus)
    : Descriptor {
    val outcome = atomic(DescriptorStatus.UNDECIDED)

    override fun complete() {
        val curStatus = outcome.value
        if (outcome.value != DescriptorStatus.UNDECIDED) {
        ...
        if (currentB === expectB)  {
            this.outcome.compareAndSet(DescriptorStatus.UNDECIDED, DescriptorStatus.SUCCESS)
        } else {
            this.outcome.compareAndSet(DescriptorStatus.UNDECIDED, DescriptorStatus.FAIL)
        }
    }
}
...
class MyRef<out E>(init: Any?) {
    ...
    fun set(value: Any?) {
        while (true) {
            val curValue = this.ref.value
            if (curValue is Descriptor) {
                curValue.complete()
                continue
            }

            if (this.ref.compareAndSet(curValue, value)) {
                return
            }
        }
    }
