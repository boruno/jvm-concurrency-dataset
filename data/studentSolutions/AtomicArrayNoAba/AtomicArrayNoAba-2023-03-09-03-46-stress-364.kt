...
//    }
    ...
    fun cas2(index1: Int, expected1: E, update1: E,
        ...
        if (i1 > i2) i1 = i2.also{ i2 = i1 }.also{ e1 = e2 }.also{ e2 = e1 }.also{ u1 = u2 }.also { u2 = u1 }
        val descriptor = Descriptor(a[i1], e1, u1, a[i2], e2, u2)
        if (a[i1].cas(e1, descriptor)) {
            descriptor.complete()
            return descriptor.outcome.value ?: false
        }
        return false
    }
}
...
class Descriptor<E> (
    ...
    fun complete() {
        ...
        val ua = if (fl) updateA else expectedA
        val ub = if (fl) updateB else expectedB
        outcome.ref.compareAndSet(null, fl)
        a.ref.compareAndSet(this, ua)
        b.ref.compareAndSet(this, ub)
    }
}

class Ref<E>(value: E?) {
    val ref: AtomicRef<Any?> = atomic(value)
    var value: E
        get() = ref.loop {
            if (it is Descriptor<*>) it.complete() else return it as E
        }
        set(new) = ref.loop {
            if (it is Descriptor<*>) it.complete() else if (ref.compareAndSet(it, new)) return
        }
