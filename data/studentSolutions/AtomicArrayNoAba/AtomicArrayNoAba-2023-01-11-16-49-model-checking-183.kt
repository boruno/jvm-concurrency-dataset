...
class Descriptor<E>(
    ...
    b: E, val expectB: E, val updateB: E
): NodeValue {

    private val a = atomic(a)
    private val b = atomic(b)
    fun complete() {
        val update = if (b.compareAndSet(expectB, updateB)) updateA else expectA
...
class AtomicArrayNoAba<E>(size: Int, initialValue: E) {
    ...
    fun cas2(index1: Int, expected1: E, update1: E,
        ...
        while (true) {
            val descriptor = Descriptor(value1.value.value, expected1, update1, value2.value.value, expected2, update2)
            if (a[index1].compareAndSet(SimpleValue(expected1), descriptor)) {
                if (a[index2].compareAndSet(SimpleValue(expected2), SimpleValue(update2))) {
