...
class AtomicArrayNoAba<E>(size: Int, initialValue: E) {
    ...
    private inner class DescriptorCAS2(
        val a: Ref, val expectA: Any?, val updateA: Any?,
        val b: Ref, val expectB: Any?, val updateB: Any?
    ) : Descriptor() {
        override fun complete() {
    ...
    private inner class Ref(initial: Any?) {
        ...
        fun get(): Any? {
            v.loop {
                if (it is AtomicArrayNoAba<*>.Descriptor) it.complete() else return it
            }
        }

        fun set(update: E) {
            v.loop {
                if (it is AtomicArrayNoAba<*>.Descriptor) it.complete() else if (v.compareAndSet(it, update)) return
            }
        }
        ...
        fun cas(expected: Any?, update: Any?): Boolean {
            while (true) {
                ...
                if (curValue is AtomicArrayNoAba<*>.Descriptor) {
                    curValue.complete()
                } else {
