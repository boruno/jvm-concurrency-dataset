...
class AtomicArrayNoAba(size: Int, initialValue: Int) {
    ...
    fun get(index: Int): Int {
        while (true) {
            ...
            if (indexElement.second != null) {
                doCAS2WithHelp(indexElement.second!!)
                continue
            }
    ...
    fun cas(index: Int, expected: Int, update: Int): Boolean {
        while (true) {
            ...
            if (curElement.second != null) {
                doCAS2WithHelp(curElement.second!!)
                continue
            }
    ...
    fun cas2(index1: Int, expected1: Int, update1: Int,
        ...
        while (true) {
            doCAS2WithHelp(thisCASNDescriptor)
    ...
    fun doCAS2WithHelp(casnDescriptor: CASNDescriptor<Int>) {
        while (true) {
            if (casnDescriptor.outcome.value != Outcome.UNDECIDED) {
                doCAS2WhenDecided(casnDescriptor)
                return
            }
            ...
            if (index1Element.second != null && index2Element.second != null) {
                if (index1Element.second == casnDescriptor && index2Element.second != casnDescriptor) {
                    if (conflictingDescriptors(casnDescriptor, index2Element.second!!)) {
                        if (resolveConflict(casnDescriptor, index1Element, index2Element)) {
                            continue
                        }
                    }
                }
            ...
            if (index1Element.second != casnDescriptor && index1Element.second != null) { // installed another descriptor
                doCAS2WithoutRecursionAndCycles(index1Element.second!!)
                continue
            }

            if (index2Element.second != casnDescriptor && index2Element.second != null) { // installed another descriptor
                doCAS2WithoutRecursionAndCycles(index2Element.second!!)
                continue
            }

            if (casnDescriptor.outcome.value == Outcome.UNDECIDED) { // here both indexes have either casnDescriptor or null installed
                doCAS2WhenUndecided(casnDescriptor, index1Element, index2Element)
            }
        }
    }

    fun doCAS2WhenUndecided(casnDescriptor: CASNDescriptor<Int>, index1Element: Pair<Int, CASNDescriptor<Int>?>, index2Element: Pair<Int, CASNDescriptor<Int>?>) {
    ...
    fun resolveConflict(casnDescriptor: CASNDescriptor<Int>, index1Element: Pair<Int, CASNDescriptor<Int>?>, index2Element: Pair<Int, CASNDescriptor<Int>?>): Boolean {
        if (casnDescriptor.outcome.value == Outcome.UNDECIDED && index2Element.second!!.outcome.value == Outcome.UNDECIDED) {
            if (index1Element.first == index2Element.second!!.expect2) {
                if (a[casnDescriptor.index1].value!!.element.compareAndSet(index1Element, Pair(index1Element.first, index2Element.second))) {
                    ...
                    doCAS2WhenDecided(index2Element.second)
                    return true
                }
            }
            else {
    ...
    fun doCAS2WithoutRecursionAndCycles(casnDescriptor: CASNDescriptor<Int>) {
        if (casnDescriptor.outcome.value != Outcome.UNDECIDED) {
            doCAS2WhenDecided(casnDescriptor)
            return
        }
    ...
    fun doCAS2WhenDecided(casnDescriptor: CASNDescriptor<Int>?) {
