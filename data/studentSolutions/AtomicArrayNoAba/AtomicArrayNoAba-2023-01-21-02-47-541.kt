...
class AtomicArrayNoAba<E>(size: Int, initialValue: E) {
    ...
    fun cas(index: Int, expected: E, update: E) =
        a[index].reference.compareAndSet(expected, update)
    ...
    fun cas2(
        ...
        val firstUpdated = if (index1 <= index2) update1 else update2
        while (true) {
            ...
            if (a[firstIndex].reference.compareAndSet(firstExpected, caS2Descriptor)) {
                caS2Descriptor.complete()
                return caS2Descriptor.outcome.value === Outcome.SUCCESS
            } else {
                if (a[firstIndex].value === firstExpected) return false
            }
        }
    }
