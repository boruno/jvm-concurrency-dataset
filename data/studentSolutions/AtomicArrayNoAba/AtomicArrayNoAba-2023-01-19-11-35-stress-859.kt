...
class CASNDescriptor<E>(
    ...
    val status = atomic(Status.Underfined)

    fun complete() {
        if (b.v.compareAndSet(expectB, this)) {
            status.compareAndSet(Status.Underfined, Status.Success)
        } else {
            status.compareAndSet(Status.Underfined, Status.Failed)
        }
        ...
        while (true) {
            ...
            if (value1.v.compareAndSet(expected1, descriptor)) {
                ...
                when (descriptor.status.value) {
                    ...
                    Status.Failed, Status.Underfined -> return false
                }
            } else {
                return false
            }
