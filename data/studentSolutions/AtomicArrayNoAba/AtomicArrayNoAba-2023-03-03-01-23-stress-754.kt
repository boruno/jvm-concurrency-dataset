...
class AtomicArrayNoAba<E>(size: Int, initialValue: E) {
    ...
    fun completeCAS2(dc: DescriptorCAS2) {
        ...
        var realElem2 = get_real(dc.index2)
        while ( realElem2 !is Int) {
            if (realElem2 is DescriptorCAS2) {
                completeCAS2_2(realElem2)
            }
            if (realElem2 is DescriptorDCSS) {
                completeDCSS(realElem2)
            }
            realElem2 = get_real(dc.index2)
        }

        dcss(dc.index2, dc.expected2 as E, dc, dc.outcome.value, "Undecided")
        completeCAS2_2(dc)
    }

    fun completeCAS2_2 (dc: DescriptorCAS2) {
        while (true) {
            val res = dc.outcome.value
            if (res == "Success") {
    ...
    fun dcss(index1: Int, expected1: E, update1: DescriptorCAS2,
        ...
        var curElemOnIndex1 = get_real(index1)
        while (curElemOnIndex1 !is Int) {
            if (curElemOnIndex1 is DescriptorDCSS) {
                completeDCSS(curElemOnIndex1)
            }
            if (curElemOnIndex1 is DescriptorCAS2) {
                completeCAS2(curElemOnIndex1)
            }
            curElemOnIndex1 = get_real(index1)
        }
        if (cas(index1, expected1, dc)) {
            completeDCSS(dc)
        } else {
    ...
    fun completeDCSS(dc: DescriptorDCSS) {
        ...
        if (real2 == dc.expected2) {
            if (dc.outcome.compareAndSet("Undecided", "Success")) {
                cas(dc.index1, dc, dc.update1)
            } else if (dc.outcome.compareAndSet("Undecided", "Fail")){
                cas(dc.index1, dc, dc.expected1)
            } else {
