    ...
    private fun finish(descriptor: CASNDescriptor): Boolean {
        with(descriptor) {
            if (outcome.value != Outcome.UNDECIDED) {
                val valueA = if (outcome.value == Outcome.SUCCESS) updateA else expectedA
                a[indexA].compareAndSet(descriptor, valueA)
                return outcome.value == Outcome.SUCCESS
            }
            val curB = get(indexB)
            if (curB != expectedB || !a[indexB].compareAndSet(curB, updateB)) {
                println("$curB $expectedB")
