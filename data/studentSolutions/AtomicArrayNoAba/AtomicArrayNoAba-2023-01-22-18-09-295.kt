...
class DescCAS2(
    ...
    fun complete(): Boolean {
        val descDCSS = DescDCSS( b, expectB, this ) // говно+говно*говно
        if ( b.value.value != this ) {
            if ( b.value.value is DescDCSS ) {
                b.value.value
            }
            ...
            if ( b.value.compareAndSet( expectB, descDCSS ) ) {
                descDCSS.complete()
            }
        }
        if ( descDCSS.res.value.value != null && descDCSS.res.value.value == true ) {
...
class DescDCSS ( val a: Ref, val expectA: Any?, val updateA: Any? ) {
    val res = Ref(null)
    fun complete() : Boolean {
        if ( res.value.compareAndSet( null, true ) ) {
            a.value.compareAndSet(this, updateA)
            return true
        } else {
            res.value.compareAndSet( null, false )
            a.value.compareAndSet( this, a )
            return false
        }
    }
}
