...
class AtomicArrayNoAba<E>(size: Int, initialValue: E) {
  ...
  fun cas2(
    ...
    return if (a1.cas(expected1, desc)) desc.complete() else false
  }
}
...
class NDescriptor(
  ...
  val state = Ref<Boolean?>(null)
  override fun complete(): Boolean {
    if (b.value != this) dcss(b, expectB, this, state as Ref<Any?>, null)
    state.value = b.value == this

    val a1 = if (state.value!!) updateA else expectA
    a.v.compareAndSet(this, a1)
    val b1 = if (state.value!!) updateB else expectB
