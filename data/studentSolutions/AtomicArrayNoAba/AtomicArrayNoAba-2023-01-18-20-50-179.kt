...
class AtomicArrayNoAba<E>(size: Int, initialValue: E) {
    ...
    fun <A,B> cas2ref(a: Ref<A>, expectA: A, updateA: A,
        ...
        if (!a.v.compareAndSet(expectA, desc)) {
            desc.outcome.v.compareAndSet(0, 2)
            return desc.outcome.value == 2
        }
...
class CASNDescriptor<A, B>(
    ...
    fun <A,B> dcss(a: Ref<A>, expectA: A, updateA: Any,
        ...
        desc_.compareAndSet(null, RDCSSDescriptor(a, expectA, updateA, b, expectB))
        val desc = desc_.value as RDCSSDescriptor<A, B>
        if (!a.v.compareAndSet(expectA, desc)) {
            desc.outcome.v.compareAndSet(0, 2)
            return desc.outcome.value == 2
        }
