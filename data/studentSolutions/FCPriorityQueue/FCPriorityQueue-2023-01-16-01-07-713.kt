...
class FCPriorityQueue<E : Comparable<E>> {
    ...
    private val opArray = atomicArrayOfNulls<Operation<E>>(Runtime.getRuntime().availableProcessors())
    ...
    private fun tryPerform(operation: Operation<E>): E? {
        ...
        while (true) {
            if (lock.tryLock()) {
                ...
                if (arrIdx == -1) {
                    val idx = nextInt(Runtime.getRuntime().availableProcessors())
                    ...
                    if (opArray[arrIdx].value!!.operationType == OperationType.RESULT) {
                        return opArray[arrIdx].value!!.element
                    }
                }
            }
        }
