...
class FCPriorityQueue<E : Comparable<E>> {
    ...
    fun add(element: E) {
        ...
        while (true) {
            if (arr[curID] !is OperationType || arr[curID] != OperationType.ADD)
                break
            if (!lock.compareAndSet(false, true)) continue
            for (i in arr.indices) {
                when (arr[i]) {
    ...
    private fun abstractLockOperation(operation: () -> E?): E? {
        arr[index.get()] = OperationType.ADD
        ...
        while (arr[index.get()] === operation) {
            if (lock.compareAndSet(false, true)) {
                for (i in arr.indices) {
                    ...
                    if (element == OperationType.ADD) arr[i] = operation()
                }
                lock.getAndSet(false)
            }
        }
