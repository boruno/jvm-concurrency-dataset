...
class FCPriorityQueue<E : Comparable<E>> {
    ...
    private val nodes = atomicArrayOfNulls<Node<E>>(20)
    ...
    fun poll(): E? {
        while (true) {
            return if (lock.tryLock()) {
                ...
                commonMethod(Node(POLL, null))?.x ?: continue
            }
        }
    }
    ...
    fun peek(): E? {
        while (true) {
            return if (lock.tryLock()) {
                ...
                commonMethod(Node(PEEK, null))?.x ?: continue
            }
        }
    }
    ...
    fun add(element: E) {
        while (true) {
            if (lock.tryLock()) {
                ...
                commonMethod(Node(ADD, element)) ?: continue
                return
            }
        }
    }
...
fun <T> ReentrantLock.tryWithLock(blockIfUnlocked: () -> T, blockIfLocked: () -> T): T =
    if (tryLock()) {
        try {
            blockIfUnlocked()
        } finally {
