...
class FCPriorityQueue<E : Comparable<E>> {
    ...
    fun poll(): E? {
        ...
        while(true) {
            if (!tryLock()) {
                if (insert) {
                    if (arr[index].compareAndSet(null, "poll")) { insert = false } else { index = ThreadLocalRandom.current().nextInt() }
                } else {
    ...
    fun peek(): E? {
        ...
        while (true) {
            if (!tryLock()) {
                if (insert) {
                    if (arr[index].compareAndSet(null, "peek")) { insert = false } else { index = ThreadLocalRandom.current().nextInt() }
                } else {
    ...
    fun add(element: E) {
        ...
        while (true) {
            if (!tryLock()) {
                if (insert) {
                    if (arr[index].compareAndSet(null, element)) { insert = false } else { index = ThreadLocalRandom.current().nextInt() }
                } else {
