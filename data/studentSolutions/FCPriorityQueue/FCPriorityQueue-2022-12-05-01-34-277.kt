...
class FCPriorityQueue<E : Comparable<E>> {
    ...
    private fun action(opType: Int, element: Any? = null): E? {
        while(true) {
            var value: E? = null
            if(lock.tryLock()) {
                when(opType) {
...
private class Operation(val type: Int) {
    val fulfilled = atomic<Boolean>(false)
    val value = atomic<Any?>(null)
}
