...
class InsertValue <E>{
    private val _value: AtomicRef<Any?> = atomic(null)
    ...
    constructor(value: E?, operation: Operation) {
        ...
        _operation.getAndSet(operation)
    }
    fun getCurrentValue(): E? {
        return _value.value as E?
    }
    ...
    fun setStatus(status: Boolean) {
        _completed.getAndSet(status)
    }
    fun setCurrentValue(value: E?) {
        _value.getAndSet(value)
    }
        ...
        for (i in 0 until ArraySize) {
            ...
            if (iV.getOperation() == Operation.add) {
                if (!iV.getStatus()) {
                    ...
                    iV.setStatus(true)
                    arr[i].getAndSet(iV) // ToDo:: check this | null return here value
                    continue
                }
            }
        }
