...
class FCPriorityQueue<E : Comparable<E>> {
    ...
    fun tryWithLock(block: () -> Unit): Boolean {
    ...
    private fun mainCycle(operation: Operation<E>): OperationResult<E> {
        while (!operation.isPerformed) {
            ...
            if (cellToPutOperation.compareAndSet(operationInCell, operation)) {
                ...
                if (!cellToPutOperation.compareAndSet(operation, None())) {
                    ...
                    break  // 39 line -> somebody fetched it to proceed, we need to wait a bit
                }
            }
