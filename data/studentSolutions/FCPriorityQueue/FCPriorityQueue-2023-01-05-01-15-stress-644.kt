        ...
        while (true) {
            if (tryLock()) {
                ...
                for (i in 0 until FC_ARRAY_SIZE) {
                    ...
                    if (res != null && fc_array[i].compareAndSet(res, Pair(CMD.READY, res.second))) {
                        when (res.first) {
                            CMD.ADD -> {
                                q.add(res.second)
                                fc_array[i].getAndSet(Pair(CMD.EMPTY, res.second))
                            }
                            CMD.PEEK -> {
                                fc_array[i].getAndSet(Pair(CMD.EMPTY, q.peek()))
                            }
                            CMD.POLL -> {
                                fc_array[i].getAndSet(Pair(CMD.EMPTY, q.poll()))
                            }
                            else -> {}
                        }
                    }
