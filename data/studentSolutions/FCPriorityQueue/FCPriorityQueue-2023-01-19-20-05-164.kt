    ...
    fun poll(): E? {
        ...
        for (i in 0 until 50) {
            ...
            if (ar[i].value!!.op == OP.PEEK) {
                ar[i].compareAndSet(task, Operation(task.op, STATUS.DONE, null, q.peek()))
            } else if (task.op == OP.ADD) {
                q.add(task.input)
                ar[i].compareAndSet (task, Operation(task.op, STATUS.DONE, task.input, null))
            } else if (task.op == OP.POOL) {
                ar[i].compareAndSet(task, Operation(task.op, STATUS.DONE, null, q.poll()))
            }
        }
