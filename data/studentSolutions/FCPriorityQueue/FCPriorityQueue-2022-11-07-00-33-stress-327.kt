...
class FCPriorityQueue<E : Comparable<E>>() {
    ...
    fun peek(): E? {
        if (lock.tryLock()) {
            ...
            while (true) {
                ...
                if (!lock.isLocked) {
                    if (lock.tryLock()) {
                        if (!FCArray[i].compareAndSet(PEEK_SIGNAL, EMPTY_SIGNAL)) {
                            val result = FCArray[i].value
                            FCArray[i].compareAndSet(result, EMPTY_SIGNAL)
                            return result as E
                        }
    ...
    fun add(element: E) {
        if (lock.tryLock()) {
            q.add(element)
            for (i in 0 until FCArray.size) {
                if (FCArray[i].compareAndSet(POLL_SIGNAL, DequeueResponse(q.poll()))) {
                    continue
                }
                if (FCArray[i].compareAndSet(PEEK_SIGNAL, DequeueResponse(q.peek()))) {
                    continue
                }
                if (FCArray[i].value is EnqueueRequest<*>) {
                    q.add((FCArray[i].value as EnqueueRequest<E>).element)
