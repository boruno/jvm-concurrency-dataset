        ...
        while (true) {
            if (tryLock()) {
                ...
                while (true) {
                    ...
                    if (flatCombiningArray[index].value === null) throw NullPointerException("!!throw null!!")
    ...
    private fun support() {
        for (i in 0 until flatCombiningArray.size) {
            ...
            when (operation.status) {
                OperationType.POLL -> {
                    val result = q.poll()
                    if (!flatCombiningArray[i].compareAndSet(operation, Operation(OperationType.DONE, result))) {
                        q.add(result)
                    }
                }
