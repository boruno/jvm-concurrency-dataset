    ...
    fun poll(): E? {
        while (true) {
            if (!lock.isLocked) {
                ...
                val p = giveToCombiner(Operation(OperationType.POLL, null))
                if (p.first) {
                    return p.second
                }
            }
    ...
    fun peek(): E? {
        while (true) {
            if (!lock.isLocked) {
                ...
                val p = giveToCombiner(Operation(OperationType.PEEK, null))
                if (p.first) {
                    return p.second
                }
            }
    ...
    fun add(element: E) {
        while (true) {
            if (!lock.isLocked) {
                ...
                val p = giveToCombiner(Operation(OperationType.POLL, element))
                if (p.first) return
            }
        }
    }
