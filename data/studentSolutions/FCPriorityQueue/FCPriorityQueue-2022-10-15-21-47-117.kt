...
class FCPriorityQueue<E : Comparable<E>> {
     ...
     */
    fun poll(): E? {
        val req = Request<E>(ReqType.Poll, null, lastRequestId.getAndIncrement())
        var index = 0
        while (true) {
            if (lock.tryLock()) {
     ...
     */
    fun peek(): E? {
        val req = Request<E>(ReqType.Peek, null, lastRequestId.getAndIncrement())
        var index = 0
        while (true) {
            if (lock.tryLock()) {
                val leftMsg = getHelpedRequest(req.id)
     ...
     */
    fun add(element: E) {
        val req = Request<E>(ReqType.Add, element, lastRequestId.getAndIncrement())
        var index = 0
        while (true) {
            if (lock.tryLock()) {
                val leftMsg = getHelpedRequest(req.id)
    ...
    private fun help(): String {
        ...
        for (i in 1 until LINE_SIZE) {
            ...
            if (req != null) {
                ...
                s += line[i].value!!.msg.toString() + " at " + i.toString() + " "
            }
        }
        return s
    }

    fun getHelpedRequest(id: Long): Request<E>? {
        for (i in 1 until LINE_SIZE) {
            val req = line[i].value
            if (req != null) {
                if (req.op == ReqType.Response && req.id == id) {
                    return line[i].getAndSet(null)
                }
            }
        }
        return null
