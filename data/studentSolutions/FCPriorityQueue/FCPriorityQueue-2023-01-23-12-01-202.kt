...
class FCPriorityQueue<E : Comparable<E>> {
    ...
    private fun getRandomIndex(): Int {
        return (FC_ARRAY_SIZE + ThreadLocalRandom.current().nextInt() % FC_ARRAY_SIZE) % FC_ARRAY_SIZE
    }
    ...
    fun poll(): E? {
        while (true) {
            if (locked.compareAndSet(expect = false, update = true)) {
                ...
                var attempts = 0
                while (fcArray[i].value is Operation<*> && attempts++ < ATTEMPTS)
