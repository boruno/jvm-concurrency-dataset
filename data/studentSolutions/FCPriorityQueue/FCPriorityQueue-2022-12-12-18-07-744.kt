...
class FCPriorityQueue<E : Comparable<E>> {
        ...
        lok@ while (!lock.compareAndSet(false, true)) {
            ...
            while (true) {
                if (fc_array[i].compareAndSet(null, Pair(1, null))) {
                    var x = fc_array[i].value?.second
                    while (x == null) {
                        if (lock.compareAndSet(false, true)) { //не поменяли и взяли блокировку выйти из while(23)
                            break@lok
                        }
                        x = fc_array[i].value?.second
                    }
                    //кто-то помог
                    fc_array[i].compareAndSet(Pair(1, x), null) //освобождаем массив
                    return x as E
                }
    ...
    fun add(element: E) {
        lok@ while (!lock.compareAndSet(false, true)) {
            var i = random.nextInt(0,  fc_array.size)
            while (true) {
                if (fc_array[i].compareAndSet(null, Pair(2, element))) {
                    while (!fc_array[i].compareAndSet(Pair(2, DONE), null)) {
    ...
    private fun help() {
        ...
        while (i < fc_array.size) {
            ...
            if (cur != null) {
                if (cur.first == 1) {
                    fc_array[i].compareAndSet(Pair(1, null), Pair(1, q.poll()))
                } else {
                    val x = cur.second
                    if (x != DONE) {
                        q.add(x as E)
                        fc_array[i].compareAndSet(Pair(2, null), Pair(2, DONE))
                    }
                }
            }
            i++
