...
class FCPriorityQueue<E : Comparable<E>> {
    ...
    private fun executeQuery(queryType: QueryType, element: E? = null): E? {
        ...
        while (!tryLock()) {
            if (index == null) {
                ...
                if (maybeExecuted.isExecuted()) {
                    if (!queries[index].compareAndSet(maybeExecuted, null)) {
                        throw Exception("Cell ownership rights violated")
                    }
        ...
        val result = if (index == null) {
            ...
            val executed = queries[index].value ?: throw Exception("Cell ownership rights violated")
            if (!queries[index].compareAndSet(executed, null)) {
                throw Exception("Cell ownership rights violated")
            }
            executed.getElement()
        }
