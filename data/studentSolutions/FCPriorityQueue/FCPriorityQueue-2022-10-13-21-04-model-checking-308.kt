...
class FCPriorityQueue<E : Comparable<E>> {
    ...
    private val tasks = Array(arraySize) {emptyTask}
    private val results = Array(arraySize) {emptyTask}
    ...
    private fun process(name : Int, element: E?) : E? {
        ...
        var index = 0
        while (true) {
            if (stages[index].compareAndSet(0, 0)) {
                tasks[index] = task
                stages[index].set(1)
                break
            }
        ...
        while (true) {
            if (stages[index].compareAndSet(2, 2)) {
