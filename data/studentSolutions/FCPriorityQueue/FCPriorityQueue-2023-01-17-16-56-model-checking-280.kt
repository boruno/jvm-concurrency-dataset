...
class FCPriorityQueue<E : Comparable<E>> {
    ...
    fun poll(): E? {
        ...
        val index = bookFC(Operation(null, OperationType.POLL))

       return checkProcessed(index) ?: pollLocked()
    }
    ...
    fun peek(): E? {
        ...
        return checkProcessed(bookFC(Operation(null, OperationType.PEEK))) ?: peekLocked()
    }
    ...
    fun add(element: E) {
        ...
        if (checkProcessed(bookFC(Operation(element, OperationType.ADD))) == null)
            addLocked(element)
    }
    ...
    private fun help() {
        for (i in (0..FC_ARRAY_SIZE)) {
            ...
            if (fci.type == OperationType.POLL) {
                ...
                q.add(fci.value)
                fcArray[i].set(Operation(null, OperationType.RESULT))
            }
        }
    }
    ...
    private fun checkProcessed(index: Int): E? {
        while (!tryLock()) {
