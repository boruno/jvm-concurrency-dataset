import kotlinx.atomicfu.atomic
import kotlinx.atomicfu.atomicArrayOfNulls
import java.util.*
import java.util.concurrent.ConcurrentLinkedQueue
import kotlin.collections.ArrayDeque

class FCPriorityQueue<E : Comparable<E>> {
    private val line = atomicArrayOfNulls<Request<E>>(100000)
    private val q = PriorityQueue<E>()
    private val lock = Lock()

    /**
     * Retrieves the element with the highest priority
     * and returns it as the result of this function;
     * returns `null` if the queue is empty.
     */
    fun poll(): E? {
        val req = Request<E>(ReqType.Poll)
        val index = postReqAtFreeIndexGetIndex(req)
        while (true) {
            if (lock.tryLock()) {
                help()
                lock.unlock()
                return q.poll()
            } else if (line[index].value != req) { // if request was processed
                val found: E? = line[index].value?.value
                line[index].compareAndSet(Request(ReqType.Response, found), null)
                return found
            }
        }
    }

    /**
     * Returns the element with the highest priority
     * or `null` if the queue is empty.
     */
    fun peek(): E? {
        val req = Request<E>(ReqType.Peek)
        val index = postReqAtFreeIndexGetIndex(req)
        while (true) {
            if (lock.tryLock()) {
                help()
                lock.unlock()
                return q.peek()
            } else if (line[index].value != req) { // if request was processed
                val found: E? = line[index].value?.value
                line[index].compareAndSet(Request(ReqType.Response, found), null)
                return found
            }
        }
    }

    /**
     * Adds the specified element to the queue.
     */
    fun add(element: E) {
        val req = Request(ReqType.Add, element)
        val index = postReqAtFreeIndexGetIndex(req)
        while (true) {
            if (lock.tryLock()) {
                help()
                lock.unlock()
                q.add(element)
                return
            } else if (line[index].value != req) { // if request was processed
                line[index].compareAndSet(Request(ReqType.Response), null)
                return
            }
        }
    }

    private fun postReqAtFreeIndexGetIndex(req: Request<E>): Int {
        var index = 0
        while (!line[index].compareAndSet(null, req)) {
            index++;
        }
        return index
    }

    private fun help() {
        for (i in 0 until line.size) {
            val req = line[i].value
            if (req != null) {
                var res: E?
                when (req.op) {
                    ReqType.Add -> {
                        res = null
                        q.add(req.value)
                    }

                    ReqType.Poll -> {
                        res = q.poll()
                    }

                    ReqType.Peek -> {
                        res = q.peek()
                    }

                    else -> {
                        res = null
                    }
                }
                line[i].value = Request(ReqType.Response, res)
            }
        }
    }
}

class Lock {
    private val locked = atomic(false)

    fun tryLock(): Boolean {
        return locked.compareAndSet(expect = false, update = true)
    }

    fun unlock() {
        locked.value = false
    }
}

class Request<E>(val op: ReqType, val value: E? = null) {

}

enum class ReqType {
    Poll, Peek, Add, Response
}