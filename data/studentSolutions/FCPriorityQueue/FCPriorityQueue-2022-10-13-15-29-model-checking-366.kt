...
class FCPriorityQueue<E : Comparable<E>> {
    private val line = atomicArrayOfNulls<Request<E>>(50)
    private val q = atomic(LinkedList<E>())
    ...
    fun poll(): E? {
        ...
        while (true) {
            if (lock.tryLock()) {
                ...
                lock.unlock()
                return ans
            } else if (index != -1) { // if request was made
    ...
    fun peek(): E? {
        ...
        while (true) {
            if (lock.tryLock()) {
                ...
                lock.unlock()
                return ans
            } else if (index != -1) { // if request was processed
                if (line[index].value!!.op == ReqType.Response) {
                    val found: E? = line[index].value?.value
                    line[index].value = null
                    return found
    ...
    private fun help(msg: String = "") {
        for (i in 0 until line.size) {
            ...
            if (req != null) {
                var res: E?
                System.out.println("Help op " + req.op + " " + msg)
                when (req.op) {
                    ReqType.Add -> {
                        res = null
