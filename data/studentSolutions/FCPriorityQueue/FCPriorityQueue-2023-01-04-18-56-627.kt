...
class FCPriorityQueue<E : Comparable<E>> {
    ...
    private val fcArray: AtomicArray<Operation<E>?> = atomicArrayOfNulls(10)
    ...
    fun poll(): E? {
        ...
        while (true) {
            if (lock.compareAndSet(false, true)) {
                ...
                if (ourOp!!.status == opStatus.DONE) {
                    println("set for $arrayNum in else")
                    fcArray[arrayNum].compareAndSet(ourOp, null)
                    return ourOp.result
                }
            } else {
        ...
        for (ind in 0..(fcArray.size - 1)) {
            ...
            if (op != null) {
                ...
                if (op.op == opType.ADD) {
                    ...
                    if (!fcArray[ind].compareAndSet(op, answ)) {
                        throw Exception("add wtf, ind: $ind new val: ${fcArray[ind].value}")
                    }
                    ...
                    answ.result = res
                    if (!fcArray[ind].compareAndSet(op, answ)) {
                        throw Exception("peek wtf, ind: $ind new val: ${fcArray[ind].value}")
                    }
                } else if (op.op == opType.POLL) {
                    val res = q.poll()
                    answ.status = opStatus.DONE
                    answ.result = res
//                    println("set answ for $ind with res $res")
                    if (!fcArray[ind].compareAndSet(op, answ)) {
                        throw Exception("poll wtf, ind: $ind new val: ${fcArray[ind].value}")
                    }
                }
            }
        }
