    ...
    private fun evaluate(type: Op, argument: E?): E? {
        ...
        while (true) {
            if (tryLock()) {
                ...
                for (i in 0 until workers) {
                    ...
                    if (op != null) {
                        when (op.type) {
                            ...
                            Op.ADD -> {
                                q.add(op.attr)
                                fcArray[i].compareAndSet(op, Operation(Op.COMPLETED, null))
                            }
                            Op.PEEK -> fcArray[i].compareAndSet(op, Operation(Op.COMPLETED, q.peek()))
                            Op.POLL -> fcArray[i].compareAndSet(op, Operation(Op.COMPLETED, q.poll()))
                        }
                    }
                }
                unlock()
                ...
                if (fcArray[idx].compareAndSet(null, Operation(type, argument))) {
                    put = true
                }
