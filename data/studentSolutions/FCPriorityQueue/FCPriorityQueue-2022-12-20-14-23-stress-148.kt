...
class FCPriorityQueue<E : Comparable<E>> {
    ...
    fun add(element: E) {
        while (true) {
            if (lock.compareAndSet(expect = false, update = true)) {
                q.add(element) // тут точно добавляется, если poll null, то не возвращает poll
                help()
                lock.compareAndSet(expect = true, update = false)
                return
            } else {
    ...
    private fun help() {
        for (i in 0 until ARRAY_SIZE) {
            ...
            when(cur.type.value) {
                NodeType.ADD -> {
                    if (cur.type.compareAndSet(NodeType.ADD, NodeType.RESULT_PENDING)) {
                        q.add(cur.value) // тут может не сработать если нет в массиве или не тот тип ноды
                        println("add in help")
