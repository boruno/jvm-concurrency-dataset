...
class FCPriorityQueue<E : Comparable<E>> {
     ...
     */
    fun poll(): E? {
        val node = addToTail(Node<E>(Operation.POLL, null))
...
//                    println("[${Thread.currentThread().id}] poll $res")
     ...
     */
    fun peek(): E? {
        val node = addToTail(Node<E>(Operation.PEEK, null))
     ...
     */
    fun add(element: E) {
        val node = addToTail(Node(Operation.ADD, element))

        while (true) {
            if (lock.compareAndSet(expect = false, update = true)) {
                if (!node.finished) {
                    q.add(element)
//                    println("[${Thread.currentThread().id}] add lock $element")
                    node.finished = true
                }
                help()
                lock.compareAndSet(expect = true, update = false)
    ...
    private fun addToTail(node: Node<E>) : Node<E>{
        while (true) {
    ...
    private fun help() {
        while (true) {
            ...
            when (cur.type) {
                Operation.ADD -> {
                    q.add(cur.value)
//                    println("[${Thread.currentThread().id}] add help ${cur.value}")
                    cur.finished = true
                }
                Operation.PEEK -> {
                    cur.value = q.peek()
