package day1

import kotlinx.atomicfu.*
import java.util.EmptyStackException

class MSQueue<E> : Queue<E> {
    private val head: AtomicRef<Node<E>>
    private val tail: AtomicRef<Node<E>>

    init {
        val dummy = Node<E>(null)
        head = atomic(dummy)
        tail = atomic(dummy)
    }

    override fun enqueue(element: E) {
        while (true) {
            val node = Node<E>(element)
            val curTail = tail
            if(curTail.value.next.compareAndSet(null, node)) {
                if (tail.compareAndSet(curTail.value, node)) break
            } else {
                tail.value.next.value?.let { tail.compareAndSet(tail.value, it) }
            }
        }
        TODO("implement me")
    }

    override fun dequeue(): E? {
        while (true) {
            val headVal = head.value
            val tailVal = tail.value
            val curHead = head
            val curHeadNext = head.value.next
            if (headVal == tailVal) {
                if (tailVal.next.value == null) {
                    return null
                } else {
                   if (headVal.next.value?.let { head.compareAndSet(headVal, it) } == true){
                       return headVal.next.value!!.element
                   }
                }
            }
        }
    }

    private class Node<E>(
        var element: E?
    ) {
        val next = atomic<Node<E>?>(null)
    }
}
