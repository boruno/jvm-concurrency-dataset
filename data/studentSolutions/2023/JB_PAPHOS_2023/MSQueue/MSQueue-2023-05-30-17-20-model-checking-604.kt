...
class MSQueue<E> : Queue<E> {
    ...
    override fun enqueue(element: E) {
        while (true) {
            ...
            if (oldTail.next.compareAndSet(null, newTail)) {
                ...
                println("enqueue headEl " + head.value.element)
                println("enqueue headNextEl " + head.value.next.value?.element)
                return
            } else {
    ...
    override fun dequeue(): E? {
        println("dequeue headEl " + head.value.element)
        println("dequeue headNextEl " + head.value.next.value?.element)
        while (true) {
