...
class IntIntHashMap {
    ...
    private fun putAndRehashWhileNeeded(key: Int, value: Int): Int {
        while (true) {
            ...
            /*if (core.value.map.size == 8) {
                var state = ""
                (0..3).forEach { i -> state = state + curCore.map[i].value + ", "}
                state += "\n"
                (0..7).forEach { i -> state = state + core.value.map[i].value + ", "}
    ...
    private class Core(capacity: Int) {
                ...
                when {
                    oldValue == TRANSFERRED_VALUE -> {
                        ...
                        while (true) {
                            ...
                            val result = nextCore.putInternal(key, value, isRehash)
                    ...
                    oldValue < 0 -> {
                        ...
                        val nextCore = next.value!!
                        while (true) {
                            val result = nextCore.putInternal(curKey, -oldValue, isRehash)
                        ...
                        map[index+1].compareAndSet(oldValue, TRANSFERRED_VALUE)
                        continue
                    }

                    else -> {
                        if (isRehash && oldValue != NULL_VALUE) return NULL_VALUE
        ...
        fun rehash(): Core {
            ...
            while (index < map.size) {
                ...
                //throw Exception("Why index not increasing?")
                index += 2
            }
            return next.value!!
        }
