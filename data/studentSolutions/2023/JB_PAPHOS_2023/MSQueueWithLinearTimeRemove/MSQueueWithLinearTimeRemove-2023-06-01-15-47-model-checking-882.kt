    ...
    override fun dequeue(): E? {
        ...
        while (true) {
            ...
            if (curHead.value == curTail.value) {
                if (curTail.value.next.value == null) return null
                else curTail.value.next.value?.let {
                    tail.compareAndSet(curTail.value, it)
                }
                continue
            }

            nextVal?.let {
                if (head.compareAndSet(curHead.value, it) && curHead.value.markExtractedOrRemoved()) {
...
//            while (true) {
            ...
            while (floating != tail.value || floating.next.value == null) {
                ...
                if (next.value == state) return floating
                if (next.value != null) {
                    floating = next.value!!
                } else return null
            }
