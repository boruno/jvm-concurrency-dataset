...
class MSQueueWithLinearTimeRemove<E> : QueueWithRemove<E> {
    ...
    override fun enqueue(element: E) {
        ...
        while (true) {
            ...
            if (last == tail.value) {
                if (next == null) {
                    if (tail.value.extractedOrRemoved) {
                        tail.value.remove()
                    }
        ...
        while (true) {
            ...
            if (start == head.value) {
                if (start == last) {
                    ...
                    if (next != null) {
                        if (head.compareAndSet(start, next) && next.markExtractedOrRemoved()) {
                            return next.element
                        }
                    }
                }
...
//                }
    ...
    private inner class Node(
        ...
        private fun findPrev(): Node {
            ...
            var prev: Node? = null
            while (true) {
                if (node.next.value == this && !node.extractedOrRemoved) {
                    break
                } else {
                    prev = node
                    node = node.next.value ?: return prev
                }
            }
            return node
        }
