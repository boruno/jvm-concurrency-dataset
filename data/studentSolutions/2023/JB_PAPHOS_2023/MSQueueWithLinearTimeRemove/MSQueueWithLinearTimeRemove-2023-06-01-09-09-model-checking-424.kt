...
class MSQueueWithLinearTimeRemove<E> : QueueWithRemove<E> {
    ...
    override fun dequeue(): E? {
        ...
        while (true) {
            ...
            if (head.compareAndSet(oldHead, newHead) && newHead.markExtractedOrRemoved()) {
                //println("dequeue - ${newHead.element}")
    ...
    private inner class Node(
        ...
        fun remove(): Boolean {
            ...
            //println("remove of ${this.element}")
            ...
            if (this == head.value || this == tail.value) return res
            ...
            var foundPreviousNode: Node? = head.value // dummy node
            while (foundPreviousNode!!.next.value != this) {
                foundPreviousNode = foundPreviousNode.next.value
                if (foundPreviousNode == null) return res // already deleted by someone
            }

            val nextNode: Node? = this.next.value

            if (foundPreviousNode.next.compareAndSet(this, nextNode)) {
                if (foundPreviousNode.extractedOrRemoved) foundPreviousNode.remove()
