...
class MSQueueWithLinearTimeRemove<E> : QueueWithRemove<E> {
    ...
    override fun dequeue(): E? {
        while (true) {
            ...
            if (head.compareAndSet(currentHead, currentNext) && currentNext.markExtractedOrRemoved()) {
        ...
        while (true) {
            if (node !== head.value && node !== tail.value) {
                check(!node.extractedOrRemoved) {
                    println("Removed node with element ${node.element} found in the middle of this queue: $this")
    ...
    private inner class Node(
        ...
        fun remove(): Boolean {
            ...
            val nextHead = next.value ?: this
            if (head.compareAndSet(this, nextHead)) return result
            val previous = findPrev()
            val next = this.next.value
            if (next != null && previous != null) {
                previous.next.value = next
            }
            if (previous?.extractedOrRemoved == true) previous.remove()
