...
class MSQueueWithLinearTimeRemove<E> : QueueWithRemove<E> {
    ...
    override fun enqueue(element: E) {
        ...
        val toEnqueue = Node(element)

        while (true) {
            if (tail.value.next.compareAndSet(null, toEnqueue)) {
                tail.compareAndSet(tail.value, toEnqueue)
                checkAndRemove(tail.value)
                return
            } else {
                val next = tail.value.next.value ?: continue
                tail.compareAndSet(tail.value, next)
                checkAndRemove(tail.value)
            }
        }
    }
    ...
    override fun dequeue(): E? {
        ...
        while (true) {
            ...
            if (curHeadNext.markExtractedOrRemoved() && head.compareAndSet(head.value, curHeadNext)) {
    ...
    private fun checkAndRemove(toCheck: Node) {
    ...
    private inner class Node(
        ...
        fun deleteNode() {
            if (this == tail.value || this == head.value || this.next.value == null) { return }
            fun findPrevious(): Node? {
                var prev = head.value
                while (prev.next.value !== this) {
                    requireNotNull(prev.next.value) { return null}
                    prev = prev.next.value ?: return null
                }
                return prev
            }
            val previousNode = findPrevious() ?: return
            val nextNode = next.value
            previousNode.next.getAndSet(nextNode)
            if (previousNode.extractedOrRemoved) { previousNode.deleteNode() }
            if (nextNode != null && nextNode.extractedOrRemoved) { nextNode.deleteNode() }
        }
    }
}
