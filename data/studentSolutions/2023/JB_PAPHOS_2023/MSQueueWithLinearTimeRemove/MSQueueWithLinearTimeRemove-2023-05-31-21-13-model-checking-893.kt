...
class MSQueueWithLinearTimeRemove<E> : QueueWithRemove<E> {
    ...
    override fun enqueue(element: E) {
        ...
        if (curTail.value.next.compareAndSet(null, newNode)){
            ...
            val nextTail = curTail.value.next.value;
    ...
    override fun dequeue(): E? {
        while(true){
            ...
            if(head.compareAndSet(curHead.value,curHeadNext.value!!) &&
                   curHeadNext.value!!.markExtractedOrRemoved() )
                {
    ...
    private inner class Node(
        ...
        fun findPrev(head: Node, goal: Node): Node
        {
            var cursor = head;
            while(true){
                if(cursor.next.value == goal)
                    return cursor;
        ...
        fun remove(): Boolean {
            ...
            if (markExtractedOrRemoved()){
                ...
                var curPrev = findPrev(head.value, this)
                curPrev.next.value = curNext.value;
                if(curPrev.extractedOrRemoved) curPrev.remove()
                if(curNext.value!!.extractedOrRemoved) curNext.value!!.remove()
            }
            else return false;
