...
class MSQueueWithLinearTimeRemove<E> : QueueWithRemove<E> {
    ...
    override fun enqueue(element: E) {
        ...
        while (true) {
            ...
            // Check whether the previous tail is logically removed
            if (curTail.extractedOrRemoved) {
                // Try to physically remove the previous tail
                val prevTail = curTail.next.value
                if (prevTail != null && tail.compareAndSet(curTail, prevTail)) {
                    // Successfully physically removed
                    continue
                }
            }
    ...
    override fun dequeue(): E? {
        ...
        while (true) {
            ...
            val tmp = curHead.next.value ?: return null
            if (tmp.extractedOrRemoved) continue
            if (tmp.markExtractedOrRemoved() && head.compareAndSet(curHead, tmp)) {
                return tmp.element
            }
        }
    }
    ...
    private inner class Node(
        ...
        fun remove(): Boolean {
            ...
            var prev = head.value
            while (true) {
                val next = prev.next.value ?: break

                // If the node to be removed is found,
                // update the `next` field of the previous node to `this.next.value`.
                if (next == this) {
            ...
            // The tail node is removed physically by `enqueue(..)`.
            if (this == tail.value && extractedOrRemoved) {
                tail.compareAndSet(this, prev)
            }
