    ...
    private inner class IncrementDescriptor(
        ...
        fun applyOperation() {
            ...
            while (true){
                ...
                } else if (val2 == valueBeforeIncrement2) {
                    if (array[index2].compareAndSet(val2, this)) {
                        if (val1 == valueBeforeIncrement1) { // <-- marking this place
                            status.compareAndSet(UNDECIDED, SUCCESS)
                            break
                        }
                        else {
                        ...
                        // someone put desc or increment value. Try again? Create new desc?
                        continue
                    }
                } else {
                    // maybe another desc or incremented value. Create new desc?
