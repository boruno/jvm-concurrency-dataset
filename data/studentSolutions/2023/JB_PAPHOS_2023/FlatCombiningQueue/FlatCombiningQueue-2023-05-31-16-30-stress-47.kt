            ...
            if (combinerLock.compareAndSet(expect = false, update = true)) {
                try {
                    ...
                    if (randomIndex == -1 || tasksForCombiner[randomIndex].value is ToEnqueue<*>) {
                        if (tasksForCombiner[randomIndex].value is ToEnqueue<*> &&
                            (tasksForCombiner[randomIndex].value as ToEnqueue<*>).value != element) {
                            println((tasksForCombiner[randomIndex].value as ToEnqueue<*>).value)
                            println(element)
                        }
                        queue.addLast(element)
                    }
                    else {
                        if (tasksForCombiner[randomIndex].value !== PROCESSED) {
                            println(tasksForCombiner[randomIndex].value)
                        }
                    }
                    if (randomIndex != -1) {
