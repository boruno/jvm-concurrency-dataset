...
class FlatCombiningQueue<E> : Queue<E> {
    ...
    override fun enqueue(element: E) {
        ...
        while (true) {
            if (combinerLock.compareAndSet(expect = false, update = true)) {
                ...
                if (stateCell.compareAndSet(null, element)) {
                    if (stateCell.compareAndSet(element, null)) continue
    ...
    private fun combine() {
        for (i in 0 until TASKS_FOR_COMBINER_SIZE) {
            ...
            val value = stateCell.value as E?
            if (stateCell.compareAndSet(DEQUE_TASK, State(queue.removeFirstOrNull()))) {
            } else if (value != null && stateCell.compareAndSet(value, PROCESSED)) {
        ...
        while (true) {
            if (combinerLock.compareAndSet(expect = false, update = true)) {
                ...
                if (stateCell.compareAndSet(null, DEQUE_TASK)) {
                    ...
                    val value = tasksForCombiner[index].value as State
                    tasksForCombiner[index].compareAndSet(value, null)
                    return value.number as E?
                }
            }
        }
    }
    ...
    private data class State(val number: Any?)
}
