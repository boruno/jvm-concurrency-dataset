...
class FlatCombiningQueue<E> : Queue<E> {
        ...
        while (true) {
            if (combinerLock.compareAndSet(false, true)) {
                ...
                if (task.compareAndSet(null, element)) {
                    while (combinerLock.value == true && task.value != PROCESSED) {}
                    if (task.compareAndSet(element, null)) continue // try to catch next lock
                    if (task.compareAndSet(PROCESSED, null)) return // processed successfully
                }
            }
        }
    }
        ...
        while (true) {
            if (combinerLock.compareAndSet(false, true)) {
                ...
                if (task.compareAndSet(null, DEQUE_TASK)) {
                    while (combinerLock.value == true && task.value != PROCESSED) {}
                    if (task.compareAndSet(DEQUE_TASK, null)) continue
                    val taskValue = task.value
                    if (taskValue == PROCESSED) {
                        val elem = resultsForCombiner[randId].value
                        resultsForCombiner[randId].compareAndSet(elem, null)
    ...
    private fun isEnqueTask(taskValue: Any?): Boolean =
        taskValue != DEQUE_TASK && taskValue != null && taskValue != PROCESSED
