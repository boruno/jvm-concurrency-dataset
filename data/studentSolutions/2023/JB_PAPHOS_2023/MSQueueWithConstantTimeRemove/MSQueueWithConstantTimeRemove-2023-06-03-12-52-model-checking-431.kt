...
class MSQueueWithConstantTimeRemove<E> : QueueWithRemove<E> {
    ...
    override fun dequeue(): E? {
        ...
        while (true) {
            ...
            if (head.compareAndSet(curHead, newHead)) {
                ...
                if (newHead.markExtractedOrRemoved()) return newHead.element
            }
        }
    }
    ...
    override fun toString(): String {
        ...
        while (true) {
            nodes += (if (head.value === node) "HEAD = " else "") +
                    ...
                    "<${node.element}" +
                    (node.prev.value?.let { " (prev = ${it.element})" } ?: "") +
    ...
    private class Node<E>(
        ...
        fun physicalRemove() {
            // do not remove tail
            // it will be removed on enqueue
            val curNext = next.value ?: return
            val curPrev = prev.value ?: return

            curPrev.next.value = curNext

            if (!curPrev.extractedOrRemoved) curNext.prev.value = curPrev
