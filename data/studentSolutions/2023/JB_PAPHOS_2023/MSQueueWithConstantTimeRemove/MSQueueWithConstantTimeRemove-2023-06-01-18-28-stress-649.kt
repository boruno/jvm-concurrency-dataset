...
//                currentHead.next.value = null
        ...
        while (true) {
            if (node !== head.value && node !== tail.value) {
                check(!node.extractedOrRemoved) {
                    "Removed node with element ${node.id}:${node.element} found in the middle of the queue"
                }
            }
            ...
            check(nodeNextPrev != null) {
                "The `prev` pointer of node with element ${node.id}:${node.element} is `null`, while the node is in the middle of the queue. " +
                "==> Head(${head.value.id}): ${head.value.dump()} " +
                "==> Tail(${tail.value.id}): ${tail.value.dump()} " +
                "==> Node: ${node.dump()} " +
                "==> Operations: ${operations.joinToString(", ")}"
            }
            check(nodeNextPrev == node) {
                "node.next.prev != node; `node` contains ${node.id}:${node.element}, `node.next` contains ${nodeNext.element}. " +
                "==> Head(${head.value.id}): ${head.value.dump()} " +
                "==> Tail(${tail.value.id}): ${tail.value.dump()} " +
                "==> Node: ${node.dump()} " +
                "==> Operations: ${operations.joinToString(", ")}"
    ...
    override fun toString(): String {
        ...
        return "HEAD(${head.value.id}): ${head.value.dump()} TAIL(${tail.value.id}): ${tail.value.dump()}"
    }
