...
class AtomicArrayWithCAS2<E : Any>(size: Int, initialValue: E) {
    ...
    fun get(index: Int): E? {
        ...
        if (value is AtomicArrayWithCAS2<*>.CAS2Descriptor.DSSDescriptor) {
            ...
            if (status == Status.UNDECIDED || status == Status.FAILED) return value.expected1 as? E
            value.apply()
    ...
    inner class CAS2Descriptor(
        ...
        fun apply() {
            while (true) {
                ...
                if (localStatus == Status.SUCCESS) {
                    tss(index1, expected1, update1)
                    tss(index2, expected2, update2)
                    return
                }

                if (localStatus == Status.FAILED) {
                    tss(index1, update1, expected1)
                    tss(index2, update2, expected2)
                    return
                }
