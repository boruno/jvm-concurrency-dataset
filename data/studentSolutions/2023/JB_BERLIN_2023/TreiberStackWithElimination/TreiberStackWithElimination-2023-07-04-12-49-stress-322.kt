...
class TreiberStackWithElimination<E> : Stack<E> {
    ...
    private fun tryPushElimination(element: E): Boolean {
        ...
        if (eliminationArray[index].compareAndSet(CELL_STATE_EMPTY, element)) {
            ...
            assert(eliminationArray[index].compareAndSet(element, CELL_STATE_EMPTY))
            return false
        } else {
            return false
        }
    ...
    private fun tryPopElimination(): E? {
        ...
        if (element != CELL_STATE_EMPTY && eliminationArray[index].compareAndSet(element, CELL_STATE_RETRIEVED)) {
            return element as E
        }
        return null
        // TODO: Choose a random cell in `eliminationArray`
