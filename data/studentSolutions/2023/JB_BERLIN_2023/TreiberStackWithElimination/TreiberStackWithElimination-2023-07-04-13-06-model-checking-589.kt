...
class TreiberStackWithElimination<E> : Stack<E> {
    ...
    private fun tryPushElimination(element: E): Boolean {
        ...
        if (eliminationArray[index].compareAndSet(CELL_STATE_EMPTY, element)) {
            for (iter in 0 until ELIMINATION_WAIT_CYCLES) {
                if (eliminationArray[index].compareAndSet(CELL_STATE_RETRIEVED, CELL_STATE_EMPTY)) {
                    return true
                }
            }
            eliminationArray[index].compareAndSet(element, CELL_STATE_EMPTY) // TODO
            return false
        } else {
            return false
        }
    ...
    private fun tryPopElimination(): E? {
        ...
        if (element != CELL_STATE_EMPTY && element != CELL_STATE_RETRIEVED && eliminationArray[index].compareAndSet(element, CELL_STATE_RETRIEVED)) {
            return element as E?
        }
        return null
        // TODO: Choose a random cell in `eliminationArray`
