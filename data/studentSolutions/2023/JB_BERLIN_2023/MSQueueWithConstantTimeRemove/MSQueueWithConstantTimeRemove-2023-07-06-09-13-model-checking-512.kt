...
class MSQueueWithConstantTimeRemove<E> : QueueWithRemove<E> {
    ...
    override fun remove(element: E): Boolean {
        ...
        while (true) {
            ...
            node = next
            if (node.element == element && node.remove(false)) return true
        }
    }
...
//        return
    ...
    private inner class Node<E>(
        ...
        fun remove(force: Boolean): Boolean {
            ...
            if (localTail.next.value == this || localHead == this || localTail == this)
                return removed            // that's tail or head

            val localNext = this.next.value ?: return removed
            val localPrev = this.prev.value ?: return removed

//            if (localNext == null || localPrev == null)
//                return removed

            localPrev.next.value = localNext
            if (localNext.extractedOrRemoved)
                localNext.remove(false)

            if (localNext.prev.compareAndSet(this, localPrev))
                if (localPrev.extractedOrRemoved)
                    localPrev.remove(false)
