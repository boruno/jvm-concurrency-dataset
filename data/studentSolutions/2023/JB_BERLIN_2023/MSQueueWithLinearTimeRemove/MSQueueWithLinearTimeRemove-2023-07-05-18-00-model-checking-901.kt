...
class MSQueueWithLinearTimeRemove<E> : QueueWithRemove<E> {
    ...
    override fun enqueue(element: E) {
        ...
        while (true) {
            ...
            if (curTail.extractedOrRemoved) {
                val prev = curTail.findPrev(head.value)!!
                prev.next.compareAndSet(curTail, newTail)
            }
            else if (curTail.next.compareAndSet(null, newTail)) {
    ...
    private inner class Node(
        ...
        fun findPrev(head: Node): Node? {
            var cur: Node? = head
            while (cur != null) {
                if (cur.next.value == this) return cur
                cur = cur.next.value
            }
            return null
        }
        ...
        fun remove(): Boolean {
            ...
            if (this != head.value && this != tail.value) {
                val curPrev = findPrev(head.value)
