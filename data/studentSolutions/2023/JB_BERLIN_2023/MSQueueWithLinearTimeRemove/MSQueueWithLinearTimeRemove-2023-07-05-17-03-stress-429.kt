...
class MSQueueWithLinearTimeRemove<E> : QueueWithRemove<E> {
    ...
    override fun enqueue(element: E) {
        ...
        while (true) {
            ...
            if (localTail.extractedOrRemoved) {
                val previous = findPrevious(localTail)
                if (previous != null) {
                    val value = Node(element)
                    if (previous.next.compareAndSet(localTail, value)) {
                        tail.compareAndSet(localTail, value)
                        return
                    } else {
                        tail.compareAndSet(localTail, previous.next.value!!)
                    }
                    continue
                }
            }

            if (localTail.next.compareAndSet(localTail, Node(element))) {
    ...
    private inner class Node(
        ...
        fun remove(): Boolean {
            ...
            if (head.value == this || tail.value == this) return removed

            val previous = findPrevious(this) ?: /*no found previous node, it was physically removed*/return removed
            previous.next.value = this.next.value

            if (previous.extractedOrRemoved)
                previous.remove()

            if (this.next.value!!.extractedOrRemoved)
