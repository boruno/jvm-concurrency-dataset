...
//            TODO("Implement me!")
        ...
        internal fun forceRemove() {
            val nextValue = next.value
            if (nextValue == null) return  //It's tail, will remove late

            var node = head.value
            while (true) {
                val nodeNext = node.next.value
                if (nodeNext == null) return //it's head or already deleted

                if (nodeNext == this) {
                    if (node.next.compareAndSet(this, nextValue)) {
