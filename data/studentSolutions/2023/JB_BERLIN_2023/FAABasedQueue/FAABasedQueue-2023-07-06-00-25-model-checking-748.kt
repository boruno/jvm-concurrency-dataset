...
class FAABasedQueue<E> : Queue<E> {
    ...
    private fun findSegment(index: Int): Segment<E> {
        ...
        while (true) {
            ...
            if (current.index == index) {
                var poisonedCount: Int = 0
                for (i in 0 until size) {
                    if (current.array[i].value == POISONED) {
                        poisonedCount++
                    }
                }

                val currentNext = current.next.value
                if (currentNext != null && poisonedCount == size) {
                    head.compareAndSet(current, currentNext)
                }
            }
    ...
    override fun dequeue(): E? {
        while (true) {
            ...
            val value = segment.array[curDeqId % size].value
            segment.array[curDeqId % size].value = POISONED
            return value as E
        }
    }
