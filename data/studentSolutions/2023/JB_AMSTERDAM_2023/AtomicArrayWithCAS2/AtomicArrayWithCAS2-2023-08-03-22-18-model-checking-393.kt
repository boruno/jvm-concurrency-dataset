...
class AtomicArrayWithCAS2<E : Any>(size: Int, initialValue: E) {
    ...
    inner class DCSSDescriptor(
            ...
            while (true) {
                ...
                when {
                    ...
                    value1 == expected1 && cas2Status == expectedStatus -> {
                        // todo here cas2Desc.status can be already SUCCESS and accidentally compareAndSet may succeed
                        // so we repeat the operations, ABA problem?
                        if (array[index1].compareAndSet(expected1, this)) {
                            // installed successfully
                            return true
                        }
        ...
        private fun updateStatus(installed: Boolean) {
            if (installed && cas2Desc.status.value == UNDECIDED) {
                status.compareAndSet(UNDECIDED, SUCCESS)
            } else {
                status.compareAndSet(UNDECIDED, FAILED)
            }
        ...
        private fun updateCell() {
            if (status.value == SUCCESS /*&& cas2Desc.status.value == UNDECIDED*/) {
