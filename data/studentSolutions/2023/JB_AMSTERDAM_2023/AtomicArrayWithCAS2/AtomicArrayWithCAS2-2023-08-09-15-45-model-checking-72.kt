...
class AtomicArrayWithCAS2<E : Any>(size: Int, initialValue: E) {
    ...
    fun get(index: Int): E {
        ...
        if (cellState is AtomicArrayWithCAS2<*>.CAS2Descriptor) {
            return if (cellState.status.get() == Status.SUCCESS) {
    ...
    fun dcss(
        ...
        val descriptor = DCSSDescriptor(
            index = index, expected = expectedCellState, update = updateCellState,
    ...
    inner class DCSSDescriptor(
        ...
        fun apply() {
            val success = install() && statusReference.get() == expectedStatus
            updateStatus(success)
            updatePhysically()
        }
