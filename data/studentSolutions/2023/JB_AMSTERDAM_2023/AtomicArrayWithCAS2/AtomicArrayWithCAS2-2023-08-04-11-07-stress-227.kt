...
//            val (e1, e2) = if (cix1 == index1) expected1 to expected2 else expected2 to expected1
            ...
            while (!dcss(index1, expected1, this, this)) {
                val curState = array[index1].value
                if (curState is AtomicArrayWithCAS2<*>.CAS2Descriptor) {
                    if (curState == this) {
                        break
                ...
                } else if (curState != expected1 || status.value != Status.UNDECIDED) {
                    status.compareAndSet(Status.UNDECIDED, Status.FAILED)
                    applyValues()
                    return
                }
            }
            while (!dcss(index2, expected2, this, this)) {
                ...
                } else if (curState != expected2 || status.value != Status.UNDECIDED) {
                    status.compareAndSet(Status.UNDECIDED, Status.FAILED)
                    applyValues()
                    return
                }
