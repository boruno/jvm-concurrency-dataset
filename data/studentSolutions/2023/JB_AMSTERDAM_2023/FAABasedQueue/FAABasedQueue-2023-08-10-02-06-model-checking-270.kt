...
class FAABasedQueue<E> : Queue<E> {
    ...
    private val settledEnqIdx = AtomicLong(-1)
...
//            return infiniteArray.getAndSet(i, null) as? E
            ...
            val (shouldDequeue, lastSettledIndex) = shouldDequeue()
            if (!shouldDequeue) return null
            ...
            if (itemIndex < lastSettledIndex && segment.cells.compareAndSet(itemIndex, null, POISONED)) {
                continue
            }
    ...
    private fun shouldDequeue(): Pair<Boolean, Long> {
        while (true) {
            val enqueueIdx = settledEnqIdx.get()
            val dequeueIdx = deqIdx.get()
            if (settledEnqIdx.get() != enqueueIdx) { continue }
            return if (dequeueIdx <= enqueueIdx) (true to enqueueIdx) else (false to -1) // todo: and enqueue idx set
        }
    }
}
