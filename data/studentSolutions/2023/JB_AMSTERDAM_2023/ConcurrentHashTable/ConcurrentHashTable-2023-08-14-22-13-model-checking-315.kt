...
class ConcurrentHashTable<K : Any, V : Any>(initialCapacity: Int) : HashTable<K, V> {
    ...
    override fun put(key: K, value: V): V? = put(key, value, false)
    fun put(key: K, value: V, onlyNotInitialized: Boolean): V? {
        while (true) {
    ...
    private fun moveFixedValue(
        ...
        val value = curTable.values[i]
        if (value is Fixed<*>) {
            put(curTable.keys[i] as K, value.value as V, onlyNotInitialized = true)
                    ...
                    key -> {
                        ...
                        return when {
                            ...
                            oldValue === Moved || oldValue is Fixed<*> -> {
                                ...
                                this@ConcurrentHashTable.table.get()!!.put(key, value, onlyNotInitialized)
                            }
                ...
                when (curKey) {
                    ...
                    key -> {
                        if (curValue === Moved || curValue is Fixed<*>) {
                            applyStartedResize(this)
                            return this@ConcurrentHashTable.table.get()!!.get(key)
                        }
                ...
                when (curKey) {
                    ...
                    key -> {
                        ...
                        if (oldValue === Moved || oldValue is Fixed<*>) {
                            applyStartedResize(this)
                            return this@ConcurrentHashTable.table.get()!!.remove(key)
                        }
