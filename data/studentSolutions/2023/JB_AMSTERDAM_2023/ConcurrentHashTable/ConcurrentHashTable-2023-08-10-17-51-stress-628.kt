...
class ConcurrentHashTable<K : Any, V : Any>(initialCapacity: Int) : HashTable<K, V> {
    ...
    class Table<K : Any, V : Any>(val capacity: Int) {
        ...
        object MovedCell
        data class MovedValue(val value: Any?)
        ...
        fun isMoved(index: Int): Boolean {
            ...
            return values[index] is MovedCell
        }
            ...
            while (true) {
                when (val prevValue = values[index]) {
                    null -> {
                        ...
                        if (values.compareAndSet(index, null, MovedCell)) {
                            return true
                        }
                    }
                    MovedCell -> {
                    ...
                    is MovedValue -> {
                        // Claim new slot.
                        val newIndex = claimNewSlot(keys[index] as K)
                        // Copy the value (only if it wasn't already changed).
                        if (nextTable.get().values.compareAndSet(newIndex, null, prevValue.value)) {
                            // Mark as moved.
                            if (values.compareAndSet(index, prevValue, MovedCell)) {
                                return true
                            }
                        }
                        ...
                        val movedValue = MovedValue(prevValue)
                ...
                when (curKey) {
                    ...
                    key -> {
                        if (nextTable != null) {
                            // Move the value.
                            move(index)
                            // Go to 'MovedValue' branch if it was successfully moved.
                        }
                        else {
            ...
            while (true) {
                ...
                when (curKey) {
                    ...
                    key -> {
                        ...
                        if (value is MovedValue) return value.value as V?
                        if (value is MovedCell) return nextTable.get().get(key)
            ...
            while(true) {
                ...
                when (curKey) {
                    ...
                    key -> {
                        ...
                        if (oldValue is MovedCell) return nextTable.get().remove(key)
                        if (values.compareAndSet(index, oldValue, null)) return oldValue as V?
                    }
                    // Empty cell.
                    null -> {
                        // The key has not been found.
