...
//    private val table = AtomicReference(Table<K, V>(initialCapacity))
        private val table = AtomicReference(ConcurrentHashMap<K, V>())
            ...
            repeat(MAX_PROBES) {
                ...
                when (curKey) {
                    ...
                    key -> {
                        ...
                        // values[index] = value
                        return if (!values.compareAndSet(index, oldValue, value)) put(key, value) else oldValue
                    }
                    ...
                    null -> {
                        ...
                        return if (!values.compareAndSet(index, null, value)) put(key, value) else null
                    }
                }
            ...
            repeat(MAX_PROBES) {
                ...
                when (curKey) {
                    ...
                    key -> {
                        ...
                        val oldValue = values[index]
                        // values[index] = null
                        return if (!values.compareAndSet(index, oldValue, null)) remove(key) else oldValue
                    }
                    // Empty cell.
                    null -> {
                        // The key has not been found.
