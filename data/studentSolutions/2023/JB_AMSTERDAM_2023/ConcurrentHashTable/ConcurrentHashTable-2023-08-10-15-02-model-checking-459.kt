...
class ConcurrentHashTable<K : Any, V : Any>(initialCapacity: Int) : HashTable<K, V> {
    ...
    class Table<K : Any, V : Any>(val capacity: Int) {
                ...
                while (true) {
                    ...
                    when (curKey) {
                        ...
                        key -> {
                            ...
                            return exchangeValueAtIndex(index, value)
                        }
            ...
            repeat(MAX_PROBES) {
                ...
                when (curKey) {
                    ...
                    key -> {
                        ...
                        return exchangeValueAtIndex(index, null)
                    }
                    // Empty cell.
                    null -> {
                        // The key has not been found.
        ...
        private fun exchangeValueAtIndex(
            index: Int,
            newV: V?
        ): V? {
            while (true) {
                val oldValue = values[index]
                if (values.compareAndSet(index, oldValue, newV)) return oldValue
            }
        }
