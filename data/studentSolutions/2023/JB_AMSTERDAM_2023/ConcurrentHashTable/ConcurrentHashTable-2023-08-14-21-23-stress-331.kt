...
class ConcurrentHashTable<K : Any, V : Any>(initialCapacity: Int) : HashTable<K, V> {
    ...
    private fun resize(curTable: Table<K, V>) {
        val myNewTable = Table<K, V>(capacity = curTable.capacity * 2)
        curTable.next.compareAndSet(null, myNewTable)
        val newTable: Table<K, V> = curTable.next.get()!!
    ...
    companion object {
        private fun <K: Any, V: Any> moveValue(
            curTable: Table<K, V>,
            newTable: Table<K, V>,
            i: Int,
        ) {
        ...
        private fun <K: Any, V: Any> moveValueIfFixed(
            curTable: Table<K, V>,
            newTable: Table<K, V>,
            i: Int,
        ) {
            val value = curTable.values[i]
            if (value is Fixed<*>) {
                newTable.put(curTable.keys[i] as K, value.value as V, onlyNotInitialized = true)
                ...
                when (curKey) {
                    ...
                    key -> {
                        ...
                        return when {
                            ...
                            values.compareAndSet(index, oldValue.takeUnless { onlyNotInitialized }, value) -> oldValue.convertToUserValue()
                    ...
                    null -> {
                        ...
                        return when {
                            ...
                            !values.compareAndSet(index, oldValue.takeUnless { onlyNotInitialized }, value) -> put(key, value, onlyNotInitialized)
