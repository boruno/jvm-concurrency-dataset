...
class MSQueueWithLinearTimeNonParallelRemove<E> : QueueWithRemove<E> {
        ...
        while (true) {
            ...
            if (curTail.next.compareAndSet(null, node)) {
                ...
                if (curTail.extractedOrRemoved)
                {
                    ...
                    while (prevNode.next.value != curTail)
                    {
                        prevNode = prevNode.next.value!!
                    }
                    prevNode.next.compareAndSet(curTail, node);
                }
                return
            } else {
    ...
    override fun dequeue(): E? {
        ...
        while (true) {
            ...
            var curHeadNext = curHead.next.value
    ...
    private inner class Node(
        ...
        fun remove(): Boolean {
            ...
            //TODO("Implement me!")
            var result =  markExtractedOrRemoved()
            if (result && this != tail.value)
            {
                var prevNode = head.value
                while (prevNode.next.value != this.element)
                {
                    prevNode = prevNode.next.value!!
                }
