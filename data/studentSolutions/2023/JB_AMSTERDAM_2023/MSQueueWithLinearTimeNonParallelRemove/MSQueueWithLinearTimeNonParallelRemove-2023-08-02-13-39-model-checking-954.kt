    ...
    override fun remove(element: E): Boolean {
        ...
        while (true) {
            ...
            if (node.element == element && node.remove()) {
				val prev = findPrevious(node) ?: return false
				return prev.next.compareAndSet(node, node.next.value)
			}
        }
    }

	private fun findPrevious(needle: Node): Node? {
		var node = head.value
		while (true) {
			val next = node.next.value
			if (next == null) return null
			if (next == needle) return node
			node = next
		}
	}
