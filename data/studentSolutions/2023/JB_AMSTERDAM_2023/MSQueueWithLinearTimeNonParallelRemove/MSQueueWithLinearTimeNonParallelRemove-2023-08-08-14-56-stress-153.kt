...
class MSQueueWithLinearTimeNonParallelRemove<E> : QueueWithRemove<E> {
    ...
    override fun enqueue(element: E) {
        ...
        while (true) {
            ...
            val tailPrev = curTail.previous()
            if (tailPrev != null && tailPrev.markExtractedOrRemoved()) {
                val tailPrevPrev = tailPrev.previous()
                if (tailPrevPrev != null && !tailPrevPrev.next.compareAndSet(tailPrev, curTail))
                    continue
            }
    ...
    private fun Node?.previous() =
        generateSequence(head.get()) { it.next.get() }
            .firstOrNull { it.next.get() == this }
