    ...
    override fun enqueue(element: E) {
        ...
        while (true) {
            ...
            if (curTail.next.compareAndSet(null, newNode)) {
                if (tail.compareAndSet(curTail, newNode)) curTail.remove()
                return
            } else {
    ...
    private inner class Node(
        ...
        fun remove(): Boolean {
            ...
            if (extractedOrRemoved) {
                // remove physically in case not a tail
//                if (tail.get() == this || head.get() == this) return false
                val curPrev = findPrev()
                val curNext = next
                if (curPrev?.next?.compareAndSet(this, curNext.get()) == true) return true
            }
        ...
        private fun findPrev(): Node? {
            ...
            while (curNode.next.get() != this) {
                curNode = curNode.next.get() ?: return null
            }
            return curNode
        }
    }
