...
class MSQueueWithLinearTimeNonParallelRemove<E> : QueueWithRemove<E> {
        ...
        while (true) {
            ...
            if (currHead == currLast) {
                if (nextCurrHead == null) {
                    ...
                    tail.compareAndSet(currLast, nextCurrHead)
                }
            } else {
                if (head.compareAndSet(currHead, nextCurrHead)) {
                    if (!nextCurrHead!!.markExtractedOrRemoved()) {
                        continue
                    }
    ...
    private inner class Node(
        ...
        fun remove(): Boolean {
                ...
                if (this.markExtractedOrRemoved() || this.extractedOrRemoved) {
                    val prev: AtomicReference<Node?>
                    var target = head.get()
                    while (true) {
                        if (target.next.get() == this) {
                            prev = AtomicReference(target)
                            break
                        } else {
                    ...
                    if (this !== tail.get() && this !== head.get()) {
                        prev.get()!!.next.compareAndSet(this, this.next.get())
                    }
                    return true
                }
                return false
