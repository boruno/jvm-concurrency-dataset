...
class MSQueueWithLinearTimeNonParallelRemove<E> : QueueWithRemove<E> {
    ...
    override fun enqueue(element: E) {
        ...
        while (true) {
            val currentTail: Node = tail.value
            val nextTail: Node? = currentTail.next.value
    ...
    private inner class Node(
        ...
        fun removePhysically() {
            val cursor = head.value

            val next = cursor.next.value
            if (next == null) return
            ...
            prev.next.value = cursor
        }

        private fun findPrev(): MSQueueWithLinearTimeNonParallelRemove<E>.Node? {
            var cursor = head.value
            while(true)
            {
                val curNext = cursor.next.value;
                if (curNext == this) return cursor
