...
//             TODO: it will be removed physically by `enqueue(..)`.
            return if (markExtractedOrRemoved()) {
                ...
                var currentNode = prevNode.next.value
                while (true) {
                    currentNode = prevNode.next.value
                    if (prevNode.next.value?.element == this) break
                    else prevNode = prevNode.next.value?: break
                }
                prevNode.next.compareAndSet(currentNode, this.next.value)
                true
            } else false
        }
    }
