...
class MSQueueWithConstantTimeRemove<E> : QueueWithRemove<E> {
    ...
    private class Node<E>(
		...
		fun destroy() {
			...
			if (curPrev.prev.value == null) return
			// @@@: do we need this check? We never try to remove the dummy head value anyway.
			// if (curPrev == head.value) return
			curPrev.next.value = curNext
			curNext.prev.value = curPrev
			if (curNext.extractedOrRemoved) curNext.destroy()
			if (curPrev.extractedOrRemoved) curPrev.destroy()
		}
    }
}
