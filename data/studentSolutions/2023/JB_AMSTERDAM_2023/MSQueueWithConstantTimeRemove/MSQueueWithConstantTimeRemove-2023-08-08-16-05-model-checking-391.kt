...
class MSQueueWithConstantTimeRemove<E> : QueueWithRemove<E> {
    ...
    private fun findPrevious(n: Node<E>): Node<E>? {
        var curr: Node<E>? = head.get()
        while (true) {
    ...
    override fun enqueue(element: E) {
        ...
        while (true) {
            ...
            val node = Node<E>(element, null)
            if (currentTail.next.compareAndSet(null, node)) {
                node.prev.compareAndSet(null, currentTail)
                if (currentTail.extractedOrRemoved) {
                    currentTail.removedMarkedPhysically(currentTail)
                }
    ...
    private class Node<E>(
        ...
        fun removedMarkedPhysically(node: Node<E>) {
            val next = node.next.get()
            if (next == null) {
                return // we don't do anything for tails
            }
            val prev = node.prev.get() ?: return
