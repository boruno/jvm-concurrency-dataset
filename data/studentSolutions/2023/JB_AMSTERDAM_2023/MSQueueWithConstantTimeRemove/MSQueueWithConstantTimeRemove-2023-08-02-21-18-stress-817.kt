...
class MSQueueWithConstantTimeRemove<E> : QueueWithRemove<E> {
    ...
    override fun enqueue(element: E) {
        while (true) {
            ...
            val n = if (currentTail.extractedOrRemoved) {
                Node(element, null)
            } else {
                Node(element, currentTail)
            }
    ...
    override fun dequeue(): E? {
        while (true) {
            ...
            if (currentNext.prev.compareAndSet(currentHead, null)) {
                head.compareAndSet(currentHead, currentNext)
                if (currentNext.markExtractedOrRemoved()) {
                    return currentNext.element
                }
            } else {
                val actualNext = currentHead.next.value!!
                head.compareAndSet(currentHead, actualNext)
            }
        }
    }
    ...
    private class Node<E>(
        ...
        fun remove(): Boolean {
            ...
            val currentPrevious = prev.value
            // if previous is not already deleted, we remove it physically
            if (currentPrevious != null) {
//                if (previous.element == null) return removed // if head do not try to delete it

                currentNext.prev.value = currentPrevious
                currentPrevious.next.value = currentNext
