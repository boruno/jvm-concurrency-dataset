...
class MSQueueWithConstantTimeRemove<E> : QueueWithRemove<E> {
    ...
    override fun dequeue(): E? {
        ...
        while (true) {
            ...
            // It's about to be deleted, so if remove doesn't have previous as null it's fine
    ...
    private class Node<E>(
        ...
        fun tryRemovePhysically() {
            val previous = this.prev.get() // TODO: this can be non-working
            val curNext = this.next.get()
            if (previous != null && curNext != null) {
                if (previous.next.compareAndSet(this, curNext)) {
                    val newPrevious = this.prev.get()

                    curNext.prev.compareAndSet(this, newPrevious)

                    if (curNext.extractedOrRemoved) {
                        curNext.tryRemovePhysically()
                    }
                }
            }
        }
