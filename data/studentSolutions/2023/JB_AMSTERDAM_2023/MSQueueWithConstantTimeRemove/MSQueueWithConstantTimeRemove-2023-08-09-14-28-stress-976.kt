...
class MSQueueWithConstantTimeRemove<E> : QueueWithRemove<E> {
    ...
    override fun enqueue(element: E) {
        ...
        while(true) {
            ...
            val nextNode = Node(element, null)
            // add node after the current tail snapshot
            if (curTailNode.next.compareAndSet(null, nextNode)) {
                // add prev link to the current tail snapshot
                nextNode.prev.compareAndSet(null, curTailNode)
                // try to move tail the new node
                tail.compareAndSet(curTailNode, nextNode)
                // helps operation remove to remove node physically
                if (curTailNode.extractedOrRemoved) {
                    curTailNode.remove()
                }
                return
            }
            // move tail anyway even though we failed to set new node
    ...
    override fun dequeue(): E? {
        ...
        while (true) {
            ...
            // return null if there is no next node after the head
            val curHeadNext = curHead.next.get() ?: return null
            if (head.compareAndSet(curHead, curHeadNext)) {
                if (curHeadNext.prev.compareAndSet(curHead, null)) {
    ...
    private class Node<E>(
        ...
        fun remove(): Boolean {
            ...
            val curPrevNode = prev.get() ?: return marked
            val curNextNode = next.get() ?: return marked
            curPrevNode.next.set(curNextNode)
            curNextNode.prev.set(curPrevNode)
            if (curPrevNode.extractedOrRemoved) {
                curPrevNode.remove()
            }
            if (curNextNode.extractedOrRemoved) {
                curNextNode.remove()
            }
            return marked
        }
