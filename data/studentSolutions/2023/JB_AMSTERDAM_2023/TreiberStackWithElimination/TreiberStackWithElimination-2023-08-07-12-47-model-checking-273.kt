...
open class TreiberStackWithElimination<E> : Stack<E> {
    ...
    protected open fun tryPushElimination(element: E): Boolean {
        ...
        if (eliminationArray.compareAndSet(celNum, CELL_STATE_EMPTY, element)) {
            var cycle = 0
            while (cycle++ < ELIMINATION_WAIT_CYCLES + 100) {
    ...
    private fun tryPopElimination(): E? {
        val celNum = Random.nextInt(ELIMINATION_ARRAY_SIZE)
        val value = eliminationArray.get(celNum)
        if (value != CELL_STATE_EMPTY && value != CELL_STATE_RETRIEVED) {
            if (eliminationArray.compareAndSet(celNum, value, CELL_STATE_RETRIEVED)) return value as E
        }
        return null
        TODO("Implement me!")
        // TODO: Choose a random cell in `eliminationArray`
