...
class TreiberStackWithElimination<E> : Stack<E> {
  ...
  private fun tryPushElimination(element: E): Boolean {
    ...
    for (i in (0 until ELIMINATION_WAIT_CYCLES)) {
      if (cell.compareAndSet(CELL_STATE_RETRIEVED, null)) return true
    }
  ...
  private fun tryPopElimination(): E? {
    ...
    for (i in (0 until ELIMINATION_WAIT_CYCLES)) {
      val oldValue = cell.getAndUpdate { oldV ->
        if (oldV == CELL_STATE_EMPTY) {
          CELL_STATE_EMPTY
        } else {
      ...
      if (oldValue != CELL_STATE_RETRIEVED) return oldValue as? E?
    }
