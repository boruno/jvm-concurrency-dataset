...
open class TreiberStackWithElimination<E> : Stack<E> {
    ...
    protected open fun tryPushElimination(element: E): Boolean {
        ...
        val currentElement = eliminationArray[index]
        if (currentElement != CELL_STATE_RETRIEVED && currentElement != CELL_STATE_EMPTY) return false
        val addedNode = Node(element)
        if (!eliminationArray.compareAndSet(index, currentElement, addedNode)) return false
        ...
        return !eliminationArray.compareAndSet(index, addedNode, CELL_STATE_EMPTY)
    }
