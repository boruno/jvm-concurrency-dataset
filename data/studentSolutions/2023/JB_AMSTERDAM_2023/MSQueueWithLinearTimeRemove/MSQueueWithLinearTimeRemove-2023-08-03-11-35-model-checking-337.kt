...
class MSQueueWithLinearTimeRemove<E> : QueueWithRemove<E> {
    ...
    private fun physicallyRemove(target: Node, newNext: Node): Boolean {
        var node = head.value
        while(true) {
            when(val next = node.next.value) {
                target -> {
                    node.next.value = newNext
                    return true
                }
                null -> return false
                else -> node = next
    ...
    private fun setTail(expectedTail: Node, newTail: Node?) {
        if (newTail == null)
            return
        tail.compareAndSet(expectedTail, newTail)
        if (expectedTail.extractedOrRemoved)
            physicallyRemove(expectedTail, newTail)
    }
