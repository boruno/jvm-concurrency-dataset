...
class MSQueueWithLinearTimeRemove<E> : QueueWithRemove<E> {
    ...
    override fun dequeue(): E? {
        while (true) {
            ...
            val nextCurHead = curHead.next.get() ?: return null
            if (head.compareAndSet(curHead, nextCurHead)) {
                if (nextCurHead.remove()) {
                    return nextCurHead.element
                }
            }
        }
    }
    ...
    private inner class Node(
        ...
        fun physicallyRemove() {
            ...
            prev.next.set(nextNode)
            if (nextNode?.extractedOrRemoved == true) throw Exception("123")
