...
class MSQueueWithLinearTimeRemove<E> : QueueWithRemove<E> {
    ...
    override fun enqueue(element: E) {
        ...
        while (true) {
            ...
            if (currentTailValue.next.compareAndSet(null, next)) {
                ...
                next.prev.value = currentTailValue
    ...
    private inner class Node(
        ...
        fun removeP() {
            ...
            val thisNodePrev = this.prev.value
            if (thisNodeNext != null) {

                if(thisNodeNext != null) {
                    thisNodeNext.prev.value = thisNodePrev
                    if(thisNodeNext.extractedOrRemoved) {
                        thisNodeNext.removeP()
                    }
                }

                if(thisNodePrev != null) {
                    thisNodePrev.next.value = thisNodeNext
                    if(thisNodePrev.extractedOrRemoved) {
                        thisNodePrev.removeP()
                    }
                }
//
//                if(prev != null) {
...
//                }
        ...
        private fun findPrevNode(nodeToFind: Node): Node? {
            var node = head.value
            while (true) {
                val nextNode = node.next.value
                if (nextNode == null) return null
                if (nextNode == nodeToFind) {
                    return node
                }
                node = nextNode
            }
