            ...
            if (currentTailNode.next.compareAndSet(null, newTailNode)) {
                tail.compareAndSet(currentTailNode, newTailNode)
                    if (currentTailNode.extractedOrRemoved)
                        currentTailNode.remove()
//                if (tail.compareAndSet(currentTailNode, newTailNode))
                return
            } else {
                tail.compareAndSet(currentTailNode, currentTailNode.next.value!!)
//                if (tail.compareAndSet(currentTailNode, currentTailNode.next.value!!))
//                    if (currentTailNode.extractedOrRemoved)
    ...
    private inner class Node(
        ...
        fun remove(): Boolean {
            ...
            return if (markExtractedOrRemoved()) {
                ...
                if (nextNode != null) {
                    // This is not a tail. Remove it physically.
                    removePhysically()
                }
                true
            } else
        ...
        fun removePhysically() {
            ...
            while (true) {
                ...
                if (next === this) {
                    if (node.next.compareAndSet(next, next.next.value))
                        break
                }
                node = next
            }
