            ...
            repeat(MAX_PROBES) {
                ...
                when (curKey) {
                    ...
                    key -> {
                        ...
                        // values[index] = value
                        values.compareAndSet(index, oldValue, value)
                        return oldValue
                    }
                    // The cell does not store a key.
                    null, REMOVED_KEY -> {
                        // Insert the key/value pair into this cell.
                        val keyAtIndex = keys.get(index)
                        if ((keyAtIndex == null || keyAtIndex == key) && keys.compareAndSet(index, keyAtIndex, key)) {
                            values[index] = value
                            return keyAtIndex
                        }
                        if (keyAtIndex != null && keyAtIndex != key) {
