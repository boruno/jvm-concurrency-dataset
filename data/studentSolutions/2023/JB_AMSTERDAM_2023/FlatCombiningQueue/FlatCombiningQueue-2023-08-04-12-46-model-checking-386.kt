...
class FlatCombiningQueue<E> : Queue<E> {
    ...
    override fun enqueue(element: E) {
        ...
        var randomCellIndex: Int? = null
        while (true) {
            if (combinerLock.compareAndSet(false, true)) {
                if (randomCellIndex != null) {
                    val t = tasksForCombiner[randomCellIndex].getAndSet(null)
                    if (t !is Result<*>) {
        ...
        for (i in 0 until TASKS_FOR_COMBINER_SIZE) {
            ...
            when (task) {
                ...
                is Dequeue -> {
                    ...
                    tasksForCombiner[i].value = Result(removeFirstOrNull)
                }
                null -> continue
                else -> {
