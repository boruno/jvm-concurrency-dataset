...
class FlatCombiningQueue<E> : Queue<E> {
    ...
    override fun dequeue(): E? = enqueueOperation(Dequeue).value as? E

    private fun enqueueOperation(operation: Any?): Result<out Any?> {
    ...
    private fun applyOperation(operation: Any?): Result<Any?> {
    ...
    private fun helpOthers() {
        ...
        (0 until TASKS_FOR_COMBINER_SIZE)
            .map { tasksForCombiner.get(it) }
            .filter { it != null && it !is Result<*> }
            .forEachIndexed { actionIndex, action ->
                tasksForCombiner.set(actionIndex, executeOperation(action))
            }
    }

    private fun executeOperation(operation: Any?): Result<Any?> =
        when (operation) {
