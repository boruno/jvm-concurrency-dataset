...
class FlatCombiningQueue<E> : Queue<E> {
    ...
    private fun waitUntilResultOrEnqueueWithLock(index: Int, element: E) {
        while (true) {
            ...
            if (tryLock()) {
                checkResult(index) ?: enqueueWithLock(element)
                return
            }
        }
    }
    ...
    private fun waitForResultOrDequeueWithLock(index: Int): E? {
        while (true) {
            checkResult(index)?.also { result ->
                clean(index)
    ...
    private fun checkResult(index: Int) = (tasksForCombiner.get(index) as? Result<*>)
