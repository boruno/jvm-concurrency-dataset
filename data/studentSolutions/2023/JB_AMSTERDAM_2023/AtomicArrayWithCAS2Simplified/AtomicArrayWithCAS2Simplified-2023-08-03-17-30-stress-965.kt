...
*/
    ...
    inner class CAS2Descriptor(
        ...
        private fun installAtIndex(index: Int, isOneAlreadyInstalled: Boolean): Boolean {
            ...
            while (true) {
                when (val firstValue = array[index].value) {
                    expected -> {
                        if (array[index].compareAndSet(firstValue, this)) return true
                    }
                    is AtomicArrayWithCAS2Simplified<*>.CAS2Descriptor -> {
                        firstValue.receiveHelp(index)
                    }
                    else -> {
                        if (isOneAlreadyInstalled && status.value == UNDECIDED) {
                           cleanupFirst()
        ...
        private fun install(): Boolean {
           ...
           if(!installAtIndex(minIndex, false)) return false
            ...
            return installI(maxIndex, maxExpected, minIndex, minExpected)
        }

        private fun installI(maxIndex: Int, maxExpected: E, minIndex: Int, minExpected: E): Boolean {
            while (true) {
                when (val secondValue = array[maxIndex].value) {
                    maxExpected -> {
                        if (array[maxIndex].compareAndSet(secondValue, this)) return true
