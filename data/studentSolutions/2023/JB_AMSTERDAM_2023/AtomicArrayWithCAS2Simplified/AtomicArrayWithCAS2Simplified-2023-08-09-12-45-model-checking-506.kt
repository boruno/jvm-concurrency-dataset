    ...
    inner class CAS2Descriptor(
        ...
        private fun install(): Boolean {
            while (true) {
                ...
                if (array.get(pairs[1].first) == this || array.compareAndSet(pairs[1].first, pairs[1].second, this)) {
                    return if (array.get(pairs[2].first) == this || array.compareAndSet(pairs[2].first, pairs[2].second, this)) {
                        true
                    } else {
                        array.set(pairs[1].first, pairs[1].second)
                        false
                    }
                } else {
                    tryHelpForeignDescriptor(pairs[1].first)
                }
            }
        }
