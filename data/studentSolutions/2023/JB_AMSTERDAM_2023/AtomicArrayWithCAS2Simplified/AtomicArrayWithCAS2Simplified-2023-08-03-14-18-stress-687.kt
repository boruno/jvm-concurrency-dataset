...
class AtomicArrayWithCAS2Simplified<E : Any>(size: Int, initialValue: E) {
    ...
    inner class CAS2Descriptor(
        ...
        fun install(): Pair<Boolean, Boolean> {
            ...
            var idx2 = false
            while (true) {
                idx1 = when (array[index1].value) {
                    this -> true
                    is AtomicArrayWithCAS2Simplified<*>.CAS2Descriptor -> completeRunningOperation()
                    else -> if (array[index1].compareAndSet(expected1, this)) true else continue
                }
                idx2 = when (array[index2].value) {
                    this -> true
                    is AtomicArrayWithCAS2Simplified<*>.CAS2Descriptor -> completeRunningOperation()
                    else -> if (array[index2].compareAndSet(expected2, this)) true else continue
                }
