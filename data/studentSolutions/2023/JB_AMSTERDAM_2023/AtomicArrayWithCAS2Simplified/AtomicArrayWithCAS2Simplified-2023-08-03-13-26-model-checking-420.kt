    ...
    inner class CAS2Descriptor(
        ...
        private fun installDescriptor(): Boolean =
            (array[index1].compareAndSet(expected1, this) || array[index1].value == this)
                    && (array[index2].compareAndSet(expected1, this) || array[index2].value == this)
        ...
        private fun installationSuccess(): Boolean? {
            val actual1 = (array[index1].value as? AtomicArrayWithCAS2Simplified<*>.CAS2Descriptor ?: return false)
            val actual2 = array[index2].value as? AtomicArrayWithCAS2Simplified<*>.CAS2Descriptor ?: return false
            if (actual1 == this && actual2 == this)
                return true
            else {
                actual1.takeIf { it != this && it.isUndecided() }?.apply()
                actual2.takeIf { it != this && it.isUndecided() }?.apply()
                return null
            }
        }

        private fun needsHelp(i: Int): Boolean {
