    ...
    inner class CAS2Descriptor(
        ...
        fun apply() {
            ...
            if (elem1 is AtomicArrayWithCAS2Simplified<*>.CAS2Descriptor) {
                if (elem1.status.value != UNDECIDED) elem1.applyStatus()
            } else {
            ...
            if (elem2 is AtomicArrayWithCAS2Simplified<*>.CAS2Descriptor) {
                if (elem2.status.value != UNDECIDED) elem2.applyStatus()
            } else {
            ...
            if (flag) status.compareAndSet(UNDECIDED, SUCCESS)
        ...
        {
            val elem1 = array[index1].value
            if (elem1 is AtomicArrayWithCAS2Simplified<*>.CAS2Descriptor && elem1 == this)
                array[index1].compareAndSet(elem1, expected1)

            val elem2 = array[index2].value
            if (elem2 is AtomicArrayWithCAS2Simplified<*>.CAS2Descriptor && elem2 == this)
