        ...
        private fun installDescriptor() {
            while (status.value == Status.UNDECIDED) {
                ...
                if (array[index1].value == this) {
                    // All good, the descriptor is already installed to A
                } else {
                    if (array[index1].compareAndSet(expected1, this)) {
                        // All good, the descriptor is installed to A
                    } else {
                ...
                if (array[index2].value == this) {
                    // All good, the descriptor is already installed to B
                } else {
                    if (array[index2].compareAndSet(expected2, this)) {
                        // All good, the descriptor is installed to B
                    } else {
                        if (tryHelpAnotherDescriptor(index2))
                            continue
