    ...
    fun cas2(
        ...
        while (true) {
            ...
            if (cell1descriptor == null && cell2descriptor == null)
                ...
                cell1descriptor.apply()
            else
                cell2descriptor?.apply()
        }
        descriptor.apply()
...
//        }
        ...
        fun apply() {
            ...
            val smallerIdx = if (index1 < index2) index1 else index2
            val biggerIdx = if (index1 < index2) index2 else index1
            if (array[smallerIdx].compareAndSet(expected1, this) || array[smallerIdx].value == this) {
                if (array[biggerIdx].compareAndSet(expected2, this) || array[biggerIdx].value == this) {
                    if (status.compareAndSet(UNDECIDED, SUCCESS) || status.value == SUCCESS) {
                        array[smallerIdx].compareAndSet(this, update1)
                        array[biggerIdx].compareAndSet(this, update2)
                    } else {
                        return
                    }
                } else {
                    array[smallerIdx].compareAndSet(this, expected1)
                    status.compareAndSet(UNDECIDED, FAILED)
                }
            } else {
                status.compareAndSet(UNDECIDED, FAILED)
