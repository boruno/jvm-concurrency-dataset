...
class AtomicArrayWithDCSS<E : Any>(size: Int, initialValue: E) {
        ...
        while (true) {
            when (val value = array[index].value) {
                is AtomicArrayWithDCSS<*>.DCSSDescriptor -> when (value.status.value) {
                    ...
                    val result = array[index].compareAndSet(expected, update)
                    if (result) return true
                    if (array[index].value !is AtomicArrayWithDCSS<*>.DCSSDescriptor) return false
                }
            }
        }
    }
    ...
    inner class DCSSDescriptor(
        ...
        fun apply() {
            if (install() && array[index2].value == expected2 && updateStatus(Status.SUCCESS)) {
