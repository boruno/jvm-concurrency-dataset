        ...
        private fun applyLogically () {
            while (true) {
                ...
                when (val cell2 = array[index2].value) {
                    ...
                    !is AtomicArrayWithDCSS<*>.DCSSDescriptor -> {
                        // TODO: WEIRD: next line works but replacing next line with the scrutinee fails on modelChecking tests only
                        // TODO: seems to be incorrect
                        // if (status.compareAndSet(Status.UNDECIDED, Status.FAILED)) { return } // INCORRECT -- the WEIRD decision
                        status.compareAndSet(Status.UNDECIDED, Status.FAILED)
                    }
                    else -> if (index1 < cell2.index1) { tryToHelp(cell2) }
                            else {
                                // val newStatus = if (cell2.expected1 == expected2) Status.SUCCESS else Status.FAILED // INCORRECT -- the WEIRD decision
                                val newStatus = when (val cell2Status = cell2.status.value) {
                                    Status.FAILED    -> Status.FAILED
                                    Status.SUCCESS   -> if (cell2.update1   == expected2) Status.SUCCESS else Status.FAILED
                                    Status.UNDECIDED -> if (cell2.expected1 == expected2) Status.SUCCESS else Status.FAILED
                                }
