...
//            val maybeDescriptor = array[index].value as? AtomicArrayWithDCSS<*>.DCSSDescriptor
//
//
//
...
//                return true
//            else {
//                val existingDescriptor = array[index].value as? AtomicArrayWithDCSS<*>.DCSSDescriptor
    ...
    fun dcss(
        ...
        while (true) {
            if (!array[index1].compareAndSet(expected1, descriptor)) {
                val existingDescriptor = array[index1].value as? AtomicArrayWithDCSS<*>.DCSSDescriptor
                if (existingDescriptor != null)
                    existingDescriptor.proceed()
                else
                    return false
