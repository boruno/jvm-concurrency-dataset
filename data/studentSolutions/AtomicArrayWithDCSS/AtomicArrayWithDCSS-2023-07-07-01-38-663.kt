...
class AtomicArrayWithDCSS<E : Any>(size: Int, initialValue: E) {
    ...
    inner class DCSSDescriptor(
        ...
        fun apply(helpOnly: Boolean = false) {
            if (status.value === Status.UNDECIDED) {
                if (!tryToSetDiscriptor(helpOnly)) {
        ...
        private fun tryToSetDiscriptor(helpOnly: Boolean = false): Boolean {
            while (true) {
                val currentState: Any? = array[index1].value
                if (!helpOnly && array[index1].compareAndSet(expected1, this)) return true
