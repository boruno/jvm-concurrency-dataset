    ...
    fun get(index: Int): E? {
        when (val curValue = array[index].value) {
            is AtomicArrayWithDCSS<*>.DCSSDescriptor -> {
                val observationValue = array[curValue.index2].value
                return if (curValue.status.value == Status.SUCCESS) curValue.update1 as E?
                else {
                    if (observationValue == curValue.expected2)
                        curValue.expected1 as E?
                    else {
                        curValue.status.compareAndSet(Status.UNDECIDED, Status.FAILED)
                        if (curValue.status.value == Status.FAILED) curValue.expected1 as E?
                        else curValue.update1 as E?
                    }
                }
            }
        ...
        while (true) {
            when(val curValue = array[index].value) {
                ...
                expected -> {
                    ...
                    val curValueAfterCompare = array[index].value
                    if (curValueAfterCompare is AtomicArrayWithDCSS<*>.DCSSDescriptor) {
                        curValueAfterCompare.applyOperation()
                        continue
                    }
                    if (curValueAfterCompare == expected) continue
                    else return false
                }
