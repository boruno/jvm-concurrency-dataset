        ...
        while (true) {
            when(val curValue = array[index].value) {
                ...
                expected -> {
                    ...
                    val curCell = array[index].value
                    if (curCell is AtomicArrayWithDCSS<*>.DCSSDescriptor) {
                        curCell.applyOperation()
                        continue
                    }
                    if (curCell == expected) continue
                    else return false
                }
        ...
        while (true) {
            when (val curValue = array[index1].value) {
                is AtomicArrayWithDCSS<*>.DCSSDescriptor -> {
                    ...
                    } else if (array[index1].value is AtomicArrayWithDCSS<*>.DCSSDescriptor) {
                        continue
                    }
                    else false
                }
