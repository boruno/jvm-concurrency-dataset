...
class AtomicArrayWithDCSS<E : Any>(size: Int, initialValue: E) {
    ...
    private inner class DcssDescriptor(
            ...
            while (true) {
                when (val current1 = array[index1].value) {
                    ...
                    is AtomicArrayWithDCSS<*>.DcssDescriptor -> {
                        if (current1.status.value == Status.UNDECIDED) {
        ...
        fun updateStatus(caller: DcssDescriptor = this) {
            while (true) {
                when (val value2 = array[index2].value) {
                    this -> break
                    caller -> break
                    expected2 -> {
                    ...
                    is AtomicArrayWithDCSS<*>.DcssDescriptor -> {
                        if (value2.status.value == Status.UNDECIDED) {
                            (value2 as AtomicArrayWithDCSS<E>.DcssDescriptor).updateStatus(caller)
                        }
                        value2.updateValue()
                    }
                    else -> {
