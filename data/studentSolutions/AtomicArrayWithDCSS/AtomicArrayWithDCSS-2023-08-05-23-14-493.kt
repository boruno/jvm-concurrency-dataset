...
//        return array[index].value as E?
        when (val cell = array[index].value) {
            is AtomicArrayWithDCSS<*>.DCSSDescriptor -> {
                if (cell.status.value == Status.SUCCESS || cell.status.value == Status.MAYBE) {
                    cell.update1
                } else {
                    cell.expected1
                }
        ...
        public fun tryToHelp (descr : AtomicArrayWithDCSS<*>.DCSSDescriptor) {
            while (true) {
                when (descr.status.value) {
                    ...
                    Status.SUCCESS -> {
                        array[descr.index1].compareAndSet(descr, descr.update1)
                        return
                    }
                }
            }
                ...
                if (cell2 == expected2) {
                    ...
                    if (cell2 !is AtomicArrayWithDCSS<*>.DCSSDescriptor) {
                        ...
                        if (index1 < cell2.index1) {
                            val v = when (cell2.status.value) {
                                Status.SUCCESS -> cell2.update1
                                Status.FAILED -> cell2.expected1
                                Status.MAYBE -> cell2.tryToHelp(cell2)
                                else -> cell2.expected1
                            }
                            if (v == expected2) {
