...
class SynchronousQueue<E> {
    ...
    suspend fun send(element: E) {
        while (true) {
            ...
            if (curHead.next.value == null || curHead.next.value is Node.SenderNode<*>) {
                try {
    ...
    suspend fun receive(): E {
        while (true) {
            ...
            if (curHead.next.value == null || curHead.next.value is Node.ReceiverNode<*>) {
                try {
    ...
    private fun dequeueAndResumeSender(element: E, curHead: Node<E>): Boolean {
        ...
        return if (head.compareAndSet(curHead, curHeadNext)) {
            (curHeadNext as Node.ReceiverNode).continuation?.resume(element)
            true
        } else {
            false
        }
    ...
    private fun dequeueAndResumeReceiver(curHead: Node<E>): E? {
        ...
        return if (head.compareAndSet(curHead, curHeadNext)) {
            (curHeadNext as Node.SenderNode).continuation?.resume(Unit)
            curHeadNext.x
        } else {
            null
        }
