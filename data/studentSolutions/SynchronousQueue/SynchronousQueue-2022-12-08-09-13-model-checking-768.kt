    ...
    suspend fun send(element: E) {
        ...
        while (true) {
            ...
            if (h == t || !t.isReceive) {
                ...
                if (t == tail.value) {
                    if (n != null) {
                        ...
                        suspendCoroutine<Unit?> sc@{ cont ->
                            offer.sender.value = cont to element
                ...
                val data = n.receiver.value ?: continue
                val success = n.receiver.compareAndSet(data, null)
                if (success) {
                    data.resume(element)
                }
    ...
    suspend fun receive(): E {
        ...
        while (true) {
            ...
            if (h == t || t.isReceive) {
                ...
                val data = n.sender.value ?: continue
                val success = n.sender.compareAndSet(data, null)
                if (success) {
                    data.component1().resume(Unit)
                }
                head.compareAndSet(h, n)
                if (success) {
...
private class Node<E>(val isReceive: Boolean) {
    ...
    val sender = atomic<Pair<Continuation<Unit>, E>?>(null)
    val receiver = atomic<Continuation<E>?>(null)
}