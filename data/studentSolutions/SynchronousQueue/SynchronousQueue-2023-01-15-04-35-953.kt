...
class SynchronousQueue<E> {
    ...
    private var curHead: Node<E>? = null
    private var curTail: Node<E>? = null
    ...
    suspend fun send(element: E) {
        while (true) {
            curHead = head.value
            curTail = tail.value
            if (checkSenderAndReceiver(Node(NodeType.SENDER, element))) return
    ...
    suspend fun receive(): E {
        while (true) {
            ...
            if (checkSenderAndReceiver(node)) return node.value.value!!
            val next = curHead!!.next.value ?: continue
            if (predicate()) continue
    ...
    private suspend fun checkSenderAndReceiver(node: Node<E>): Boolean {
