...
class SynchronousQueue<E> {
    ...
    suspend fun send(element: E): Unit {
        while(true) {
            ...
            if(curSendIdx < receiveIdx.value && segment.get(idx) != null) {
                (segment.get(idx) as Node<E>).cont.resume(element!!)
                return
            } else {
                if(segment.get(idx) is KILL) continue
                val result1 = suspendCoroutine sc@ { cont ->
                    if (!segment.cas(idx,null, Node(element, cont))) {
                        cont.resume(RETRY)
                        return@sc
                    }
                }
                if (result1 != RETRY) return
            }
        }
    }
    ...
    suspend fun receive(): E {
        while(true) {
            ...
            if(curReceiveIdx < sendIdx.value) {
                ...
                val result2 = suspendCoroutine sc@ { cont ->
                    if (!segment.cas(idx,null, Node(null, cont))) {
                        cont.resume(RETRY)
                        return@sc
                    }
                }
                if (result2 != RETRY) return result2 as E
            }
        }
    }
