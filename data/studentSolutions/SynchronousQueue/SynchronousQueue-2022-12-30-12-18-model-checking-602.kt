...
class SynchronousQueue<E> {
    ...
    suspend fun send(element: E) {
        while (true) {
            ...
            if (q.isEmpty() || tail.x != null && tail.x.isSend.value) {
    ...
    suspend fun receive(): E {
        while (true) {
            ...
            val head = q.head.value
            if (q.isEmpty() || tail.x != null && !tail.x.isSend.value) {
                val core = Core(null, null, false)
                val res = suspendCoroutine sc@ { cont ->
                    core.cont.compareAndSet(null, cont)
                    if (!q.enqueue(tail, Core(cont, null, false))) {
                        cont.resume(false)
                        return@sc
                    }
                }
...
class MSQueue<E> {
    ...
    fun enqueue(curTail: Node<E>, x: E): Boolean {
        return if (tail.value.next.compareAndSet(null, Node(x))) {
