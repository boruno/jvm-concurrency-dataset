...
class SynchronousQueue<E> {
    ...
    suspend fun receive(): E {
        ...
        if (r< sendIdx.value){
            val segment = findSegment(curr_head, (r/ SEGMENT_SIZE).toInt())
            val el = segment.elements[((r % SEGMENT_SIZE).toInt())].value as Ref<E>
            val cor = el.coroutine
            cor.resume(el)
            return el.x!!
        }
        else
        {
            val res = suspendCoroutine { cont->
                ...
                segment.elements[((r % SEGMENT_SIZE).toInt())].compareAndSet(null, el)
            }
            return res.x!!
        }
    }
