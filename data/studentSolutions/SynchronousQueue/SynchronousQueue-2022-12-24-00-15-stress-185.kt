...
class SynchronousQueue<E> {
    ...
    private val movedElement = MovedElement()

    init {
        val dummy = Node(false)
        dummy.element.compareAndSet(null, movedElement)
        head = atomic(dummy)
        tail = atomic(dummy)
    }
    ...
    suspend fun receive(): E {
        ...
        while (true) {
            ...
            if (curTail == curHead || !curTail.isSend) { // if empty or all elements in queue are senders
                ...
                if (result != null && result != movedElement) {
                    if (nextHead.element.compareAndSet(result, movedElement)) {
                        head.compareAndSet(curHead, nextHead)
                        nextHead.continuation.resume(true)
                        return result as E
                    } else {
