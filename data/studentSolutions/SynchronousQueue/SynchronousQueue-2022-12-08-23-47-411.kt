...
class SynchronousQueue<E> {
    ...
    private inner class Node(type: String? = null, var element: E? = null) {
    ...
    suspend fun send(element: E) {
        while (true) {
            ...
            val node = Node("SENDER", element)

            if (tail == head || tail.type.value  == "SENDER") {
                enqueueAndSuspend(tail, node)
    ...
    suspend fun receive(): E {
        while (true) {
            ...
            val node = Node("RECEIVER", null)

            if (tail == head || tail.type.value  == "RECEIVER") {
