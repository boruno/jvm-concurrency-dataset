...
class SynchronousQueue<E> {
    ...
    suspend fun send(element: E): Unit {
        ...
        while (true) {
            ...
            val isNormalized = normalizeQueueState(curTail)
            if (isNormalized) {
                continue
            }
            ...
            if (!nextHead.isSender && nextHead.cont != null && head.compareAndSet(curHead, nextHead)) {
                nextHead.x = element // для ресиверов надо положить, чтобы те знали, что вернуть надо
    ...
    private fun normalizeQueueState(curTail: Node<E>): Boolean {
