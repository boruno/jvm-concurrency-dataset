...
class SynchronousQueue<E> {
    ...
    suspend fun send(element: E) {
        while (true) {
            ...
            if (curTail == curHead || curTail.type == Type.SEND) {
                ...
                if (headNext.type == Type.RECEIVE && head.compareAndSet(curHead, headNext)) {
    ...
    suspend fun receive(): E {
        while (true) {
            val curTail = tail.value
            val curHead = head.value
            if (curTail == curHead || curTail.type == Type.RECEIVE) {
                val result = suspendCoroutine sc@ { cont ->
                ...
                val headNext = curHead.next.value ?: continue
                if (headNext.type == Type.SEND && head.compareAndSet(curHead, headNext)) {
                    headNext.operation!!.resume(Status.OK)
