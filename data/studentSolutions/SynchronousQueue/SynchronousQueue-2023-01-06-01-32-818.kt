    ...
    suspend fun send(element: E): Unit {
        while(true) {
            ...
            if(curSendIdx < receiveIdx.value) {
                val node = segment.get(getSegmentIndex(curSendIdx)) as Node<E>
                node.cont.resume(element!!)
                return
            } else {
                val result = suspendCoroutine sc@ { cont ->
                    if (!segment.cas(getSegmentIndex(curSendIdx),null, Node(element, cont))) {
                        cont.resume(RETRY)
                        return@sc
                    }
                }
    ...
    suspend fun receive(): E {
        while(true) {
            ...
            if(curReceiveIdx < sendIdx.value) {
                ...
                val result = suspendCoroutine sc@ { cont ->
                    if (!segment.cas(getSegmentIndex(curReceiveIdx),null, Node(null, cont))) {
                        cont.resume(RETRY)
                        return@sc
                    }
                }
