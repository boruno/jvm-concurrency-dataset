...
class SynchronousQueue<E> {
    ...
    suspend fun send(element: E): Unit {
        while (true) {
            ...
            if (curHead == curTail || curTail.x.category == TaskCategory.Send) {
                ...
                val (res, value) = suspendCoroutine<Pair<Boolean, Any?>> sc@{ cont ->
                    ...
                    if (!enqueue(wrap, curTail)) {
                        cont.resume(false to element) // try transfer again if smth went wrong
                        return@sc
                    }
                }
                if (res == true) return
...
//                res.element.getAndSet(element)
    ...
    suspend fun receive(): E {
        while (true) {
            val curTail = tail.value
            val curHead = head.value
            if (curHead == curTail || curTail.x.category == TaskCategory.Receive) {
                // transfer coroutine to the rendevouz moment
                val (res, value) = suspendCoroutine<Pair<Boolean, Any?>> sc@{ cont ->
