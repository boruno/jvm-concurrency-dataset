...
class SynchronousQueue<E : Any> {
    ...
    suspend fun send(element: E): Unit {
        ...
        while (true) {
            ...
            if (receiver != null) {
                assert(receiver.resume(element))
                return
            }
    ...
    private fun tryEnqueueSender(x: E, t: Continuation<Unit>) : Boolean {
        ...
        while (true) {
            ...
            if ((head.value != tail.value) && currentTail.isReceiver()) {
    ...
    private fun tryEnqueueReceiver(t: Continuation<E>) : Boolean {
        ...
        while (true) {
            val currentTail = tail.value
            if ((head.value != tail.value) && currentTail.isSender()) {
                tail.compareAndSet(currentTail, currentTail.next.value!!)
                return false
            }
            if (currentTail.next.compareAndSet(null, node)) {
    ...
    fun isEmpty(): Boolean {
        while (true) {
            ...
            if (head.compareAndSet(currentHead, currentHead)) {
                return currentHeadNext!!.x == null
            }
        }
    }
