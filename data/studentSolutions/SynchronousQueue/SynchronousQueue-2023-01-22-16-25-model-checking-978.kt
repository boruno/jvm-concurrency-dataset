...
class SynchronousQueue<E> {
    ...
    private fun dequeueAndResume(h: Coroutine<E>, element: E): Boolean {
        ...
        val next = current_head.next.value
        if (next is Receiver<*>) {
            if (head.compareAndSet(h, next)) {
                (next.cont as Continuation<E>).resume(element)
                return true
            } else {
                head.compareAndSet(current_head, current_head.next.value!!)
            }
        } else {
