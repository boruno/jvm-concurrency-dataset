...
// TODO: 05.01.2023 по ходу что-то не так с особыми случаями (когда делаем BREAK или RETRY) 
    /**
    ...
    suspend fun send(element: E) {
        while (true) {
            ...
            file.appendText("send started " + element + " at " + s + '\n')
            ...
            if (s < receiveIdx.value) {
                if (segment.cas((s % SEGMENT_SIZE).toInt(), null, element)) {
                    file.appendText("send " + element + " wrote the value" + '\n')
                    return
                }
                if (segment.get((s % SEGMENT_SIZE).toInt()) == BREAK_SIGNAL) {
                    file.appendText("send " + element + " restarting because BREAK" + '\n')
                    continue
                }
                val receiver = segment.get((s % SEGMENT_SIZE).toInt()) as Continuation<E>
                file.appendText("send " + element + " resuming receiver at " + s + '\n')
                receiver.resume(element)
                return
            } else {
                val res = suspendCoroutine<Any> sc@{ cont ->
                    if (!segment.cas((s % SEGMENT_SIZE).toInt(), null, Pair(cont, element))) {
                        file.appendText("send " + element + " RETRY because" + segment.get((s % SEGMENT_SIZE).toInt()) + '\n')
                        cont.resume(RETRY)
                        return@sc
                    }
                }
                if (res == RETRY) {
                    file.appendText("send " + element + " restarting because RETRY" + '\n')
                    continue
                }
                file.appendText("send " + element + " finished" + '\n')
                return
            }
        }
    }
    ...
    suspend fun receive(): E {
        while (true) {
            ...
            file.appendText("receive started at " + r + '\n')
            val segment = findSegment(curHead, r / SEGMENT_SIZE)
            moveHeadForward(segment)
            if (r < sendIdx.value) {
                if (segment.cas((r % SEGMENT_SIZE).toInt(), null, BREAK_SIGNAL)) {
                    file.appendText("receive restarting at " + r + " because BREAK" + '\n')
                    continue
                }
                val sent = segment.get((r % SEGMENT_SIZE).toInt())
                if (sent is Pair<*, *>) {
                    val (sender, elem) = sent as Pair<Continuation<Unit>, E>
                    file.appendText("receive resuming sender elem " + elem + '\n')
                    sender.resume(Unit)
                    return elem
                }
                file.appendText("receive returning value sent " + sent + '\n')
                return sent as E
            } else {
                val res = suspendCoroutine<Any> sc@ { cont ->
                    if (!segment.cas((r % SEGMENT_SIZE).toInt(), null, cont)) {
                        file.appendText("receive RETRY because" + segment.get((r % SEGMENT_SIZE).toInt()) + '\n')
                        cont.resume(RETRY)
                        return@sc
                    }
                }
                if (res == RETRY) {
                    file.appendText("receive restart because retry at " + r + '\n')
                    continue
                }
                file.appendText("receive return res " + res + '\n')
                return res as E
            }
        }
    }
