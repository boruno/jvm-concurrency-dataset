...
class SynchronousQueue<E> {
    ...
    suspend fun send(element: E) {
        while (true) {
            ...
            if (tail == head || tail.x!!.isSend.value) {
    ...
    suspend fun receive(): E {
        while (true) {
            ...
            if (tail == head || !tail.x!!.isSend.value) {
                ...
                val res = suspendCoroutine sc@{ cont ->
                    core.cont.compareAndSet(null, cont)
...
class Core<E>(continuation: Continuation<Any?>?, element: E?, isForSending: Boolean) {
    val cont: AtomicRef<Continuation<Any?>?> = atomic(continuation)
    val elem: AtomicRef<E?> = atomic(element)
    val isSend: AtomicBoolean = atomic(isForSending)
}
...
class MSQueue<E> {
    ...
    fun enqueue(curTail: Node<E>, x: E): Boolean {
        val node = Node(x)
        return if (curTail.next.compareAndSet(null, Node(x))) {
