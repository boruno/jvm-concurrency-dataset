...
//                        queue.enqueue(operation)
                        ...
                        if (curTail.next.compareAndSet(null, operation)) {
                            queue.tail.compareAndSet(curTail, operation)
                        } else {
                            if (queue.tail.value is Receiver<*>) {
...
//                        queue.enqueue(Receiver<E>(null, it))
                        ...
                        if (curTail.next.compareAndSet(null, operation)) {
                            ...
                            if (queue.tail.value is Sender<*>) {
                                val receiver = queue.dequeue()
                                receiver?.continuation?.resume(null)
                            } else {
                                val next = curTail.next.value
                                if (next != null) {
                                    queue.tail.compareAndSet(curTail, next)
