...
class SynchronousQueue<E> {
    ...
    suspend fun receive(): E {
        ...
        val currHead = head.value
        if(currTail == currHead || currTail.type == NodeType.SENDER){
            return enqueueAndSuspend(currTail, null, NodeType.SENDER) as E
        }
        else{
    ...
    suspend fun enqueueAndSuspend(t: Node<E>, x: E?, type: NodeType): Any?{
        while(true){
            ...
            else
            {
                ...
                val res = suspendCoroutine<Node<E>> sc@{ cont ->
                    ...
                    if(t.next.compareAndSet(next,node)){
    ...
    suspend fun dequeueAndResume(h: Node<E>): Any?{
        while(true) {
