    ...
    suspend fun send(element: E): Unit {
        while (true) {
            ...
            if (i < q.deqIdx.value) {
                ...
                val corRes = suspendCoroutine<Any> sc@ { cont ->
                    if (!segment.elements[(i % SEGMENT_SIZE).toInt()].compareAndSet(null, cont to element)) {
                        cont.resume('T')
                        return@sc
                    }
                }
                if (corRes == 'T') continue
                else return
            }
        }
    }
    ...
    suspend fun receive(): E {
        while (true) {
            ...
            if (i < q.enqIdx.value) {
                ...
                if (result == null) {
                    segment.elements[(i % SEGMENT_SIZE).toInt()].compareAndSet(result, 'T')
                }
            } else {
                val corRes = suspendCoroutine<Any> sc@ { cont ->
                    if (!segment.elements[(i % SEGMENT_SIZE).toInt()].compareAndSet(null, cont)) {
                        cont.resume('T')
                        return@sc
                    }
                }
                if (corRes == 'T') continue
                else return corRes as E
            }
        }
    }
}
