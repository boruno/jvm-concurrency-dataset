...
class SynchronousQueue<E> {
    ...
    suspend fun send(element: E) {
        ...
        while (true) {
            ...
            println("${Thread.currentThread().id}: send before info: tail is ${curTail.x}, head is ${curHead.x}")
            ...
            println("${Thread.currentThread().id}: send after info: tail is ${curTail.x}, head is ${curHead.x}")
        }
    }
    ...
    suspend fun receive(): E {
        ...
        while (true) {
            val curTail = tail.value
            val curHead = head.value
            println("${Thread.currentThread().id}: receive info: tail is ${curTail.x}, head is ${curHead.x}")
    ...
    private suspend fun enqueueAndSuspend(element: E?, operation: Operation) {
        suspendCoroutine<Any?> action@{ process ->
            while (true) {
