        ...
        while (true) {
            ...
            if (t == h || t.type.value == Operation.SEND) {
                val res = suspendCoroutine sc@ { cont ->
                    val newNode = Node(element, Operation.SEND, cont)
                ...
                if (head.compareAndSet(h, headNext)) {
                    headNext.continuation.value!!.resume(element)
                    return
                }
            }
        }
    ...
    suspend fun receive(): E {
        while (true) {
            ...
            if (t == h || t.type.value == Operation.RECEIVE) {
                val res = suspendCoroutine sc@ { cont ->
                    val newNode = Node(null, Operation.RECEIVE, cont)
                ...
                if (head.compareAndSet(h, headNext)) {
                    ...
                    return headNext.element.value!!
                }
            }
        }
    }
    private inner class Node(x: E?, type: Operation? = null, continuation: Continuation<Any?>?) {
        val continuation: AtomicRef<Continuation<E?>?> = atomic(continuation)
