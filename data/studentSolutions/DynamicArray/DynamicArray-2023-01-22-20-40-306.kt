...
class DynamicArrayImpl<E> : DynamicArray<E> {
    ...
    override fun get(index: Int): E {
        ...
        while (true) {
            ...
            if (curCore.capacity > index) {
                val res : Element<E> = curCore.array[index].value ?: continue
                if (res is MovedElement) {
                    return curCore.nextCore.value!!.array[index].value?.element ?: continue
                }
                return curCore.array[index].value?.element ?: continue
            }
        }
    }
    ...
    private fun resize(oldCore: Core<E>) {
        val newCore = Core<E>(oldCore.capacity * 4)
