...
class DynamicArrayImpl<E> : DynamicArray<E> {
    ...
    override fun pushBack(element: E) {
        while (true) {
            val cur = core.value
            val size = cur.updatePos()
            val capacity = cur.capacity
            if (size < capacity) {
                if (cur.cas(size, null, element)) {
                    cur.updateSize()
                    var next: Core<E>? = core.value.next.value
                    while (next != null) {
                        next.set(size, element)
                ...
                next.size = capacity
                next.pos.compareAndSet(0, capacity)
...
//    private fun getInternal(index: Int): E? {
//        require(index < pos.value)
//        return array[index].value as E?
//    }
