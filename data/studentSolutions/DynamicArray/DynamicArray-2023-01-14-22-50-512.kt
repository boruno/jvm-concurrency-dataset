    ...
    override fun pushBack(element: E) {
        while ( true ) {
            ...
            if ( sizeC >= core.value.cap.value ) {
                ...
                copyC._size.getAndSet( old_core._size.value )
                for (i in 0 until old_core.cap.value) {
                    while (true) {
                        val old_value = old_core.array[i].value
                        if ( old_value != BROKEN_VALUE && old_value != null ) {
                            if ( old_core.array[i].compareAndSet(old_value, BROKEN_VALUE as E)) {
                                if ( copyC.array[i].compareAndSet(null, old_value)) break
                            }
                        } else {
                            break
                        }
                ...
                if ( core.compareAndSet(old_core, copyC ) ) {
                    core.value.cap.compareAndSet(t, copyC.cap.value )
                }
            } else {
                if ( core.value.array[ sizeC ].compareAndSet( null, element ) ) {
                    if ( core.value._size.compareAndSet( sizeC, sizeC + 1 ) ) {
                        return
                    }
                } else {
