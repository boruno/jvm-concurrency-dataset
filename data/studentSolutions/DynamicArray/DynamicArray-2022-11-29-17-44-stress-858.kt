...
class DynamicArrayImpl<E> : DynamicArray<E> {
    ...
    override fun pushBack(element: E) {
        ...
        while (true) {
            ...
            if (result == RESIZED && core.value.next.value != null) core.compareAndSet(
                expectedCore,
                core.value.next.value!!
            )
        }
    }
        ...
        if (expectedSize == array.size) {
            if (next.compareAndSet(null, Core(2 * array.size))) {
                for (i in 0 until array.size) {
                    next.value!!._size.compareAndSet(i, i + 1)
                    next.value!!.put(i, get(i))
                }
                return RESIZED
            }
            return NOT_SUCCESS
        }
        if (array[expectedSize].compareAndSet(null, element)) {
            _size.compareAndSet(expectedSize, expectedSize + 1)
            return SUCCESS
        }
        return NOT_SUCCESS
    }
