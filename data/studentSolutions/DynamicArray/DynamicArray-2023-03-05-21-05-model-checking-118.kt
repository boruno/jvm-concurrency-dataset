...
class DynamicArrayImpl<E> : DynamicArray<E> {
    ...
    override fun put(index: Int, element: E) {
        ...
        while (true) {
            when (val oldValue = core.value.array[index].value!!) {
                is Common<E> -> {
                    if (core.value.array[index].compareAndSet(oldValue, Common(element))) break
                }

                is Moving<E> -> {
                    core.value.next.value.array[index].compareAndSet(null, Common(oldValue.element))
                    core.value.array[index].compareAndSet(oldValue, Moved(oldValue.element))
                    continue
                }
                    ...
                    if (cur.array[index].compareAndSet(cur.array[index].value, Common(element))) {
                        return
                    }
                    continue
                }
