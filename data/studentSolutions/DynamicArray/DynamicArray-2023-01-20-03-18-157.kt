...
class DynamicArrayImpl<E> : DynamicArray<E> {
    ...
    private fun helpCopy() {
        val curCore = core.value
        if (curCore.size > core.value._size.value) return
        // Если мы тут, значит:
        // 1) Идет фаза копирования из array -> next
        // 2) Фаза закончилась и в array лежат Move
        curCore.nextCore.compareAndSet(null, Core(curCore.size * 2))
    ...
    override fun put(index: Int, element: E) {
        ...
        while (true) {
            ...
            if (el is RemovedNode) {
                ...
                if (core.value.array[index].compareAndSet(el, ActiveNode(element)))
                    return
            } else if (el == null) {
                throw IllegalStateException("Жопа")
            } else {
    ...
    override fun pushBack(element: E) {
        while (true) {
            ...
            val curCore = core.value
            if (curSize < curCore.size) {
                if (core.value.array[curSize].compareAndSet(null, ActiveNode(element))) {
                    // interrupt и сразу не lock-free, надо форсить
                    core.value._size.compareAndSet(curSize, curSize + 1)
                    return
                } else {
                    core.value._size.compareAndSet(curSize, curSize + 1)
                }
    ...
    private fun checkIndex(index: Int) {
        if (index < 0 || index >= core.value._size.value)
            throw IllegalArgumentException()
    }
