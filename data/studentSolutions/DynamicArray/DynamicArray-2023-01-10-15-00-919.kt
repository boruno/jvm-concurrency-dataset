        ...
        while (true) {
            ...
            if (size < core.value.capacity) {
                if (core.value.array[size].compareAndSet(null, element)) {
                    core.value._size.compareAndSet(size, size + 1)
                    return
                } else core.value._size.compareAndSet(size, size + 1)
            } else {
                val newNode = Core<E>(2 * core.value.capacity, null)
                newNode._size.getAndSet(core.value.capacity)
                if (core.value.next.compareAndSet(null, newNode)) move(core.value, newNode)
                else if (core.value.next.value != null) move(core.value, core.value.next.value!!)
            }
        }
    }
