...
class DynamicArrayImpl<E> : DynamicArray<E> {
    ...
    override fun get(index: Int): E {
        ...
        while(true) {
            ...
            if (curCore.nextCore.value != null) {
        ...
        while (true) {
            ...
            if (curCore.pushBack(element)) {
                ...
                if (curCore.nextCore.value != null) {
                    ...
                    val newCore = Core<E>(curCore.capacity * 2, curCore.capacity) // переставить местами???????
...
private class Core<E>(
    ...
    fun get(idx: Int): E? {
        ...
        if (idx >= capacity) {
            return null
        } // what if array is not copied yet????????????????????????????????????????????????????????????????????????
        if (array[idx].value!!.state.value > State.FIXED) {
            return null
        }
        return array[idx].value as E
    }
