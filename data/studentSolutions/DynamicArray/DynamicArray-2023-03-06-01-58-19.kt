        ...
        while (true) {
            ...
            while (true) {
                ...
                if (size == coreValue.capacity) {
                    core.value.next.compareAndSet(null, Core(size * 2, size))
                    core.compareAndSet(core.value, core.value.makeNewTable())
                    break
                } else if (coreValue.array[size].compareAndSet(null, Wrapper(element))) {
