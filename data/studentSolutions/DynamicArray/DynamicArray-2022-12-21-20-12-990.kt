...
//                putIfNull(index, element) //???
     ...
     * */
    private fun putIfNull(index: Int, element: E): Boolean {
        if (index >= capacity) {
            val newCore = Core<E>(capacity * 2, size)
            if (nextCore.compareAndSet(null, newCore)) {
                moveElements()
            }
            return nextCore.value!!.putIfNull(index, element)
        }
        ...
        if (nextCore.value != null) {
            ...
            res = nextCore.value!!.putIfNull(index, element)
    ...
    fun pushBack(element: E) {
        while (true) {
            ...
            if (idx < capacity) { // -1
                ...
                if (nextCore.value!!.putIfNull(idx, element)) {
                    _size.compareAndSet(idx, idx + 1)
                    return
                } else {
                    _size.compareAndSet(idx, idx + 1)
