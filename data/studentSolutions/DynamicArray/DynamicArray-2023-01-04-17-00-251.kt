...
class DynamicArrayImpl<E> : DynamicArray<E> {
    ...
    override fun put(index: Int, element: E) {
        ...
        if (index >= curHead.getSize()) {
            throw IllegalArgumentException()
        }
        curHead.array[index].getAndSet(Cell(element))
        while (true) {
            val nextNode = curHead.nextCore.value
            when {
                nextNode != null -> {
    ...
    override fun pushBack(element: E) {
        while (true) {
            ...
            val curSize = curHead.getSize()
            if (curSize < curHead.getCapacity()) {
                if (curHead.array[curSize].compareAndSet(null, Cell(element))) {
                    curHead.incSize()
                    return
                } else {
                    curHead.incSize()
                ...
                val newNode = Core<E>(2 * curHead.getCapacity())
                if (curHead.nextCore.compareAndSet(null, newNode)) {
