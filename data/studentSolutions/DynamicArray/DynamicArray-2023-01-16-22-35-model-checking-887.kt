...
class DynamicArrayImpl<E> : DynamicArray<E> {
    ...
    override fun put(index: Int, element: E) {
        ...
        while (true) {
            ...
            if (!moveIfNeeded(coreValue, valueByIndex)) {
                if (coreValue.set(index, valueByIndex, element)) {
                    return
                }
            }
        }
    ...
    private fun moveIfNeeded(coreValue: Core<E>, value: E): Boolean {
        if (value is Movable<*>) {
            move(coreValue)
            return true
        } else {
    ...
    private fun move(coreValue: Core<E>) {
        if (coreValue.next.value == null) {
            coreValue.next.compareAndSet(null, Core(2 * coreValue.size))
        }
        val coreValueNext = coreValue.next.value
        if (coreValueNext != null) {
            for (i in 0 until coreValue.size) {
                ...
                if (elemValue is Movable<*>) {
                    coreValue.set(i, null, elemValue.inner)
                } 
    ...
    override fun pushBack(element: E) {
        while (true) {
            val sizeCopy = size
            val coreValue = core.value
            if (sizeCopy >= coreValue.size) {
                move(coreValue)
            } else if (coreValue.set(sizeCopy, null, element)) {
