    ...
    override fun pushBack(element: E) {
        ...
        for (i in 0 until oldCore.getSize()) {
            if (core.value.tryLock(i)) {
                newCore.value?.tryPut(i, get(i))
            }
        }
        if (newCore.value?.tryPut(oldCore.getSize(), element) == true) {
            newCore.value?.trySetSize(oldCore.getSize(), oldCore.getSize() + 1)
            println(newCore.value!!.getSize())
        }
        println(oldCore.getSize())
        newCore.value?.let { println(it.getSize()) }
...
private class Core<E>(
    ...
    fun tryPlace(index: Int, element: E): Boolean {
        println(Cell<E>())
        return array[index].compareAndSet(Cell(), Cell(element))
    }
