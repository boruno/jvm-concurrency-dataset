...
class DynamicArrayImpl<E> : DynamicArray<E> {
    ...
    override fun get(index: Int): E {
        ...
        while (true) {
            if (index < currentCore.capacity) {
                ...
                if (currentValue is NodeMoved) {
                    currentCore = currentCore.nextList.value!!
                    continue
                }
                if (currentValue is NodeFixed) {
                    if (currentCore.nextList.value!!.get(index) == null) // Checking if node already updated
                        return currentValue.value!!
                    else return currentCore.nextList.value!!.get(index)!!.value!!
                }
                return currentValue.value!!
            } else {
    ...
    override fun put(index: Int, element: E) {
        ...
        while (true) {
            ...
            if (currentValue is NodeFixed) {
                ...
                if (nextElement is NodeMoved) {
                    currentCore.putCas(index, currentValue, NodeMoved(element))
                    continue
                }
                if (nextElement is NodeFixed) {
                    currentCore = currentCore.nextList.value!!
                    continue
                }
    ...
    private fun tryPlaceInArray(element: E): Boolean {
        ...
        while (currentCore != null) {
                ...
                if (currentCore.putCas(currentIndex, null, Node(element))) {
                    ...
                    if (size < currentIndex + 1) {
                        while (true) {
                            ...
                            if (currentCoreSize >= currentIndex + 1)
                                break
                            if (currentCore.sizeCas(currentCoreSize, currentIndex + 1))
                                break
                        }
                    }
                    return true
            ...
            if (currentElement is NodeMoved) {
                currentCore = currentCore.nextList.value
                continue
            }
            if (currentElement is NodeFixed) {
                val nextCore = currentCore.nextList.value!!
                val nextElement = nextCore.get(currentIndex)
                if (nextElement == null) {
                    var needMoing = false
                    if (nextCore.putCas(currentIndex, null, Node(currentElement.value)))
                        needMoing = true
                    var oldSize = nextCore.size
                    while (true) {
                        if (oldSize >= currentIndex + 1)
                            break
                        if (nextCore.sizeCas(oldSize, currentIndex + 1))
                            break
                        oldSize = nextCore.size
                    }
                    if (needMoing)
                        currentCore.putCas(currentIndex, currentElement, NodeMoved(currentElement.value))
                    continue
                }
                if (nextElement is NodeMoved || nextElement is NodeFixed) {
                    currentCore = core.value
                    currentIndex++
                    continue
                }
                    ...
                    while (true) {
                        ...
                        if (currentElement == null) {
                            ...
                            while (initialCore != newCore) {
                                ...
                                if (element == null) {
                                    initialCore.putCas(index, null, NodeMoved(element))
                                    continue
                                }
                        ...
                        if (currentElement!!.getType() == "NodeFixed") {
                            newCore.putCas(index, null, Node(currentElement.value))
