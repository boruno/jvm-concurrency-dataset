...
//        }
    ...
    private fun move() {
        ...
        var core = Core<E>(curCore.capacity * 2)
        core.size.value = curCore.size.value
        if (!nextCore.compareAndSet(null, core)) {
            core = nextCore.value ?: return
        }
        for (i in 0 until curCore.size.value) {
            ...
            v!!
            core.array[i].compareAndSet(null, NotMoved(v.value))
        }
//        if (nextCore.compareAndSet(core, null)) {
            this.curCore.compareAndSet(curCore, core)
        this.nextCore.compareAndSet(core, null)
//        }
    }
        ...
        while(true) {
            ...
            if (s >= curCore.value.capacity) {
                ...
                if (curCore.value.array[s].compareAndSet(null, NotMoved(element))) {
                    curCore.value.size.compareAndSet(s, s + 1)
                    return
                } else {
                    curCore.value.size.compareAndSet(s, s + 1)
                }
