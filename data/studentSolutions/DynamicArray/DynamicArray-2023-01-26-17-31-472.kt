...
class DynamicArrayImpl<E> : DynamicArray<E> {
    private val core = atomic(Core<E>(null, INITIAL_CAPACITY, 0 ))

    override fun get(index: Int): E {
        if (index < core.value.sizeAt.value && index >= 0) {
    ...
    override fun put(index: Int, element: E) {
        var corv = core.value
        when {
            index >= core.value.sizeAt.value || index < 0 -> {
                throw IllegalArgumentException("Incorrect")
            }
            else -> {
        ...
        while (true) {
            ...
            when {
                cnstSize >= cnstCap -> {
                    ...
                    when {
                        cnstCore.next.compareAndSet(null, biggerCore) -> corRep = biggerCore
                    }
                    when (corRep) {
                        null -> return
                        else -> {
                            (0 until cnstCore.capacity).forEach { i ->
                                val coreVal = cnstCore.array[i].value ?: return@forEach
