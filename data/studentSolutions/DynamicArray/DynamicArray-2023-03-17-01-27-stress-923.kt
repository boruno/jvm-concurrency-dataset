        ...
        while (true) {
            ...
            if (size < head.capacity) {
                ...
                if (head.next.compareAndSet(null, newNode)) {
                    for (i in 1..head.capacity) if (head.get(i - 1) != null) newNode.array[i - 1].compareAndSet(
                        null,
                        head.get(i - 1)
                    )
                    core.compareAndSet(head, newNode)
                } else if (head.next.value != null) {
                    for (i in 1..head.capacity) if (head.get(i - 1) != null) head.next.value!!.array[i - 1].compareAndSet(
                        null,
                        head.get(i - 1)
                    )
