    ...
    override fun pushBack(element: E) {
        while (true) {
            ...
            if (nextCore != null) {
                ...
                newCore.casSize(expectedSize = 0, newSize = currentSize)
...
private class Core<E>(
    ...
    fun tryPushBackWithoutExtending(index: Int, element: E): Boolean {
        ...
        if (!array[index].compareAndSet(null, CommonElement(element)))
            return false

        if (!_size.compareAndSet(index, index + 1))
            error("Failed to CAS size from $index to ${index + 1}. Current size is ${_size.value}.")
