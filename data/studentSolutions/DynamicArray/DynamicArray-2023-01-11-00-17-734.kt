...
class DynamicArrayImpl<E> : DynamicArray<E> {
    ...
    fun resizeCore(){
        ...
        val old_size = size
        val new_core = Core<E>(old_size*2)
        for(i in 0 until old_size)
        {
            new_core.put_copy(i,core.value.get(i))
        }
...
private class Core<E>(
    ...
    fun put(index: Int, element: E){
        while(true){
            ...
            if(arr_val == null && _size.value <= index) {
                if(!_size.compareAndSet(curr_size,index+1))
                    continue
                size = _size.value
            }
    ...
    fun put_copy(index: Int, element: E){
