...
class DynamicArrayImpl<E> : DynamicArray<E> {
    ...
    override fun pushBack(element: E) {
        while (true) {
            ...
            val curSize = coreValue.getSize
            val curCapacity = coreValue.getCapacity

            if (curSize < curCapacity) {
                if (!coreValue.array[curSize].compareAndSet(null, element)) {
                    coreValue.size.compareAndSet(curSize, curSize + 1)
                    continue
                }
                ...
                next.size.compareAndSet(0, curSize)
