...
class DynamicArrayImpl<E> : DynamicArray<E> {
    ...
    override fun get(index: Int): E = core.get().get(index)
...
private class Core<E>(
    ...
    private val next = AtomicReference<Core<E>?>(null)

    private val isMoving = AtomicBoolean(false)

    fun getSize(): Int = size.get()

    fun get(index: Int): E = when (val node = array[index].get()) {
        is Empty -> {
    ...
    private fun createNextArray() {
        val nextRef = next.get()
        if (nextRef == null) {
            val newNextRef = Core<E>(2 * capacity)
            next.compareAndSet(null, newNextRef)
        }
    }
