    ...
    override fun pushBack(element: E) {
        while (true) {
            ...
            if (pushBackAttemptResult.isSuccessful) {
                // Trying to update inserted value in case if new core was create while push backing into current.
                var updatedCore = currentCore.next.value
                while (updatedCore != null) {
                    updatedCore.copyElement(pushBackAttemptResult.index, element)
                    updatedCore = currentCore.next.value!!
                }
            ...
            val pushBackResult = newCore.tryPushBackWithoutExtending(element)

            if (!pushBackResult.isSuccessful)
                continue
