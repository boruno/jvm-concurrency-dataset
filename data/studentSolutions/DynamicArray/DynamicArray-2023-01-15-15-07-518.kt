...
class DynamicArrayImpl<E> : DynamicArray<E> {
    ...
    override fun pushBack(element: E) {
        while (true) {
            val nextCore = nextCoreRef.value
            if (core.value.pushBack(element) || (nextCore != null && nextCore.pushBack(element))) {
                return
            } else {
                val curCapacity = core.value.size()
                if (nextCoreRef.compareAndSet(null, Core(curCapacity * 2, curCapacity))) {
