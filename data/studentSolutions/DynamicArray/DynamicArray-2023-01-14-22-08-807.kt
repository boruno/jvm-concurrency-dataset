...
class DynamicArrayImpl<E> : DynamicArray<E> {
        ...
        while (true) {
            ...
            if (containerSize >= currentArray.array.size) {
                ...
                if (currentArray.next.compareAndSet(null, newArray)) {
                    ...
                    if (nextArray.array[containerSize].compareAndSet(null, element)) {
                        if (core.compareAndSet(currentArray, nextArray)) {
                            nextArray._size.compareAndSet(containerSize, containerSize + 1)
                            return
                        } else {
                            nextArray._size.compareAndSet(containerSize, containerSize + 1)
                        }
    ...
    private fun helpToMove(from: Core<E>, to: Core<E>) {
