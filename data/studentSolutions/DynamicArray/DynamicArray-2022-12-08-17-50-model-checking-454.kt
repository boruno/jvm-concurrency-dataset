...
private class Core<E>(
    ...
    private val array = Array<AtomicReference<Node>>(capacity) {
    ...
    fun get(index: Int): E {
        return if (index < capacity) {
            when (val node = array[index].get()) {
                is WithValueTemp<*> -> {
                    (node as WithValueTemp<E>).value
                }

                is WithValue<*> -> {
                    (node as WithValue<E>).value
                }

                Empty -> {
                    throw Exception("trying to accept empty cell")
                }
                Moved -> {
                    next.get()!!.get(index)
                }
    ...
    fun casAssistance(elem: E, index: Int = -1, oldValueNeeded: Node? = null): Boolean {
        ...
        val res = array[indexToPush].compareAndSet(oldValue, WithValue(elem))
        return res
    }
