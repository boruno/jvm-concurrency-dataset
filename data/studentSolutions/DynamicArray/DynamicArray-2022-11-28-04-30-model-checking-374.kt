...
class DynamicArrayImpl<E> : DynamicArray<E> {
    ...
    override fun get(index: Int): E {
        ...
        var currentElement = currentCore.get(index)

        while (currentElement is MovedElement) {
            currentCore = currentCore.next.value ?: break
            currentElement = currentCore.get(index)
        }
    ...
    private fun finishCoreExtending(oldCore: Core<E>, newCore: Core<E>) {
        ...
        markCoreAsNotExtending(oldCore)
    }

    private fun moveElementsToExtendedCore(currentCore: Core<E>, extendedCore: Core<E>) {
        var oldCore = currentCore
        var newCore = extendedCore
        ...
        for (index in 0 until currentBound) {
            ...
            val elementValue = movableElement.element

            if (movableElement is MovedElement) {
                oldCore = core.value
                newCore = oldCore.next.value ?: extendedCore
            }

            val fixedElement = movableElement.fixElement()

            oldCore.casElement(index, movableElement, fixedElement)

            // Copy element to the extended array
            newCore.copyElement(index, elementValue)

            // Mark element as Moved (so it couldn't be used by reader anymore)
            oldCore.casElement(index, fixedElement, MovedElement(elementValue))
        }
    }
...
private class Core<E>(
    ...
    fun tryPushBackWithoutExtending(element: E): PushBackResult {
        ...
        if (!_size.compareAndSet(pushIndex, pushIndex + 1))
            println("Failed to CAS size from $pushIndex to ${pushIndex + 1}. Current size is ${_size.value}.")
