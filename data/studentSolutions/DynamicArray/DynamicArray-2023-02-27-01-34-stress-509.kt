...
class DynamicArrayImpl<E> : DynamicArray<E> {
    ...
    override fun pushBack(element: E) {
        ...
        while (true) {
            ...
            val curLen = core.value.len.value

            val cCore = core.value
            if (curLen + 1 > curSize) {
                val newCore = Core<E>(curSize * 2)
                for (i in 0 until curSize) {
                    val transferValue = cCore.get(i).value
                    newCore.cas(i,null, Elem(transferValue, false))
                }
                if (!core.compareAndSet(cCore, newCore)){
                    continue
                }
            } else {
                if (core.value.cas(curLen + 1, null, elem)){
                    core.value.len.incrementAndGet()
                    return
                }
            }
        }
...
private class Core<E>(
    ...
    fun get(index: Int): Elem<E> {
        require(index < size)
        return array[index].value as Elem<E>
    }
}
