...
class DynamicArrayImpl<E> : DynamicArray<E> {
    ...
    override fun pushBack(element: E) {
        while (true) {
            val head = core.value
            if (head.size.value < head.capacity) {
                if (head.array[head.size.value].compareAndSet(null, element)) {
                    head.size.compareAndSet(head.size.value, head.size.value + 1)
                    return
                } else head.size.compareAndSet(head.size.value, head.size.value + 1)
            } else {
