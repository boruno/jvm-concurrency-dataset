...
class DynamicArrayImpl<E> : DynamicArray<E> {
    ...
    override fun pushBack(element: E) {
        while(true) {
            ...
            if (!core.value!!.pushBack(element)) // Если массив в данный момент расширяется, то 100% не сработает, так как расширение происходит при переполнении
            {
                ...
                println(String.format("Pushed back. New size %d. Capacity: %d", size, core.value!!.capacity))
    ...
    override val size: Int get(){
        println(String.format("Getting size: %d", core.value!!.size))
...
private class Core<E>(
    ...
    fun pushBack(element: E): Boolean { // Через FAA пушим, если получается size > capacity, то отказываем
       val result = _size.getAndAdd(1)
        if (_size.value > _capacity.value) {
            _size.value = _capacity.value
            return false
        }
        array[result].value = element
        return true
    }
}
