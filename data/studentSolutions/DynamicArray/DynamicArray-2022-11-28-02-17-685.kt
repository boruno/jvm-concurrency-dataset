...
private class Core<E>(
    ...
    fun incrementSize(index: Int) {
        ...
        val newSize = max(index + 1, currentSize + 1)
        if (!_size.compareAndSet(currentSize, newSize))
            error("Failed to CAS size from $currentSize to ${newSize}. Current size value is ${_size.value}.")
    }
}
