            ...
            while (true) {
                if (!migrationStatus.value) {
                    ...
                    if (core.value.tryPlace(place, element)) {
                        core.value.trySetSize(place, place + 1)
                        break
                    }
                } else {
                    val place = newCore.value?.getSize() ?: continue
                    if (newCore.value!!.tryPlace(place, element)) {
                        newCore.value!!.trySetSize(place, place + 1)
                        break
                    }
                }
            }
...
private class Core<E>(
    ...
    fun tryPlace(index: Int, element: E): Boolean {
        ...
        val cell = Cell(element)
        return if (array[index].compareAndSet(prev, cell)) {
