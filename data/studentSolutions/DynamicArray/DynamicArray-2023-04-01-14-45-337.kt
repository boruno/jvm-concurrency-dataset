...
class DynamicArrayImpl<E> : DynamicArray<E> {
    ...
    override fun put(index: Int, element: E) {
        ...
        while (true) {
            ...
            head = head.next.value ?: return
        }
    }

    override fun pushBack(element: E) {
        val size = core.value.size.incrementAndGet()
        while (true) {
            val head = core.value
//            val size = head.size.value

            if (size < head.capacity) {
                if (head.array[size].compareAndSet(null, element)) break else continue
//                val flag = head.array[size].compareAndSet(null, element)
//                head.size.compareAndSet(size, size+1)
//                if (flag) break else continue
            }

            var next: Core<E>? = Core(head.capacity * 2, head.capacity)
...
private class Core<E>(val capacity: Int, _size: Int) {
