...
class DynamicArrayImpl<E> : DynamicArray<E> {
    ...
    override fun pushBack(element: E) {
        ...
        while (true) {
            curCore.fixCapacity()
            updateCore()
            val pushIndex = curCore.size
            if (pushIndex >= curCore.capacity) {
                curCore = curCore.toNextForPush()
            } else {
                if (curCore.casFirst(Value(element), pushIndex)) {
                    return
                }
                val pushCellValue = curCore.array[pushIndex].value
                if (pushCellValue is FixedValue || pushCellValue is MovedValue) {
                    curCore = curCore.toNextForPush()
                }
            }
        }*/
...
private class Core<E>(
    ...
    /*fun fixCapacity() {
        while (capacityExpandCondition() && next.value == null) {
    ...
    fun updateNext() {
        ...
        /*for (i in 0 until capacity) {
            while (true) {
                ...
                if (value is FixedValue || fixValue(value as Value<E>?, i)) {
                    if (value != null) {
                        moveFirstValue(value, i)
                    }
                    break
                }
            }
