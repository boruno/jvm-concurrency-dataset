        ...
        while (true) {
            ...
            if (curSize >= curCore._capacity.value) { // Need update
                if (curCore._needResize.compareAndSet(false, true)) {
                    ...
                    if (curCore._nextCore.value == null) {
                        core.value._nextCore.compareAndSet(null, newCore)
                    }
                    ...
                    if (!core.value._needResize.compareAndSet(true, false) && !core.compareAndSet(curCore, newCore))
                        continue
                } else {
                    val newCore = Core<E>(curCore._capacity.value * 2, curSize)
                    if (core.value._nextCore.value == null) {
                        core.value._nextCore.compareAndSet(null, newCore)
                    }
                    for (i in curSize - 1 downTo 0) {
                        if (curCore.array[i].value == null || curCore.array[i].value!!.isWriting()) { continue }
                        val curElement = curCore.array[i].value
                        if (curCore.array[i].compareAndSet(curElement, InsertValue(curElement!!.getValue(), true))) {
                            curCore._nextCore.value!!.array[i].getAndSet(curElement)
                        }
                    }
                    if (!core.value._needResize.compareAndSet(true, false) && !core.compareAndSet(curCore, newCore))
                        continue
...
private class Core<E>(capacity: Int, size: Int) {
    ...
    val _nextCore: AtomicRef<Core<E>?> = atomic(null)
