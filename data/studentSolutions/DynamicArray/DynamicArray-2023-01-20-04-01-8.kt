...
class DynamicArrayImpl<E> : DynamicArray<E> {
    ...
    private fun helpCopy() {
        val curCore = core.value
        if (curCore.capacity > _size.value) return
        newCore.compareAndSet(null, Core(curCore.capacity * 2))
        var index = -1
        while (++index < curCore.array.size) {
            var elFromNext = newCore.value!!.array[index].value
            var el = curCore.array[index].value
            while (el is ActiveNode) {
                if (newCore.value!!.array[index].compareAndSet(elFromNext, el))
                    if (curCore.array[index].compareAndSet(el, RemovedNode()))
                        break
                elFromNext = newCore.value!!.array[index].value
                el = curCore.array[index].value
            }
        }
        core.compareAndSet(curCore, newCore.value!!)
        newCore.getAndSet(null)
    }
