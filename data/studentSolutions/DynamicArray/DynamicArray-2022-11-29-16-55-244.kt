...
class DynamicArrayImpl<E> : DynamicArray<E> {
    ...
    override fun pushBack(element: E) {
        ...
        while (true) {
            ...
            if (result == RESIZED && core.value.next.value != null) core.compareAndSet(expectedCore, core.value.next.value!!)
        }
    }
...
class Core<E>(
    ...
    fun pushBack(element: E, expectedSize: Int): Int {
        if (expectedSize == array.size) {
            ...
            while (next.value!!._size.value != array.size) {
                for (i in 0 until array.size) {
                    if (next.value!!.array[i].compareAndSet(null, get(i))) {
