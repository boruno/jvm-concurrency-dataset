...
class DynamicArrayImpl<E> : DynamicArray<E> {
        ...
        while (true){
            ...
            if (curCore.size < curCore.cap){
                ...
                if (nextCore.compareAndSet(curNextCore, newCore)){
                    ...
                    newCore.casToNull(curCore.cap, element)
    ...
    override val size: Int get() {
        ...
        val curNextCore = nextCore.value
        val curCore = core.value
        for (i in 0 until min(curNextCore.cap, curNextCore.size)){
            if ((i < curCore.cap && curCore.get(i) != null) || (curNextCore.get(i) != null)){
                counter++
            }
        }
        return counter
...
//        _size.getAndSet(initSize)
    ...
    fun casToNull(index: Int, element: E) {
        require(index < size)
        try {
            array[index].compareAndSet(null, CasualValue(element))
        } catch (e: ArrayIndexOutOfBoundsException){
            throw IllegalArgumentException()
        }
    }
