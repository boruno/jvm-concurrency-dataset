...
class DynamicArrayImpl<E : Any> : DynamicArray<E> {
    ...
    override fun put(index: Int, element: E) {
        ...
        file.appendText("after put " + element + " array[index] " + core.value.get(index) + " size " + size + '\n')
    }

    override fun pushBack(element: E) {
        val curCore = core.value
        val newCore = Core<E>(curCore.capacity + 1, size) //тут ли плюсуем сайз
        if (curCore.next.compareAndSet(null, newCore)) {
            ...
            if (newCore.cas(curCore.capacity, null, element)) {
                newCore._size.incrementAndGet()
            }
...
//            pushBack(element)
        ...
        file.appendText("after pushBack " + element + " size " + core.value._size.value + '\n')
    }
...
//    val size: Int = _size.value
    ...
    fun get(index: Int): E {
        require(index < capacity || next.value != null) //?
        if (index >= capacity) {
