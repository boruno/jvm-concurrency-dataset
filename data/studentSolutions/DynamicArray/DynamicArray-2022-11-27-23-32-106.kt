...
class DynamicArrayImpl<E> : DynamicArray<E> {
    ...
    override fun pushBack(element: E) {
        while (true) {
            ...
            if (newCore.tryPushBackWithoutExtending(element))
                break
        }
    }
...
private class Core<E>(
    ...
    fun tryPushBackWithoutExtending(element: E): Boolean {
        ...
        val pushIndex = currentSize + 1

        return if (array[pushIndex].compareAndSet(null, CommonElement(element))) {
            if (!_size.compareAndSet(currentSize, pushIndex))
                error("Failed to cas size from $currentSize to $pushIndex")
            true
        } else false
    }
