...
class DynamicArrayImpl<E> : DynamicArray<E> {
    ...
    override fun put(index: Int, element: E) {
        ...
        while (true) {
            ...
            if (value is Moved) {
                println("Put($index, $element) - Moved")
                val currentCoreNext = currentCore.next.value
                if (currentCoreNext != null) {
                    currentCore = currentCoreNext
                }
            ...
            if (value is Busy<*>) {
                println("Put($index, $element) - Busy")
                currentCore.next.value?.let {
                    move(currentCore, it)
                }
                continue
            ...
            if (currentCore.array[index].compareAndSet(value, element)) {
                println("Put($index, $element) - Norm, [${currentCore.array[0].value}, ${currentCore.array[1].value}]")
                return
            }
            currentCore = core.value
        }
    ...
    private fun move(from: Core<*>, to: Core<*>) {
        for (i in 0 until from.array.size) {
            while (true) {
                ...
                if (from.array[i].compareAndSet(currentValue, busyElement)) {
                    to.array[i].compareAndSet(null, settableValue)
