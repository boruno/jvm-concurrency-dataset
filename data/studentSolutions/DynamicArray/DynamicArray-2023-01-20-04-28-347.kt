...
class DynamicArrayImpl<E> : DynamicArray<E> {
    ...
    private fun ensureCapacity() {
        val curCore = core.value
        val curSize = size
        //if (curCore.capacity <= curSize) {
        curCore.newCore.compareAndSet(null, Core(curCore.capacity * (curSize / curCore.capacity + 1)))
        var index = -1
        while (++index < curCore.array.size) {
            var elFromNext = curCore.newCore.value!!.array[index].value
            var el = curCore.array[index].value

            while (true) {
                if (el is RemovedNode) break
                if (curCore.newCore.value!!.array[index].compareAndSet(elFromNext, el))
                    if (curCore.array[index].compareAndSet(el, RemovedNode()))
                        break
                elFromNext = curCore.newCore.value!!.array[index].value
                el = curCore.array[index].value
            }
