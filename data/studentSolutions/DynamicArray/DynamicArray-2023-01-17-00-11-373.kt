...
class DynamicArrayImpl<E> : DynamicArray<E> {
    ...
    override fun put(index: Int, element: E) {
        ...
        while (true) {
            ...
            if (newCore == null) {
                core.value.array[index].value= CoreElementRecreate(element)
                return
                if (core.value.array[index].compareAndSet(currentValue, CoreElementRecreate(element))) {
                    return
                }
            } else {
                if (newCore.size() > index) {
                    newCore.array[index].value = CoreElementRecreate(element)
                    return
                }
            }
        }
    ...
    override fun pushBack(element: E) {
        ...
        while (true) {
            ...
            if ((coreSize + 1) > currCap) {
                ...
                if (!this.newCore.compareAndSet(null, newCore)) {
                    ...
                    newCore = this.newCore.value ?: continue
                    for (idx in 0 until coreSize) {
                        val currCoreElem = currCore.get(idx)!!
                        val currValue = currCoreElem.putAtomicStatus.value
                        currCore.get(idx)!!.putAtomicStatus.compareAndSet(null, Any())
