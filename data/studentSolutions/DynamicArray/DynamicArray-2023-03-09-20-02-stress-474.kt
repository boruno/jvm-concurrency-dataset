...
private class Core<E>(
    ...
    fun get(index: Int): E {
        require(index < size)
        when (val value = array[index].value as Movement<E>) {
    ...
    fun pushBack(element: E) {
        _size.value = -2
        debug.compareAndSet(debug.value, "start")
        while (true) {
            val localSize = size
            debug.compareAndSet(debug.value, "localSize: $localSize , capacit: $capacity")
            ...
            if (array[localSize].compareAndSet(null, Movement.Still(element))) {
                ...
                _size.compareAndSet(localSize, -1)
