...
class DynamicArrayImpl<E> : DynamicArray<E> {
    ...
    private fun tryResizeArray() {
        var currentCore = core.value
        while (true) {
            if (currentCore.nextList.compareAndSet(null, Core(currentCore.capacity * 2, currentCore.capacity))) {
                    ...
                    while (true) {
                        ...
                        if (currentElement == null)
                        {
                            ...
                            while(initialCore != newCore) {
                                val element = initialCore.get(index) ?: throw Exception("It is not suppoused to happen")
                                if (element.getType() == "Node")
                                {
                                    if(!initialCore.putCas(index, element, NodeFixed(element.value)))
                                        continue;
                                    val fixedElement = currentCore.get(index);
                                    initialCore.nextList.value!!.putCas(index, fixedElement, Node(fixedElement!!.value))
