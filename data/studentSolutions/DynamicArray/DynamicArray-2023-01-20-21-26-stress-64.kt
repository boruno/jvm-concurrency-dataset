...
class DynamicArrayImpl<E> : DynamicArray<E> {
    ...
    override fun pushBack(element: E) {
        while (true) {
            ...
            val index = x.first.size.value
            if (index < x.first.capacity) {
                if (x.first.array[index].compareAndSet(null, element)) {
                    x.first.size.getAndIncrement()
                    return
                }
                x.first.size.compareAndSet(index, index + 1)
                continue
            }
            val newCore = Core(x.first.capacity + 1)
            newCore.size.value = x.first.capacity + 1
            newCore.array[x.first.capacity].value = element
            val newPair = Pair(x.first, newCore)
            if (!core.compareAndSet(x, newPair)) { continue }
            helpExtendSize(newPair, x.first, newCore)
            return
        }
    }
