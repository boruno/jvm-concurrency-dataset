    ...
    override fun put(index: Int, element: E) {
        ...
        while (true) {
            ...
            if (prevElementWrapped != null) {
                ...
                if (prevElementRef.compareAndSet(prevElementRef.value, element)) return
            }
        }
    }
        ...
        if (curArrayState.next.compareAndSet(null, newArrayState)) {
            for (i in 0 until curArrayState.capacity) {
                while (true) {
                    ...
                    if (element != null) {
                        if (newArrayState.array[i].compareAndSet(null, element)) break
                    }
                }
            }
            if (core.compareAndSet(curArrayState, newArrayState)) return
        }
    }
