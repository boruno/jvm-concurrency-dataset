...
class DynamicArrayImpl<E> : DynamicArray<E> {
    ...
    override fun pushBack(element: E) {
        while (true) {
            val curCore = core.value
            val localCurSize = curSize.value
            if (localCurSize < curCore.capacity) {
                if (curCore.array[localCurSize].value != null) {
                    curCore.size.compareAndSet(localCurSize, localCurSize + 1)
                    continue
                }
                if (curCore.array[localCurSize].compareAndSet(null, Element(element))) {
                    curCore.size.compareAndSet(localCurSize, localCurSize + 1)
                    return
                }
            } else {
