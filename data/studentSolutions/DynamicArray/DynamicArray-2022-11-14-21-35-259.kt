...
class DynamicArrayImpl<E> : DynamicArray<E> {
    ...
    override val size: Int get() = publicIndexFactory.current()

    val publicIndexFactory = IndexFactory()
    ...
    override fun pushBack(element: E) {
        return pushBack(element, size)
    }

    fun pushBack(element: E, _proposition: Int) {
        //val i = indexFactory.next()


        var currentCore = core.value
        var proposition = _proposition
        ...
        while (i < 0) {
            if (i == FULL) {
                ...
                //proposition = indexFactory.current()
                proposition = size
                i = currentCore.insertToEnd(proposition, element, this)
            } else if (i == LEAVE_ME_ALONE) {
                proposition++
                i = currentCore.insertToEnd(proposition, element, this)
            }
...
//            if (currentCore.locked[i].compareAndSet(null, false) || currentCore.locked[i].compareAndSet(true, false)) {
...
//            }
        ...
        var currPSize = publicIndexFactory.current()
        val currVal = currentCore.array[i].value
        //if (currentCore.locked[i].compareAndSet(true, false)) {
        if (!currentCore.array[i].compareAndSet(currVal, element)) {
            pushBack(element, i)
            return
        }
        if (!publicIndexFactory.nextCas(currPSize)) {
            pushBack(element, i)
            return
        }
//        } else {
//            return pushBack(element, i + 1)
//        }
...
//                if (locked[i].compareAndSet(true, false)) {
...
//                }
                ...
                if (!c.array[i].compareAndSet(currVal, value)) {
                    return RACE
                }
                //val currVal = c.array[i].value

//                if (i < arr.publicIndexFactory.current()) {
//                    return LEAVE_ME_ALONE
//                }
//
//                if (c.locked[i].value == false) {
//                    return LEAVE_ME_ALONE
//                }
...
//                }
            ...
            if (locked[i].value == null) {
                locked[i].compareAndSet(null, true)
            }
