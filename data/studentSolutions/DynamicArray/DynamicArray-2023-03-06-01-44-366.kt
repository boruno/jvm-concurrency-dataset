        ...
        while (true) {
            ...
            while (true) {
                ...
                if (size == coreValue.capacity) {
                    if (coreValue.next.compareAndSet(null, Core(size * 2))) {
                        coreValue.size.getAndSet(size)
                    }
                    core.compareAndSet(coreValue, coreValue.makeNewTable())
                    break
                } else if (coreValue.array[size].compareAndSet(null, Value(element))) {
