...
class DynamicArrayImpl<E> : DynamicArray<E> {
    ...
    private fun ensureCapacity() {
        val curCore = core.value
        val curSize = size
        if (curCore.capacity > curSize) return
        val resize = max(2, curSize / curCore.capacity + 1)
        curCore.newCore.compareAndSet(null, Core(curCore.capacity * resize))
        (0 until curCore.array.size).forEach {
            ...
            while (true) {
                ...
                if (!curCore.array[it].compareAndSet(el, RemovedNode())) continue
                break
            }
        }
