        ...
        while (true) {
            ...
            if (value is NeedToMove<*>){
                ...
                (0..cor.capacity-1).forEach { num ->
                    var v: Any?
                    do v = cor.array[num].value
                    while (v is NotNeedToMove<*> && !cor.array[num].compareAndSet(v as E?, v.value as E?))
                    if (v is NeedToMove<*>) next.array[num].compareAndSet(null, v.value as E?)
                    if (v is NotNeedToMove<*>) next.array[num].compareAndSet(null, v.value as E?)
                }
                this.core.compareAndSet(cor, next)
            } else {
                if (cor.array[size].compareAndSet(null, element)) {
                    return
                }
            }
        }
