        ...
        while (true) {
            ...
            for (i in 0 until size) {
                if (curCore.array[i].value != null) {
                    while (true) {
                        if (newCore.array[i].compareAndSet(null, curCore.array[i].value)) {
                            break;
                        }
                    }
                }
            ...
            while (true) {
                if (newCore.array[size].compareAndSet(null, element)) {
                    break
                }
            }
            var err = false
            for (i in 0 until size) {
                if (newCore.array[size - 1 - i].value != core.value.array[size - 1 - i].value) {
                   err = true
                }
            }
            if (err) {
