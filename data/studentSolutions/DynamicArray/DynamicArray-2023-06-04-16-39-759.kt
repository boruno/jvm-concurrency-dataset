                ...
                while (index < curCore.size.value) {
                    when (val curElement = curCore.array[index].value) {
                        is Frozen -> {
                            ...
                            if (curCore.array[index].compareAndSet(curElement, Frozen(curElement!!))) {
                                nextCore.array[index].compareAndSet(null, curElement)
                                index++
                                continue
                            }
                            else continue
    ...
    fun set(index: Int, element: E) {
        while (true) {
            ...
            when (val curCellValue = curCore.array[index].value) {
                is Frozen -> {
                    val nextCore = curCore.next.value!!
                    if (curCore.array[index].compareAndSet(curCellValue, Frozen(element))) {
                        nextCore.array[index].compareAndSet(curCellValue.element, element)
                    } else continue
                    /*var copyIndex = 0
