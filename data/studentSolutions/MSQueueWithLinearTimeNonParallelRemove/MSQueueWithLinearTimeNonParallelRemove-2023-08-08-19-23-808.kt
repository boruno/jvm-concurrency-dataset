...
//        }
    ...
    private inner class Node(
        ...
        fun remove(): Boolean {
            ...
            var curHead: Node? = head.get()
            while (curHead != null) {
                var next = curHead.next.get()
                if (curHead == this || curHead.extractedOrRemoved) {
                    curHead = curHead.next.get()
                    continue
                }
                if (curHead.next.compareAndSet(this, this.next.get())
                    || curHead.next.compareAndSet(null, this.next.get())) {
                    return removed
                }
                curHead = next
            }
