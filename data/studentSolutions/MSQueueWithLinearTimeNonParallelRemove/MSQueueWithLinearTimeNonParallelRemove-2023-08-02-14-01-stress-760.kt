...
class MSQueueWithLinearTimeNonParallelRemove<E> : QueueWithRemove<E> {
    ...
    private inner class Node(
        ...
        fun remove(): Boolean {
            ...
            // is it a tail?
            if (currentNext == null) return removed
            val previous = findPrevious()
            previous.next.value = currentNext // no concurrency issue in this implem, so no CAS
            return removed
        }
        ...
        fun findPrevious(): Node {
            ...
            while (true) {
                val next = node.next.value
                if (next == null) error("should not have reached tail")
