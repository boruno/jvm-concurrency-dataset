...
class MSQueueWithLinearTimeNonParallelRemove<E> : QueueWithRemove<E> {
    ...
    override fun dequeue(): E? {
        ...
        while(true) {
            ...
            if (curHeadNext.extractedOrRemoved) continue

            if (head.compareAndSet(curHead, curHeadNext)) {
                val element = curHeadNext.element
                if (curHeadNext.markExtractedOrRemoved()) return element else continue
            }
        }
    }
    ...
    private inner class Node(
        ...
        fun remove(): Boolean {
            ...
            val removed = if (extractedOrRemoved) false else markExtractedOrRemoved()

            val curHead = head.get()
            val curTail = tail.get()
            if (extractedOrRemoved && this != curHead && this != curTail) {
                val curPrev = previous()
                val curNext = next
                if (curPrev?.next?.compareAndSet(this, curNext.get()) == true) {
                    next.set(null)
                    element = null
                }
            }
