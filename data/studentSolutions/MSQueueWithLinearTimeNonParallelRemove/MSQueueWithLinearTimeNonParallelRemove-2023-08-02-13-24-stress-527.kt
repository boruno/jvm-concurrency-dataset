...
class MSQueueWithLinearTimeNonParallelRemove<E> : QueueWithRemove<E> {
    ...
    override fun enqueue(element: E) {
        ...
        while (true) {
            ...
            if (curTail == tail.value) {
                if (curNext == null) {
                    if (curTail.next.compareAndSet(curNext, newNode)) {
    ...
    private inner class Node(
        ...
        fun remove(): Boolean {
            ...
            val removed = _extractedOrRemoved.compareAndSet(false, true)
            if (removed) {
                val next = next.value
                if (next != null) {
                    next.next.compareAndSet(next, null)
