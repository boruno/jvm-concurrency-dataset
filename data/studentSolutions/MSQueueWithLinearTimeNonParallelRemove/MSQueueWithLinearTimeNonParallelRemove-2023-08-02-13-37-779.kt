...
class MSQueueWithLinearTimeNonParallelRemove<E> : QueueWithRemove<E> {
    ...
    override fun enqueue(element: E) {
        ...
        while (true) {
            ...
            if (currentTail.next.compareAndSet(null, newNode)) {
                ...
                val myHead = head.value
                if (currentTail.extractedOrRemoved && myHead != currentTail) {
                    val prevNode = currentTail.findPreviousNode(head.value)
                    if (prevNode != null) {
                        prevNode.next.value = newNode
                    }
                }
                return
            } else {
    ...
    private inner class Node(
        ...
        fun remove(): Boolean {
            ...
            if (markExtractedOrRemoved()) {
                if (head.value != this && tail.value != this){
                    val prevNode = findPreviousNode(head.value)
                    if (prevNode != null) {
                        prevNode.next.value = next.value
                    }
                }
                return true
            }
        ...
        fun findPreviousNode(myHead: Node): Node? {
            var node = myHead
            while (true) {
                if (node.next.value == this) return node
                node = node.next.value ?: break
            }
            return null
        }
