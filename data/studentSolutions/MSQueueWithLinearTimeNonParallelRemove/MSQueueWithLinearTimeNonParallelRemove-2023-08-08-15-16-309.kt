...
class MSQueueWithLinearTimeNonParallelRemove<E> : QueueWithRemove<E> {
    ...
    override fun enqueue(element: E) {
        ...
        while (true) {
            if (tail.get().extractedOrRemoved) {
                if (tail.compareAndSet(this.tail.get(), tail.get().next.get()))
                    break
            }
        }
        while (true) {
    ...
    private fun findPrev(search: Node): Node? {
        var node = head.get()
        while(true) {
            if (node == null) return null
            if (node.element == search) {
                return node
            }
            node = node.next.get()
        }
    ...
    private inner class Node(
        ...
        fun remove(): Boolean {
            ...
            val removed = this.markExtractedOrRemoved()
            val curPrev = findPrev(this)
            if (curPrev == null || this.next.get() == null) {
                return true
            }
            val curNext = this.next
            curPrev.next.compareAndSet(curPrev.next.get(), curNext.get())
            return removed
        }
    }
}
