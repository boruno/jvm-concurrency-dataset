...
//                    val currElement = currTail.element
//                    val prev = currTail.findPreviousNode(currTail.element)
                    val prev = currTail.findPreviousNode(currTail)
                    prev?.next?.set(currTail)
                }
                return
            } else {
    ...
    private inner class Node(var element: E?) {
        ...
        fun remove(): Boolean {
            ...
            if (/*this != headNode &&*/ this != tailNode) {
                val previousNode = findPreviousNode(this) ?: return false
...
//        fun findPreviousNode(element: E?): Node? {
//            var node = head.get()
//            while (true) {
...
//        }
        ...
        fun findPreviousNode(findNode: Node): Node? {
            var node = head.get()
            while (true) {
                val next = node.next.get() ?: return null
