...
class MSQueueWithLinearTimeNonParallelRemove<E> : QueueWithRemove<E> {
    ...
    private inner class Node(
        ...
        fun remove(): Boolean {
            ...
            val markRes = markExtractedOrRemoved()
            var prevNode = head.get()
            while (prevNode != null && prevNode.next.get() != this) prevNode = prevNode.next.get()
            if (prevNode != null) prevNode.next.set(this.next.get())
            return markRes
        }
    }
}
