...
class MSQueueWithLinearTimeNonParallelRemove<E> : QueueWithRemove<E> {
    ...
    override fun enqueue(element: E) {
        while (true) {
            ...
            if (next.compareAndSet(null, node)) {
                if (!tail.compareAndSet(curTail, node)) continue
                return
            } else {
                val value = next.value ?: continue
    ...
    private inner class Node(
        ...
        fun remove(): Boolean {
            ...
            val previousNode = findPreviousNode() ?: return true
            val value = next.value
            previousNode.next.compareAndSet(this, value)
            next.compareAndSet(value, null)
            return true
        ...
        private fun findPreviousNode(): Node? {
            ...
            while (currentNode != null) {
                val next = currentNode.next.value
                if (next == this) break
                currentNode = next
            }
