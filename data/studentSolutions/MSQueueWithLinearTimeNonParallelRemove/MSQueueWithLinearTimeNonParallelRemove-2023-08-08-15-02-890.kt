...
class MSQueueWithLinearTimeNonParallelRemove<E> : QueueWithRemove<E> {
        ...
        while (true) {
            ...
            if (curTail.next.compareAndSet(null, node)) {
                if (curTail.extractedOrRemoved) {
                    var prev = head.get()
                    while (prev.next.get() != curTail && prev.next.get() != null) prev = prev.next.get()
                    prev.next.set(node)
                }
    ...
    private inner class Node(
        ...
        fun remove(): Boolean {
            ...
            if (shouldBeRemoved) {
                if (this == head.get() || this == tail.get()) return true
                var prev = head.get()
                while (prev.next.get() != this && prev.next.get() != null) prev = prev.next.get()
