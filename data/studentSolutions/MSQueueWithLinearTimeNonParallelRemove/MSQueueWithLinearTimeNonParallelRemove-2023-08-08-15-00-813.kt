    ...
    override fun enqueue(element: E) {
        ...
        while (true) {
            ...
            if (curTail.extractedOrRemoved) {
                val prevTail = tail.get().findPrev()
                if (tail.compareAndSet(curTail, prevTail)) curTail.remove()
            }
    ...
    private inner class Node(
        ...
        fun remove(): Boolean {
            ...
            if (this.markExtractedOrRemoved()) {
                if (this == tail.get() || this == head.get()) return true

                val curPrev = this.findPrev()
                val curNext = this.next
                val prevNext = curPrev?.next

                return curPrev?.next?.compareAndSet(prevNext?.get(), curNext.get()) ?: false
            } else return false
        }
        ...
        fun findPrev(): Node? {
            ...
            while (current != tail.get()) {
                if (current.next.get() == this) return current.next.get()
