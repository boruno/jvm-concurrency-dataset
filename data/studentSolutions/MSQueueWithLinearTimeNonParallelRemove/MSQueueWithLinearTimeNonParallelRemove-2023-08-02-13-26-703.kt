...
class MSQueueWithLinearTimeNonParallelRemove<E> : QueueWithRemove<E> {
    ...
    override fun enqueue(element: E) {
        ...
        while (true) {
            ...
            val curTail = tail
            val curTailValue = curTail.value
            if (curTailValue.extractedOrRemoved == true) {
                curTailValue.remove()
                continue
            }
            if (curTailValue.next.compareAndSet(null, node)) {
                tail.compareAndSet(curTailValue, node)
                return
            } else {
                val curTailNext = curTailValue.next
                val curTailNextValue = curTailNext.value ?: continue
                tail.compareAndSet(curTailValue,  curTailNextValue)
            }
        }
    }
    ...
    override fun dequeue(): E? {
        ...
        while(true) {
            ...
            val curHeadNext = curHeadValue.next
            val curHeadNextValue = curHeadNext.value ?: return null
            if (head.compareAndSet(curHeadValue, curHeadNextValue)) {
                if (curHeadNextValue.markExtractedOrRemoved()) {
                    return curHeadNextValue.element
                }
            }
        }
    }
    ...
    private inner class Node(
        ...
        fun remove(): Boolean {
            ...
            val wasMarked = markExtractedOrRemoved()
            var cur = head.value
            while (true) {
                if (cur.next.value == this) {
                    cur.next.update { this.next.value }
                    return wasMarked
                } else {
                    cur = cur.next.value ?: return wasMarked
                }
            }
        }
    }
