...
class MSQueueWithLinearTimeNonParallelRemove<E> : QueueWithRemove<E> {
    ...
    override fun dequeue(): E? {
        ...
        while (true) {
            ...
            if (newHead != null && newHead.extractedOrRemoved) {
                return null
            }
            if (curHead == curTail) {
                if (newHead == null)
                ...
                if (curHead == head.get() && head.compareAndSet(curHead, newHead)) {
...
//            return result
            ...
            if (prevNode == head.get() && nextNode != null && !nextNode.extractedOrRemoved) {
                head.compareAndSet(this, nextNode)
            }

            if (this == tail.get() && nextNode == null) {
                tail.compareAndSet(this, head.get())
            }
