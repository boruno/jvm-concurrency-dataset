...
class MSQueueWithLinearTimeNonParallelRemove<E> : QueueWithRemove<E> {
    ...
    override fun enqueue(element: E) {
        ...
        while (true) {
            ...
            if (tailNode.next.compareAndSet(null, el)) {
                ...
                if (tailNode.extractedOrRemoved && tailNode != head.get()) {
                    findPrev(tailNode)?.next?.compareAndSet(tailNode, el)
                }
    ...
    override fun dequeue(): E? {
        ...
        while (true) {
            ...
            val first = headNode.next.get() ?: return null
            if (head.compareAndSet(headNode, first)) {
                if (first.markExtractedOrRemoved()) return first.element
            }
        }
    }
    ...
    private inner class Node(
        ...
        fun remove(): Boolean {
            ...
            if (markExtractedOrRemoved()) {
                // remove tail only logically
                if (next.get() == null) return true

                var prevNode = head.get()
                // remove head only logically
                if (prevNode == this) return true

                while (prevNode.next.get() != this) {
                    prevNode = prevNode.next.get()!!
                }

                prevNode.next.compareAndSet(this, next.get())
                return true
            }
    ...
    private fun findPrev(node: Node): Node? {
        var prevNode = head.get()
        while (true) {
            if (prevNode.next.get() == null) return null
            if (prevNode.next.get()!! == node) return prevNode
            prevNode = prevNode.next.get()!!
        }
    }
}
