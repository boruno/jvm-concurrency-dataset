...
class MSQueueWithLinearTimeNonParallelRemove<E> : QueueWithRemove<E> {
    ...
    override fun enqueue(element: E) {
        ...
        while (true) {
            ...
            if (curTail.next.compareAndSet(null, node)) {
                ...
                if (curTail.markExtractedOrRemoved()) curTail.removePhysically()
                return
            }
            else {
    ...
    private inner class Node(
        ...
        fun remove(): Boolean {
            ...
            val wasExtractedOrRemoved = markExtractedOrRemoved()
            // no need to check wasExtractedOrRemoved
            if (wasExtractedOrRemoved) return removePhysically()
            return false
        }

        fun removePhysically(): Boolean {
            val curNext = this.next.value
            if (curNext == null) return true
            val prev = findPrev()
        ...
        private fun findPrev(): Node? {
            ...
            val curTail = tail.value
            if (curNode == curTail) return null
            while (true) {
