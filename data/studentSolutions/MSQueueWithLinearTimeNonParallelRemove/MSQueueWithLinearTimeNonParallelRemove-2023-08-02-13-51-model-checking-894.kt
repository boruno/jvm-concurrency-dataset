...
class MSQueueWithLinearTimeNonParallelRemove<E> : QueueWithRemove<E> {
    ...
    override fun enqueue(element: E) {
        ...
        while (true) {
            ...
            if (currentTail.extractedOrRemoved) {
                val newNext = currentTail.next.value
                if (newNext != null)
                    setPreviousNext(currentTail, newNext)
                return
    ...
    private fun setPreviousNext(target: Node, newNext: Node) {
        var node = head.value
        while(true) {
            val next = node.next.value
            when(next) {
                target -> {
                    node.next.compareAndSet(target, newNext)
                    return
                }
                null -> return
                else -> node = next
