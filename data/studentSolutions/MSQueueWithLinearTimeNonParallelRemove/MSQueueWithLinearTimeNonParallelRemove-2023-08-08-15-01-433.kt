...
class MSQueueWithLinearTimeNonParallelRemove<E> : QueueWithRemove<E> {
    ...
    private fun findPrev(curNode: Node): Node? {
        var node = head.get()
        while (node != null) {
            val next = node.next.get()
            if (next == null) return null
            if (next.element == curNode.element) return node
            node = next
        }
        return null
    }

    private fun removePhysically(curNode: Node): Boolean {
    ...
    private inner class Node(
        ...
        fun remove(): Boolean {
            ...
            val prevNode = findPrev(this) ?: return removed
