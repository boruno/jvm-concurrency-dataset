...
//            if (oldTail.value.extractedOrRemoved)
//                remove(oldTail.value.element!!)
        }
    }
    ...
    override fun dequeue(): E? {
        ...
        while (true) {
            ...
            if (head.compareAndSet(curHead, curNextHead)) {
                if (curNextHead.markExtractedOrRemoved()) {
                    return curNextHead.element
                }
            }
        }
    }
    ...
    private inner class Node(
         ...
         */
        fun remove(): Boolean {
            val isSuccess = markExtractedOrRemoved()

            val curNext = next.value
            if (curNext == null) return isSuccess

            val prev = findPrevNode()
            if (prev != null) prev.next.value = curNext
    ...
    private fun Node.findPrevNode1(): Node {
        var curNode = head.value

        while (true) {
            if (curNode.next.value == this)
                break
    ...
    private fun Node.findPrevNode(): Node? {
        var curNode = head.value
        while (true) {
            val curNodeNext = curNode.next.value
            if (curNodeNext == null) return null
            if (curNodeNext == this) return curNodeNext
