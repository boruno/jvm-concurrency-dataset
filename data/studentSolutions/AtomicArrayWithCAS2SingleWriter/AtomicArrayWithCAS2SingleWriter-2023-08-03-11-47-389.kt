...
class AtomicArrayWithCAS2SingleWriter<E : Any>(size: Int, initialValue: E) {
    ...
    inner class CAS2Descriptor(
        ...
        fun apply(): Boolean {
            ...
            // create functions for each of these three phases.
            // In this task, only one thread can call cas2(..),
            // so cas2(..) calls cannot be executed concurrently.

            // Phase 1: Install the descriptor
            if (!array[index1].compareAndSet(expected1, this)) {
                status.value = FAILED
                return false
            }
            if (!array[index2].compareAndSet(expected2, this)) {
                // Failed to install the descriptor in the second cell,
                // need to roll back the first cell.
                array[index1].compareAndSet(this, expected1)
                status.value = FAILED
                return false
            }

            // Phase 2: Update the status
            status.value = SUCCESS

            // Phase 3: Update the cells
