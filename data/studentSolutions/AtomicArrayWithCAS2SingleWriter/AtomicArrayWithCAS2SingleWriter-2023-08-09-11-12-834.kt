...
class AtomicArrayWithCAS2SingleWriter<E : Any>(size: Int, initialValue: E) {
    ...
    fun get(index: Int): E {
        ...
        return when (val value = array[index]) {
            is AtomicArrayWithCAS2SingleWriter<*>.CAS2Descriptor -> when (value.status.get()) {
                UNDECIDED, FAILED -> if (index == value.index1) value.expected1 else value.expected2
    ...
    inner class CAS2Descriptor(
        ...
        private fun install(): Boolean {
            if (array.compareAndSet(index1, expected1, update1)) {
                if (array.compareAndSet(index2, expected2, update2)) {
                    return true
                } else {
