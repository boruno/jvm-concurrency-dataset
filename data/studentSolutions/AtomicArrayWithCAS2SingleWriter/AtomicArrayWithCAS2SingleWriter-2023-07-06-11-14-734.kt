        ...
        if (value is AtomicArrayWithCAS2SingleWriter<*>.CAS2Descriptor) {
            ...
            return if (status == SUCCESS) {
                if (index == value.index1) {
                    array[index].compareAndSet(value, value.update1)
                    value.update1 as E
                } else {
                    array[index].compareAndSet(value, value.update2)
                    value.update2 as E
                }
            } else {
                if (index == value.index1) {
                    array[index].compareAndSet(value, value.expected1)
                    value.expected1 as E
                } else {
                    array[index].compareAndSet(value, value.expected2)
