...
class AtomicArrayWithCAS2SingleWriter<E : Any>(size: Int, initialValue: E) {
    ...
    fun cas2(
        ...
        if (array[index1].compareAndSet(expected1, ds) &&
            array[index2].compareAndSet(expected2, ds)) {
            ds.status.compareAndSet(UNDECIDED, SUCCESS)
            array[index1].compareAndSet(ds, update1)
            array[index2].compareAndSet(ds, update2)
            return true
        }
        return false
    }
