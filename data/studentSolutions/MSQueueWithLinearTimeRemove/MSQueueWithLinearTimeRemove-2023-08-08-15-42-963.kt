...
class MSQueueWithLinearTimeRemove<E> : QueueWithRemove<E> {
    ...
    override fun enqueue(element: E) {
        ...
        while (true) {
            ...
            if (nextRef.compareAndSet(null, newNode)) {
                ...
                tail.compareAndSet(tail.get(), nextRef.get())
            }
        }
    ...
    private inner class Node(
        ...
        fun removePhysically() {
            if (this == head.get() || this == tail.get()) return
            ...
            while (true) {
                ...
                if (next == null) return
                if (next == this) {
                    val target = next.next.get()
                    node.next.compareAndSet(next, target)
                    if (target != null && target.extractedOrRemoved) {
                        target.removePhysically()
                    }
                    return
                }
                node = next
