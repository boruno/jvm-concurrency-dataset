...
class MSQueueWithLinearTimeRemove<E> : QueueWithRemove<E> {
    ...
    private inner class Node(
        ...
        fun remove(): Boolean {
            ...
            while (true) {
                val next = node.next
                val afterNext = next.get()?.next ?: return removed
                if (afterNext.get() == null) return removed
                if (next.get()?.element == element && next.get()?.extractedOrRemoved == true) {
                    next.set(afterNext.get())
                    if (afterNext.get()?.extractedOrRemoved == true) {
                        afterNext.get()?.remove()
                    }
                    return removed
                }
                node = next.get()
