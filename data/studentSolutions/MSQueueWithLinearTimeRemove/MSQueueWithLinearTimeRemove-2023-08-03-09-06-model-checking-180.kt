...
//    }
    ...
    private fun physicallyRemove(target: Node): Boolean {
        var node = head.value
        while(true) {
            when(val next = node.next.value) {
                target -> return next.next.value?.let {
                    node.next.compareAndSet(target, next.next.value)
                } == true
                null -> return false
                else -> node = next
            }
