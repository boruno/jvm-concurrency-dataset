...
class MSQueueWithLinearTimeRemove<E> : QueueWithRemove<E> {
    ...
    override fun enqueue(element: E) {
        ...
        while (true) {
            ...
            if (currentTailRef.next.compareAndSet(null, newNode)) {
                ...
                if (currentTailRef.extractedOrRemoved) {
                    currentTailRef.remove()
                }
                return
            }
            else {
    ...
    override fun dequeue(): E? {
        ...
        while (true) {
            ...
            if (head.compareAndSet(headRef, headRefNext)) {
                if (headRefNext.markExtractedOrRemoved()) {
                    return headRefNext.element.also {
                        headRefNext.element = null
                    }
                }
            }
        }
    ...
    private inner class Node(
        ...
        fun remove(): Boolean {
            ...
            return markExtractedOrRemoved().also {
                ...
                if (head.get() != this && tail.get() != this) {
                    val prev = findPrev() ?: return@also
                    if (curNext?.extractedOrRemoved == true) {
                        curNext.remove()
                    }
                    prev.next.set(curNext)
                }
            }
        }
        ...
        private fun findPrev(): Node? {
            var curr = head.get()
            if (curr.next.get() == null) return null
            while (curr.next.get() !== this) {
                curr = curr.next.get() ?: return null
                if (curr === tail.get()) return null
                if (curr.next.get() == null) return null
            }
            return curr
        }
