...
class MSQueueWithLinearTimeRemove<E> : QueueWithRemove<E> {
    ...
    override fun enqueue(element: E) {
        ...
        while (true) {
            ...
            if (curTail == tail.value) {
                if (next == null) {
                    if (curTail.next.compareAndSet(next, newNode)) {
                        if (newNode.extractedOrRemoved) return
                        tail.compareAndSet(curTail, newNode)
                        if(curTail.extractedOrRemoved){
                            removePhysically(curTail)
                        }
                        return
                    }
                } else {
    ...
    private fun removePhysically(node: Node) {
        var prev = head.value
        while (true) {
            val next = prev.next.value ?: return
            if (next == node) {
                prev.next.compareAndSet(next, node.next.value)
                val curNext = prev.next.value
                if (prev.extractedOrRemoved) removePhysically(prev)
                if (curNext != null && curNext.extractedOrRemoved) {
                    removePhysically(curNext)
                }
                return
            }
            prev = next
    ...
    private inner class Node(
        ...
        fun remove(): Boolean {
            ...
            if (this.next.value == null || this == head.value) return true
            removePhysically(this)
            return true
        }
    }
}
