...
//            if (current.extractedOrRemoved && current != head.value) {
//                val prev = findLastNodeNotMe(current)
//                prev.next.compareAndSet(current, current.next.value)
//                tail.compareAndSet(current, prev)
//                continue
//            }
            ...
            if (current?.extractedOrRemoved == true && current != head.value)
            {
                val currPrev = current.findPrev()
                val currNext = current.next.value
                if (currPrev != null)
                    currPrev.next.compareAndSet(current, currNext)
            }
        }
    }

    private fun findLastNodeNotMe(me: Node): Node {
        var current: Node = head.value
        while (true) {
            val next = current.next.value
            current = if (next != null && next != me) next else return current
        }
    }
...
//                    tail.compareAndSet(this, currPrev)
            ...
            if (currPrev != null && currPrev.extractedOrRemoved)
                currPrev.remove()
