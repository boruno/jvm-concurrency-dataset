...
class MSQueueWithLinearTimeRemove<E> : QueueWithRemove<E> {
    ...
    private inner class Node(
        ...
        fun remove(): Boolean {
            ...
            var currentNext = next.get()
            while (currentNext?.extractedOrRemoved == true) {
                currentNext.remove()

                currentNext = currentNext.next.get()
            }
        ...
        fun findPreviousNode(): Node? {
            ...
            var nextNode = current.next.get()

            while (nextNode != null) {
                if (nextNode == this) return current
                current = nextNode
