...
class MSQueueWithLinearTimeRemove<E> : QueueWithRemove<E> {
        ...
        while(true) {
            ...
            if (curTail.next.compareAndSet(null, node)) {
                ...
                if (curTail.extractedOrRemoved) {
                    ...
                    while(true) {
                        i += 1
                        if (prev.next.compareAndSet(curTail, curTail.next.value)) {
                            break
                        }
                        prev = prev.next.value ?: throw Exception(prev.extractedOrRemoved.toString() + curTail.element.toString() + prev.element.toString() + (curTail == prev).toString())
                    }
            ...
            if (curTail.extractedOrRemoved) {
                var prev = head.value
                while(true) {
                    if (prev.next.value == curTail) {
                        break
                    }
                    prev = prev.next.value ?: throw Exception(this.toString())
                }
                prev.next.value = curTail.next.value
                if (prev.extractedOrRemoved) prev.remove()
            }
        }
    }
    ...
    private inner class Node(
        ...
        fun remove(): Boolean {
            ...
            if (markExtractedOrRemoved()) return true
            if (this != head.value && this != tail.value) {
                var prev = head.value
                while(prev.next.value != this) {
                    prev = prev.next.value ?: return false
                }
