...
class MSQueueWithLinearTimeRemove<E> : QueueWithRemove<E> {
    ...
    private inner class Node(
        ...
        fun deleteNode(): Boolean {
            check (this == tail.value || this == head.value || this.next.value == null) { return true }
            ...
            val previousNode = findPrevious() ?: return true
            val nextNode = next.value
            previousNode.next.getAndSet(nextNode)
            check(previousNode.extractedOrRemoved) { previousNode.deleteNode() }
            check (nextNode != null && nextNode.extractedOrRemoved) { nextNode!!.deleteNode() }
            return true
        }
    }
}
