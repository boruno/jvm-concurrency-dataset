...
class MSQueueWithLinearTimeRemove<E> : QueueWithRemove<E> {
    ...
    private inner class Node(
        ...
        fun remove(): Boolean {
            ...
            if (this.next.value == null || this == head.value) {
                return res
            }

            val curPrev = prev()
            if (curPrev == null) return res
            val curNext = next
            curPrev.next.compareAndSet(this, curNext.value)

            if (curPrev.extractedOrRemoved) {
                curPrev.remove()
            }
            if (curNext.value!!.extractedOrRemoved) {
                curNext.value!!.remove()
            }
            return res
        }
        ...
        fun prev(): Node? {
            var cur = head.value
//            while (cur.next.value != null && cur.next.value != this) {
//                cur = cur.next.value!!
//            }
