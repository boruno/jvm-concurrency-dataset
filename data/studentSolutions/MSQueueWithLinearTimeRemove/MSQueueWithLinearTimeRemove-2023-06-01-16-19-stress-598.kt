...
class MSQueueWithLinearTimeRemove<E> : QueueWithRemove<E> {
    ...
    override fun dequeue(): E? {
        ...
        while (true) {
            ...
            if (head.compareAndSet(expect = curHead, update = curHeadNext) && curHeadNext.markExtractedOrRemoved()) {
    ...
    private inner class Node(
        ...
        fun removePhysically() {
            while (true) {
                ...
                if (prev == null) return
                // Don't delete the last element in list
                if (this.next.value == null) return

                if (prev.next.compareAndSet(expect = this, update = next.value)) {
                    return
                }
