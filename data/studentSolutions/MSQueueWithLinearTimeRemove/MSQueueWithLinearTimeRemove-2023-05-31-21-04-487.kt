...
class MSQueueWithLinearTimeRemove<E> : QueueWithRemove<E> {
    ...
    private inner class Node(
        ...
        fun remove(): Boolean {
            ...
            // try to remove physycally
            val currPrev = findPrev()
            if (currPrev == null) return false
        ...
        fun findPrev(): Node? {
            var res: Node = head.value

            // if there is no prev
            if (res == this) {
                return null
            }
            while (res.next.value != this && res.next.value != null) {
                if (res.next.value!!.extractedOrRemoved) {
                    res.next.value!!.remove()
                    continue
                }
                res = res.next.value!!
            }
            return if (res.next.value == this) res else null
        }
    }
}
