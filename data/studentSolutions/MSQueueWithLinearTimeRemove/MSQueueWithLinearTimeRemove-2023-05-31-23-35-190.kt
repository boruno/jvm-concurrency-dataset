...
class MSQueueWithLinearTimeRemove<E> : QueueWithRemove<E> {
    ...
    private inner class Node(
        ...
        fun removePhysic() {
            while (true) {
                val prevNode = getPrev() ?: return
                val nextNode = getNext() ?: return

                if (prevNode.extractedOrRemoved) {
                    prevNode.removePhysic()
                    continue
                }
                if (nextNode.extractedOrRemoved) {
                    nextNode.removePhysic()
                    continue
                }
        ...
        private fun getPrev(): Node? {
            ...
            var nextAfterPrevNode = prevNode.next.value
            while (nextAfterPrevNode !== this) {
                if (nextAfterPrevNode == null) return null
                if (nextAfterPrevNode.extractedOrRemoved) break
                prevNode = nextAfterPrevNode
                nextAfterPrevNode = prevNode.next.value
            }
            return prevNode
        }

        private fun getNext(): Node? {
            var nextNode = next.value ?: return null
            while (nextNode.extractedOrRemoved) {
                nextNode = nextNode.next.value ?: return null
            }
            return nextNode
        }
    }
