...
class MSQueueWithLinearTimeRemove<E> : QueueWithRemove<E> {
    ...
    override fun enqueue(element: E) {
        ...
        while (true) {
            ...
            if (curTail.next.compareAndSet(null, node)) {
                if (tail.compareAndSet(curTail, node)) //note(vg): if we fail here, it's ok, it means that some other thread moved the tail pointer for us
                    if (curTail.extractedOrRemoved) {
                        val curPrev = curTail.findPrevious()
                        val curNext = curTail.next.get()
                        curPrev?.next?.set(curNext)
                        if (curNext?.extractedOrRemoved == true && curNext != head.get() && curNext != tail.get())
                            curNext.remove()
                    }
                return
            } else {
                if (tail.compareAndSet(curTail, curTail.next.get())) //note(vg): help another thread to move the tail pointer
                    if (curTail.extractedOrRemoved) {
                        val curPrev = curTail.findPrevious()
                        val curNext = curTail.next.get()
                        curPrev?.next?.set(curNext)
    ...
    private inner class Node(
        ...
        fun findPrevious(): Node? {
            var previous = head.get() //note(vg): we consider that head has no value actually
            var node = previous.next.get()
            while (true) {
...
//            if (curPrev != null) {
//                curPrev.next.set(curNext)
//                if (curNext != null && curNext.extractedOrRemoved && curNext != head.get() && curNext != tail.get())
//                    curNext.remove()
//            }
//        }
