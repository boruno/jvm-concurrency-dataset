...
class MSQueueWithLinearTimeRemove<E> : QueueWithRemove<E> {
    ...
    private inner class Node(
        ...
        fun remove(): Boolean {
            ...
            val curretNext = this.next.get()
            while (true) {
                val next = curHead.next.get()
                if (next == null) {
                    break
                }
                if (curHead.next.compareAndSet(this, curretNext)) {
                    if (curretNext != null && curretNext.extractedOrRemoved) {
                        curretNext.remove()
                    }
                    return removed
                }
            ...
            if (curHead != tail.get()) {
                ...
                curHead.next.compareAndSet(null, curretNext)
            }
            return removed
        }
    }
