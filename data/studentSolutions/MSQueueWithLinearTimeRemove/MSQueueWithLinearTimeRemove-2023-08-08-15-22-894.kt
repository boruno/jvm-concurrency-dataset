...
class MSQueueWithLinearTimeRemove<E> : QueueWithRemove<E> {
    ...
    private inner class Node(
        ...
        fun remove(): Boolean {
            ...
            while (currentNode != null && currentNode != this && currentNode != currentTail) {
                val nextNode = currentNode.next.get()

                // physically, updating the `next` field of the previous
                // node to `this.next.value`.
                if (nextNode == this) {
                    val newNextNode = this.next.get()

                    if (newNextNode != null) { // check if tail or not
                        currentNode.next.compareAndSet(this, newNextNode)
                    }
                    break
                }
