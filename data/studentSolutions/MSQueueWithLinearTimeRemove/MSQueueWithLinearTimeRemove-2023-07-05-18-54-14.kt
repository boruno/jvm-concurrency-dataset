...
class MSQueueWithLinearTimeRemove<E> : QueueWithRemove<E> {
    ...
    private fun actualizeTail() {
        while (true) {
            ...
            val next = tailCandidate.next.value
            if (next == null) {
                // We've found tail!
                return
            }
            if (tail.compareAndSet(tailCandidate, next)) {
                // Aha, we moved tail forward. What if our last known tail wanted to be removed, though?
    ...
    private inner class Node(
        ...
        private fun erase() {
            ...
            if (this === this@MSQueueWithLinearTimeRemove.tail.value)
                return
            val prev = prev() ?: return // If we can't find the previous node, this is already erased
            val next = next.value
