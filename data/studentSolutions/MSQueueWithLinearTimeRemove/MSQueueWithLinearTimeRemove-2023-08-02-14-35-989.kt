...
class MSQueueWithLinearTimeRemove<E> : QueueWithRemove<E> {
    ...
    override fun enqueue(element: E) {
        ...
        while (true) {
            ...
            if (curTail.next.compareAndSet(null, node)) {
                ...
                val curTailNextValue = curTail.next.value ?: continue
                tail.compareAndSet(curTail,  curTailNextValue)
            }
        }
    }
    ...
    private inner class Node(
        ...
        fun removePhysically() {
            // only remove tail when adding a new element
            if (this == tail.value || this.next.value == null) return

            var cur = head.value
            while (true) {
                if (cur.next.value == this) {
                    cur.next.update { this.next.value } // prev.next.value = curNext
                    return
                } else {
