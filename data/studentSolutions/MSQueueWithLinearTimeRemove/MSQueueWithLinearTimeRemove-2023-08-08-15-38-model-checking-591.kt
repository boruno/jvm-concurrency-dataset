...
class MSQueueWithLinearTimeRemove<E> : QueueWithRemove<E> {
    ...
    fun printQueue() {
        println("The whole queue")
        var node = head.get().next.get()
        while (node != null) {
        ...
        while (true) {
            ...
            if (currentTail.next.compareAndSet(null, newNode)) {
                ...
                if (currentTail.extractedOrRemoved) {
                    currentTail.tryPhysicallyRemove()
                }
                return
            } else {
    ...
    override fun dequeue(): E? {
        while (true) {
            ...
            val newHead = currentHead.next.get() ?: return null
            val elementToReturn = newHead.element
    ...
    private inner class Node(
         ...
         */
        fun remove(markAsRemoved: Boolean = true): Boolean {
        ...
        fun tryPhysicallyRemove() {
            this.element = null
            val prevNode = findPredecessor() ?: return
            val nextNode = this.next.get() ?: return
            prevNode.next.compareAndSet(this, nextNode)
        }

        fun findPredecessor(): Node? {
            var node = head.get()
            while (true) {
                val next = node.next.get()
