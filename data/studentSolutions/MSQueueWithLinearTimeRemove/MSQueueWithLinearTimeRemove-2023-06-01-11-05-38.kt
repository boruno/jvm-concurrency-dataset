...
class MSQueueWithLinearTimeRemove<E> : QueueWithRemove<E> {
    ...
    override fun dequeue(): E? {
        while (true) {
            ...
            // TODO not forget to handle with tail
        ...
        while (true) {
            if (node !== head.value && node !== tail.value) {
                check(!node.extractedOrRemoved) {
                    "Removed node with element ${node.element} found in the middle of this queue; STATE = ${toString()}"
                }
            }
    ...
    private inner class Node(
        ...
        fun removePhysically() {
            ...
            curPrev.next.compareAndSet(curPrev.next.value, curNext)
            if (curPrev.extractedOrRemoved) curPrev.remove()
            if (curNext?.extractedOrRemoved == true) curNext.remove()
        }
