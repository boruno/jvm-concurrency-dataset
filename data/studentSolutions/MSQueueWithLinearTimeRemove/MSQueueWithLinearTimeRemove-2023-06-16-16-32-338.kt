...
class MSQueueWithLinearTimeRemove<E> : QueueWithRemove<E> {
    ...
    override fun dequeue(): E? {
        ...
        while (true) {
            ...
            var curHeadNext = curHead.next.value ?: return null

            while (!curHeadNext.markExtractedOrRemoved()) {
                curHead = curHeadNext
            ...
            // TODO сделать получение указателя на следующего потенциального результата
            // TODO в случае, если этот был удален
        }
    }
    ...
    private inner class Node(
        ...
        fun remove(): Boolean {
            ...
            if (curPrev.element == this.element) return true // Don't delete first element

            while (true)
            {
                val curPrevNext = curPrev.next.value
                if (curPrevNext == null) return false // there is no such element
                if (curPrevNext.element == this.element) break // found it

                curPrev = curPrevNext
            }

            val curNext = next.value
            if (curNext == null) return true // Don't delete the last element
