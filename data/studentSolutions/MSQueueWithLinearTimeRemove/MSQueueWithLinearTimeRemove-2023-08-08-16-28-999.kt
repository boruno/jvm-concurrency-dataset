...
class MSQueueWithLinearTimeRemove<E> : QueueWithRemove<E> {
    ...
    private fun findPrev(node: Node): Node? {
        ...
        while (true) {
            val cursorNext = cursor.next.get()
            if (cursorNext == node) return cursor
            if (cursorNext == null) return null
            cursor = cursorNext
        }
    }
    ...
    private fun removeNodePhysicallyThreadUnsafe(node: Node) {
        ...
        val nodePrev = findPrev(node) ?: return

//        val queue = this
//        val tail = queue.tail.get()
//        if (node == tail) return
//
//        val head = queue.head.get()
//        if (node == head) return
//
//        if (head == null) return
//        if (head.next.get() == null) return

        val nodeNext = node.next.get()
        nodePrev.next.set(nodeNext)
        if (nodeNext?.extractedOrRemoved == true) removeNodePhysicallyThreadUnsafe(nodeNext)
    }
