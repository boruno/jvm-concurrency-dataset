...
class MSQueueWithLinearTimeRemove<E> : QueueWithRemove<E> {
    ...
    override fun enqueue(element: E) {
        // TODO: When adding a new node, check whether the previous tail is logically removed. If so, remove it physically from the linked list.
        while (true) {
    ...
    override fun dequeue(): E? {
        // TODO: After moving the `head` pointer forward, mark the node that contains the extracting element as "extracted or removed", restarting the operation if this node has already been removed.
        while (true) {
            val curHead = head.value
            val nextHead = curHead.next.value ?: return null
            if (head.compareAndSet(curHead, nextHead) && !nextHead.extractedOrRemoved) {
    ...
    private inner class Node(
        ...
        fun remove(): Boolean {
            ...
            while (true) {
                ...
                if (prev.next.compareAndSet(this, next.next.value)) return true
            }
            return true
        }
    }
