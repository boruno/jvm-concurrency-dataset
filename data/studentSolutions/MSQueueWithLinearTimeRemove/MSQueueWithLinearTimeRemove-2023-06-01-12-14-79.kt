...
class MSQueueWithLinearTimeRemove<E> : QueueWithRemove<E> {
        ...
        while (true) {
            if (node !== head.value && node !== tail.value) {
                check(!node.extractedOrRemoved) {
                    "Head: ${head.value} Tail: ${tail.value} Node: $node Commands: ${commands.joinToString(",")}"
                }
            }
    ...
    private inner class Node(
        ...
        fun remove(): Boolean {
            ...
            val prevNode = findPrev() ?: return result
        ...
        private fun findPrev() : Node? {
            ...
            while (true) {
                ...
                if (nextNode === this) return node
                node = nextNode
            }
        }

        override fun toString(): String = "(<element: $element, extractedOrRemoved: $extractedOrRemoved> next) -> ${next.value}"
    }
}
