...
class MSQueueWithLinearTimeRemove<E> : QueueWithRemove<E> {
    ...
    override fun dequeue(): E? {
        while (true) {
            ...
            val nextHead = head.value.next.value ?: return null
            if (head.compareAndSet(curHead, nextHead) && nextHead.markExtractedOrRemoved())
    ...
    private inner class Node(
        ...
        fun remove(): Boolean {
            ...
            val prevNode = findPrev() ?: return false
            val nextNode = next.value ?: return false
            prevNode.next.compareAndSet(this, nextNode)
            if (prevNode != null && prevNode.extractedOrRemoved)
                prevNode.remove()
        ...
        private fun findPrev() : Node? {
            ...
            while (true) {
                when (val next = curNode.next.value) {
                    null -> return null
                    this -> return curNode
                    else -> curNode = next
                }
