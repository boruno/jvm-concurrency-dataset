...
class MSQueueWithLinearTimeRemove<E> : QueueWithRemove<E> {
        ...
        while (true) {
            ...
            if (curTail.next.compareAndSet(null, newNode)) {
                ...
                if (curTail.extractedOrRemoved)
                    curTail.removePhyscally()
                return
            } else {
                val valueNext = curTail.next.value
                if (valueNext != null) {
                    tail.compareAndSet(curTail, valueNext)
                }
            }
        }
    }
    ...
    override fun dequeue(): E? {
        while (true) {
            ...
            val curHeadNext = value.next
            val curNextHead = curHeadNext.value ?: return null
            if (head.compareAndSet(value, curNextHead)) {
