...
class MSQueueWithLinearTimeRemove<E> : QueueWithRemove<E> {
        ...
        while (true) {
            ...
            if (curTail == tail.value) {
                ...
                if (curTail.next.compareAndSet(null, newNode)) {
                    ...
                    // if the current tail is marked as extractedOrRemoved, we should update head to the new node
                    if (curTail == head.value && curTail.extractedOrRemoved) {
        ...
        while (true) {
            ...
            if (head.compareAndSet(curHead, curHeadNext)) {
                ...
                if (curHeadNext.next.value != null) {
                    // Physically remove the current head node if it's not the last node in the queue
                    head.compareAndSet(curHead, curHeadNext.next.value!!)
                }
    ...
    private inner class Node(
        ...
        fun remove(): Boolean {
            ...
            if (result) {
                val prev = referrer()
                if (prev.next.value == this) {
                    prev.next.compareAndSet(this, next.value)
                }
                // If this node is at the tail, update the tail to point to the previous node
                if (this == tail.value) {
