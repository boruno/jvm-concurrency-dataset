        ...
        while (true) {
            ...
            if (curTail.next.compareAndSet(null, node)) {
                if (curTail.extractedOrRemoved) {
                    val ref = curTail.referrer()
                    if (ref != null)
                        ref.next.compareAndSet(curTail, node)
                }
                tail.compareAndSet(curTail, node)
                return
            } else {
                val nextVal = curTail.next.value
                if (nextVal !=null)
                    tail.compareAndSet(curTail, nextVal)
            }
        }
    }
...
//        TODO("Implement me!")
    ...
    private inner class Node(
        ...
        fun remove(): Boolean {
            ...
            if (referrer != null && this != head.value && this.next.value != null) {
                val prevNext = referrer.next.value
                val myNext = next.value
                referrer.next.compareAndSet(this, myNext)
                if (referrer.extractedOrRemoved) referrer.remove()
                if (myNext!=null) if (myNext.extractedOrRemoved) myNext.remove()
            }
            return result
        }
    }
