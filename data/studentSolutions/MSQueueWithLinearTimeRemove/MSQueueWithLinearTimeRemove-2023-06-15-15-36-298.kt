...
class MSQueueWithLinearTimeRemove<E> : QueueWithRemove<E> {
    ...
    private fun prev(element : MSQueueWithLinearTimeRemove<E>.Node): MSQueueWithLinearTimeRemove<E>.Node? {
        var node = head.value
        while (true) {
            val next = node.next.value
            if (next == null) return null

            if (next.element == element) return node
    ...
    override fun dequeue(): E? {
        ...
        while (true) {
            ...
            if (head.compareAndSet(curHead, curHeadNext)) {
                if (curHeadNext.extractedOrRemoved)
                    continue // someone grabbed the value before us
                else
                {
    ...
    private inner class Node(
        ...
        fun remove(): Boolean {
            ...
            {
                curPrev = curPrev.next.value ?: return false
            }

            val curNext = next.value ?: return true // Don't delete the last element
