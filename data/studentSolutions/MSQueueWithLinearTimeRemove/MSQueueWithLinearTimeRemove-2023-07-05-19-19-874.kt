...
//            TODO("Implement me!")
            if (markExtractedOrRemoved()) {
                ...
                while (true) {
                    ...
                    if (nodeNext == null) return false //it's head should never delete it

                    if (nodeNext == this) {
                        val nextValue = next.value
                        if (nextValue == null) return true //It's tail, will return late

                        node.next.value = nextValue
                        if (node.extractedOrRemoved) node.remove()
                        if (nextValue?.extractedOrRemoved == true) nextValue.remove()
                        return true
                    }
                    node = nodeNext
                }
