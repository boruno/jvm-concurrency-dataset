...
class MSQueueWithLinearTimeRemove<E> : QueueWithRemove<E> {
    ...
    override fun dequeue(): E? {
        ...
        return if (curHeadNext == null) null
            else if (head.compareAndSet(curHead, curHeadNext) && curHeadNext.markExtractedOrRemoved()) {
                curHeadNext.element
            }
            else dequeue()
    }
    ...
    private inner class Node(
        ...
        fun remove(): Boolean {
            ...
            if (this != this@MSQueueWithLinearTimeRemove.head.value && next.value != null) doRemove()
            return result
        }

        fun doRemove() {
            var n = this@MSQueueWithLinearTimeRemove.head.value
            while (n.next.value != this && n.next.value != null) {
                n = n.next.value!!
            }
            val next = this.next.value
            if (n.next.compareAndSet(this, next)) {
                if (n.markExtractedOrRemoved()) n.doRemove()
                next?.let { if (it.markExtractedOrRemoved()) it.remove() }
            }
        }
    }
}
