...
class MSQueueWithLinearTimeRemove<E> : QueueWithRemove<E> {
    ...
    init {
        val dummy = Node(null).also { it.markExtractedOrRemoved() }
        head = atomic(dummy)
        tail = atomic(dummy)
    }
        ...
        while (true) {
            ...
            if (next.compareAndSet(null, elementNode)) {
                ...
                if (nextValue != null) {
                    ...
                    throw IllegalStateException("Next value is null")
                }
            }
        }
    }

    private fun updateTail(oldValue: Node, newValue: Node) {
...
//            TODO("Implement me!")
            if (markExtractedOrRemoved()) {
                ...
                while (true) {
                    ...
                    if (nodeNext == null) throw IllegalStateException("Tried to remove head")//it's head. should never delete it

                    if (nodeNext == this) {
                        node.next.value = nextValue
                        if (node.extractedOrRemoved) node.remove()
                        if (nextValue.extractedOrRemoved) nextValue.remove()
                        return true
                    }
                    node = nodeNext
                }
