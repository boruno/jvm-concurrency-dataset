...
class MSQueueWithLinearTimeRemove<E> : QueueWithRemove<E> {
            ...
            if (curTail.next.compareAndSet(null, node)) {
                if (tail.compareAndSet(curTail, node)) {
                    if (curTail.extractedOrRemoved) {
                        findPrev(curTail)?.next?.set(node)
                    }
                }
                return
            } else {
    ...
    private fun findPrev(node: Node): Node? {
        ...
        while (prev != null && prev.next.get() != node) prev = prev.next.get()
        return prev
    }
    ...
    private inner class Node(
        ...
        fun remove(): Boolean {
            ...
            val shouldBeRemoved = markExtractedOrRemoved()
            if (shouldBeRemoved) {
                var nodeToRemove = this
                while (true) {
                    if (nodeToRemove == head.get() || nodeToRemove == tail.get()) break
                    val prev = findPrev(this)
                    if (prev == null) break
                    val curNext = nodeToRemove.next.get()
                    prev.next.set(curNext)
                    if (curNext == null) break
                    if (!curNext.extractedOrRemoved) break
                    nodeToRemove = curNext
                }
            }
