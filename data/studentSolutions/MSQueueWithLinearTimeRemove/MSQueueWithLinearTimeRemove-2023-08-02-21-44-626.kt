...
class MSQueueWithLinearTimeRemove<E> : QueueWithRemove<E> {
    ...
    private inner class Node(
        ...
        fun remove(): Boolean {
            ...
            if (next.value == null) return marked // don't remove tail
            removePhysically()
            return marked
        }
        ...
        fun removePhysically() {
            ...
            if (prev != null) {
                //prev.next.compareAndSet(this, this.next.value)//value = this.next.value
                val nextValue = next.value
                prev.next.value = nextValue
                //if (prev.extractedOrRemoved) prev.removePhysically()
                if (nextValue?.extractedOrRemoved == true) nextValue.removePhysically()
            }
        }

        fun removeTailPhysically() {
            val prev = findPrevNode()
            if (prev != null) {
                val newValue = this.next.value
                //!!!!
                prev.next.compareAndSet(this, newValue)// = newValue
                //if (newValue?.extractedOrRemoved == true) newValue.removeTailPhysically()
                if (prev.extractedOrRemoved) prev.removePhysically()
            }
        }
