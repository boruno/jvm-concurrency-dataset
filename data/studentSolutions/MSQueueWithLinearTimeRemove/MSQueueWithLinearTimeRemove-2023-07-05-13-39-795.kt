...
class MSQueueWithLinearTimeRemove<E> : QueueWithRemove<E> {
        ...
        while (true) {
            if (node !== head.value && node !== tail.value) {
                check(!node.extractedOrRemoved) {
                    "Removed node with element ${node.element} found in the middle of this queue (${head.value} ${tail.value})"
                }
            }
    ...
    private inner class Node(
        ...
        fun getPrev(): Node? {
            ...
            return if (node != head.value && node != tail.value) node else null
        }
    }
}
