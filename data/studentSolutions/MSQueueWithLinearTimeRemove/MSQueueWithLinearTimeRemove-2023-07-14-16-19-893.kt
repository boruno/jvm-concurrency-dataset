    ...
    override fun enqueue(element: E) {
        ...
        while (true) {
            ...
            if (curTail.extractedOrRemoved) {
                val curTailReferrer = curTail.referrer()
                if (curTailReferrer != null && curTailReferrer.next.compareAndSet(curTail, node)) {
                    tail.compareAndSet(curTail, node)
                    return
                }
            } else {
                // Ensure the next node of curTail is not null before assigning.
                val curTailNext = curTail.next.value
                if (curTailNext == null && curTail.next.compareAndSet(null, node)) {
                    // Only update the tail if it hasn't changed during the operation
                    tail.compareAndSet(curTail, node)
                    return
                } else if (curTailNext != null) {
                    // If tail's next is not null, try to update tail to the next node
