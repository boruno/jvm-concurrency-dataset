...
class LinkedListSet<E : Comparable<E>> {
    ...
    fun add(element: E): Boolean {
        while (true) {
            ...
            if (node[1] != last && node[1].element == element) {
                return false
            } else {
                val newNode = Node(node[1].prev, element, node[1])
                if (node[1].prev!!.casNext(node[1], newNode)) {
                    return true
                } else {
                    continue
                }
    ...
    fun remove(element: E): Boolean {
        while (true) {
            ...
            if (node[1] == last || node[1].element != element) {
                ...
                node[1].prev!!.casNext(node[1], node[1].next!!)
                return true
            }
        }
    }
