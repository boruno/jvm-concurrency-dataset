...
//        while (true) {
        ...
        for (aaa in 1..1000) {
            ...
            assert(node == head.value || node.element < element)

            println(nodeNext?.optElement().toString() + "-----")

            val nodeNextAndAlive = node.nextAndAlive.value
            nodeNext = nodeNextAndAlive.first
            ...
            val newNode = Node(element, nodeNext)

            assert(node == first || node!!.element != element)
            assert(nodeNext == last || nodeNext!!.element != element)

            if (nodeNextAndAlive.second && node.casNext(nodeNextAndAlive, Pair(newNode, true))) {
                assert (head.value.next != null)
