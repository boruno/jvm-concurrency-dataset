...
class LinkedListSet<E : Comparable<E>> {
    ...
    private fun findWindow(element: E): Pair<Node<E>, Node<E>> {
        var cur = head.value
        var next = head.value.next ?: return (cur to last)
        while (next.element < element) {
            val nodeAfterNode = next.next
            if (nodeAfterNode != null && nodeAfterNode.isRemoved) {
                if (!cur.casNext(next, Node(nodeAfterNode.element, nodeAfterNode.next))) {
                    return findWindow(element)
                }
                next = nodeAfterNode
            } else {
                cur = next.also { next = cur.next ?: return (cur to last)}
            }
        }
        return (cur to next)
    }
