...
class LinkedListSet<E : Comparable<E>> {
    ...
    fun remove(element: E): Boolean {
        ...
        while (true) {
            val subset = findSubset(element)
            val removingNode = if (subset.first == element) subset.first
                               else subset.second

            val deletedNode = DeletedNode(removingNode.prev, removingNode.element, removingNode.next)

            if (removingNode.prev is DeletedNode)
                continue

            if (!removingNode.prev?.casNext(removingNode, deletedNode)!!)
                continue

            if (removingNode.prev?.casNext(deletedNode, removingNode.next)!!)
                return true
        }
    }
