...
class LinkedListSet<E : Comparable<E>> {
    ...
    fun add(element: E): Boolean {
        while (true) {
            ...
            when {
                !(second == last) -> return false
                else -> {
                    val node = Node(prev = first, element = element, next = second)
                    if (first.casNext(second, node)) return true
                }
            }
        }
    }
    ...
    fun remove(element: E): Boolean {
        while (true) {
            val (first, second) = findWindow(element)
            when {
                !(second != last && second.element <= element) -> return false
                else -> {
                    if (second.casNext(
                            second.next, Wrapper(prev = second.prev, element = second.element, next = second.next)
                        ) && first.casNext(second, second.next)
                    ) return true
                }
            }
        }
    ...
    private fun findWindow(element: E): Pair<Node<E>, Node<E>> {
        ...
        while (true) {
            ...
            when {
                next == last || next.element >= element -> {
...
private open class Node<E : Comparable<E>>(prev: Node<E>?, element: E?, next: Node<E>?) {
    ...
    fun casPrev(expected: Node<E>?, update: Node<E>?) = _prev.compareAndSet(expected, update)
