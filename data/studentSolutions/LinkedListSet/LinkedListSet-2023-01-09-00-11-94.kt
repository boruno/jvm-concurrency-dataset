...
class LinkedListSet<E : Comparable<E>> {
    ...
    fun add(element: E): Boolean {
        outer@ while (true) {
            ...
            while ( current != null) {
                if (current != last && current.node.element == element && current is SomeNode<E> && !current.isRemoved()) {
                    return true
                }
                if ((prev == first || prev.node.element <= element) && (current == last || element < current.node.element)) {
...
private class SomeNode<E : Comparable<E>>(val inner: Node<E>) : WrappedNode<E> {
    ...
    fun isRemoved() = setted.value is RemovedNode<E>
