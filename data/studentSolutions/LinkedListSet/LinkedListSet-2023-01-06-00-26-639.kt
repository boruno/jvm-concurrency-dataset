...
class LinkedListSet<E : Comparable<E>> {
    ...
    fun findWindow(x: E): Window<E> {
       loop@ while (true) {
            var curNode = first
            var (nextNode, removed) = curNode.nextAndRemoved.value
            while (curNode.element < x) {
                val nextNextNode = nextNode?.next?.nextAndRemoved!!.value.first
                if (removed) {
...
class AtomicNodeWrapper<E : Comparable<E>>(
    ...
    private val atomicNode: AtomicRef<NodeWrapper> = atomic(Alive(node))
...
class Node<E : Comparable<E>>(
    ...
    val nextAndRemoved = AtomicNodeWrapper(next, false)
