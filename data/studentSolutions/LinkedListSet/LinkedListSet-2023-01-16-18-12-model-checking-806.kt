...
class LinkedListSet<E : Comparable<E>> {
    ...
    fun add(element: E): Boolean {
        while(true) {
            ...
            while (currentElement.next != last) {
                if (currentElement == first && currentElement.next == last)
                    break
            ...
            val newNode = Node(currentElement, element, currentElement.next!!)
            if(!currentElement.next!!.casPrev(currentElement, newNode))
                continue
            if(!currentElement.casNext(newNode.next, newNode))
            {
                currentElement.next!!.casPrev(newNode, currentElement)
                continue
            }
        }
    }
    ...
    fun contains(element: E): Boolean {
        ...
        if (currentElement.next === last && currentElement.element != element)
            return false
        if (currentElement.element == element)
            return true
        throw Exception("Not last and not found!")
    }
}
