...
class LinkedListSet<E : Comparable<E>> {
    ...
    fun add(element: E): Boolean {
       mainLoop@ while(true) {
            ...
            while(node != last){
                val cur_next = node.next
                if (contains(element))
                    return false
                if(node == first){
                    if(cur_next == last){
                        insert.setNext(cur_next)
                        insert.setPrev(node)
                        if(node.casNext(last,insert))
                            break@mainLoop
                    }
                    else if(element < cur_next!!.element){
                        insert.setNext(cur_next)
                        insert.setPrev(node)
                        if(node.casNext(cur_next,insert))
                            break@mainLoop
                    }
                }
                else if(cur_next == last){
                    insert.setNext(last)
                    insert.setPrev(node)
                    if(node.casNext(cur_next,insert))
                        break@mainLoop
                }
                else if(node.element < element && cur_next!!.element > element){
