...
class LinkedListSet<E : Comparable<E>> {
    ...
    private fun findWindow(element: E): Window<E> {
        var cur = head.value
        while (cur.next != last && cur.next?.element!! < element) {
            cur = cur.next!!
        }
    ...
    fun add(element: E): Boolean {
        while (true) {
            ...
            if (window.next.element != last.element && window.next.element === element) {
                return false
            }
    ...
    fun contains(element: E): Boolean {
        return findWindow(element).next.element === element
    }
}
