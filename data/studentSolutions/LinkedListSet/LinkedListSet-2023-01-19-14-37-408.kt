...
class LinkedListSet<E : Comparable<E>> {
    ...
    fun add(element: E): Boolean {
        var node = head.value
        val newNode = Node(element = element, next = null, prev = null)
        while(true){
            if(contains(element))
                return false
            val curr_next = node.next
            if(node == first)
            {
                if(curr_next == last)
                {
                    ...
                    if(node.casNext(curr_next,newNode))
                        return true
                    else
                    {
                        node = first
            ...
            if(node.element < element){
                ...
                else if(curr_next!!.element > element){
                    newNode.setNext(curr_next)
                    newNode.setPrev(node)
                    if(node.casNext(curr_next,newNode))
                        return true
