...
class LinkedListSet<E : Comparable<E>> {
    ...
    private fun getPosition(element: E): Pair<Node<E>> {
        while (true) {
            var cur = first
            var next = cur.next
            while (next!!.isRemoved() || next.element < element) {
                if (cur.isRemoved()) {
                    break
                }
                if (next.isRemoved()) {
    ...
    fun remove(element: E): Boolean {
        while (true) {
            ...
            if (pos.second._nextAndRemoved.attemptMark(pos.second, true)) {
                return true
            }
        }
    }
    ...
    fun contains(element: E): Boolean {
        val pos = getPosition(element)
        return pos.second.element == element
    }
}
