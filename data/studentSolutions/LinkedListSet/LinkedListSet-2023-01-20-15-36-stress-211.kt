...
class LinkedListSet<E : Comparable<E>> {
    ...
    private fun elementEqual(element: E, node: ReferenceInterface<E>): Boolean {
        if (node.getReference1().prev == null || node.getReference1().next == null) {
            return false
        }

        return node.getReference1().element == element
    }
    ...
    fun contains(element: E): Boolean {
        ...
        while (!isNextGreater(element, next.getReference1()) && !(elementEqual(element, next))) {
            next = next.getReference1().next!!
        }

        return (elementEqual(element, next) && next !is RemovedReference)
    }
