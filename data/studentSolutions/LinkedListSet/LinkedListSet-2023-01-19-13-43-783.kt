...
class LinkedListSet<E : Comparable<E>> {
    ...
    fun add(element: E): Boolean {
        ...
        while (node != last) {
            if (contains(element))
                return false
            val curNext = node.next
            if (node == first) {
                if (curNext == last) {
                    insert.setNext(curNext)
                    insert.setPrev(node)
                    if (node.casNext(curNext, insert))
                        return true
                } else if (element < curNext!!.element) {
                    insert.setNext(curNext)
                    insert.setPrev(node)
                    if (node.casNext(curNext, insert))
                        return true
                }
            } else if (curNext == last) {
                insert.setNext(last)
                insert.setPrev(node)
                if (node.casNext(curNext, insert))
                    return true
            } else if (node.element < element && curNext!!.element > element) {
