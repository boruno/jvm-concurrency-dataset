...
private class PairSegment<E: Comparable<E>>(var cur: Node<E>, var next: Node<E>?) {
    companion object {
        ...
        fun <E : Comparable<E>> find(element: E, set: LinkedListSet<E>): PairSegment<E> {
            outer@ while (true) {
                val segment = PairSegment(set.getFirst(), set.getFirst().next)
                while((segment.next!!.element as E) < element) {
                    val nextNext = segment.next!!.next
                    if(!nextNext!!.alive) {
...
const val MINUS_INF = Int.MIN_VALUE
const val PLUS_INF = Int.MAX_VALUE