    ...
    fun enqueue(x: E) {
        while(true){
            ...
            if(curr_tail.next.compareAndSet(null,node)) {
                if(tail.compareAndSet(curr_tail,node))
                    return
            }
            else
            {
                if(tail.compareAndSet(curr_tail, curr_tail.next.value!!))
                    return
            }
        }
    }
    ...
    fun dequeue(): E? {
       while(true){
           ...
           if(isEmpty())
               ...
               val curr_head_next = curr_head.next.value!!
               tail.compareAndSet(curr_tail, curr_head_next)
           }
