...
class MSQueue<E> {
    ...
    fun enqueue(x: E) {
        ...
        while (true) {
            ...
            val currentTailNext = this.tail.value.next
            if (currentTail.value == tail.value) {
                if (currentTail.value.x == null) {
                    if (currentTail.value.next.compareAndSet(null, newNode)) {
                        this.tail.compareAndSet(currentTail.value, newNode)
                        return
                    }
                } else {
                    this.tail.compareAndSet(currentTail.value, currentTailNext.value!!)
                }
            }
        }
    }
