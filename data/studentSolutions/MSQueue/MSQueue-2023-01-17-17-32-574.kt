...
class MSQueue<E> {
    ...
    fun enqueue(x: E) {
        while (true) {
            ...
            if (tail.value.next.compareAndSet(null, nodeToEnqueue)) {
    ...
    fun dequeue(): E? {
        while (true) {
            ...
            val headNextNode = headNode.next.value
            val tailCur = tail.value

            if (headNode == tail.value) {
                if (headNextNode == null) {
                    return null
                }
                tail.compareAndSet(tailCur, headNextNode)
            } else if (head.compareAndSet(headNode, headNextNode!!)) {
