...
class FAABasedQueue<E> : Queue<E> {
    ...
    override fun enqueue(element: E) {
        ...
        if (!moveTailForward(segment)) return enqueue(element)
    ...
    private fun moveTailForward(segment: Segment): Boolean {
        while (true) {
            ...
            if (curTail.id < segment.id) {
                infiniteArray.tail.compareAndSet(curTail, curTail.next.get())
            } else {
                return curTail.id == segment.id
            }
        }
    }
    ...
    private fun moveHeadForward(segment: Segment): Boolean {
        while (true) {
            ...
            if (curHead.id < segment.id) {
                infiniteArray.head.compareAndSet(curHead, curHead.next.get())
            } else {
                return curHead.id == segment.id
            }
        }
    }
    ...
    override fun dequeue(): E? {
        ...
        if (!moveHeadForward(segment)) return dequeue()
        val iModulo = (i % SEGMENT_SIZE).toInt()
