...
class FAABasedQueue<E> : Queue<E> {
    ...
    override fun dequeue(): E? {
        while (true) {
            ...
            if (segmentIdx == 0 && curIdx != 0) {
                val newHead = curHead.next.value ?: return null // return null if there's no next node
                head.compareAndSet(curHead, newHead)
            }

            val value = curHead.elements[segmentIdx].getAndSet(POISONED)
            if (value != null && value != POISONED) return value as E?
        }
    }
}
