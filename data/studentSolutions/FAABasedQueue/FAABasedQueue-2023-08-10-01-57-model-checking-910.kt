...
//            return infiniteArray.getAndSet(i, null) as? E
            ...
            val (shouldDequeue, currentEnqueueIndex) = shouldDequeue()
            if (!shouldDequeue) return null
            ...
            if (itemIndex < currentEnqueueIndex && segment.cells.compareAndSet(itemIndex, null, POISONED)) {
                continue
            }
    ...
    private fun shouldDequeue(): Pair<Boolean, Long> {
        while (true) {
            ...
            return if (dequeueIdx < enqueueIdx) (true to (enqueueIdx - 1)) else (false to -1) // todo: and enqueue idx set
        }
    }
}
