...
class FAABasedQueue<E> : Queue<E> {
    private val SEGM_SIZE = 10
    class dumby<E>(val arr: AtomicArray<Any?> = atomicArrayOfNulls(10),
                   val id: Int = 0) {
    ...
    override fun dequeue(): E? {
        while (true) {
            ...
            if (s.arr[i % SEGM_SIZE].compareAndSet(null, POISONED)) {
                if (i % 2 == 0) tmp = s else  throw Exception("${i}${tmp.id}${s.id}${tmp.arr[0].value==POISONED}${tmp.arr[1].value}")
                continue
            }
            if (s.arr[i % SEGM_SIZE].value == POISONED) throw Exception("${i}${curHead.id}${s.id}${s.arr[0].value==POISONED}${s.arr[1].value}")
