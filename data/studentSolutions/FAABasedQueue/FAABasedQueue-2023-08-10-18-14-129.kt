...
class FAABasedQueue<E> : Queue<E> {
    ...
    override tailrec fun dequeue(): E? {
        ...
        while (head.id < segmentId) {
            val nextHead = head.next.get()
            if (nextHead == null) {
                val newReadySegment = Segment(head.id + 1)
                if (head.next.compareAndSet(null, newReadySegment)) {
                    infiniteArray.head.compareAndSet(head, newReadySegment)
                }
            } else {
