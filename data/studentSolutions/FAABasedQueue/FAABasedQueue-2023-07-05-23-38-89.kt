...
class FAABasedQueue<E> : Queue<E> {
    ...
    private operator fun MSQueue<Segment>.get(idx: Long): Segment {
        ...
        val curtailSegment = curTail.element
        if (curtailSegment ==null){
            val result = Segment(0)
            segments.enqueue(result)
            return result
        }
        else if (curtailSegment.idx == idx) return curtailSegment
        else if (curtailSegment.idx > idx) {
