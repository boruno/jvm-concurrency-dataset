...
class FAABasedQueue<E> : Queue<E> {
    ...
    private fun moveHeadTo(section: Section) {
        while (true) {
            val curHead = head.value
            if (curHead.index >= section.index) return
    ...
    private fun moveTailTo(section: Section) {
        while (true) {
            val curTail = tail.value
            if (curTail.index >= section.index) return
            tail.compareAndSet(curTail, curTail.next.value!!)
...
//            }
    ...
    private fun getOrAddSection(isHead: Boolean, sectionIndex: Int): Section? {
        ...
        if (section.index > sectionIndex) return null
    ...
    override fun enqueue(element: E) {
        while (true) {
            ...
            val section = getOrAddSection(false, sectionIndex) ?: continue
            moveTailTo(section)
    ...
    override fun dequeue(): E? {
        while (true) {
            ...
            val sectionIndex = i / sectionSize
            val section = getOrAddSection(true, sectionIndex) ?: continue
            moveHeadTo(section)
            val indexInSection = i % sectionSize
            val elementRef = section.array[indexInSection]
