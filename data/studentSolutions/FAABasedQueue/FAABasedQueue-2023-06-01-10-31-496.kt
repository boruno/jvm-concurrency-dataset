    ...
    override fun dequeue(): E? {
        while (true) {
            ...
            if (s.arr[i % SEGM_SIZE].compareAndSet(null, POISONED)) {
                throw Exception("${i % SEGM_SIZE}${curHead.id}${curHead.arr[0].value}${curHead.arr[1].value}")
                continue
            }
            if (s.arr[i % SEGM_SIZE].value == POISONED) throw Exception("${i}${curHead.id}${curHead.arr[0].value}${curHead.arr[1].value}")
