...
class FAABasedQueue<E> : Queue<E> {
    ...
    private fun moveHeadTo(section: Section) {
        while (true) {
            ...
            if (currHead.index >= section.index) return
            if (head.compareAndSet(currHead, section)) return
        }
    }

    private fun moveTailTo(section: Section) {
        while (true) {
            val currTail = tail.value
            if (currTail.index >= section.index) return
            if (tail.compareAndSet(currTail, section)) return
        }
    }
    ...
    private fun getOrAddSection(isHead: Boolean, sectionIndex: Int): Section {
        var section: Section = if (isHead) head.value else tail.value
    ...
    override fun enqueue(element: E) {
        while (true) {
            ...
            val section = getOrAddSection(false, sectionIndex)
            moveTailTo(section)
    ...
    override fun dequeue(): E? {
        while (true) {
            ...
            val sectionIndex = i / sectionSize
            val section = getOrAddSection(true, sectionIndex)
            moveHeadTo(section)
            val indexInSection = i % sectionSize
            val elementRef = section.array[indexInSection]
