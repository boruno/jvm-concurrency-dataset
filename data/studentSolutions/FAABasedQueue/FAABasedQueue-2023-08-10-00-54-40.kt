...
private class InfiniteArray {
    ...
    init {
        val initialHead = Segment(getNewSegmentId())
        val initialTail = Segment(getNewSegmentId())
        initialHead.next.set(initialTail)

        head = AtomicReference(initialHead)
        tail = AtomicReference(initialTail)
    }
