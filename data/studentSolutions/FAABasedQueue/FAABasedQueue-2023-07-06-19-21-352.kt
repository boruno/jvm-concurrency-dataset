...
class FAABasedQueue<E> : Queue<E> {
    ...
    private fun findSegment(start: Segment<E>, id: Long): Segment<E> {
        ...
        while (true) {
            val addLastSegmentnewSegment = addLastSegment()
            if (addLastSegmentnewSegment.id == id) return addLastSegmentnewSegment
            if (addLastSegmentnewSegment.id < id) continue
            var curSeg = start
            while (true) {
