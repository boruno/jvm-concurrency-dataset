...
class FAABasedQueue<E> : Queue<E> {
    ...
    private fun tryMoveTail(segment: Segment) {
        val notNull = segment.notNull.getAndIncrement()

        if (notNull == SEGMENT_SIZE) {
            val curTail = tail.value
    ...
    private fun tryMoveHead(segment: Segment) {
        val done = segment.processed.getAndIncrement()

        if (done == SEGMENT_SIZE) {
