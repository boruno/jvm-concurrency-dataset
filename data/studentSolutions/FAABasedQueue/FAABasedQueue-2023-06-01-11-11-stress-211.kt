...
class FAABasedQueue<E> : Queue<E> {
    ...
    fun moveTailForward(newTail: Segment<E>) {
        while (true)
        {
            ...
            if (newTail.id == null || currentTail.id == null)
                continue // Может это неправильная обработка, потом подумать
    ...
    fun moveHeadForward(newHead: Segment<E>) {
        while (true) {
            ...
            if (newHead.id == null || currentHead.id == null)
                continue // Может это неправильная обработка, потом подумать
            if (newHead.id * SEGM_SIZE > deqIdx.value) // мы хотим сделать head таким, что до deqIdx не достать
                return
            if (newHead.id > currentHead.id)
            {
                if (tail.compareAndSet(currentHead, newHead))
                    return
            } else {
                return
            }
...
//            deqIndex = deqIdx.value
            ...
            moveTailForward(s) // another function
