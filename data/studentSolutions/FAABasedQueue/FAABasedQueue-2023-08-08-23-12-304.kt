...
class FAABasedQueue<E> : Queue<E> {
    ...
    override fun validate() {
        val (deqSeg, deqCellsIdx) = getSegmentAndCellIdx(head.get(), deqIdx.get())
        check(deqSeg === head.get()) {
            "`deqIdx = ${deqIdx.get()}` points to segment with id `${deqSeg.id}` but `head` points to segment with id `${head.get().id}"
        }

        val (enqSeg, enqCellsIdx) = getSegmentAndCellIdx(tail.get(), enqIdx.get())
        check(enqSeg === tail.get()) {
