...
class FAABasedQueue<E> : Queue<E> {
    ...
    override fun dequeue(): E? {
        while (true) {
            ...
            val segmentIndex = idx / SEGMENT_SIZE
            val insideSegment = (idx % SEGMENT_SIZE).toInt()
            val xxx = segments[segmentIndex].data[insideSegment]
            if (xxx.compareAndSet(null, POISONED)) continue
            return xxx.value as E
        }
    }
    ...
    private operator fun MSQueue<Segment>.get(idx: Long): Segment {
        if (tail.value.element!!.idx == idx) return tail.value.element!!
            ...
            while (curseg==null||(curidx != null && curidx != idx)) {
