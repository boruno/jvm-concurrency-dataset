...
class AtomicCounterArray(size: Int) {
    ...
    fun inc2(index1: Int, index2: Int): Boolean {
        ...
        while (true){
            ...
            if (firstValue is IncrementDescriptor){
                if (firstValue.index1 == index2 && firstValue.index2 == index1)
                    continue
            ...
            if (secondValue is IncrementDescriptor){
                if (secondValue.index1 == index2 && secondValue.index2 == index1)
                    continue
    ...
    private inner class IncrementDescriptor(
        ...
        fun applyOperation() {
            ...
            while (true) {
                val firstValue = array[index1].value
                if (firstValue is IncrementDescriptor && firstValue != this){
//                    if (firstValue.index1 == index2 && firstValue.index2 == index1)
//                        continue
                    firstValue.applyOperation()
                    continue
                ...
                val secondValue = array[index2].value
                if (secondValue is IncrementDescriptor && secondValue != this){
//                    if (secondValue.index1 == index2 && secondValue.index2 == index1)
//                        continue
                    secondValue.applyOperation()
                    continue
                ...
                // apply logically
                if (firstValue == valueBeforeIncrement2) {
                    if (array[index1].compareAndSet(firstValue, this)) {
                        continue
                    } else {
                        // someone put desc or increment value. Try again?
                        continue
                    }
                } else if (firstValue == this && secondValue != this) {
                    if (secondValue == valueBeforeIncrement2){
                        array[index2].compareAndSet(secondValue, this)
                    }
