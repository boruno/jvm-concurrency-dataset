    ...
    private class Core internal constructor(capacity: Int) {
        ...
        fun getInternal(key: Int): Int {
            ...
            while (true) {
                ...
                return when (val currentValue = map[index + 1].value) {
                    in (0..DEL_VALUE) -> currentValue
                    MOVED_VALUE -> if (nextCore !== null)
                        nextCore.getInternal(key) else throw IllegalArgumentException("Something wrong")
            ...
            while (true) { // optimize for successful lookup
                ...
                if (currentKey == key) {
                    ...
                    return when (currentValue) {
                        MOVED_VALUE -> if (nextCore !== null)
                            nextCore.putInternal(key, value) else throw IllegalArgumentException("Something wrong")
                        ...
                        in (0..DEL_VALUE) ->
                            if (map[index + 1].compareAndSet(currentValue, value)) currentValue else continue
        ...
        fun rehash(): Core {
            ...
            while (index < map.size) {
                while (true) {
                    val currentKey = map[index].value
                    when (val currentValue = map[index + 1].value) {
                        MOVED_VALUE -> break
                        in (MOVED_VALUE until 0) -> {
                            val fixedValue = -currentValue
