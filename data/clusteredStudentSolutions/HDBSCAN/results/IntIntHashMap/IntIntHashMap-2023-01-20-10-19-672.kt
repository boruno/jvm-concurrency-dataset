...
class IntIntHashMap {
    ...
    */
    ...
    private class Core internal constructor(capacity: Int) {
        ...
        val shift: Int
        val new: AtomicRef<Core?> = atomic(null)
        ...
        fun putInternal(key: Int, value: Int): Int {
            ...
            while (true) {
                ...
                if (curKey == key) {
                    return when {
                        res < 0 -> NEEDS_REHASH
                        !map[index + 1].compareAndSet(res, value) -> continue
                        else -> res
                    }
                }
        ...
        fun rehash(): Core {
            val tmpCore = Core(map.size * 2)
            new.compareAndSet(null, tmpCore)
            val newCore = new.value!!
            var index = map.size - 2
            while (index >= 0) {
                val curKey = map[index].value
                val res = map[index + 1].value
                if (res == Int.MIN_VALUE) continue
                val newRes = -res
                if (isValue(res)) {
                    if (!map[index + 1].compareAndSet(res, newRes)) continue
                    newCore.copyMap(curKey, res)
                    map[index + 1].compareAndSet(newRes, Int.MIN_VALUE)
                }
                if (res < 0)
                    newCore.copyMap(curKey, newRes)
                if (res <= 0 || res == DEL_VALUE) {
                    val flag = map[index + 1].compareAndSet(res, Int.MIN_VALUE)
                    if (!flag) continue
                }
                index -= 2
            }
        ...
        fun copyMap(key: Int, value: Int) {
            ...
            while (true) {
                ...
                if (curKey == NULL_KEY) {
                    when {
                        map[index].compareAndSet(curKey, key) -> break
