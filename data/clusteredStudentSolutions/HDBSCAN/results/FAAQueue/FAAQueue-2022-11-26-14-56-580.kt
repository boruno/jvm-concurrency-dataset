...
//            println((segment.get().get((index % SEGMENT_SIZE).toInt()) as Just<E>).value)
            if (segment.get().cas((index % SEGMENT_SIZE).toInt(), Null, Just(element)))
                return
        }
    }
...
//        println("Dequeue")
        while (true) {
            ...
            if (segment.get().cas((index % SEGMENT_SIZE).toInt(), Null, None))
                continue
            val cellValue = segment.get().get((index % SEGMENT_SIZE).toInt())
