...
class AtomicArrayWithCAS2AndImplementedDCSS<E : Any>(size: Int, initialValue: E) {
    ...
    inner class CAS2Descriptor(
        ...
        private fun helpToProcessDescriptor(anotherDescriptor: CAS2Descriptor): Boolean {
            ...
            if (anotherDescriptor.status.get() != SUCCESS && !dcss(indexA, expectedA, anotherDescriptor, anotherDescriptor.status, UNDECIDED))
                if (array.get(indexA) != anotherDescriptor) {
                    anotherDescriptor.updateStatus(FAILED)
                    return false
                }
            if (anotherDescriptor.status.get() != SUCCESS && !dcss(indexB, expectedB, anotherDescriptor, anotherDescriptor.status, UNDECIDED))
        ...
        private fun installDescriptor(): Boolean {
            ...
            while (true) {
                if (dcss(indexA, expectedA, this, status, UNDECIDED) || array.get(indexA) == this) {
                    if (dcss(indexB, expectedB, this, status, UNDECIDED) || array.get(indexB) == this) {
