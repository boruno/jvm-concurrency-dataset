...
class TreiberStackWithElimination<E> {
    ...
    fun push(x: E) {
        ...
        if (eliminationArray[elim_pos].compareAndSet(null, myval)) {
            ...
            while (LocalDateTime.now() < time) {
                if (eliminationArray[elim_pos].compareAndSet("pop", null)) {
                    return
                }
            }
            while (true) {
                if (eliminationArray[elim_pos].compareAndSet(myval, null)) {
                    break
                }
                if (eliminationArray[elim_pos].compareAndSet("pop", null)) {
                    return
    ...
    fun pop(): E? {
        ...
        val time = LocalDateTime.now() + Duration.ofMillis(delay)
        while (LocalDateTime.now() < time) {
            val old = eliminationArray[elim_pos].value
            if (old is Pair<*, *> && eliminationArray[elim_pos].compareAndSet(old, myval)) {
                return old.first as E?
            }
        }
...
/*class TreiberStackWithElimination<E> {
    ...
    fun push(x: E) {
        val elim_pos = 0//ThreadLocalRandom.current().nextInt(ELIMINATION_ARRAY_SIZE)
        var shouldstop = true//false
        val myval = Pair("push", x)
