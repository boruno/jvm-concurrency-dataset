...
def process_task_folder(task_folder, skip_folders):
    ...
    print(f"Processing task folder: {task_name}")
    ...
    if len(files) == 1:
        ...
        # End timing for single-file case
        end_time = time.time()
        elapsed_time = end_time - start_time
        print(f"Time taken for {task_name}: {elapsed_time:.2f} seconds")
        return
    ...
    # End timing for multi-file case
    end_time = time.time()
    elapsed_time = end_time - start_time
    print(f"Time taken for {task_name}: {elapsed_time:.2f} seconds")
