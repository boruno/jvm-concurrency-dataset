...
package org.jetbrains.research.cav23.caTreeMapAVL;
...
public class DualLFCASAVLTreeMapSTD<K, V> extends AbstractMap<K,V> implements SplitableAndJoinableMap<K, V>, Invalidatable, AnyKeyProviding<K>{
    ...
    final private void addAllToList(final STDAVLNodeDCAS<K,V> node, ArrayList<Entry<K, V>> list){
        if(node!=null){
            addAllToList(node.getLeft(), list);
            SimpleImmutableEntry<K,V> entry = new SimpleImmutableEntry<K,V>(node.key, node.getValue()){
                    public int hashCode(){
    ...
    final public void addAllToList(ArrayList<Entry<K, V>> list){
